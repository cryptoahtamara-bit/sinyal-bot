...
def mexc_place_order(symbol, sinyal, tp1=None, sl=None):
    ...
    try:
        from pymexc import futures as mexc_futures
        futures_client = mexc_futures.HTTP(
            api_key=MEXC_API_KEY,
            api_secret=MEXC_API_SECRET
        )
        res = futures_client.order(
            symbol=sym,
            price=0,
            vol=vol,
            side=side,
        ...
        print(f"[MEXC] pymexc yaniti: {res}")
        if res.get("success"):
