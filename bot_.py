...
def mexc_update_tpsl(symbol, is_long, tp1, sl):
    ...
    try:
        ...
        # TP ayrı istek olarak gönder
        if tp1:
            tp_body = {
                "symbol": sym,
                "positionId": pid,
                "stopLossOrTakeProfits": 2,  # 1=SL, 2=TP
                "price": float(str(tp1).replace(",", ".")),
                "priceType": 2,  # Fair Price
                "triggerType": 1,  # >= trigger
                "executeCycle": 87600,
                "trend": 1
            }
            print(f"[MEXC TPSL] TP body: {tp_body}")
            r_tp = mexc_private_post(f"{MEXC_BASE_URL}/api/v1/private/stoporder/place", tp_body)
            print(f"[MEXC TPSL] TP yanit: {r_tp.text[:200]}")

        # SL ayrı istek olarak gönder
        if sl:
            sl_body = {
                "symbol": sym,
                "positionId": pid,
                "stopLossOrTakeProfits": 1,  # 1=SL, 2=TP
                "price": float(str(sl).replace(",", ".")),
                "priceType": 2,  # Fair Price
                "triggerType": 2,  # <= trigger
                "executeCycle": 87600,
                "trend": 2
            }
            print(f"[MEXC TPSL] SL body: {sl_body}")
            r_sl = mexc_private_post(f"{MEXC_BASE_URL}/api/v1/private/stoporder/place", sl_body)
            print(f"[MEXC TPSL] SL yanit: {r_sl.text[:200]}")

        # Son yanıtı kontrol et
        last_r = r_sl if sl else r_tp if tp1 else None
        if last_r:
            print(f"[MEXC TPSL] Son yanit: {last_r.text}")
            return last_r.json().get("success", False)
        return False
    except Exception as e:
...
BOT_VERSIYON = "v274"
print(f"[BASLANGIC] ========== BOT VERSIYON: v274 ==========")
