...
def mexc_sign(body_dict, ts):
    sign_str = MEXC_API_KEY + ts + json.dumps(body_dict, separators=(",", ":"))
    return hmac.new(MEXC_API_SECRET.encode(), sign_str.encode(), hashlib.sha256).hexdigest()
...
def mexc_place_order(symbol, sinyal, tp1=None, sl=None):
    ...
    try:
        ...
        sign_chk = hmac.new(MEXC_API_SECRET.encode(),
                            (MEXC_API_KEY + ts_chk).encode(),
        ...
        if res_chk.get("success"):
            ...
            for a in (assets if isinstance(assets, list) else [assets]):
                if a.get("currency") == "USDT":
                    ...
                    if avail < MEXC_MARGIN_USDT:
                        return {"success": False,
                                "msg": f"Marginsiz islem acilamadi: Bakiye ${avail:.2f}, Gereken ${MEXC_MARGIN_USDT:.2f}"}
                    break
    except Exception as e:
        print(f"[MEXC] Marjin kontrol hatasi: {e}")
