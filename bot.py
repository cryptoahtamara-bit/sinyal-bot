...
def webhook():
    ...
    try:
        ...
        if isinstance(data, dict) and ("message" in data or "channel_post" in data):
            ...
            elif text.startswith("/yorum"):
                if chat_id in yetkili:
                    print(f"[KOMUT] /yorum islendi. chat_id={chat_id}")
                    _telegram_mesaj_gonder(chat_id, "🤖 Piyasa yorumu hazırlanıyor, lütfen bekleyin...")
                    threading.Thread(target=_yorum_gonder, daemon=True).start()
                else:
                    print(f"[KOMUT] /yorum yetkisiz. chat_id={chat_id}")
...
# ==========================================
# YAPAY ZEKA PİYASA YORUMU
# ==========================================

YORUM_SAATLER = [0, 6, 12, 18]  # TR saatiyle her 6 saatte bir

def _yorum_veri_topla():
    """BTC/ETH fiyat, değişim, balina ve tarayıcı verisini toplar."""
    veri = {}

    # BTC ve ETH fiyat + 24s değişim
    try:
        r = requests.get("https://contract.mexc.com/api/v1/contract/ticker", timeout=10)
        if r.status_code == 200:
            data = r.json()
            if data.get("success") and data.get("data"):
                for t in data["data"]:
                    sym = t.get("symbol", "")
                    if sym == "BTC_USDT":
                        veri["btc_fiyat"] = float(t.get("lastPrice", 0))
                        veri["btc_degisim"] = float(t.get("riseFallRate", 0)) * 100
                        veri["btc_hacim"] = float(t.get("volume24", 0))
                    elif sym == "ETH_USDT":
                        veri["eth_fiyat"] = float(t.get("lastPrice", 0))
                        veri["eth_degisim"] = float(t.get("riseFallRate", 0)) * 100
    except Exception as e:
        print(f"[YORUM] Fiyat verisi hatasi: {e}")

    # Tarayıcı verisi (ilk 3)
    try:
        tickers = _tarayici_veri_cek()
        if tickers:
            liste = []
            for t in tickers:
                ...
                if not sym.endswith("_USDT"):
                    continue
                pct = float(t.get("riseFallRate", 0)) * 100
                liste.append({"sym": sym.replace("_USDT", ""), "pct": pct})
            veri["yukselenler"] = sorted(liste, key=lambda x: x["pct"], reverse=True)[:3]
            veri["dusenler"]    = sorted(liste, key=lambda x: x["pct"])[:3]
    except Exception as e:
        print(f"[YORUM] Tarayici verisi hatasi: {e}")

    # Son balina hareketi
    try:
        for sym in WHALE_SYMBOLS:
            deals = _whale_fetch(sym)
            for deal in (deals or [])[:50]:
                fiyat  = float(deal.get("p", 0))
                miktar = float(deal.get("v", 0))
                tutar  = fiyat * miktar
                if tutar >= WHALE_LIMIT_USD:
                    ...
                    break
            if "son_balina" in veri:
                break
    except Exception as e:
        print(f"[YORUM] Balina verisi hatasi: {e}")

    return veri

def _yorum_prompt_olustur(veri):
    btc  = veri.get("btc_fiyat", 0)
    btcd = veri.get("btc_degisim", 0)
    eth  = veri.get("eth_fiyat", 0)
    ethd = veri.get("eth_degisim", 0)

    yuksel_str = ", ".join(
        f"{x['sym']} {'+' if x['pct']>0 else ''}{x['pct']:.1f}%"
        for x in veri.get("yukselenler", [])
    )
    dusen_str = ", ".join(
        f"{x['sym']} {x['pct']:.1f}%"
        for x in veri.get("dusenler", [])
    )

    balina_str = ""
    if "son_balina" in veri:
        b = veri["son_balina"]
        balina_str = f"Son büyük işlem: {b['sym']} {b['yon']} ${b['tutar']:,.0f}"

    prompt = (
        f"Sen deneyimli bir kripto piyasa analistisin. "
        f"Aşağıdaki verilere dayanarak kısa ve öz Türkçe piyasa yorumu yap. "
        f"Maksimum 4 kısa paragraf yaz. Yatırım tavsiyesi verme, sadece teknik gözlem yap.\n\n"
        f"Güncel Veriler:\n"
        f"- BTC: ${btc:,.1f} ({btcd:+.2f}% 24s)\n"
        f"- ETH: ${eth:,.4f} ({ethd:+.2f}% 24s)\n"
    )
    if yuksel_str:
        prompt += f"- En çok yükselenler: {yuksel_str}\n"
    if dusen_str:
        prompt += f"- En çok düşenler: {dusen_str}\n"
    if balina_str:
        prompt += f"- {balina_str}\n"

    prompt += (
        "\nŞu başlıkları ele al: genel piyasa durumu, öne çıkan hareketler, dikkat edilmesi gerekenler, kısa özet.\n"
        "Her başlık için 2-3 cümle yaz. Emojiler kullan."
    )
    return prompt
...
def _yorum_ai_al(prompt):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            print(f"[YORUM] API hata: {r.status_code} {r.text[:200]}")
    except Exception as e:
        print(f"[YORUM] API istegi hatasi: {e}")
    return None

def _yorum_mesaj_olustur(ai_yorum, veri, zaman_str):
    btc  = veri.get("btc_fiyat", 0)
    btcd = veri.get("btc_degisim", 0)
    eth  = veri.get("eth_fiyat", 0)
    ethd = veri.get("eth_degisim", 0)
    ...
    msg = (
        ...
        "🕐 " + zaman_str + "\n"
        + ayrac + "\n\n"
        f"📊 BTC: <b>${btc:,.1f}</b> ({btcd:+.2f}%)  "
        f"ETH: <b>${eth:,.4f}</b> ({ethd:+.2f}%)\n\n"
        + ayrac + "\n\n"
        + ai_yorum + "\n\n"
        + ayrac + "\n"
        "⚠️ <i>Bu yorum yatırım tavsiyesi değildir. Kendi araştırmanızı yapın.</i>\n\n"
        "Siz de kulübe katılıp, alarmları kaçırmamak için lütfen iletişime geçin.\n"
        "İletişim: " + KANAL_TAG
    )
    return msg
...
def _yorum_gonder():
    print("[YORUM] Piyasa yorumu hazırlanıyor...")
    veri = _yorum_veri_topla()

    if not veri.get("btc_fiyat"):
        print("[YORUM] Yeterli veri yok, yorum atlandı.")
        return
    ...
    prompt   = _yorum_prompt_olustur(veri)
    ai_yorum = _yorum_ai_al(prompt)

    if not ai_yorum:
        print("[YORUM] AI yorumu alınamadı.")
        return

    mesaj = _yorum_mesaj_olustur(ai_yorum, veri, zaman_str)
    _telegram_mesaj_gonder(TELEGRAM_CHAT_ID, mesaj)
...
def _yorum_zamanlayici():
    """Her 6 saatte bir (00:00, 06:00, 12:00, 18:00 TR) yorum gönder."""
    import datetime as dt_mod
    print(f"[YORUM] Zamanlayici basladi. Saatler: {YORUM_SAATLER}")
    while True:
        try:
            if TR_TZ:
                simdi = datetime.now(tz=TR_TZ)
            ...
            # Bir sonraki hedef saati bul
            hedef = None
            for saat in sorted(YORUM_SAATLER):
                aday = simdi.replace(hour=saat, minute=0, second=0, microsecond=0)
                if aday > simdi:
                    hedef = aday
                    break
            if hedef is None:
                yarin = simdi + dt_mod.timedelta(days=1)
                hedef = yarin.replace(hour=YORUM_SAATLER[0], minute=0, second=0, microsecond=0)

            bekle = (hedef - simdi).total_seconds()
            print(f"[YORUM] Sonraki yorum: {hedef.strftime('%H:%M')} ({int(bekle//60)} dk sonra)")
            time.sleep(bekle)
            _yorum_gonder()
            time.sleep(10)
        except Exception as e:
            print(f"[YORUM] Zamanlayici hata: {e}")
            time.sleep(60)
...
threading.Thread(target=_yorum_zamanlayici, daemon=True).start()
