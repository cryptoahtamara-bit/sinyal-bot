...
def koin_sirala_bugun(snapshot=None):
    ...
    Yeni mantık (v612):
    - Her coin için filtre_notu = Güçlü olan sinyallerin TP oranına bak
    - Güçlü sinyal sayısı < 5 ise veri yetersiz — coin whitelist dışı
    - TRADE_WHITELIST_MIN_PERCENT eşiğine göre whitelist kararı verilir
    """
    with gunluk_kilit:
        kayitlar = list(gunluk_sinyaller) if snapshot is None else list(snapshot)

    TF_GECERLI = {"1","5","15","60","240","D"}
    MIN_GUCLU  = 5  # whitelist için minimum Güçlü sinyal sayısı

    koin_istat = {}
    for s in kayitlar:
        sym = sembol_grup(s.get("symbol", ""))
        if not sym or sym == "OTHERS": continue
        if str(s.get("timeframe","?")) not in TF_GECERLI: continue

        if sym not in koin_istat:
            koin_istat[sym] = {
                "guclu_tp": 0, "guclu_sl": 0, "guclu_devam": 0,
                "orta_tp":  0, "orta_sl":  0, "orta_devam":  0,
                "zayif_tp": 0, "zayif_sl": 0, "zayif_devam": 0,
                "toplam": 0, "long": 0, "short": 0,
            }

        sin = s.get("sinyal","").upper()
        if any(x in sin for x in ["BUY","LONG"]): koin_istat[sym]["long"] += 1
        else: koin_istat[sym]["short"] += 1
        koin_istat[sym]["toplam"] += 1

        # Filtre kategorisi — filtre_notu alanından oku
        ...
        # TP/SL/devam durumu
        kontrol = any(s.get(k) is not None for k in ["tp1_ok","tp2_ok","tp3_ok","tp4_ok","tp5_ok"])
        if kontrol:
            if any(s.get(k) is True for k in ["tp1_ok","tp2_ok","tp3_ok","tp4_ok","tp5_ok"]):
                koin_istat[sym][f"{kat}_tp"] += 1
            elif s.get("sl_ok"):
                koin_istat[sym][f"{kat}_sl"] += 1
            else:
                koin_istat[sym][f"{kat}_devam"] += 1
        else:
            koin_istat[sym][f"{kat}_devam"] += 1

    sonuc = []
    for sym, v in koin_istat.items():
        guclu_toplam = v["guclu_tp"] + v["guclu_sl"] + v["guclu_devam"]
        orta_toplam  = v["orta_tp"]  + v["orta_sl"]  + v["orta_devam"]
        zayif_toplam = v["zayif_tp"] + v["zayif_sl"] + v["zayif_devam"]

        # Güçlü TP oranı — whitelist kararı için
        guclu_oran = round(v["guclu_tp"] / guclu_toplam * 100, 1) if guclu_toplam > 0 else 0
        yeterli_veri = guclu_toplam >= MIN_GUCLU

        yon = "LONG" if v["long"] >= v["short"] else "SHORT"
        sonuc.append({
            "sym":          sym,
            "oran":         guclu_oran,       # Güçlü sinyallerin TP oranı
            "toplam":       v["toplam"],
            "guclu_n":      guclu_toplam,
            "guclu_tp":     v["guclu_tp"],
            "orta_n":       orta_toplam,
            "zayif_n":      zayif_toplam,
...
def _filtre_zamanlayici():
    ...
    Saat başına hizalanır: XX:00 ve XX:30'da çalışır.
    """
    print("[FILTRE] Zamanlayici basladi. XX:00 ve XX:30'da gorsel gonderilecek.")
    import time as _t
    # İlk çalışma için bir sonraki :00 veya :30'u bekle
    while True:
        try:
            simdi   = datetime.now(tz=TR_TZ) if TR_TZ else datetime.utcnow()
            dakika  = simdi.minute
            saniye  = simdi.second
            # Bir sonraki :00 veya :30
            if dakika < 30:
                bekle = (30 - dakika) * 60 - saniye
            else:
                bekle = (60 - dakika) * 60 - saniye
            bekle = max(bekle, 1)
...
def _tp_kosul_rapor_zamanlayici():
    """Her saatin :10'unda TP koşul raporunu TOPIC_RAPOR'a gönder."""
    ...
    while True:
        try:
            ...
            dk = simdi.minute
            if dk < 10:
                sonraki = simdi.replace(minute=10, second=0, microsecond=0)
            else:
                sonraki = (simdi.replace(minute=10, second=0, microsecond=0)
                           + dt_mod.timedelta(hours=1))
            bekle = (sonraki - simdi).total_seconds()
            ...
            # Bot restart sonrası gunluk_sinyaller boşsa diskten yükle
            if len(snapshot) < 100 and os.path.exists(VERI_DOSYASI):
                try:
                    with open(VERI_DOSYASI, "r", encoding="utf-8") as _f:
                        snapshot = _json.load(_f)
                except Exception:
                    pass
            veri      = _tp_kosul_rapor_hesapla(snapshot=snapshot)

            # Confluance skoru hesapla ve log yaz
            try:
                if TR_TZ:
                    zaman_str = datetime.now(tz=TR_TZ).strftime("%Y-%m-%d %H:%M")
                else:
                    zaman_str = datetime.utcnow().strftime("%Y-%m-%d %H:%M")
                confluance = _tp_confluance_skoru_hesapla(veri, kayitlar_snapshot=snapshot)
                if confluance:
                    _tp_confluance_log_yaz(confluance, zaman_str)
            except Exception as ce:
                print(f"[TP_CONFLUANCE] Hesaplama hatasi: {ce}")

            gorseller = _tp_kosul_rapor_gorsel(veri=veri, snapshot=snapshot)
            if gorseller:
                for img, caption in gorseller:
                    _topic_foto_gonder_filigranli(TOPIC_RAPOR, img.read() if hasattr(img, "read") else img, caption)
                    time.sleep(2)
                # Yorum metni en sona
                maddeler = _tp_kosul_yorum_uret(veri, snapshot=snapshot)
                if maddeler:
                    yorum_txt = "📊 <b>TP Performans Yorumu</b>\n\n" + "\n".join(f"• {m}" for m in maddeler)
