...
def _filtre_detay_gorsel_uret(combo_tab, genel_tp, kapali_tum, esik_g, esik_z, tf_map,
    ...
    try:
        ...
        tam_toplam   = sum(1 for s in _snap if s.get("tp1_ok") is not None or s.get("sl_ok") is not None)
        devam_toplam = toplam_sin - tam_toplam
        ...
        ozet_data = [
            ...
            ('Tamamlanan',    str(tam_toplam),    '#FFFFFF'),
        ...
        for ri, st in enumerate(SINYAL_TIPLERI):
            ...
            # sinyaller.json'dan TP/SL/devam — bu sinyal tipi için
            st_sinyaller = [s for s in gunluk_sinyaller
                            if SINYAL_NORM_MAP.get(s.get("sinyal","").upper()) == st]
            st_tp    = sum(1 for s in st_sinyaller if s.get("tp1_ok") == True)
            st_sl    = sum(1 for s in st_sinyaller if s.get("sl_ok") == True)
            st_tam   = sum(1 for s in st_sinyaller
                           if s.get("tp1_ok") is not None or s.get("sl_ok") is not None)
            st_devam = len(st_sinyaller) - st_tam
            ...
            # Tamamlanan sütunu — sinyaller.json'dan
            ax.text(cols[5], y - 0.018, str(st_tam), fontsize=15,
                    color='#FFFFFF', ha='left', va='top', fontweight='bold')
