...
def mexc_test():
    ...
    sonuc = {"version": "v186", "proxy": bool(MEXC_PRIVATE_PROXY)}
...
def webkey_test():
    ...
    # Test 4: Dogru endpoint /private/order/create
    if MEXC_API_KEY and MEXC_API_SECRET:
        test_body = {"symbol": "BTC_USDT", "price": 0, "vol": 0.001, "side": 1, "type": 5, "openType": 2, "leverage": 20}

        # Yontem 1: /private/order/create endpoint
        try:
            ...
            sonuc["create_endpoint"] = {"status": r1.status_code, "body": r1.text[:200]}
        except Exception as e:
            sonuc["create_endpoint"] = {"error": str(e)[:50]}

        # Yontem 2: Farkli base URL ile
        try:
            hdrs2, _ = mexc_headers(test_body)
            r2 = requests.post("https://futures.mexc.com/api/v1/private/order/create",
                               headers=hdrs2, json=test_body, timeout=8)
            sonuc["create_futures_domain"] = {"status": r2.status_code, "body": r2.text[:200]}
        except Exception as e:
            sonuc["create_futures_domain"] = {"error": str(e)[:50]}

        # Yontem 3: /private/order/submit yerine /private/order/create + bos body imza
        try:
            hdrs3, _ = mexc_headers({})
            r3 = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/create",
                               headers=hdrs3, json=test_body, timeout=8)
            sonuc["create_bos_imza"] = {"status": r3.status_code, "body": r3.text[:200]}
        except Exception as e:
            sonuc["create_bos_imza"] = {"error": str(e)[:50]}
...
print(f"[BASLANGIC] ========== BOT VERSIYON: v186 ==========")
