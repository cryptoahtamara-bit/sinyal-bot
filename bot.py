...
def _tp_kosul_rapor_hesapla(snapshot=None):
    ...
    kosullar = {
        ...
        "Seans — Asya":            [s for s in kayitlar if s.get("seans") == "Asya"],
        "Seans — Londra":          [s for s in kayitlar if s.get("seans") == "Londra"],
        "Seans — New York":        [s for s in kayitlar if s.get("seans") == "New York"],
    }
    ...
    # Sinyal tipi + seans
    for tip, ses in iterproduct(tipler, ["Asya","Londra","New York"]):
        sub = [s for s in kayitlar if s.get("sinyal_tipi")==tip and s.get("seans")==ses]
        if sub:
            kombinasyonlar[f"{tip} + {ses}"] = satir(sub)
    # Coin + TF
    for sym in sorted(semboller):
...
def _tp_kosul_yorum_uret(veri):
    """Veriden otomatik Türkçe yorum paragrafı üret."""
    satirlar = []

    # En iyi sinyal tipi
    en_iyi_tip = max(veri["sinyal_tipi"].items(), key=lambda x: x[1][1] if x[1][0] >= 5 else -1, default=None)
    if en_iyi_tip and en_iyi_tip[1][0] >= 5:
        satirlar.append(f"Sinyal tipi bazında en yüksek TP oranı {en_iyi_tip[0]} sinyallerinde "
                       f"görülüyor ({en_iyi_tip[1][3]}, {en_iyi_tip[1][0]} sinyal).")
    else:
        satirlar.append("Sinyal tipi verisi henüz yetersiz — sistem yeni parametreleri kayıt altına almaya başladı.")

    # Piyasa koşulu yorumu
    kosul_veri = {k: v for k, v in veri["kosul"].items() if v[0] >= 5}
    if kosul_veri:
        en_iyi_kosul = max(kosul_veri.items(), key=lambda x: float(str(x[1][3]).replace("%","")) if "%" in str(x[1][3]) else 0)
        en_kotu_kosul = min(kosul_veri.items(), key=lambda x: float(str(x[1][3]).replace("%","")) if "%" in str(x[1][3]) else 100)
        satirlar.append(f"Piyasa koşullarında en başarılı ortam: {en_iyi_kosul[0]} ({en_iyi_kosul[1][3]}). "
                       f"En düşük performans: {en_kotu_kosul[0]} ({en_kotu_kosul[1][3]}).")
    else:
        satirlar.append("Piyasa koşulları verisi toplanmaya devam ediyor (sinyaller yeni kayıt altına alınıyor).")

    # En iyi coin
    coin_veri = {k: v for k, v in veri["coin"].items() if v[0] >= 10}
    if coin_veri:
        en_iyi_coin = max(coin_veri.items(), key=lambda x: float(str(x[1][3]).replace("%","")) if "%" in str(x[1][3]) else 0)
        en_kotu_coin = min(coin_veri.items(), key=lambda x: float(str(x[1][3]).replace("%","")) if "%" in str(x[1][3]) else 100)
        satirlar.append(f"En başarılı coin: {en_iyi_coin[0]} ({en_iyi_coin[1][3]} TP). "
                       f"En düşük performanslı coin: {en_kotu_coin[0]} ({en_kotu_coin[1][3]} TP).")

    # En iyi TF
    tf_veri = {k: v for k, v in veri["tf"].items() if v[0] >= 10}
    if tf_veri:
        en_iyi_tf = max(tf_veri.items(), key=lambda x: float(str(x[1][3]).replace("%","")) if "%" in str(x[1][3]) else 0)
        satirlar.append(f"Zaman dilimi bazında en güvenilir sinyal: {en_iyi_tf[0]} "
                       f"({en_iyi_tf[1][3]} TP, {en_iyi_tf[1][0]} sinyal).")
        # Düşük TF uyarısı
        for tf, tv in veri["tf"].items():
            if tv[0] >= 20 and "%" in str(tv[3]) and float(str(tv[3]).replace("%","")) < 25:
                satirlar.append(f"Dikkat: {tf} sinyallerinde TP oranı {tv[3]} — bu zaman diliminde dikkatli olunması önerilir.")

    # En iyi kombinasyon
    if veri["oneri"]:
        en_iyi_komb, en_iyi_kv = veri["oneri"][0]
        satirlar.append(f"En güvenilir kombinasyon: {en_iyi_komb} — {en_iyi_kv[3]} TP oranı "
                       f"({en_iyi_kv[0]} sinyal üzerinden).")
    elif veri["kombinasyon"]:
        komb_sirali = sorted(veri["kombinasyon"].items(),
                            key=lambda x: float(str(x[1][3]).replace("%","")) if "%" in str(x[1][3]) else 0,
                            reverse=True)
        if komb_sirali:
            k, kv = komb_sirali[0]
            if kv[0] >= 5:
                satirlar.append(f"Öne çıkan kombinasyon (min. 20 sinyal bekleniyor): "
                               f"{k} — {kv[3]} TP ({kv[0]} sinyal).")
...
def _tp_kosul_rapor_gorsel(veri=None, snapshot=None):
    ...
    yorum = _tp_kosul_yorum_uret(veri)
    ...
    def toplam_satir_yap(satirlar, sutunlar):
        """Tablonun alt toplamını hesapla."""
        try:
            n = len(sutunlar)
            toplam_sayilar = [0] * n
            for satir in satirlar:
                for ci in range(1, n):
                    try:
                        ...
                        pass
            # Oran sütununu hesapla (TP/Toplam)
            t_row = ["Toplam"]
            for ci in range(1, n):
                if ci == n-1:  # Oran sütunu
                    try:
                    ...
                    v = int(toplam_sayilar[ci]) if toplam_sayilar[ci] == int(toplam_sayilar[ci]) else round(toplam_sayilar[ci],1)
                    t_row.append(str(v) if v > 0 else "—")
            return tuple(t_row)
        except Exception:
            return None
    ...
    satirlar1 = [(t,)+v for t,v in veri["sinyal_tipi"].items()]
    t1 = toplam_satir_yap(satirlar1, ["Sinyal Tipi","Toplam","TP","TP Olmayan","Oran"])
    g1 = gorsel_yap(
        [("1. Sinyal Tipi — Karşılaştırma", BLUE, satirlar1,
          ["Sinyal Tipi","Toplam","TP","TP Olmayan","Oran"])],
        "TP Koşul Raporu — 1/4",
        toplam_satirlari=[t1],
        yorum_metni=yorum
    )
    ...
    satirlar2 = [(t,)+v for t,v in veri["kosul"].items()]
    g2 = gorsel_yap(
        [("2. Piyasa Koşulları — Karşılaştırma", GREEN2, satirlar2,
          ["Koşul","Toplam","TP","TP Olmayan","Oran"])],
        "TP Koşul Raporu — 2/4",
    ...
    satirlar4 = [(t,)+v for t,v in veri["coin"].items()]
    satirlar5 = [(t,)+v for t,v in veri["tf"].items()]
    t4 = toplam_satir_yap(satirlar4, ["Coin","Toplam","TP","TP Olmayan","Oran"])
    t5 = toplam_satir_yap(satirlar5, ["TF","Toplam","TP","TP Olmayan","Oran"])
    g3 = gorsel_yap(
        [
            ("3. TP Seviyesi — Dağılımı", AMBER2, satirlar3,
             ["Seviye","Toplam TP","Oran","",""]),
            ("4. Coin Bazlı — Karşılaştırma", PURPLE, satirlar4,
             ["Coin","Toplam","TP","TP Olmayan","Oran"]),
            ("5. Zaman Dilimi — Karşılaştırma", TEAL, satirlar5,
             ["TF","Toplam","TP","TP Olmayan","Oran"]),
        ],
        "TP Koşul Raporu — 3/4",
    ...
    if veri["oneri"]:
        ...
        satirlar7 = [("Veri birikmesi bekleniyor (min. 20 sinyal)", "—", "—", "—", "—")]
    g4 = gorsel_yap(
        [
            ("6. En Kârlı Kombinasyonlar", CORAL, satirlar6,
             ["Kombinasyon","Toplam","TP","TP Olmayan","Oran"]),
            ("7. Öneri Mekanizması — En Güvenilir Koşullar", PINK, satirlar7,
             ["Kombinasyon","Toplam","TP","TP Olmayan","Oran"]),
        ],
        "TP Koşul Raporu — 4/4",
        alt_yazi="TP Olmayan = SL + Devam Eden  |  Öneri: min. 20 sinyal  |  %50+ Önerilen  |  %35- Kaçınılan",
        toplam_satirlari=[None, None]
    )
