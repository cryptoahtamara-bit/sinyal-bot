...
TOPIC_MEXC        = int(os.getenv("TOPIC_MEXC",    "6018")) # ⚡ MEXC İşlemleri
...
def mexc_notify(symbol, sinyal, vol=0, leverage=0, margin=0,
    ...
    if bilgi_msg:
        ...
        metin = "\n".join(parts)
    try:
        _mexc_topic_mesaj_gonder(metin)
    except Exception as e:
...
def mexc_update_tpsl(symbol, is_long, tp1, sl):
    ...
    try:
        ...
        if tp1:
            ...
            body["profitTrend"]     = 1   # Last Price
        if sl:
            body["stopLossPrice"] = float(str(sl).replace(",", "."))
            body["lossTrend"]     = 1   # Last Price
...
def _gunluk_pnl_gonder(hedef_chat_id=None):
    """PNL gonder — hedef_chat_id None ise TOPIC_MEXC kullanilir."""
...
def _gunluk_pnl_gonder_topic():
    """PNL'yi TOPIC_MEXC'e gonder — wrapper fonksiyon."""
...
def _mexc_topic_mesaj_gonder(metin, parse_mode="HTML"):
    ...
    if not MEXC_NOTIFY_CHAT_ID:
        ...
        return None
    return _telegram_mesaj_gonder(MEXC_NOTIFY_CHAT_ID, metin, parse_mode=parse_mode)
...
def _analiz_gorsel(veriler, zaman_str):
    ...
    try:
        ...
        # Figür boyutu — likidasyon haritasıyla aynı DPI (130), daha geniş
        fig_w = 14.0
        fig_h = 17.4
        ...
        for i, (lbl, val, col, sub) in enumerate(kartlar):
            ...
            ax.text(kx + kart_w/2, y - 0.52, val,
                    ha="center", va="top", fontsize=16, color=col, fontweight="bold")
            ax.text(kx + kart_w/2, y - 0.88, sub,
        ...
        for cx, lbl, ha in col_defs:
            ax.text(cx, y, lbl, ha=ha, va="top", fontsize=11.5, color=WHITE, fontweight="bold")
        y -= 0.18
        ...
        for i, sym in enumerate(SEMBOLLER):
            ...
            ax.text(COL["coin"], my, sym_kisa, ha="left", va="center",
                    fontsize=14, color=sym_renk, fontweight="bold")
            ...
            ax.text(COL["d24"], my, f"{d24:+.1f}%" if d24 is not None else "—",
                    ha="right", va="center", fontsize=12, color=renk_pct(d24))
            ...
            if oi_d:
                ...
                ax.text(COL["oi"], my, f"{dp:+.1f}%", ha="right", va="center",
                        fontsize=12, color=renk_pct(dp))
            else:
                ax.text(COL["oi"], my, "—", ha="right", va="center", fontsize=12, color=GRAY)
            ...
            if taker:
                ...
                ax.text(COL["taker"], my, f"{bp:.0f}%B", ha="right", va="center",
                        fontsize=12, color=t_col)
            else:
                ax.text(COL["taker"], my, "—", ha="right", va="center", fontsize=12, color=GRAY)
            ...
            if cvd:
                ...
                ax.text(COL["cvd"], my, cvd_str, ha="right", va="center",
                        fontsize=12, color=c_col)
            else:
                ax.text(COL["cvd"], my, "—", ha="right", va="center", fontsize=12, color=GRAY)
            ...
            if lik and lik["toplam_usd"] > 0:
                ...
                ax.text(COL["lik"], my, lik_str, ha="right", va="center",
                        fontsize=11.5, color=l_col)
            else:
                ax.text(COL["lik"], my, "—", ha="right", va="center", fontsize=11.5, color=DGRAY)
            ...
            if hc and hc > 0:
                ...
                ax.text(COL["hac"], my, hc_str, ha="right", va="center",
                        fontsize=11.5, color=hc_col)
            else:
                ax.text(COL["hac"], my, "—", ha="right", va="center", fontsize=11.5, color=DGRAY)
        ...
        for pi, sym in enumerate(["BTCUSDT", "ETHUSDT", "SOLUSDT"]):
            ...
            if taker:
                ...
                ax.text(px + panel_w/2, y - 0.42,
                        ...
                        ha="center", va="top", fontsize=8.5, color=t_col)
            ...
            def satir(label, val1, val2=None, renk=WHITE):
                ...
                ax.text(pad_r, ly, vstr, ha="right", va="top",
                        fontsize=12, color=renk, fontweight="bold")
                ly -= 0.41
        ...
        for sym2, col, lbl in legend_items:
            ax.text(lx, y - 0.16, sym2, ha="left", va="top", fontsize=9, color=col)
            ax.text(lx + 0.20, y - 0.16, lbl, ha="left", va="top", fontsize=8.5, color=WHITE)
            lx += 2.20
        # Sağ legend — 2 satır
        ax.text(fig_w - 0.20, y - 0.08,
                "CVD=Net Alış/Satış  ·  LİK.M=Likidasyon($M)",
                ha="right", va="top", fontsize=7.5, color=GRAY)
        ax.text(fig_w - 0.20, y - 0.26,
                "HAC.Ç=Fut/Spot Hacim Oranı  ·  UYM⚠=CVD Uyumsuz",
                ha="right", va="top", fontsize=7.5, color=GRAY)
