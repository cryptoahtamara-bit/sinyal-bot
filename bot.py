...
def _atr(highs, lows, closes, periyot):
    ...
    # ATR = EMA(TR, periyot)
    return _ema(tr_list, periyot)

def _cci(highs, lows, closes, periyot=21):
...
def _kulyutmaz_sinyal_hesapla(closes, highs, lows, volumes):
    ...
    # --- EMA 200 ---
    ema200_list = _ema(closes, EMA200_LEN)

    # --- ATR hesapları ---
    atr_beyaz = _atr(highs, lows, closes, BEYAZ_H)
    atr_sari  = _atr(highs, lows, closes, SARI_P)
    atr14     = _atr(highs, lows, closes, 14)

    # --- CCI ---
    cci_list = _cci(highs, lows, closes, 21)

    # --- CVWAP: EMA(close, vwaplength) — Pine'da ta.ema(ta.vwap, 1) = close ---
    # vwaplength=1 olduğu için EMA(close, 1) = close'un kendisi
    cvwap_list = closes[:]  # vwaplength=1 → CVWAP = close
    ...
    # Son bar sinyal üretiyor mu?
    son_i = n - 1
    if son_i != long_sinyal_i and son_i != short_sinyal_i:
        return None, None, None

    is_long = (son_i == long_sinyal_i)
    ...
    # --- TP: Hacim bazlı (f_collectTPs mantığı) ---
    atr14_son = atr14[son_i] if atr14[son_i] else entry * 0.005
    lookback_start = max(0, son_i - TP_LOOKBACK)

    c_prices = []
    c_vols   = []
    for i in range(lookback_start, son_i):
        if is_long:
            p_lvl = highs[i]
            if p_lvl > entry:
                c_prices.append(p_lvl)
                c_vols.append(volumes[i])
        else:
            p_lvl = lows[i]
            if p_lvl < entry:
                c_prices.append(p_lvl)
    ...
    # Hacme göre sırala, en yüksek TP_COUNT tane al
    tp_listesi = []
    if c_prices:
    ...
    # Yeterli seviye yoksa ATR ile tamamla
    ...
    if needed > 0 and atr14_son > 0:
        ...
        for k in range(start_k, start_k + needed):
            offset   = atr14_son * k * 1.5
            extra_tp = entry + offset if is_long else entry - offset
            tp_listesi.append(extra_tp)

    # Sırala (LONG: artan, SHORT: azalan)
...
def _kulyutmaz_alarm_kontrol():
    """Tüm MEXC Futures coinleri için KÜLYUTMAZ V.4.3 sinyali tara → TOPIC_YUKSELENLER'e bildir."""
    try:
        tickers = _tarayici_veri_cek()
        if not tickers:
        ...
        for sym in semboller:
            try:
                ...
                if not closes or not highs or not lows or not volumes:
                    continue
                # Veri bütünlük kontrolü: ilk close değeri makul fiyat aralığında olmalı
                # MEXC bazen string döndürdüğünde her karakter float oluyor (ör: "1","2","3",...)
                if not (0.000001 <= closes[0] <= 10_000_000):
                    print(f"[KULYUTMAZ] {sym} veri bozuk atlanıyor: closes[0]={closes[0]}")
                    continue
                if len(closes) < 210:
                    continue

                # DEBUG: veri tipi kontrolü
                if not isinstance(closes, list) or not isinstance(highs, list) or not isinstance(lows, list):
                    print(f"[KULYUTMAZ] {sym} TIP HATASI: closes={type(closes).__name__} highs={type(highs).__name__} lows={type(lows).__name__}")
                    continue
                if closes and not isinstance(closes[0], float):
                    print(f"[KULYUTMAZ] {sym} ELEMAN TIP HATASI: closes[0]={type(closes[0]).__name__} val={closes[0]}")
                    continue
