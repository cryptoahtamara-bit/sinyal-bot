...
def koin_sirala_metni(snapshot=None):
    ...
    for sym, v in koin_sirali[:10]:
        ...
        g_str = f"\U0001f7e2{g2['tp']}/{g2['sin']}" if g2["sin"] > 0 else "\U0001f7e2\u2014"
        o_str = f"\U0001f7e1{o2['tp']}/{o2['sin']}" if o2["sin"] > 0 else "\U0001f7e1\u2014"
        z_str = f"\U0001f534{z2['tp']}/{z2['sin']}" if z2["sin"] > 0 else "\U0001f534\u2014"
        aktif = "\u2705" if genel_oran >= TRADE_WHITELIST_MIN_PERCENT else "  "
        satirlar.append(f"{aktif}<b>{sym}</b>  {g_str} {o_str} {z_str} | %{genel_oran:.0f}")
