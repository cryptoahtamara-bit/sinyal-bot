...
def _analiz_ozet_telegram(coin_verileri, veriler, zaman_str):
    ...
    try:
        ...
        # CVD uyumsuzlukları — CVD değeri de yönü destekliyorsa listele
        ...
        for sym, v in coin_verileri.items():
            ...
            # Birikim: fiyat ↓ CVD ↑ VE CVD değeri pozitif → gerçek alım var
            if uyum == "uyumsuz_asagi" and cvd_val > 0:
                uyumsuz_asagi.append(kisa)
            # Sahte pump: fiyat ↑ CVD ↓ VE CVD değeri negatif → satış gizleniyor
        ...
        if uyumsuz_asagi or uyumsuz_yukari or btc_cvd_d:
            parcalar = []
            # BTC'yi birikim listesinden çıkar — CVD satırında ayrıca gösteriliyor
            birikim_listesi = [c for c in uyumsuz_asagi if c != "BTC"]
            pump_listesi    = [c for c in uyumsuz_yukari if c != "BTC"]
            if birikim_listesi:
                parcalar.append(f"<b>{', '.join(birikim_listesi)}</b> düşüş suni olabilir, dip alma fırsatı")
            if pump_listesi:
        ...
        if asiri_alim and asiri_satim:
            ...
            str_as = ", ".join([n for n,_ in sorted(asiri_satim, key=lambda x:x[1])])
            satirlar.append(f"📊 <b>RSI:</b> <b>{str_as}</b> aşırı satım bölgesinde.")
        else:
        ...
        # CVD bazlı coin önerileri — sadece gerçek birikim varsa
        if uyumsuz_asagi and puan >= 0:
            dip_adaylar = []
            dip_bekle   = []
            for sym in [s+"USDT" for s in uyumsuz_asagi[:4]]:
                v    = coin_verileri.get(sym, {})
                rsi  = v.get("rsi", 50)
                skor = v.get("skor", 0)
                cvd  = v.get("cvd", {}) or {}
                isim = sym.replace("USDT","")
                if rsi <= 35 and skor >= 45 and cvd.get("cvd", 0) > 0:
                    dip_adaylar.append(isim)
                elif cvd.get("cvd", 0) > 0:
                    dip_bekle.append(isim)
            if dip_adaylar:
...
BOT_VERSIYON = "v489"
print(f"[BASLANGIC] ========== BOT VERSIYON: v489 ==========")
