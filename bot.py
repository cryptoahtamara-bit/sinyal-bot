...
def _liq_yorum(veri):
    ...
    if grid_data:
        try:
            import numpy as np
            gl   = grid_data["grid_long"]
            gs   = grid_data["grid_short"]
            pmin = grid_data["price_min"]
            pmax = grid_data["price_max"]
            n_pb = grid_data["n_pb"]
            fiyat_bin = int((fiyat - pmin) / (pmax - pmin) * n_pb)
            fiyat_bin = max(1, min(n_pb - 2, fiyat_bin))

            def _bolge_oku(grid, alt, ust, pmin, pmax, n_pb):
                """Eşik üstündeki komşu bantları birleştir, en yüksek hacimli bölge."""
                adaylar = [(grid[i], i) for i in range(alt, ust) if grid[i] > 0]
                if not adaylar: return None
                esik = max(w for w,_ in adaylar) * 0.25
                aktif = {i for w,i in adaylar if w >= esik}
                bolgeler = []
                ziyaret  = set()
                for i in sorted(aktif):
                    ...
                    while j in aktif:
                        ...
                        j += 1
                    if bolge:
                        hacim = sum(grid[k] for k in bolge)
                        bolgeler.append((hacim, bolge))
                if not bolgeler: return None
                bolgeler.sort(reverse=True)
                en_iyi = bolgeler[0][1]
                p_alt = pmin + en_iyi[0] / n_pb * (pmax - pmin)
                p_ust = pmin + (en_iyi[-1] + 1) / n_pb * (pmax - pmin)
                return round(p_alt, 2), round(p_ust, 2)

            long_bolge  = _bolge_oku(gl, 0,         fiyat_bin, pmin, pmax, n_pb)
            short_bolge = _bolge_oku(gs, fiyat_bin, n_pb,      pmin, pmax, n_pb)

            if long_bolge:
                liq_long_alt, liq_long_ust = long_bolge
            if short_bolge:
                liq_short_alt, liq_short_ust = short_bolge
        except Exception as e:
            print(f"[LIQ_YORUM] Grid okuma hata: {e}")
...
BOT_VERSIYON = "v505"
print(f"[BASLANGIC] ========== BOT VERSIYON: v505 ==========")
