...
def tp1_karlilik_kontrol(mark_price, tp1, tp2, is_long, tp3=None, tp4=None, tp5=None, symbol=None, hedge_fallback=False):
    ...
    Coin bazlı eşik varsa onu, yoksa genel MIN_KARLI_MESAFE_PCT'yi kullanır.
    Hiçbiri geçemiyorsa (None, notu) döner → işlem AÇILMAZ.
    NOT: TP5 varsa önce dener. hedge_fallback=True ise TP5 geçemezse TP1-TP4'e düşer.
    ...
    # TP5 varsa önce onu dene — geçiyorsa direkt kullan
    # hedge_fallback=False ise geçemezse işlem açma
    # hedge_fallback=True ise geçemezse TP1-TP4'e düş
    if tp5 and str(tp5) != "null":
        try:
            tp5_f = float(str(tp5).replace(",", "."))
            if is_long and tp5_f <= mp_f:
                print(f"[FEE] TP5 yön hatası: LONG TP5 ({tp5_f}) <= giriş ({mp_f}) — {'fallback' if hedge_fallback else 'işlem açılmıyor'}")
                notu = f"TP5 yanlış yönde (mark={mark_price})"
            elif not is_long and tp5_f >= mp_f:
                print(f"[FEE] TP5 yön hatası: SHORT TP5 ({tp5_f}) >= giriş ({mp_f}) — {'fallback' if hedge_fallback else 'işlem açılmıyor'}")
                notu = f"TP5 yanlış yönde (mark={mark_price})"
            else:
                mesafe_pct = abs(tp5_f - mp_f) / mp_f
                print(f"[FEE] TP5 mesafe: %{mesafe_pct*100:.3f} (esik: %{esik*100:.2f})")
                if mesafe_pct >= esik:
                    notu = f"TP5 ({tp5}) kullanildi"
                    print(f"[FEE] {notu}")
                    return tp5, notu
                else:
                    notu = f"TP5 çok yakın (mark={mark_price})"
                    print(f"[FEE] {notu} — {'fallback TP1-TP4' if hedge_fallback else 'işlem açılmıyor'}")
        except Exception as e:
            print(f"[FEE] TP5 parse hatasi: {e}")
            notu = f"TP5 parse hatası"

        if not hedge_fallback:
            return None, notu + ", işlem açılmıyor"
        # hedge_fallback=True → TP1-TP4'e devam et

    # TP5 yoksa eski sırayla dene: TP1→TP2→TP3→TP4
    tp_listesi = [(tp1, "TP1"), (tp2, "TP2"), (tp3, "TP3"), (tp4, "TP4")]
...
def mexc_update_tpsl(symbol, is_long, tp1, sl):
    ...
    try:
        ...
        # TP yön kontrolü: LONG için tp > avg_price, SHORT için tp < avg_price
        if tp1 and avg_price > 0:
            tp1_f = float(str(tp1).replace(",", "."))
            if is_long and tp1_f <= avg_price:
                print(f"[MEXC TPSL] UYARI: LONG TP ({tp1_f}) <= giriş ({avg_price}) — TP iptal edildi")
                tp1 = None
            elif not is_long and tp1_f >= avg_price:
                print(f"[MEXC TPSL] UYARI: SHORT TP ({tp1_f}) >= giriş ({avg_price}) — TP iptal edildi")
                tp1 = None
...
def _pnl_islem_kaydet(sinyal_kayit, tp1_ok, tp2_ok, tp3_ok, tp4_ok, tp5_ok, sl_ok):
    ...
    try:
        ...
        if oim <= 0:
            oim = float(os.getenv("MEXC_MARGIN_USDT", "10"))
        ...
        if mexc_acildi and MEXC_NOTIFY_CHAT_ID:
            ...
            try:
                ...
                if sinyal_kayit:
                    ...
                    leverage = float(sinyal_kayit.get("mexc_leverage") or 1)
                    ...
                    if giris > 0 and cikis > 0:
                        ...
                        if oim_c <= 0:
                            oim_c = float(os.getenv("MEXC_MARGIN_USDT","10"))
    ...
    if AUTO_TRADE_ENABLED:
        try:
            ...
            if is_trade:
                ...
                if not is_futures:
                ...
                elif not top3_whitelist:
                    # Hiçbir coin %50+ başarı oranına ulaşmamış — işlem açma
                    # Ama ters yön açık pozisyon varsa hedge izni ver
                    _mevcut_wl = pozisyon_al(sym_key, is_long) or pozisyon_al(sym_key, not is_long)
                    _hedge_wl  = _mevcut_wl and (_mevcut_wl["is_long"] != is_long)
                    if not _hedge_wl:
                        print(f"[MEXC] {timeframe}dk sinyal, {sym_key} atlandı — whitelist bos (%50+ yok)")
                elif sym_key.replace("USDT","") not in top3_whitelist:
                    # %50+ başarı listesinde değilse atla — ama hedge ise izin ver
                    _mevcut_wl = pozisyon_al(sym_key, is_long) or pozisyon_al(sym_key, not is_long)
                    _hedge_wl  = _mevcut_wl and (_mevcut_wl["is_long"] != is_long)
                    if not _hedge_wl:
                    ...
                    if mevcut:
                        ...
                        if mevcut_long != is_long:
                            ...
                            # Hedge için TP5 zorunlu değil — geçemezse TP1-TP4'e fallback
...
def _liq_heatmap_gercek(symbol, saat=6):
    ...
    try:
        ...
        # Long likidasyon = giriş * (1 - 1/kaldıraç)  → fiyatın ALTINDA
        # Short likidasyon = giriş * (1 + 1/kaldıraç) → fiyatın ÜSTÜNDE
        kaldıraçlar = [
            (10,  0.30),   # 10x  — en yaygın, ağır ağırlık
            (25,  0.25),   # 25x
            (50,  0.20),   # 50x
            (100, 0.15),   # 100x
            (5,   0.10),   # 5x   — muhafazakar
        ]
        ...
        for i, k in enumerate(klines):
            ...
            k_high   = float(k[2])
            k_low    = float(k[3])
            k_usd    = float(k[5]) * k_close  # mum USD hacmi
            yukari   = k_close > k_open
            mum_boy  = abs(k_close - k_open)

            for lev, agirlik in kaldıraçlar:
                # Long pozisyon giriş → likidasyon fiyatı aşağıda
                long_lik = k_open * (1 - 1 / lev)
                # Short pozisyon giriş → likidasyon fiyatı yukarıda
                short_lik = k_open * (1 + 1 / lev)

                # Ağırlık: hacim * kaldıraç ağırlığı * yön katsayısı
                long_w  = k_usd * agirlik * (1.3 if not yukari else 0.7)
                short_w = k_usd * agirlik * (1.3 if yukari else 0.7)

                # Long likidasyon bölgesi (price_min-price_max içindeyse yaz)
                if price_min < long_lik < price_max:
                    p_bin = int((long_lik - price_min) / (price_max - price_min) * n_pb)
                    p_bin = max(0, min(n_pb - 1, p_bin))
                    grid[p_bin, t_bin] += long_w

                # Short likidasyon bölgesi
                if price_min < short_lik < price_max:
                    p_bin = int((short_lik - price_min) / (price_max - price_min) * n_pb)
                    p_bin = max(0, min(n_pb - 1, p_bin))
                    grid[p_bin, t_bin] += short_w

            # Yüksek hacimli mumlar = birikmiş pozisyonlar → kuvvetlendir
            if k_usd > np.mean(volumes) * 1.5:
                # Tüm kaldıraç seviyelerine ekstra baskı
                for lev, _ in kaldıraçlar:
                    for entry in [k_open, k_close, k_high, k_low]:
                        for lik in [entry * (1 - 1/lev), entry * (1 + 1/lev)]:
                            if price_min < lik < price_max:
                                p_bin = int((lik - price_min) / (price_max - price_min) * n_pb)
                                p_bin = max(0, min(n_pb - 1, p_bin))
                                grid[p_bin, t_bin] += k_usd * 0.05
        ...
        if len(events) >= 5:
            for ts_ms, price, usd, side in events:
                ...
                if price_min < price < price_max:
                    ...
                    p_bin = max(0, min(n_pb - 1, p_bin))
                    grid[p_bin, t_bin] += usd * 3  # gerçek veriyi kuvvetlendir

        # 5) Yumuşat — küçük sigma = net bölgeler
        grid = gaussian_filter(grid, sigma=[0.8, 0.5])

        # Normalize — power scale ile kontrast artır
        if grid.max() > 0:
            grid = grid / grid.max()
            grid = np.power(grid, 0.4)  # düşük değerleri bastır, yüksekleri öne çıkar
        ...
        # ── Dolar birikim etiketleri — yoğun bölgelerde ──
        try:
            band_usd = np.zeros(n_pb)
            ...
            for ts_ms, ev_price, ev_usd, ev_side in events:
                if price_min < ev_price < price_max:
                    ...
                    p_bin = max(0, min(n_pb - 1, p_bin))
                    band_usd[p_bin] += ev_usd
            esik = band_usd.max() * 0.40
            etiket_konumlar = []
            i_e = 0
            while i_e < n_pb:
                if band_usd[i_e] >= esik:
                    j_e = i_e
                    while j_e < n_pb and band_usd[j_e] >= esik:
                        j_e += 1
                    merkez_bin   = (i_e + j_e) // 2
                    toplam_usd   = band_usd[i_e:j_e].sum()
                    merkez_fiyat = price_min + (merkez_bin / n_pb) * (price_max - price_min)
                    etiket_konumlar.append((merkez_fiyat, toplam_usd))
                    i_e = j_e
                else:
                    i_e += 1
            for ep_fiyat, ep_usd in etiket_konumlar[:6]:
                if ep_usd >= 1e9:
                ...
                renk = "#FFD700" if abs(ep_fiyat - fiyat) / fiyat < 0.02 else "#E0E0E0"
                ax.annotate(
                    usd_str,
                    xy=(0.01, ep_fiyat),
                    xycoords=("axes fraction", "data"),
                    color=renk, fontsize=6, fontweight="bold",
                    va="center",
                    bbox=dict(boxstyle="round,pad=0.15", facecolor="#050A14", alpha=0.75, edgecolor="none")
                )
        ...
        ax.set_title(
            f"BEN KUL YUTMAM  |  {sym_kisa}/USDT  Likidasyon Isi Haritasi\n"
...
def _analiz_olta_seviyeleri(sym, veri):
    ...
    # ── Short Giriş ──
    # Mutlaka fiyatın ÜSTÜNDE bir direnç seviyesi
    direnc_ust_short = [r for r in (direnc or []) if r and r > fiyat]
    direnc_ust_short.sort()
    if direnc_ust_short:
        short_ref = direnc_ust_short[0]  # en yakın direnç
    else:
        short_ref = high_24h if high_24h > fiyat else fiyat * 1.015

    short_giris = round(short_ref * 0.999, dec)
    short_stop  = round(short_ref * 1.012, dec)

    # Short TP = fiyatın altındaki en yakın destek
    destek_alt = [d for d in (destek or []) if d and d < fiyat]
...
BOT_VERSIYON = "v377"
print(f"[BASLANGIC] ========== BOT VERSIYON: v377 ==========")
