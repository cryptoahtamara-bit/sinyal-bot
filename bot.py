...
def tp_kontrol_gonder(symbol, sinyal, timeframe, tp1, tp2, tp3, tp4, tp5, sl,
    ...
    def bildirim_gonder(msg):
        # Sadece bot tarafından açılan işlemlerde bildirim gönder
        mexc_acildi = False
        mexc_msg_id = None
        with gunluk_kilit:
            for s in gunluk_sinyaller:
        ...
        if not mexc_acildi:
            print(f"[TP] {symbol} — bot açmadı, bildirim atlandı.")
            return
                                ...
                                with gunluk_kilit:
                                    for s in reversed(gunluk_sinyaller):
                                        if s.get("symbol") == symbol and not s.get("mexc_acildi"):
                                            ...
                                            s["mexc_acildi"] = True
                                            print(f"[MEXC] mexc_acildi=True set edildi: {symbol} msg_id={s.get('message_id')}")
                                            break
                            elif sonuc:
                                mexc_notify(symbol, sinyal, hata_msg=sonuc["msg"])
        except Exception as e:
