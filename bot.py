...
def _liq_gonder():
    """BTC ve ETH için likidisyon baskı haritası gönder + yorum. Her :00'da çalışır."""
    ...
    for sym in ["BTCUSDT", "ETHUSDT"]:
        try:
            ...
            _telegram_topic_mesaj_gonder(TOPIC_ANALIZ, yorum_raw)
            print(f"[LIQ] {sym} gonderildi.")
            time.sleep(3)
        except Exception as e:
...
def _liq_zamanlayici():
    """Her saatin :00'ında likidisyon baskı haritası + yorum gönder. (v281 — 15dk döngü :00)"""
    import datetime as dt_mod
    print("[LIQ] Zamanlayici basladi. Her saat :00'da tetiklenecek.")
    while True:
        try:
            if TR_TZ:
                simdi = datetime.now(tz=TR_TZ)
...
def _likidasyon_cek(symbol, limit=50):
    """
    Son 30 dakikadaki zorunlu likidasyon emirlerini çek.
    Dönen: {"long_usd": float, "short_usd": float, "toplam_usd": float, "agirlik": str}
    """
    try:
        r = requests.get(
            "https://fapi.binance.com/fapi/v1/forceOrders",
            params={"symbol": symbol, "autoCloseType": "LIQUIDATION", "limit": limit},
            timeout=6, proxies=BINANCE_PROXY
        )
        if r.status_code != 200:
            return None
        data = r.json()
        if not data:
            return {"long_usd": 0, "short_usd": 0, "toplam_usd": 0, "agirlik": "yok"}

        # Son 30 dakika filtresi
        if TR_TZ:
            simdi_ms = int(datetime.now(tz=TR_TZ).timestamp() * 1000)
        else:
            simdi_ms = int(datetime.utcnow().timestamp() * 1000)
        limit_ms = simdi_ms - 30 * 60 * 1000
        ...
        for o in data:
            ...
            if t < limit_ms:
                continue
            deger = float(o.get("origQty", 0)) * float(o.get("price", 0))
            # SELL = long likidasyon, BUY = short likidasyon
            if o.get("side") == "SELL":
                long_usd  += deger
            else:
                short_usd += deger
    ...
    def _tek_coin(sym):
        ...
        try:
            ...
            try:
                ...
                spt_h = float(rs2.json().get("quoteVolume", 0)) if rs2.status_code == 200 else 0
...
def _analiz_gorsel(veriler, zaman_str):
    ...
    try:
        ...
        # Figür boyutu — içerik: başlık~1.3 + kartlar~1.2 + tablo~7.6 + olta~4.2 + legend~0.5 = ~14.8
        fig_w = 13.0
        fig_h = 16.2
        ...
        ax.text(fig_w / 2, y, "GENİŞLETİLMİŞ PİYASA ANALİZİ",
                ha="center", va="top", fontsize=19, color=WHITE, fontweight="bold")
        y -= 0.32
        ...
        for cx, lbl, ha in col_defs:
            ax.text(cx, y, lbl, ha=ha, va="top", fontsize=10.5, color=WHITE, fontweight="bold")
        y -= 0.16
        ...
        for i, sym in enumerate(SEMBOLLER):
            ...
            ax.text(COL["coin"], my, sym_kisa, ha="left", va="center",
                    fontsize=13, color=sym_renk, fontweight="bold")
            ...
            ax.text(COL["d24"], my, f"{d24:+.1f}%" if d24 is not None else "—",
                    ha="right", va="center", fontsize=11, color=renk_pct(d24))
            ...
            if oi_d:
                ...
                ax.text(COL["oi"], my, f"{dp:+.1f}%", ha="right", va="center",
                        fontsize=11, color=renk_pct(dp))
            else:
                ax.text(COL["oi"], my, "—", ha="right", va="center", fontsize=11, color=GRAY)
            ...
            if taker:
                ...
                ax.text(COL["taker"], my, f"{bp:.0f}%B", ha="right", va="center",
                        fontsize=11, color=t_col)
            else:
                ax.text(COL["taker"], my, "—", ha="right", va="center", fontsize=11, color=GRAY)
            ...
            if cvd:
                ...
                ax.text(COL["cvd"], my, "—", ha="right", va="center", fontsize=11, color=GRAY)
            ...
            if lik and lik["toplam_usd"] > 0:
                ...
                ax.text(COL["lik"], my, lik_str, ha="right", va="center",
                        fontsize=10.5, color=l_col)
            else:
                ax.text(COL["lik"], my, "—", ha="right", va="center", fontsize=10.5, color=DGRAY)
            ...
            if hc and hc > 0:
                ...
                ax.text(COL["hac"], my, hc_str, ha="right", va="center",
                        fontsize=10.5, color=hc_col)
            else:
                ax.text(COL["hac"], my, "—", ha="right", va="center", fontsize=10.5, color=DGRAY)
...
def _analiz_gonder(chat_id=None):
    """Genişletilmiş Piyasa Analizi görselini + özet metnini TOPIC_ANALIZ'a gönder."""
...
def _analiz_zamanlayici():
    """Her saatin :00 ve :30'unda Genişletilmiş Piyasa Analizi gönder."""
    import datetime as dt_mod
    print("[ANALIZ] Zamanlayici basladi. Her saat :00 ve :30'da tetiklenecek.")
    while True:
        try:
            if TR_TZ:
                simdi = datetime.now(tz=TR_TZ)
