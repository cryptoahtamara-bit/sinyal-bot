...
def mexc_update_tpsl(symbol, is_long, tp1, sl):
    """
    Position TP/SL ekranından ayarla — /api/v1/private/position/change_tpsl
    Bu endpoint MEXC UI'daki 'Position TP/SL' butonuyla aynı işlevi görür.
    stoporder/place (Batches) değil, pozisyon bazlı doğrudan TP/SL atar.
    """
    sym = mexc_format_symbol(symbol)
    try:
        # 1) Açık pozisyonu bul — positionId gerekli
        r = mexc_private_get(f"{MEXC_BASE_URL}/api/v1/private/position/open_positions",
                             params={"symbol": sym})
        raw = r.json()
        print(f"[MEXC TPSL] open_positions raw: {raw}")
        pozlar = raw.get("data", [])
        hedef  = None
        for p in pozlar:
            if is_long and p.get("positionType") == 1:
                ...
                hedef = p; break
        if not hedef:
            print(f"[MEXC TPSL] Pozisyon bulunamadi — is_long={is_long} pozlar={pozlar}")
            return False
        pid = hedef.get("positionId") or hedef.get("id")
        print(f"[MEXC TPSL] Pozisyon bulundu: pid={pid}")

        # 2) Position TP/SL endpoint — change_tpsl (UI'daki 'Position TP/SL' ekranı)
        body = {"positionId": pid}
        if tp1:
            body["takeProfit"] = float(str(tp1).replace(",", "."))
        if sl:
            body["stopLoss"] = float(str(sl).replace(",", "."))

        print(f"[MEXC TPSL] change_tpsl body: {body}")
        r2 = mexc_private_post(
            f"{MEXC_BASE_URL}/api/v1/private/position/change_tpsl", body
        )
        print(f"[MEXC TPSL] change_tpsl yanit: {r2.text[:300]}")
        result = r2.json()

        # change_tpsl başarılıysa success:true döner; başarısızsa stoporder/place ile fallback
        if result.get("success"):
            print(f"[MEXC TPSL] Position TP/SL OK (change_tpsl)")
            return True

        # Fallback: change_tpsl desteklenmiyorsa stoporder/place (Batches) dene
        print(f"[MEXC TPSL] change_tpsl basarisiz, stoporder/place fallback deneniyor...")
        ...
        print(f"[MEXC TPSL] stoporder body: {body2}")
        r3 = mexc_private_post(f"{MEXC_BASE_URL}/api/v1/private/stoporder/place", body2)
        print(f"[MEXC TPSL] stoporder yanit: {r3.text[:300]}")
        return r3.json().get("success", False)
...
def _gunluk_pnl_gonder(hedef_chat_id=None):
    ...
    try:
        ...
        try:
            ...
            kart(fig.add_subplot(gs[0,2]), "Gerceklesen PNL", realized,
                 "#26a69a" if realized >= 0 else "#ef5350")
