...
def mexc_place_order(symbol, sinyal, tp1=None, sl=None, hedge=False):
    ...
    # TP/SL order body'sine eklenmiyor — order acildiktan sonra mexc_update_tpsl ile set edilecek
...
def mexc_update_tpsl(symbol, is_long, tp1, sl):
    """
    Position TP/SL — stoporder/place ile positionId + holdVol.
    MEXC UI'daki 'Position TP/SL' ekranıyla aynı davranış:
    positionId verince tüm pozisyona bağlı TP/SL atar (Batches değil).
    """
    sym = mexc_format_symbol(symbol)
    try:
        # 1) Açık pozisyonu bul — positionId ve holdVol gerekli
        ...
        print(f"[MEXC TPSL] pid={pid} vol={vol}")

        # 2) stoporder/place — positionId ile = Position TP/SL (Batches değil)
        body = {
            "symbol":        sym,
            "positionId":    pid,
            "vol":           vol,       # tüm pozisyon = Position TP/SL davranışı
            "executeCycle":  87600,     # GTC
            "trend":         2,         # 2=Last Price (1=Fair Price, 3=Index Price)
            "triggerType":   1,
        }
        if tp1:
            body["takeProfitPrice"] = float(str(tp1).replace(",", "."))
            body["profitTrend"]     = 2   # Last Price
        if sl:
            body["stopLossPrice"] = float(str(sl).replace(",", "."))
            body["lossTrend"]     = 2   # Last Price

        print(f"[MEXC TPSL] stoporder/place body: {body}")
        # openType=1 ile Position TP/SL modunda aç (Batches değil)
        body["openType"] = 1
        r2 = mexc_private_post(f"{MEXC_BASE_URL}/api/v1/private/stoporder/place", body)
        print(f"[MEXC TPSL] yanit: {r2.text[:300]}")
        ok = r2.json().get("success", False)
...
def _gunluk_pnl_gonder(hedef_chat_id=None):
    ...
    try:
        ...
        try:
            ...
            ax_pnl.text(0.28, 0.38, f"{kum_pnl_pct:+.2f}%", transform=ax_pnl.transAxes,
                        ha="center", fontsize=12, color=renk_pnl, fontweight="bold")
            ax_pnl.text(0.72, 0.38, f"{kum_pnl_usdt:+.2f}$", transform=ax_pnl.transAxes,
                        ha="center", fontsize=12, color=renk_pnl, fontweight="bold")
            ax_pnl.axvline(0.5, ymin=0.2, ymax=0.7, color="#30363d", linewidth=0.8)
        ...
        caption = (
            "📊 <b>PNL — " + zaman_str + "</b>\n"
            "💰 Bakiye: <b>$" + f"{bakiye:.2f}" + "</b>\n"
            "📦 Kullanilan: <b>$" + f"{kullanilan:.2f}" + "</b>\n"
            "✅ Gerceklesen: <b>$" + f"{realized:+.2f}" + "</b>\n"
            "📈 Acik Poz.: <b>$" + f"{unrealized:+.2f}" + "</b>"
        )
...
def _pnl_islem_kaydet(sinyal_kayit, tp1_ok, tp2_ok, tp3_ok, tp4_ok, tp5_ok, sl_ok):
    ...
    try:
        ...
        # USDT PNL — bakiyenin %10'u pozisyon varsayımı (kaldıraç bilinmiyor)
        # Gerçekçi bir yaklaşım: sinyal_kayit'ta lot/vol bilgisi yoksa sabit 1 USDT
        pnl_usdt = round(pnl_pct / 100 * 10, 4)  # 10 USDT nominal pozisyon
        ...
        print(f"[PNL KAYIT] {symbol} {tp_label} pnl={pnl_pct:+.2f}%")
    except Exception as e:
...
def tp_kontrol_gonder(symbol, sinyal, timeframe, tp1, tp2, tp3, tp4, tp5, sl,
    ...
    def bildirim_gonder(msg):
        # TOPIC_ALARM'daki sinyal mesajına reply
        _telegram_topic_mesaj_gonder(TOPIC_ALARM, msg, reply_to=message_id)
        # MEXC_NOTIFY_CHAT_ID'deki işlem açılış mesajına reply
        mexc_msg_id = None
        with gunluk_kilit:
            for s in gunluk_sinyaller:
                if s.get("message_id") == message_id:
                    ...
                    break
        if mexc_msg_id and MEXC_NOTIFY_CHAT_ID:
            _telegram_mesaj_gonder(MEXC_NOTIFY_CHAT_ID, msg,
                                   reply_to=mexc_msg_id, parse_mode="HTML")
        print(f"[TP] {symbol} bildirim gonderildi. mexc_msg_id={mexc_msg_id}")
        if TELEGRAM_LOG_ID:
                    ...
                    if mevcut:
                        ...
                        if mevcut_long != is_long:
                            ...
                            if tp1_hedge is None:
                                ...
                                if sonuc and sonuc["success"]:
                                    ...
                                    mexc_notify(symbol, sinyal, hata_msg=sonuc["msg"])
                                mexc_notify(symbol, sinyal, hata_msg=sonuc["msg"])
                        ...
                        if tp1_yeni is None:
                            ...
                            if sonuc and sonuc["success"]:
                                ...
                                # mexc_msg_id'yi sinyal kaydına ekle — TP/SL reply için
                                with gunluk_kilit:
                                    for s in gunluk_sinyaller:
                                        if s.get("symbol") == symbol and not s.get("mexc_msg_id"):
...
def _pnl_zamanlayici():
    """Her saat başında MEXC_NOTIFY kanalına otomatik PNL gönder."""
    while True:
        try:
            simdi = datetime.now(tz=TR_TZ) if TR_TZ else datetime.utcnow()
            # Bir sonraki saat başına kadar bekle
            dakika_kalan = 60 - simdi.minute
            saniye_kalan = dakika_kalan * 60 - simdi.second
            time.sleep(max(saniye_kalan, 60))
            if MEXC_API_KEY and MEXC_API_SECRET:
                print("[PNL] Saatlik otomatik PNL gonderiliyor...")
                _gunluk_pnl_gonder(None)
        except Exception as e:
            print(f"[PNL ZAMANLAYICI] Hata: {e}")
            time.sleep(60)
