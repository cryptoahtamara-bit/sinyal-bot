...
def mexc_test():
    ...
    sonuc = {"version": "v188", "proxy": bool(MEXC_PRIVATE_PROXY)}
...
def webkey_test():
    ...
    # Test 4: Body'yi farkli sekillerde gonder
    if MEXC_API_KEY and MEXC_API_SECRET:
        test_body = {"symbol": "BTC_USDT", "price": 0, "vol": 0.001, "side": 1, "type": 5, "openType": 2, "leverage": 20}

        # Yontem 1: Body yok, query string olarak gonder
        try:
            hdrs1, _ = mexc_headers(None)
            r1 = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
                               headers=hdrs1, params=test_body, timeout=8)
            sonuc["body_query_string"] = {"status": r1.status_code, "body": r1.text[:200]}
        except Exception as e:
            sonuc["body_query_string"] = {"error": str(e)[:50]}

        # Yontem 2: Content-Length: 0 ile body gonder
        try:
            hdrs2, _ = mexc_headers(test_body)
            hdrs2["Content-Length"] = "0"
            r2 = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
                               headers=hdrs2, data=b"", timeout=8)
            sonuc["content_length_0"] = {"status": r2.status_code, "body": r2.text[:200]}
        except Exception as e:
            sonuc["content_length_0"] = {"error": str(e)[:50]}

        # Yontem 3: POST degil PUT dene
        try:
            hdrs3, _ = mexc_headers(test_body)
            r3 = requests.put(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
                              headers=hdrs3, json=test_body, timeout=8)
            sonuc["put_method"] = {"status": r3.status_code, "body": r3.text[:200]}
        except Exception as e:
            sonuc["put_method"] = {"error": str(e)[:50]}

        # Yontem 4: POST ama body'yi custom header'a koy
        try:
            hdrs4, _ = mexc_headers(None)
            hdrs4["X-Order-Data"] = json.dumps(test_body, separators=(",",":"))
            r4 = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
                               headers=hdrs4, timeout=8)
            sonuc["body_in_header"] = {"status": r4.status_code, "body": r4.text[:200]}
        except Exception as e:
            sonuc["body_in_header"] = {"error": str(e)[:50]}
...
print(f"[BASLANGIC] ========== BOT VERSIYON: v188 ==========")
