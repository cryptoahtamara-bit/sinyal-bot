...
def _analiz_gorsel(veriler, zaman_str):
    ...
    try:
        ...
        COL = {
            "coin":   0.20,
            "fiyat":  2.40,   # DOGE gibi uzun isimlerin üst üste binmemesi için
            "d24":    3.65,
            "skor":   4.75,
            "rsi":    5.75,
            "fund":   6.85,
        ...
        panel_h = 3.60  # LONG(4 satır) + SHORT(3 satır) + başlık + ayraç
        for pi, sym in enumerate(["BTCUSDT", "ETHUSDT", "SOLUSDT"]):
            ...
            if nota:
                ax.text(px + panel_w/2, y - 0.40, nota[:60],
                        ha="center", va="top", fontsize=8, color=ORANGE)

            ax.axhline(y - 0.50, xmin=px/fig_w + 0.01, xmax=(px+panel_w)/fig_w - 0.01,
            ...
            def satir(label, val1, val2=None, renk=WHITE):
                nonlocal ly
                ax.text(px + 0.18, ly, label, ha="left", va="top", fontsize=9.5, color=WHITE)
                vstr = fmt_fiyat(val1)
                if val2:
                    vstr += f" – {fmt_fiyat(val2)}"
                ax.text(px + panel_w - 0.14, ly, vstr, ha="right", va="top",
                        fontsize=10.5, color=renk, fontweight="bold")
                ly -= 0.35
            ...
            satir("Stop:",  olta["long_stop"],  RED)
            satir("TP1:",   olta["long_tp1"],   LGREEN)
            satir("TP2:",   olta["long_tp2"],   LGREEN)

            ax.axhline(ly + 0.08, xmin=px/fig_w + 0.02, xmax=(px+panel_w)/fig_w - 0.02,
            ...
            ly -= 0.30
            satir("Giriş:", olta["short_giris"], RED)
            satir("Stop:",  olta["short_stop"],  ORANGE)
            satir("TP:",    olta["short_tp"],    RED)
