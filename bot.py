        ...
        try:
            ...
            if isinstance(_pos_data, list) and _pos_data:
                for _p in _pos_data:
                    ...
                    unrealized     += _pct * _im
                    unrealized_pct += _pct * 100
                print(f"[PNL DEBUG] unrealized={unrealized:.4f} USDT | {unrealized_pct:.2f}% ({len(_pos_data)} poz)")
            else:
                ...
                if not is_futures:
                    ...
                    if not mevcut and tp1_gecildi:
                        ...
                        if mevcut_long != is_long:
                            # SENARYO 2: Ters yön — HEDGE
                            # Zaten açık HEDGE varsa yeni açma — revize et
                            if _acik_hedge and _acik_hedge.get("is_long") == is_long:
                                # Mevcut hedge aynı yönde — Senaryo 1/4 mantığıyla revize et
                                tf_hedge = TF_SIR.get(str(_acik_hedge.get("timeframe","1")), 0)
                                ...
                                pass  # Devam — aşağıda SENARYO 2 açılır
                        if mevcut_long != is_long and not (_acik_hedge and _acik_hedge.get("is_long") == is_long):
...
BOT_VERSIYON = "v406"
print(f"[BASLANGIC] ========== BOT VERSIYON: v406 ==========")
        ...
        try:
            ...
            if isinstance(_pos_data2, list) and _pos_data2:
                for _p2 in _pos_data2:
                    ...
                    unrealized_pct += _pct2 * 100
                print(f"[PNL RAPOR] unrealized={unrealized:.4f} USDT | {unrealized_pct:.2f}% ({len(_pos_data2)} poz)")
...
def _pnl_rapor_zamanlayici():
    """Her saatin :15'inde PNL raporu gönderir.
    23:15 raporundan sonra 00:00'da günlük kayıtlar sıfırlanır.
    """
    import datetime as dt_mod
    RAPOR_DAKIKA = 15  # Her saatin kaçıncı dakikasında
    print(f"[PNL RAPOR] Zamanlayici basladi. Her saatin :{RAPOR_DAKIKA:02d}'inde rapor gonderilecek.")
    while True:
        try:
            simdi = datetime.now(tz=TR_TZ) if TR_TZ else datetime.utcnow()

            # Sonraki :15'i hesapla
            if simdi.minute < RAPOR_DAKIKA:
                sonraki = simdi.replace(minute=RAPOR_DAKIKA, second=0, microsecond=0)
            else:
                sonraki = (simdi.replace(minute=RAPOR_DAKIKA, second=0, microsecond=0)
                           + dt_mod.timedelta(hours=1))
            ...
            # Gece 00:15'te günlük kayıtları sıfırla
            simdi_yeni = datetime.now(tz=TR_TZ) if TR_TZ else datetime.utcnow()
            if simdi_yeni.hour == 0 and simdi_yeni.minute <= 20:
                with _pnl_kilit:
                    _pnl_kayitlar.clear()
                    _pnl_baslangic_bak.clear()
                print("[PNL RAPOR] Gece 00:15 — PNL kayitlari sifirlandi.")
...
print("[PNL RAPOR] Her saatin :15'inde rapor zamanlayicisi basladi.")
