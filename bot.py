...
#   4. mexc_place_order() olü kod blogu temizlendi (duplicate imza+order)
#   5. mexc_update_tpsl() ve mexc_pozisyon_sorgula() mexc_headers() ile eslesti
#   6. _gunluk_pnl_gonder() manuel imza kaldirip mexc_headers() kullanacak sekilde duzeltildi
...
# Proxy devre disi — Railway'den direkt MEXC baglantisi (bot_v171)
MEXC_PROXY = None
...
def mexc_update_tpsl(symbol, is_long, tp1, sl):
    ...
    try:
        ...
        r2 = requests.post(f"{MEXC_BASE_URL}/api/v1/private/position/change_tpsl",
                           headers=hdrs2, json=body, timeout=10)
...
def mexc_test():
    ...
    sonuc = {"version": "v172", "proxy": str(MEXC_PROXY)}
    # 1. Public endpoint — imza yok, sadece baglanti testi
    try:
    ...
    # 2. Private endpoint — imza ile
    if MEXC_API_KEY and MEXC_API_SECRET:
        try:
            hdrs, ts = mexc_headers(None)
            r2 = requests.get(f"{MEXC_BASE_URL}/api/v1/private/account/assets",
                              headers=hdrs, timeout=10)
            body = r2.json()
