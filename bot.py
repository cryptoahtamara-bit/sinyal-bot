    ...
    try:
        ...
        if tip == "ls":
            ...
            for sym in ["BTCUSDT", "ETHUSDT"]:
                ...
                if heatmap:
                    _topic_foto_gonder_filigranli(TOPIC_ANALIZ, heatmap,
...
def _liq_veri_cek(symbol):
    ...
    try:
        ...
        # 8) Likidasyon bölgesi — ısı haritasıyla aynı grid mantığı (15m klines)
        liq_long_baskisi  = None
        liq_short_baskisi = None
        try:
            import numpy as np
            r_liq = requests.get("https://fapi.binance.com/fapi/v1/klines",
                                 params={"symbol": sym, "interval": "15m", "limit": 34},
                                 timeout=8, proxies=BINANCE_PROXY)
            klines_15m_liq = r_liq.json() if r_liq.status_code == 200 else []
            if klines_15m_liq and len(klines_15m_liq) >= 5 and fiyat > 0:
                highs_l = [float(k[2]) for k in klines_15m_liq]
                lows_l  = [float(k[3]) for k in klines_15m_liq]
                price_range = fiyat * 0.06
                price_min = min(min(lows_l), fiyat - price_range) * 0.999
                price_max = max(max(highs_l), fiyat + price_range) * 1.001
                n_pb = 120
                grid_long  = np.zeros(n_pb)
                grid_short = np.zeros(n_pb)
                kaldıraçlar_hm = [(10,0.30),(25,0.25),(50,0.20),(100,0.15),(5,0.10)]
                for k in klines_15m_liq:
                    k_open  = float(k[1])
                    k_close = float(k[4])
                    k_usd   = float(k[5]) * k_close
                    yukari  = k_close > k_open
                    for lev, agirlik in kaldıraçlar_hm:
                        ll = k_open * (1 - 1/lev)
                        sl = k_open * (1 + 1/lev)
                        lw = k_usd * agirlik * (1.3 if not yukari else 0.7)
                        sw = k_usd * agirlik * (1.3 if yukari else 0.7)
                        if price_min < ll < price_max:
                            p = int((ll - price_min) / (price_max - price_min) * n_pb)
                            grid_long[max(0, min(n_pb-1, p))] += lw
                        if price_min < sl < price_max:
                            p = int((sl - price_min) / (price_max - price_min) * n_pb)
                            grid_short[max(0, min(n_pb-1, p))] += sw
                fiyat_bin = int((fiyat - price_min) / (price_max - price_min) * n_pb)
                fiyat_bin = max(1, min(n_pb-2, fiyat_bin))
                def _grid_bolge(grid, alt, ust, price_min, price_max, n_pb, top_n=10):
                    adaylar = []
                    fiyat_orta = price_min + (alt + ust) / 2 / n_pb * (price_max - price_min)
                    for i in range(alt, ust):
                        if grid[i] > 0:
                            p = price_min + (i + 0.5) / n_pb * (price_max - price_min)
                            # Fiyat merkezine yakın bantları ağırlıklandır
                            uzaklik = abs(p - fiyat_orta) / max(fiyat_orta, 1)
                            skor = grid[i] * (1 + max(0, 0.5 - uzaklik * 5))
                            adaylar.append((skor, p))
                    if not adaylar: return None
                    adaylar.sort(reverse=True)
                    sevs = sorted([p for _, p in adaylar[:top_n]])
                    if not sevs: return None
                    # Birbirine en yakın %3 içindeki küme
                    en_iyi = sevs[:2] if len(sevs) >= 2 else sevs
                    en_aralik = sevs[-1] - sevs[0]
                    for i in range(len(sevs)):
                        for j in range(i+2, len(sevs)+1):
                            kume = sevs[i:j]
                            aralik = kume[-1] - kume[0]
                            pct = aralik / kume[0]
                            if pct <= 0.03 and aralik < en_aralik:
                                en_aralik = aralik
                                en_iyi = kume
                    return (round(min(en_iyi), 2), round(max(en_iyi), 2))
                liq_long_baskisi  = _grid_bolge(grid_long,  0,         fiyat_bin, price_min, price_max, n_pb)
                liq_short_baskisi = _grid_bolge(grid_short, fiyat_bin, n_pb,      price_min, price_max, n_pb)
...
def _liq_yorum(veri):
    """Likidasyon baskı yorumu — likidasyon bölgeleri, destek/direnç, risk değerlendirmesi."""
    fiyat     = veri["fiyat"]
    high_24h  = veri["high_24h"]
    low_24h   = veri["low_24h"]
    ...
    if destek and direnc:
        if liq_long:
            ...
            liq_long_alt = destek[-1]
            liq_long_ust = destek[0]
        ...
        if liq_short:
            ...
            liq_short_alt = direnc[0]
            liq_short_ust = direnc[-1]
        else:
    ...
    if destek and direnc and fiyat:
        ...
        uzaklik_long  = abs(fiyat - liq_long_ust)  / fiyat * 100 if liq_long else None
        uzaklik_short = abs(fiyat - liq_short_alt) / fiyat * 100 if liq_short else None
...
BOT_VERSIYON = "v501"
print(f"[BASLANGIC] ========== BOT VERSIYON: v501 ==========")
