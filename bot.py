...
def _tp_filtre_guncelle():
    ...
    try:
        ...
        for st, veri_st in sinyal_sonuclari.items():
            ...
            if not en_iyi:
                # Yeterli veri yok — bu sinyal tipi için filtre boş kalır (sinyal geçmez)
                continue
            best      = en_iyi[0]
            kosul_str = best["kombinasyon"]
            tp_yuzde  = float(str(best["tp_orani"]).replace("%", ""))
            # OI koşulunu filtreden çıkar — OI verisi tutarsız geliyor, sinyal sayısını düşürüyor
            kosul_parts_ham = [p.strip() for p in kosul_str.split("+")]
            kosul_parts_temiz = [p for p in kosul_parts_ham if "oi" not in p.lower()]
            yeni_filtreler[st] = {
                "kosul_parts": kosul_parts_temiz,
                "tp_yuzde"   : tp_yuzde,
            }
