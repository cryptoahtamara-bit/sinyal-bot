...
def mexc_place_order(symbol, sinyal, tp1=None, sl=None):
    ...
    body = {"symbol": sym, "price": 0, "vol": vol, "side": side, "type": 5, "openType": 1, "leverage": lev}
    # TP/SL: long icin SL < mp < TP, short icin TP < mp < SL olmali
    # Minimum %0.1 fark kontrolu
    min_diff = mp * 0.001
    if tp1:
        tp1_f = float(str(tp1).replace(",", "."))
        if is_long and tp1_f > mp + min_diff:
            body["takeProfitPrice"] = tp1_f
            body["takeProfitPriceType"] = 1
        elif not is_long and tp1_f < mp - min_diff:
            body["takeProfitPrice"] = tp1_f
            body["takeProfitPriceType"] = 1
    if sl:
        sl_f = float(str(sl).replace(",", "."))
        if is_long and sl_f < mp - min_diff:
            body["stopLossPrice"] = sl_f
            body["stopLossPriceType"] = 1
        elif not is_long and sl_f > mp + min_diff:
            body["stopLossPrice"] = sl_f
            body["stopLossPriceType"] = 1
...
def mexc_update_tpsl(symbol, is_long, tp1, sl):
    ...
    try:
        ...
        body = {"positionId": pid, "symbol": sym, "lossTrend": 1, "profitTrend": 1}
        if tp1:
            body["takeProfitPrice"]    = float(str(tp1).replace(",", "."))
            body["takeProfitPriceType"]= 2  # Fair Price
        if sl:
            body["stopLossPrice"]     = float(str(sl).replace(",", "."))
            body["stopLossPriceType"] = 2  # Fair Price
        print(f"[MEXC TPSL] change_tpsl body: {body}")
        r2 = mexc_private_post(f"{MEXC_BASE_URL}/api/v1/private/planorder/tpsl/place", body)
        print(f"[MEXC TPSL] tpsl/place yanit: {r2.text[:200]}")
        if not r2.json().get("success"):
            r2 = mexc_private_post(f"{MEXC_BASE_URL}/api/v1/private/plan/place-tp-sl", body)
            print(f"[MEXC TPSL] plan/place-tp-sl yanit: {r2.text[:200]}")
...
BOT_VERSIYON = "v270"
print(f"[BASLANGIC] ========== BOT VERSIYON: v272 ==========")
