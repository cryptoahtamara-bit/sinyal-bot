...
def _tp_kosul_yorum_uret(veri):
    """TP olan işlemlerde piyasa koşulları nasıldı — confluance odaklı yorum."""
    MIN_KOMB  = 20
    MIN_KOSUL = 5
    maddeler = []
    ...
    # 1. En başarılı kombinasyonlar — min 20 sinyal, güvenilir olanlar
    ...
    # 2. Confluance analizi — en iyi kombinasyonla örtüşen piyasa koşulları
    kosul_veri = {k: v for k, v in veri["kosul"].items()
                  if v[0] >= MIN_KOSUL and oran_val(v) >= 0 and "Seans" not in k}
    if kosul_veri and komb_veri:
        en_iyi_kosul = max(kosul_veri.items(), key=lambda x: oran_val(x[1]))
        en_iyi_komb  = sirali[0]

        maddeler.append(
            f"🔗 En başarılı kombinasyon + piyasa koşulu çakışması: "
            f"<b>{en_iyi_komb[0]}</b> ve <b>{en_iyi_kosul[0]}</b> "
            f"aynı anda aktif olduğunda TP olasılığı en yüksek "
            f"({en_iyi_komb[1][4]} × {en_iyi_kosul[1][4]})"
        )

        # Funding etkisi
        fund_pos = kosul_veri.get("Funding — Pozitif")
        fund_neg = kosul_veri.get("Funding — Negatif")
        if fund_pos and fund_neg:
            fark = round(oran_val(fund_pos) - oran_val(fund_neg), 1)
            if abs(fark) >= 3:
                daha_iyi = "Pozitif" if fark > 0 else "Negatif"
                maddeler.append(
                    f"💰 Funding etkisi: Funding <b>{daha_iyi}</b> ortamda TP oranı "
                    f"<b>{abs(fark)} puan</b> daha yüksek "
        ...
        if piy_yuk and piy_dus and oran_val(piy_yuk) >= 0 and oran_val(piy_dus) >= 0:
            ...
            if abs(fark) >= 3:
                ...
                maddeler.append(
                    f"📊 Piyasa yönü etkisi: <b>{daha_iyi}</b> trendinde TP oranı "
                    f"<b>{abs(fark)} puan</b> daha yüksek "
        ...
        if fg_korku and fg_acgoz and oran_val(fg_korku) >= 0 and oran_val(fg_acgoz) >= 0:
            ...
            if abs(fark) >= 3:
                ...
                maddeler.append(
                    f"😱 F&G etkisi: <b>{daha_iyi}</b> ortamında TP oranı daha yüksek "
                    f"({fg_korku[4]} vs {fg_acgoz[4]})"
                )
    elif kosul_veri:
        en_iyi_kosul = max(kosul_veri.items(), key=lambda x: oran_val(x[1]))
        maddeler.append(f"📈 En başarılı piyasa koşulu: <b>{en_iyi_kosul[0]}</b> → {en_iyi_kosul[1][4]} ({en_iyi_kosul[1][0]} sinyal)")
    else:
