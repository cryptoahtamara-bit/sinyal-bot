...
def send_telegram_and_schedule_tp(caption, symbol, timeframe, sinyal,
    ...
    if filtre:
        ...
        with gunluk_kilit:
            son_kayit = next(
                (s for s in reversed(gunluk_sinyaller)
                 if s.get("symbol") == symbol and s.get("sinyal_tipi") == sinyal),
                None
            )
        if son_kayit:
            tf_raw = son_kayit.get("timeframe")
            tf_str = _TF_MAP_FIL.get(str(tf_raw), str(tf_raw)) if tf_raw else None
            if tf_str:  mevcut_kosullar.add(_norm(tf_str))
            piy  = son_kayit.get("piyasa_yonu")
            if piy:   mevcut_kosullar.add(_norm(piy))
            fund = son_kayit.get("funding_yonu")
            if fund:  mevcut_kosullar.add(_norm(fund))
            oi   = son_kayit.get("oi_yonu")
            if oi:    mevcut_kosullar.add(_norm("OI " + oi))
            fg   = _fg_kat_fil(son_kayit.get("fg_deger"))
            if fg:    mevcut_kosullar.add(_norm(fg))
        ...
        oi_veri_var  = bool(son_kayit and son_kayit.get("oi_yonu"))
