...
def send_telegram_and_schedule_tp(caption, symbol, timeframe, sinyal,
    ...
    try:
        ...
        MIN_SINYAL = 10  # etiket verilmesi için gereken minimum kapalı sinyal
        ...
        if len(kapali) >= MIN_SINYAL:
            ...
            if len(combo_eslesen) >= MIN_SINYAL:
                ...
                # Kombinasyon için yeterli veri yok → Zayıf (veri yetersiz)
                filtre_notu = "🔴 Zayıf"
                print(f"[TP_FILTRE] {sinyal} {symbol} combo veri yetersiz ({len(combo_eslesen)}<{MIN_SINYAL})")
        else:
            # Sinyal tipi için henüz yeterli veri yok → etiket yok
            filtre_notu = None
            print(f"[TP_FILTRE] {sinyal} {symbol} etiket yok — {len(kapali)}<{MIN_SINYAL} kapali sinyal")
...
def koin_sirala_metni(snapshot=None):
    """Sinyal kalite raporu — filtre notu bazlı istatistik."""
    bugun = gun_str()
    if snapshot is not None:
        kayitlar = [s for s in snapshot if s.get("gun") == bugun]
    else:
    ...
    def _filtre_kat(s):
        ...
        if "g\u00fcçl\u00fc" in fn or "guclu" in fn: return "guclu"
        if "orta" in fn: return "orta"
        return "zayif"
    ...
    for s in kayitlar:
        ...
        if tp: filtre_istat[kat]["tp"] += 1
        if sym not in koin_filtre:
            koin_filtre[sym] = {
                "guclu": {"sin":0,"tp":0},
                "orta":  {"sin":0,"tp":0},
                "zayif": {"sin":0,"tp":0},
                "toplam": 0, "toplam_tp": 0
            }
        koin_filtre[sym][kat]["sin"] += 1
        koin_filtre[sym]["toplam"] += 1
        if tp:
            koin_filtre[sym][kat]["tp"] += 1
            koin_filtre[sym]["toplam_tp"] += 1

    koin_sirali = sorted(
        koin_filtre.items(),
        key=lambda x: (-(x[1]["toplam_tp"]/x[1]["toplam"]*100) if x[1]["toplam"] > 0 else 0,
                       -x[1]["toplam_tp"]),
    )
    ...
    satirlar = ["", "\ud83d\udcca <b>Sinyal Kalite Raporu (Bug\u00fcn)</b>"]
    g = filtre_istat["guclu"]; satirlar.append(f"\U0001f7e2 G\u00fc\u00e7l\u00fc \u2192 {g['tp']}/{g['sin']} %{_oran(g):.0f}")
    o = filtre_istat["orta"];  satirlar.append(f"\U0001f7e1 Orta  \u2192 {o['tp']}/{o['sin']} %{_oran(o):.0f}")
    z = filtre_istat["zayif"]; satirlar.append(f"\U0001f534 Zay\u0131f \u2192 {z['tp']}/{z['sin']} %{_oran(z):.0f}")
    satirlar.append("\n── Coin Bazlı ──")
    for sym, v in koin_sirali[:10]:
        genel_oran = round(v["toplam_tp"]/v["toplam"]*100, 0) if v["toplam"] > 0 else 0
        g2=v["guclu"]; o2=v["orta"]; z2=v["zayif"]
        g_str = f"🟢{g2['tp']}/{g2['sin']}" if g2["sin"] > 0 else "🟢—"
        o_str = f"🟡{o2['tp']}/{o2['sin']}" if o2["sin"] > 0 else "🟡—"
        z_str = f"🔴{z2['tp']}/{z2['sin']}" if z2["sin"] > 0 else "🔴—"
        aktif = "✅" if genel_oran >= TRADE_WHITELIST_MIN_PERCENT else "▪️"
        sym_pad = sym.ljust(5)
        satirlar.append(f"<code>{aktif}{sym_pad} {g_str} {o_str} {z_str} %{genel_oran:.0f}</code>")
    aktif_liste = [sym for sym, v in koin_sirali
                   if v["toplam"] > 0 and v["toplam_tp"]/v["toplam"]*100 >= TRADE_WHITELIST_MIN_PERCENT]
    if aktif_liste:
...
def _tp_filtre_guncelle():
    """En iyi TP koşullarını hesaplayıp _aktif_filtreler'e yazar, tpozet'i TOPIC_RAPOR'a gönderir."""
    global _aktif_filtreler
    import math as _math_fil
    try:
        with gunluk_kilit:
        ...
        if len(snapshot) < 100 and os.path.exists(VERI_DOSYASI):
            try:
                ...
                pass
        if not snapshot:
            print("[TP_FILTRE] Sinyal verisi yok, filtre guncellenemiyor.")
            return

        veri       = _tp_kosul_rapor_hesapla(snapshot=snapshot)
        confluance = _tp_confluance_skoru_hesapla(veri, kayitlar_snapshot=snapshot)
        if not confluance:
            print("[TP_FILTRE] Confluance verisi yok.")
            return

        sinyal_sonuclari = confluance.get("sinyal_sonuclari", {})
        yeni_filtreler   = {}
        _TF_SIRALA = ["1DK","5DK","15DK","1SA","4SA","1G"]

        for st, veri_st in sinyal_sonuclari.items():
            en_iyi = veri_st.get("en_iyi", [])
            if not en_iyi:
                continue

            # Her TF için en iyi kombinasyonu bul
            tf_filtreler = {}
            for item in en_iyi:
                kosul_str = item["kombinasyon"]
                parts     = [p.strip() for p in kosul_str.split("+")]
                tf_part   = next((p for p in parts if p in _TF_SIRALA), None)
                if not tf_part:
                    continue
                if tf_part in tf_filtreler:
                    continue  # Bu TF için zaten en iyi var
                parts_temiz = [p for p in parts if "oi" not in p.lower()]
                tp_yuzde    = float(str(item["tp_orani"]).replace("%", ""))
                tf_filtreler[tf_part] = {
                    "kosul_parts": parts_temiz,
                    "tp_yuzde"   : tp_yuzde,
                }

            # TF bazlı filtreleri kaydet: "Strong Sell_1DK", "Strong Sell_5DK" ...
            for tf_part, filtre_data in tf_filtreler.items():
                yeni_filtreler[f"{st}_{tf_part}"] = filtre_data

            # Genel filtre (TF koşulsuz) — fallback olarak
            best        = en_iyi[0]
            parts_ham   = [p.strip() for p in best["kombinasyon"].split("+")]
            parts_temiz = [p for p in parts_ham if "oi" not in p.lower() and p not in _TF_SIRALA]
            yeni_filtreler[st] = {
                "kosul_parts": parts_temiz,
                "tp_yuzde"   : float(str(best["tp_orani"]).replace("%", "")),
            }

        with _aktif_filtreler_kilit:
            _aktif_filtreler = yeni_filtreler

        print(f"[TP_FILTRE] Filtreler guncellendi: { {k: v['tp_yuzde'] for k, v in yeni_filtreler.items()} }")

        # tpozet görselini TOPIC_RAPOR'a gönder
        try:
            tarihler = []
            for s in snapshot:
                z = s.get("zaman") or s.get("tarih")
                if not z: continue
                try:
                    if isinstance(z, (int, float)):
                        tarihler.append(dt_cls.fromtimestamp(float(z)))
                    else:
                        tarihler.append(dt_cls.strptime(str(z)[:10], "%Y-%m-%d"))
                except Exception:
                    pass
            if tarihler:
                ilk = min(tarihler).strftime("%d/%m/%Y")
                son = max(tarihler).strftime("%d/%m/%Y")
            else:
                ilk = "—"; son = "—"
            tarih_aralik = f"{ilk} - {son}"

            sonuc = _tpozet_gorsel_uret(confluance, tarih_aralik, len(snapshot))
            if sonuc:
                img_bytes, caption = sonuc
                _topic_foto_gonder_filigranli(TOPIC_RAPOR, img_bytes, caption)
                print("[TP_FILTRE] tpozet TOPIC_RAPOR'a gonderildi.")

            # ── Anlık Piyasa Kontrol Raporu ─────────────────────────────────
            try:
                # _GLOBAL cache /tpozet ile aynı anda dolabilir — 5sn bekle
                _global = _son_analiz_veriler.get("_GLOBAL", {}) if _son_analiz_veriler else {}
                if not _global.get("piyasa_yonu"):
                    time.sleep(5)
                    _global = _son_analiz_veriler.get("_GLOBAL", {}) if _son_analiz_veriler else {}
                _piy    = _global.get("piyasa_yonu", "?")
                _fund   = _global.get("btc_fund")
                _fg     = _global.get("fg_deger")
                _market = _global.get("market_yonu", "?")
                ...
                def _fg_kat_str(v):
                    ...
                    if v <= 24: return f"Aşırı Korku ({int(v)})"
                    if v <= 44: return f"Korku ({int(v)})"
                    if v <= 55: return f"Nötr ({int(v)})"
                    if v <= 75: return f"Açgözlülük ({int(v)})"
                    return f"Aşırı Açgözlülük ({int(v)})"

                # Funding yönü
                fund_str = "?" if _fund is None else (
                    f"Pozitif ({_fund:+.4f}%)" if _fund > 0 else
                    f"Negatif ({_fund:+.4f}%)" if _fund < 0 else "Nötr (0.0000%)"
                )
                ...
                try:
                    _btc_oi = _son_analiz_veriler.get("BTCUSDT", {}).get("oi_delta") if _son_analiz_veriler else None
                    if _btc_oi:
                        _oi_delta = _btc_oi.get("delta_pct", 0)
                        oi_str = f"Artıyor (+{_oi_delta:.1f}%)" if _oi_delta > 0 else f"Azalıyor ({_oi_delta:.1f}%)" if _oi_delta < 0 else "Değişmiyor"
                    else:
                        oi_str = "⚠️ Veri yok"
                except Exception:
                    oi_str = "⚠️ Veri yok"

                # Mevcut piyasa koşulları seti (filtre eşleştirme için)
                def _norm_k(s):
                    if not s: return ""
                    return (s.replace("ı","i").replace("İ","I").replace("ş","s").replace("Ş","S")
                             .replace("ğ","g").replace("Ğ","G").replace("ç","c").replace("Ç","C")
                             .replace("ö","o").replace("Ö","O").replace("ü","u").replace("Ü","U").lower())

                _TF_MAP_K = {"1":"1DK","5":"5DK","15":"15DK","60":"1SA","240":"4SA"}
                mev = set()
                if _piy: mev.add(_norm_k(_piy))
                if _fund is not None:
                    mev.add(_norm_k("Pozitif" if _fund > 0 else "Negatif" if _fund < 0 else "Nötr"))
                if _fg is not None:
                    try:
                        _fgv = float(_fg)
                        fg_kat = ("Aşırı Korku" if _fgv <= 24 else "Korku" if _fgv <= 44 else
                                  "Nötr" if _fgv <= 55 else "Açgözlülük" if _fgv <= 75 else "Aşırı Açgözlülük")
                        mev.add(_norm_k(fg_kat))
                    except Exception:
                        pass
                if _btc_oi:
                    _oi_d = _btc_oi.get("delta_pct", 0)
                    mev.add(_norm_k("OI Artıyor" if _oi_d > 0 else "OI Azalıyor" if _oi_d < 0 else ""))

                # Filtre eşleşme satırları
                _SINYAL_SIRASI = ["Strong Sell", "Strong Buy", "Long", "Short"]
                _TF_SIRASI     = ["1DK", "5DK", "15DK", "1SA"]
                filtre_satirlar = []
                with _aktif_filtreler_kilit:
                    for st in _SINYAL_SIRASI:
                        # TF bazlı filtreleri göster
                        for tf in _TF_SIRASI:
                            anahtar = f"{st}_{tf}"
                            f = _aktif_filtreler.get(anahtar)
                            if not f:
                                continue
                            kosul_parts = [_norm_k(p) for p in f["kosul_parts"]]
                            # OI yoksa atla
                            oi_kosul = any("oi" in p for p in kosul_parts)
                            oi_veri  = any("oi" in k for k in mev)
                            if oi_kosul and not oi_veri:
                                kosul_parts = [p for p in kosul_parts if "oi" not in p]
                            # TF'yi mevcut'a ekleyerek eşleştir
                            mev_tf = mev | {_norm_k(tf)}
                            toplam  = len(kosul_parts)
                            eslesen = sum(1 for p in kosul_parts if p in mev_tf)
                            if eslesen == toplam:
                                durum = "🟢"
                            elif eslesen >= toplam - 1 or eslesen >= toplam / 2:
                                durum = "🟡"
                            else:
                                durum = "🔴"
                            kosul_goster = " + ".join(f["kosul_parts"])
                            filtre_satirlar.append(
                                f"{durum} <b>{st} {tf}</b>: {kosul_goster} → %{int(round(f['tp_yuzde']))} ({eslesen}/{toplam})"
                            )

                simdi_str = dt_cls.utcnow().strftime("%H:%M UTC")
                kontrol_msg = (
                    f"📡 <b>Anlık Piyasa Kontrolü</b> — {simdi_str}\n\n"
                    f"Piyasa  : {_piy}\n"
                    f"Funding : {fund_str}\n"
                    f"F&G     : {_fg_kat_str(_fg)}\n"
                    f"OI      : {oi_str}\n"
                    f"Market  : {_market}\n\n"
                    f"🔍 <b>Aktif Filtreler</b>\n"
                )
                if filtre_satirlar:
                    kontrol_msg += "\n".join(filtre_satirlar)
                else:
                    kontrol_msg += "Henüz filtre yok."

                _telegram_topic_mesaj_gonder(TOPIC_RAPOR, kontrol_msg)
                print("[TP_FILTRE] Piyasa kontrol raporu gonderildi.")
            except Exception as e:
                print(f"[TP_FILTRE] Piyasa kontrol raporu hatasi: {e}")
            # ── Anlık Piyasa Kontrol Raporu Sonu ────────────────────────────
        except Exception as e:
            print(f"[TP_FILTRE] tpozet gorsel hatasi: {e}")

    except Exception as e:
        print(f"[TP_FILTRE] Genel hata: {e}\n{traceback.format_exc()}")
...
def _filtre_skor_gorsel_uret(toplam_p, sir_p, lift_p, fark_p, kap_p, dogru, g_ort_all, kapali_n):
    ...
    try:
        ...
        try:
            ...
            fn = fs = fm = ImageFont.load_default()
        ...
        # Başlık
        d.text((W//2, 22), "Kombinasyon Sıralaması — TF + Funding + OI",
               fill=ALTIN, font=fn, anchor="mm")
        d.text((W//2, 52), f"{kapali_n} kapali sinyal  |  {dogru}/4 sinyal tipinde Güçlü>Orta>Zayıf",
               fill=GRI, font=fs, anchor="mm")

        # Büyük toplam puan
        renk_p = YESIL if toplam_p >= 70 else ALTIN if toplam_p >= 50 else (200, 80, 80)
        d.text((W//2, 110), f"{toplam_p}/100",
               fill=renk_p, font=ImageFont.truetype(
                   "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 52)
               if True else fn, anchor="mm")

        # Alt başlık
        d.text((W//2, 155), f"Ort. Güçlü TP%: {g_ort_all:.1f}",
               fill=BEYAZ, font=fm, anchor="mm")
        ...
        x0 = 80; BAR_W = 160; GAP = 30; BAR_H = 18; Y_BAR = 210
        for i, (ad, deger, maks) in enumerate(kriterler):
            xb = x0 + i * (BAR_W + GAP)
            d.text((xb + BAR_W//2, Y_BAR - 22), ad, fill=GRI, font=fs, anchor="mm")
            d.rectangle([xb, Y_BAR, xb + BAR_W, Y_BAR + BAR_H],
                        fill=(40, 44, 52), outline=(60, 65, 75))
            dolu = int(BAR_W * deger / maks) if maks > 0 else 0
            if dolu > 0:
                bar_renk = YESIL if deger >= maks * 0.8 else ALTIN if deger >= maks * 0.5 else (180, 70, 70)
                d.rectangle([xb, Y_BAR, xb + dolu, Y_BAR + BAR_H], fill=bar_renk)
            d.text((xb + BAR_W//2, Y_BAR + BAR_H + 14),
                   f"{deger}/{maks}", fill=BEYAZ, font=fs, anchor="mm")
...
def _filtre_detay_gorsel_uret(combo_tab, genel_tp, kapali_tum, esik_g, esik_z, tf_map):
    ...
    try:
        ...
        W = 900; SATIR_H = 60; UST = 80
        H = UST + len(SINYAL_TIPLERI) * SATIR_H + 40
        BG = (18, 20, 24)
        img = Image.new("RGB", (W, H), BG)
        d   = ImageDraw.Draw(img)
        try:
        ...
        d.text((W//2, 28), "TF + Funding + OI — Sinyal Tipi Detayı",
               fill=ALTIN, font=fn, anchor="mm")

        # Kolon başlıkları
        COL = [120, 230, 390, 550, 710, 840]
        basliklar = ["Sinyal", "Genel%", "🟢 Güçlü", "🟡 Orta", "🔴 Zayıf", "Kapsama"]
        for i, b in enumerate(basliklar):
            d.text((COL[i], UST - 22), b, fill=GRI, font=fs, anchor="mm")

        MIN_SINYAL = 10
        for ri, st in enumerate(SINYAL_TIPLERI):
            y = UST + ri * SATIR_H + SATIR_H // 2
            kapali = [s for s in kapali_tum if s.get("sinyal_tipi") == st]
            g_ort  = genel_tp.get(st, 0)
            tablo  = combo_tab.get(st, {})

            guclu_n=0; guclu_tp=0; orta_n=0; orta_tp=0; zayif_n=0; zayif_tp=0
            for s in kapali:
            ...
            def pct(tp, n): return f"%{tp/n*100:.1f}" if n > 0 else "—"
            toplam_k = guclu_n + orta_n + zayif_n
            kap_pct  = f"%{(guclu_n+orta_n)/toplam_k*100:.0f}" if toplam_k > 0 else "—"
            ...
            renk_st = YESIL if ok else SARI
            d.text((COL[0], y), st,          fill=renk_st, font=fs, anchor="mm")
            d.text((COL[1], y), f"%{g_ort:.1f}", fill=BEYAZ, font=fs, anchor="mm")
            d.text((COL[2], y), f"{pct(guclu_tp,guclu_n)} ({guclu_n})",
                   fill=YESIL if guclu_n>0 else GRI, font=fs, anchor="mm")
            d.text((COL[3], y), f"{pct(orta_tp,orta_n)} ({orta_n})",
                   fill=SARI  if orta_n>0  else GRI, font=fs, anchor="mm")
            d.text((COL[4], y), f"{pct(zayif_tp,zayif_n)} ({zayif_n})",
                   fill=KIRMZ if zayif_n>0 else GRI, font=fs, anchor="mm")
            d.text((COL[5], y), kap_pct, fill=BEYAZ, font=fs, anchor="mm")

            # Ayırıcı çizgi
            if ri < len(SINYAL_TIPLERI) - 1:
                ly = UST + (ri+1) * SATIR_H
                d.line([(40, ly), (W-40, ly)], fill=(40, 44, 52), width=1)
    ...
    try:
        ...
        if gorsel1:
            _topic_foto_gonder_filigranli(TOPIC_RAPOR, gorsel1,
                f"📊 Kombinasyon Sıralaması — TF+Funding+OI | {len(kapali_tum)} kapali sinyal")

        # ── GÖRSEL 2: Sinyal tipi detayı ──
        gorsel2 = _filtre_detay_gorsel_uret(combo_tab, genel_tp, kapali_tum, ESIK_G, ESIK_Z, TF_MAP)
        if gorsel2:
            _topic_foto_gonder_filigranli(TOPIC_RAPOR, gorsel2,
                "📊 TF + Funding + OI — Sinyal Tipi Detayı")
...
def _filtre_zamanlayici():
    ...
    print("[FILTRE] Zamanlayici basladi. Her 30 dakikada gorsel gonderilecek.")
    time.sleep(30)
    _filtre_gorseller_gonder()
    while True:
        try:
