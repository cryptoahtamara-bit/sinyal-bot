                ...
                # "olta ver" → BTC/ETH/SOL/XRP tum TF tablo
                    ...
                    def _olta_ver_gonder():
                        try:
                            ...
                            long_msg, short_msg = _olta_ver_tablo()
...
def _olta_ver_tablo():
    """
    BTC, ETH, XRP, SOL icin tum TF'leri paralel ceker.
    LONG ve SHORT icin ayri 2 tablo mesaji doner.
    Tablo: TF satirlari gruplu (Giris/SL/TP1/TP2), 4 coin kolonu, monospace.
    """
    import concurrent.futures
    ...
    # Her coin icin fiyat ve TF sonuclarini topla
    fiyatlar = {}
    tum_sonuclar = {}  # {coin: {tf_label: result_dict}}

    def _cek_coin_tf(coin, label, interval, limit):
        sym = coin + "USDT"
        try:
            fiyat = mexc_get_mark_price(sym)
            if not fiyat or fiyat <= 0:
                return coin, label, None, None
            closes, volumes, highs, lows = _trend_fetch_klines(sym, interval=interval, limit=limit)
            r = _olta_fib_hesapla(closes, highs, lows, fiyat, interval)
            return coin, label, fiyat, r
        except Exception as e:
            print(f"[OLTA-VER] {coin} {label} hata: {e}")
            return coin, label, None, None

    gorevler = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as ex:
        for coin in COINS:
            for lbl, iv, lim in TF_LIST:
                gorevler.append(ex.submit(_cek_coin_tf, coin, lbl, iv, lim))
        for fut in concurrent.futures.as_completed(gorevler):
            coin, lbl, fiyat, r = fut.result()
            if fiyat and coin not in fiyatlar:
                fiyatlar[coin] = fiyat
            if coin not in tum_sonuclar:
                tum_sonuclar[coin] = {}
            tum_sonuclar[coin][lbl] = r

    # Formatlama yardimcilari
    def fmt_fiyat(coin, v):
        if v is None:
            return "   ---"
        f = fiyatlar.get(coin, 1)
        dec = 6 if f < 0.01 else (5 if f < 0.1 else (4 if f < 1 else (2 if f < 10 else (1 if f < 100 else 0))))
        w = max(6, dec + 4)
        try:
            return "{:>{w},.{d}f}".format(v, w=w, d=dec).replace(",", ".")
        except Exception:
            return "   ---"

    # Sutun genislikleri: her coin icin en uzun degerin uzunlugu
    # Sabit: BTC=8, ETH=7, SOL=7, XRP=6 (yeterli)
    COL_W = {"BTC": 9, "ETH": 8, "SOL": 8, "XRP": 7}
    TF_W  = 5
    VER_W = 6

    def sutun_baslik():
        # "TF    VERİ   BTC       ETH      SOL      XRP"
        satir = "{:<{tw}}  {:<{vw}}".format("TF", "VERİ", tw=TF_W, vw=VER_W)
        for c in COINS:
            satir += "  {:>{cw}}".format(c, cw=COL_W[c])
        return satir

    def ayrac_satir(baslik):
        return "-" * len(baslik)

    def veri_satiri(tf_label, veri_adi, side, key_long, key_short, ilk_satir):
        tf_str  = tf_label if ilk_satir else ""
        satir = "{:<{tw}}  {:<{vw}}".format(tf_str, veri_adi, tw=TF_W, vw=VER_W)
        for c in COINS:
            r = tum_sonuclar.get(c, {}).get(tf_label)
            key = key_long if side == "long" else key_short
            val = r[key] if r and key in r else None
            satir += "  {:>{cw}}".format(
                fmt_fiyat(c, val).strip() if val is not None else "---",
                cw=COL_W[c]
            )
        return satir
    ...
    baslik = sutun_baslik()
    ayrac  = ayrac_satir(baslik)

    long_satirlar  = [baslik, ayrac]
    short_satirlar = [baslik, ayrac]

    for tf_label, _, _ in TF_LIST:
        for i, (veri_adi, key_l, key_s) in enumerate(VERI_SIRASI):
            ilk = (i == 0)
            long_satirlar.append( veri_satiri(tf_label, veri_adi, "long",  key_l, key_s, ilk))
            short_satirlar.append(veri_satiri(tf_label, veri_adi, "short", key_l, key_s, ilk))
        long_satirlar.append("")   # TF arasi bosluk
        short_satirlar.append("")

    # Fiyat ozeti
    fiyat_ozeti = "  ".join(
        f"{c}: {'{:,.{}f}'.format(fiyatlar[c], 0 if fiyatlar.get(c,1)>=100 else 2) if c in fiyatlar else '?'}"
        for c in COINS
    )
    ...
    try:
        import pytz
        TR_TZ = pytz.timezone("Europe/Istanbul")
        zaman = datetime.now(tz=TR_TZ).strftime("%d %b %Y %H:%M")
    except Exception:
        zaman = datetime.utcnow().strftime("%d %b %Y %H:%M UTC")

    long_msg = (
        "\U0001f7e2 <b>LONG OLTALAR — Tüm Coinler</b>\n"
        f"\U0001f4b0 {fiyat_ozeti}\n"
        f"\U0001f553 {zaman}\n"
        "<pre>" + "\n".join(long_satirlar) + "</pre>"
    )
    short_msg = (
        "\U0001f534 <b>SHORT OLTALAR — Tüm Coinler</b>\n"
        f"\U0001f4b0 {fiyat_ozeti}\n"
        f"\U0001f553 {zaman}\n"
        "<pre>" + "\n".join(short_satirlar) + "</pre>"
    )
