...
def _liq_veri_cek(symbol):
    ...
    try:
        ...
        # 8) Likidasyon bölgesi — 1h kline hacim bazlı, ısı haritasıyla tutarlı
        ...
        try:
            if len(klines) >= 10 and fiyat > 0:
                ...
                def _en_yogun_bolge_genis(yog, merkez, adim, fiyat_ref, taraf, top_n=5):
                    """En yoğun top_n bandı bul, min-max aralığını döndür."""
                    adaylar = []
                    for i, m in enumerate(merkez):
                        ...
                        if taraf == "short" and m <= fiyat_ref:
                            continue
                        if yog[i] > 0:
                            adaylar.append((yog[i], m))
                    if not adaylar:
                        return None
                    # En yoğun top_n bandı al
                    adaylar.sort(reverse=True)
                    secilen = [m for _, m in adaylar[:top_n]]
                    alt = round(min(secilen) - adim / 2, 2)
                    ust = round(max(secilen) + adim / 2, 2)
                    return (alt, ust)

                liq_long_baskisi  = _en_yogun_bolge_genis(long_yog,  merkez, adim, fiyat, "long",  top_n=5)
                liq_short_baskisi = _en_yogun_bolge_genis(short_yog, merkez, adim, fiyat, "short", top_n=5)
...
BOT_VERSIYON = "v493"
print(f"[BASLANGIC] ========== BOT VERSIYON: v493 ==========")
