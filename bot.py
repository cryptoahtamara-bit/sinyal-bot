...
def _tp_kosul_yorum_uret(veri):
    """Veriden otomatik madde madde Türkçe yorum üret. Min 5 sinyal eşiği."""
    MIN = 5
    maddeler = []
    ...
    # 1. Sinyal tipi
    tip_veri = {k: v for k, v in veri["sinyal_tipi"].items() if v[0] >= MIN and oran_val(v) >= 0}
    if tip_veri:
        en_iyi = max(tip_veri.items(), key=lambda x: oran_val(x[1]))
        en_kotu = min(tip_veri.items(), key=lambda x: oran_val(x[1]))
        maddeler.append(f"En başarılı sinyal tipi: {en_iyi[0]} → {en_iyi[1][3]} TP ({en_iyi[1][0]} sinyal)")
        if en_kotu[0] != en_iyi[0]:
            maddeler.append(f"En düşük performanslı sinyal tipi: {en_kotu[0]} → {en_kotu[1][3]} TP ({en_kotu[1][0]} sinyal)")
    else:
        maddeler.append("Sinyal tipi: Veri birikmesi devam ediyor (min. 5 sinyal bekleniyor)")

    # 2. Piyasa koşulları — sadece yeterli verisi olanlar
    kosul_veri = {k: v for k, v in veri["kosul"].items() if v[0] >= MIN and oran_val(v) >= 0}
    if kosul_veri:
        en_iyi_k = max(kosul_veri.items(), key=lambda x: oran_val(x[1]))
        en_kotu_k = min(kosul_veri.items(), key=lambda x: oran_val(x[1]))
        maddeler.append(f"En başarılı piyasa koşulu: {en_iyi_k[0]} → {en_iyi_k[1][3]} TP ({en_iyi_k[1][0]} sinyal)")
        if en_kotu_k[0] != en_iyi_k[0]:
            maddeler.append(f"En düşük piyasa koşulu: {en_kotu_k[0]} → {en_kotu_k[1][3]} TP ({en_kotu_k[1][0]} sinyal)")
    else:
        maddeler.append("Piyasa koşulları: Veri birikmesi devam ediyor")

    # 3. Coin
    coin_veri = {k: v for k, v in veri["coin"].items() if v[0] >= MIN and oran_val(v) >= 0}
    if coin_veri:
        en_iyi_c = max(coin_veri.items(), key=lambda x: oran_val(x[1]))
        en_kotu_c = min(coin_veri.items(), key=lambda x: oran_val(x[1]))
        maddeler.append(f"En başarılı coin: {en_iyi_c[0]} → {en_iyi_c[1][3]} TP ({en_iyi_c[1][0]} sinyal)")
        maddeler.append(f"En düşük performanslı coin: {en_kotu_c[0]} → {en_kotu_c[1][3]} TP ({en_kotu_c[1][0]} sinyal)")

    # 4. Zaman dilimi
    tf_veri = {k: v for k, v in veri["tf"].items() if v[0] >= MIN and oran_val(v) >= 0}
    if tf_veri:
        en_iyi_tf = max(tf_veri.items(), key=lambda x: oran_val(x[1]))
        maddeler.append(f"En güvenilir zaman dilimi: {en_iyi_tf[0]} → {en_iyi_tf[1][3]} TP ({en_iyi_tf[1][0]} sinyal)")
        for tf, tv in tf_veri.items():
            if oran_val(tv) < 25:
                maddeler.append(f"Dikkat: {tf} → {tv[3]} TP — düşük performans, dikkatli olunmalı")

    # 5. En iyi kombinasyon
    komb_veri = {k: v for k, v in veri["kombinasyon"].items() if v[0] >= MIN and oran_val(v) >= 0}
    if komb_veri:
        en_iyi_komb = max(komb_veri.items(), key=lambda x: oran_val(x[1]))
        maddeler.append(f"En güvenilir kombinasyon: {en_iyi_komb[0]} → {en_iyi_komb[1][3]} TP ({en_iyi_komb[1][0]} sinyal)")
...
def _tp_kosul_rapor_gorsel(veri=None, snapshot=None):
    ...
    g4 = gorsel_yap(
        ...
        "TP Koşul Raporu — 4/4",
        alt_yazi="TP Olmayan = SL + Devam Eden  |  Öneri: min. 5 sinyal  |  %50+ Önerilen  |  %35- Kaçınılan",
