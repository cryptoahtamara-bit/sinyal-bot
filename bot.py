...
# MEXC — IPRoyal proxy + browser headers ile WAF bypass
if _proxy_user and _proxy_pass:
    MEXC_PRIVATE_PROXY = {
        "http":  f"http://{_proxy_user}:{_proxy_pass}@{_proxy_host}:{_proxy_port}",
        "https": f"http://{_proxy_user}:{_proxy_pass}@{_proxy_host}:{_proxy_port}",
    }
    print(f"[PROXY] MEXC_PRIVATE_PROXY aktif: True ({_proxy_host}:{_proxy_port})")
else:
...
def mexc_private_post(url, body, timeout=15):
    """Private POST — json={} bos body + params=sorted_body query string ile WAF bypass
    WAF body icerigini inceler, bos body gorur ve gececek.
    Imza query string formatinda: APIKey + ts + sorted_query_string
    """
    ts = str(int(time.time() * 1000))
    sorted_body = dict(sorted(body.items()))
    # Imza: query string formati (alfabetik sirali)
    ...
    print(f"[MEXC IMZA] param_str: {param_str[:100]}")
    ...
    # json={} bos body — WAF'i atlatmak icin
    # Gercek parametreler params= ile query string'de gider
    r = requests.post(url, headers=hdrs, json={}, params=sorted_body, timeout=timeout)
...
print(f"[BASLANGIC] ========== BOT VERSIYON: v243 ==========")
