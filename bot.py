...
def send_telegram_and_schedule_tp(caption, symbol, timeframe, sinyal,
    ...
    # ── TP Filtre Notu ───────────────────────────────────────────────────────
    # Tüm sinyaller geçer. Koşul uyuştuysa TP oranı, uyuşmadıysa "Zayıf Sinyal" notu eklenir.
    # Sinyal normalize: "STRONG BUY" → "Strong Buy" vb.
    _SINYAL_NORM = {
    ...
    tf_str_fil  = _TF_MAP_FIL.get(str(timeframe), str(timeframe)) if timeframe else None

    with _aktif_filtreler_kilit:
        # Önce TF'ye özgü filtre ara: "Strong Sell_1DK"
        filtre = _aktif_filtreler.get(f"{sinyal_norm}_{tf_str_fil}") if tf_str_fil else None
        # Bulamazsa genel filtre: "Strong Sell"
        if filtre is None:
            filtre = _aktif_filtreler.get(sinyal_norm)
    ...
    def _fg_kat_fil(fg):
        ...
        if fg <= 24: return "Asiri Korku"
        if fg <= 44: return "Korku"
        if fg <= 55: return "Notr"
        if fg <= 75: return "Acgozluluk"
    ...
    if filtre:
        mevcut_kosullar = set()
        # Piyasa koşullarını doğrudan _son_analiz_veriler cache'inden oku
        # (gunluk_sinyaller'de kayıt henüz olmayabilir)
        _global_fil = _son_analiz_veriler.get("_GLOBAL", {}) if _son_analiz_veriler else {}
        _fund_fil   = _global_fil.get("btc_fund")
        _piy_fil    = _global_fil.get("piyasa_yonu")
        _fg_fil     = _global_fil.get("fg_deger")

        # Timeframe
        _TF_MAP_FIL2 = {"1":"1DK","3":"3DK","5":"5DK","15":"15DK","30":"30DK","60":"1SA","240":"4SA","D":"1G"}
        tf_str2 = _TF_MAP_FIL2.get(str(timeframe), str(timeframe)) if timeframe else None
        if tf_str2: mevcut_kosullar.add(_norm(tf_str2))

        # Piyasa yönü
        if _piy_fil: mevcut_kosullar.add(_norm(_piy_fil))

        # Funding
        if _fund_fil is not None:
            fund_yon2 = ("Pozitif" if _fund_fil > 0 else "Negatif" if _fund_fil < 0 else "Nötr")
            mevcut_kosullar.add(_norm(fund_yon2))

        # OI
        try:
            _btc_oi_fil = _son_analiz_veriler.get("BTCUSDT", {}).get("oi_delta") if _son_analiz_veriler else None
            if _btc_oi_fil:
                _oi_delta = _btc_oi_fil.get("delta_pct", 0)
                oi_yon2 = "Artıyor" if _oi_delta > 0 else "Azalıyor" if _oi_delta < 0 else None
                if oi_yon2: mevcut_kosullar.add(_norm("OI " + oi_yon2))
        except Exception:
            pass

        # F&G
        if _fg_fil is not None:
            if _fg_fil <= 24:   fg_kat2 = "Aşırı Korku"
            elif _fg_fil <= 44: fg_kat2 = "Korku"
            elif _fg_fil <= 55: fg_kat2 = "Nötr"
            elif _fg_fil <= 75: fg_kat2 = "Açgözlülük"
            else:               fg_kat2 = "Aşırı Açgözlülük"
            mevcut_kosullar.add(_norm(fg_kat2))

        kosul_parts = [_norm(p) for p in filtre["kosul_parts"]]
        tp_yuzde    = filtre["tp_yuzde"]

        # OI verisi None ise filtre koşulundaki OI kısmını sayma
        oi_kosul_var = any("oi" in p for p in kosul_parts)
        oi_veri_var  = any("oi" in k for k in mevcut_kosullar)
        if oi_kosul_var and not oi_veri_var:
            kosul_parts = [p for p in kosul_parts if "oi" not in p]

        toplam_kosul  = len(kosul_parts)
        eslesen_kosul = sum(1 for p in kosul_parts if p in mevcut_kosullar)
        tum_eslesme   = (eslesen_kosul == toplam_kosul)
        orta_eslesme  = (toplam_kosul > 0) and (
            eslesen_kosul >= toplam_kosul - 1 or          # 1 eksik
            eslesen_kosul >= toplam_kosul / 2             # en az yarısı
        ) and not tum_eslesme

        tp_yuzde_int = int(round(tp_yuzde))
        if tum_eslesme:
            filtre_notu = f"🟢 Güçlü Sinyal %{tp_yuzde_int}"
            print(f"[TP_FILTRE] {sinyal} {symbol} guclu eslesme (%{tp_yuzde}).")
        elif orta_eslesme:
            filtre_notu = f"🟡 Orta Sinyal %{tp_yuzde_int}"
            print(f"[TP_FILTRE] {sinyal} {symbol} orta eslesme ({eslesen_kosul}/{toplam_kosul}).")
        else:
            filtre_notu = "🔴 Zayıf Sinyal"
            print(f"[TP_FILTRE] {sinyal} {symbol} zayif eslesme ({eslesen_kosul}/{toplam_kosul}).")
    else:
        filtre_notu = None  # Filtre henüz yok, not eklenmez
        print(f"[TP_FILTRE] {sinyal} icin henuz filtre yok: {symbol}")

    # Filtre notunu caption icindeki ilk satirdan sonra ekle
    # Caption zaten BEN KUL YUTMAM ile basliyor, tekrar ekleme
    if filtre_notu:
        nl = chr(10)
...
def webhook():
    ...
    try:
        ...
        if isinstance(data, dict) and ("message" in data or "channel_post" in data):
            ...
            elif text.startswith("/tp_rapor") or (text.startswith("/tp") and not text.startswith("/tplog") and not text.startswith("/tpozet")):
                if chat_id in yetkili and thread_id == TOPIC_RAPOR:
                    ...
                    def _tp_rapor_gonder():
                        try:
                            ...
                            # Mega kombinasyon için tüm kayıtları kullan (bot restart sonrası da çalışsın)
            ...
            elif text.startswith("/tplog"):
                if chat_id in yetkili and thread_id == TOPIC_RAPOR:
                    print(f"[KOMUT] /tplog basliyor.")
                    parametre = text[len("/tplog"):].strip() or None
                    def _tplog_gonder(parametre=parametre):
                        try:
                            yil, ay, gun = _tplog_tarih_parse(parametre)
                            # Günün başlangıcı: ilgili günün 01:10'u
                            # Günün sonu: ertesi günün 01:09'u
                            baslangic = dt_cls(yil, ay, gun, 1, 10)
                            bitis     = baslangic + timedelta(hours=24)
                            ...
                            def _zaman_parse(z):
                                try:
                                    return dt_cls.strptime(z[:16], "%Y-%m-%d %H:%M")
                                except Exception:
                                    return None
                            kayitlar = [
                                k for k in log
                                if _zaman_parse(k.get("zaman","")) is not None
                                and baslangic <= _zaman_parse(k.get("zaman","")) < bitis
                            ]
                            if not kayitlar:
                                _telegram_topic_mesaj_gonder(
                                    TOPIC_RAPOR,
                                    f"📋 <b>TP Confluance Log — {tarih_str}</b>\n\n"
                                    f"⚠️ Bu tarihe ait kayıt bulunamadı."
                                )
                                return
                            # Max 12 saat per görsel — böl ve gönder
                            MAX_SAAT = 6
                            parcalar = [kayitlar[i:i+MAX_SAAT] for i in range(0, len(kayitlar), MAX_SAAT)]
                            toplam = len(parcalar)
                            gonderildi = False
                            for pi, parca in enumerate(parcalar):
                                parca_str = tarih_str if toplam == 1 else f"{tarih_str} ({pi+1}/{toplam})"
                                sonuc = _tplog_gorsel_uret(parca, parca_str)
                                if sonuc:
                            ...
                            if not gonderildi:
                                _telegram_topic_mesaj_gonder(
                                    TOPIC_RAPOR,
                                    f"⚠️ {tarih_str} için yeni format log kaydı bulunamadı.\n"
                                    f"İlk kayıt 01:10'dan itibaren oluşmaya başlar."
                                )
                        except Exception as e:
                            print(f"[TPLOG] Hata: {e}")
                            _telegram_topic_mesaj_gonder(TOPIC_RAPOR, f"⚠️ /tplog hatası: {e}")
                    threading.Thread(target=_tplog_gonder, daemon=True).start()
                    ...
                    def _veriindir_gonder():
                        try:
                            if not os.path.exists(VERI_DOSYASI):
                                ...
                                return
                            dosya_boyut = os.path.getsize(VERI_DOSYASI)
                            base = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}"
                            with open(VERI_DOSYASI, "rb") as f:
                                resp = requests.post(
                                    ...
                                    data={
                                        ...
                                        "caption": f"📂 sinyaller.json — {dosya_boyut // 1024} KB"
                                    },
                                    files={"document": ("sinyaller.json", f, "application/json")},
                                    timeout=60
                                )
                            if resp.ok:
                                print(f"[VERIINDIR] sinyaller.json gonderildi. {dosya_boyut} bytes")
                            else:
                                print(f"[VERIINDIR] Hata: {resp.text}")
                                _telegram_topic_mesaj_gonder(TOPIC_RAPOR, f"⚠️ Dosya gönderilemedi: {resp.text[:200]}")
                        except Exception as e:
                        ...
                        try:
                            ...
                            if len(snapshot) < 100 and os.path.exists(VERI_DOSYASI):
                                try:
                                    ...
                                    pass
                            if not snapshot:
                                _telegram_topic_mesaj_gonder(TOPIC_RAPOR, "⚠️ Sinyal verisi bulunamadı.")
                                return
                            ...
                            if not confluance:
                                _telegram_topic_mesaj_gonder(TOPIC_RAPOR, "⚠️ Yeterli veri yok.")
                                return
                            ...
                            if sonuc:
                                ...
                                _telegram_topic_mesaj_gonder(TOPIC_RAPOR, "⚠️ /tpozet görseli üretilemedi.")
                        except Exception as e:
                            print(f"[TPOZET] Hata: {e}\n{traceback.format_exc()}")
                            _telegram_topic_mesaj_gonder(TOPIC_RAPOR, f"⚠️ /tpozet hatası: {e}")
                    threading.Thread(target=_tpozet_gonder, daemon=True).start()
...
def _tp_filtre_zamanlayici():
    """Her 30 dakikada bir TP filtrelerini gunceller."""
    print("[TP_FILTRE] Zamanlayici basladi. Her 30 dakikada filtre guncellenecek.")
    # Başlangıçta 30 saniye bekle (bot tam ayağa kalksın)
    time.sleep(30)
    _tp_filtre_guncelle()
    while True:
        try:
            ...
            print(f"[TP_FILTRE] Zamanlayici hata: {e}")
            time.sleep(60)
...
def _tp_kosul_yorum_uret(veri, snapshot=None):
    ...
    if confluance:
        sinyal_sonuclari = confluance.get("sinyal_sonuclari", {})
        tavsiye_yon      = confluance.get("tavsiye_yon", "NÖTR")
        yon_ok           = {"LONG": "▲", "SHORT": "▼", "NÖTR": "●"}.get(tavsiye_yon, "●")

        maddeler.append(f"<b>En İyi TP Koşulları — {yon_ok} {tavsiye_yon}</b>")
...
threading.Thread(target=_tp_filtre_zamanlayici, daemon=True).start()  # TP filtre + tpozet otomatik
