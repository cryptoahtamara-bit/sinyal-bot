...
try:
    ...
    HAS_WS = False
# bot_v651 — 28 Temmuz 2026
# Degisiklikler (v650 -> v651):
#   1. DUZELTME: koin_sirala_guven_bugun() artik "Guclu" etiketli (filtre_notu
#      🟢) sinyalleri HARIC tutuyor. Panel B'nin "Guclu Sinyal" ve "Guven
#      Skoru Yuksek" tablolari artik BIRBIRINI DISLAYAN bir bolunme — bir
#      sinyal ikisinde birden sayilamiyor. Onceden ayni sinyal iki tabloda da
#      sayilabiliyordu, iki tablonun toplami Panel A/C'nin benzersiz toplamini
#      asiyordu (cift sayim). Not: koin_sirala_bugun'un KENDI "Guclu" hesabi
#      (bugun-ozel, ayri bir combo tablosu) ile filtre_notu'nun (kumulatif,
#      tum-zaman combo tablosu) tam ayni olmayabilir — nadir kenar durumlarda
#      kucuk sapmalar olabilir, ama ana cift-sayim sorunu cozuldu.
#   2. /ls raporuna guncel BTC/ETH fiyati eklendi (caption'da "$fiyat" olarak).
#   3. /liq raporuna guncel fiyat eklendi — heatmap zaten hesapliyordu, ekstra
#      API cagrisi olmadan caption'a tasindi.
# bot_v650 — 28 Temmuz 2026
# Degisiklikler (v649 -> v650):
#   1. KRITIK DUZELTME: rapor_gorsel() icindeki istatistik_hesapla(gun_filtre=gun)
#      cagrisina snapshot parametresi eksikti. Bu yuzden Panel C'nin (Gunluk
#      Rapor) UST 5 ozet kutusu (Toplam Sinyal/TP/SL/Devam/Basari) HER ZAMAN
#      TUM sinyalleri kullaniyordu, ama ALTTAKI parite x TF tablosu (rapor_
#      hesapla, ayni fonksiyonda birkac satir yukarida) v647'den beri dogru
#      sekilde FILTRELI (sadece TOPIC_ALARM) snapshot kullaniyordu. Sonuc:
#      ayni gorselde ust kutular "92 sinyal, 37 TP, %40.2" derken alt tablo
#      "41 sinyal, 19 TP, %46" gosteriyordu — iki celisen sayi. Artik ikisi
#      de AYNI (filtreli) snapshot'i kullaniyor, tutarli.
# bot_v649 — 28 Temmuz 2026
# Degisiklikler (v648 -> v649):
#   1. DUZELTME: Panel B'de font buyutme (v646) sirasinda kutu yukseklikleri
#      ve satir araliklari guncellenmemisti — "Sinyal Tipi Detayi" kutusunun
#      4. satiri (Short) kutunun disina tasiyor, alttaki coin tablosuyla
#      cakisiyordu. Ayrica sag tablodaki "Guven Skoru" ve "Sinyal" kolon
#      basliklari birbirine cok yakindi, ust uste biniyordu.
#      Kutu yukseklikleri ve kolon x-konumlari yeniden hesaplanip
#      DUZELTILDI — hem bos veri hem dolu veri durumunda gorsel olarak
#      test edildi, cakisma kalmadi.
# bot_v648 — 28 Temmuz 2026
# Degisiklikler (v647 -> v648):
#   1. Panel A'da "TÜM ZAMANLAR" tarafi artik FİLTRESİZ (tum_kayitlar) —
#      /istatistik'in "Tüm Zamanlar" bolumuyle (13549 sinyal, offset dahil)
#      birebir eslessin diye. "BUGÜN" tarafi degismedi, hala SADECE
#      TOPIC_ALARM sinyalleri (kullanici onayi ile netlestirildi).
#      En Iyi Sembol/TF hesabi da ayni sekilde filtresiz veriye donduruldu.
#   2. Her iki "Sinyal" basliginin altina "(X Long | Y Short)" kirilimi
#      eklendi (kullanicinin referans aldigi /istatistik formatiyla tutarli).
# bot_v647 — 27 Temmuz 2026
# Degisiklikler (v646 -> v647):
#   1. PANEL C YENIDEN AKTIF (kullanici istegiyle) — hem saatlik ISTATISTIK
#      dongusunde hem /rapor komutunda. AMA artik eskisi gibi TUM sinyalleri
#      degil, SADECE TOPIC_ALARM'a giden sinyalleri (Guclu etiketli VEYA
#      guven_skoru>=0.40 — Panel A ile AYNI filtre) olcuyor. Caption'a
#      "(Sadece TOPIC_ALARM sinyalleri)" notu eklendi ki karisiklik olmasin.
#   2. _alarm_gonderilen(s) fonksiyonu, Panel A'nin icinden modul-seviyesine
#      cikarildi — artik Panel A ve Panel C (ve ileride baska raporlar) AYNI
#      fonksiyonu kullaniyor, kopya kod yok.
#   3. Panel D hala DEVRE DIŞI (v645'teki gibi, degismedi).
#   rapor_gorsel/rapor_hesapla fonksiyonlarinin kendisine DOKUNULMADI — sadece
#      hangi snapshot'i aldiklari degisti (filtrelenmis snapshot).
# bot_v646 — 27 Temmuz 2026
# Degisiklikler (v645 -> v646):
#   1. PANEL C ve PANEL D artik TOPIC_RAPOR'a GONDERILMIYOR (kullanici istegi)
#      — hem saatlik ISTATISTIK dongusunde hem /rapor komutunda devre disi.
#      Veri kaydi (sinyaller.json, rapor_gorsel/_panel_d_gorsel fonksiyonlari)
#      DEGISMEDI — sadece gonderim durduruldu, sessizce calismaya devam ediyor.
#   2. /veriindir komutu artik eski format gorselleri (Piyasa Rejimi Bazli
#      Basari, Kombinasyon Siralamasi, Sinyal Tipi Detayi — _filtre_gorseller_
#      gonder() cikisi) GONDERMIYOR. Sadece sinyaller.json + filtre_veri.json
#      dosyalari gonderiliyor. Panel B'nin kendi basarisizlik-yedegi (baska bir
#      kod yolu) DOKUNULMADI.
#   3. "Aktif Kombinasyon — Gerçek Bileşenler" bloğu Panel B'DEN Panel A'YA
#      TASINDI (Whitelist Durumu'nun ustune, kullanici onayli taslak). Ortak
#      hesaplama _kombinasyon_ozet_hesapla() fonksiyonuna cikarildi — Panel A
#      ve Panel B artik AYNI fonksiyonu kullaniyor, kopya kod yok.
#   4. Panel B'nin coin-bazli iki-sutun bolumu, bosalan alanla genisletildi ve
#      TUM yazi boyutlari buyutuldu (kullanici: "veriler cok kucuk okunmuyor").
#   Sinyal uretimine, ALARM_GONDER kararina, TP/SL hesabina, guven_skoru'ya
#   HICBIR SEKILDE DOKUNULMADI.
# bot_v645 — 27 Temmuz 2026
# Degisiklikler (v644 -> v645):
#   1. PANEL A tamamen yeniden tasarlandi (taslak onaylandi):
#      - Eski yatay BUGÜN/TÜM ZAMANLAR satirlari ve "Piyasa Rejimi Bazli
#        Basari" kutusu KALDIRILDI. piyasa_yonu verisinin kendisi Panel B'nin
#        Guclu/Orta/Zayif siniflandirmasinda hala kullaniliyor — sadece bu
#        bagimsiz dokum tablosu kaldirildi (hicbir karari etkilemiyordu).
#      - Yeni duzen: dikeyde ikiye bolunmus — SOL "Tum Zamanlar", SAG "Bugun".
#        Ikisinde de AYNI 7 baslik: Sinyal / TP / SL / Devam / Basari /
#        En Iyi Sembol / En Iyi TF.
#      - KRITIK: Her iki taraf da artik SADECE TOPIC_ALARM'a giden sinyallerden
#        (Guclu etiketli VEYA guven_skoru>=0.40 — ALARM_GONDER ile birebir ayni
#        mantik) hesaplaniyor. Bu, "Tum Zamanlar"in artik botun basindan beri
#        degil, TOPIC_ALARM filtrelemesinin basladigi tarihten (25 Temmuz
#        v642) itibaren anlamina geldigi anlamina gelir — bilinerek onaylandi.
#      - Basliklar kalin beyaz, "Sinyal" degeri acik gri (#AEB3BC), alt baslik
#        (tarih/saat) beyaz.
#      - Whitelist Durumu korundu, alta tam genislikte tasindi; "Henuz yeterli
#        veri yok" artik kirmizi.
#      Sinyal uretimine, ALARM_GONDER kararina, TP/SL hesabina, Panel B/C/D'ye
#      DOKUNULMADI — sadece Panel A'nin GORSEL raporlama bolumu degisti.
# bot_v644 — 25 Temmuz 2026
# Degisiklikler (v643 -> v644):
#   1. PANEL B — "Coin Bazlı Güçlü Sinyallerin TP Oranı (Bugün)" bolumu
#      IKI SUTUNA bolundu (taslak onaylandi):
#      SOL: "Güçlü Sinyal % TP Oranı" — Coin/Sinyal/TP/%Oranı (koin_sirala_bugun,
#           mevcut Güçlü etiketleme mantığı, degismedi)
#      SAG: "Güven Skoru Yüksek % TP Oranı" — Coin/Güven Skoru/Sinyal/TP/%Oranı
#           (YENI: koin_sirala_guven_bugun() fonksiyonu — bugun icin
#           guven_skoru>=0.40 olan sinyallerin coin bazli TP oranini hesaplar)
#      Her iki basligin yaninda "Genel: %X (tp/n)" toplami sayisal olarak
#      gosteriliyor. Baslik satirlari kalin beyaz.
#      Sinyal uretimine, ALARM_GONDER kararina, TP/SL hesabina DOKUNULMADI —
#      sadece Panel B'nin GORSEL raporlama bolumu degisti.
# bot_v643 — 25 Temmuz 2026
# Degisiklikler (v642 -> v643):
#   1. guven_skoru artik SADECE gonderim kararinda degil, kullaniciya giden
#      caption'da da GORUNUYOR: "🎯 Güven Skoru: %XX" satiri, filtre_notu
#      (🟢/🟡/🔴 Sinyal Gücü Oranı) satirinin hemen altina ekleniyor.
#      Hesaplama mantigina, ALARM_GONDER kararina DOKUNULMADI — sadece ayni
#      deger artik metinde de gosteriliyor.
# bot_v642 — 25 Temmuz 2026
# Degisiklikler (v641 -> v642):
#   1. YENI: TOPIC_ALARM'a (ve eski kanala) gonderim artik SUSTURULABILIYOR.
#      Kural: sinyal "Guclu" etiketliyse (filtre_notu yesil 🟢) VEYA canli
#      guven_skoru >= 0.40 ise bildirim gonderilir; ikisi de degilse sessiz
#      kalir. SESSIZ KALAN SINYALLER YINE DE sinyaller.json'a kaydediliyor —
#      sadece Telegram bildirimi gitmiyor, istatistik/izleme kesintisiz devam
#      ediyor. TELEGRAM_LOG_ID her durumda mesaji almaya devam ediyor (ic log,
#      degismedi).
#      25 Temmuz analizinde gercek 79 sinyalle dogrulandi: filtresiz genel
#      basari %29.1; kurala gore GECEN sinyallerde %45.8 (n=24, hacmin %30'u);
#      SUSTURULAN sinyallerde %21.8 (n=55). MEXC entegrasyonuna, sinyal
#      uretimine (Pine Script), TP/SL hesabina HICBIR SEKILDE DOKUNULMADI —
#      sadece Telegram'a giden bildirim akisi filtrelendi.
# bot_v641 — 25 Temmuz 2026
# Degisiklikler (v640 -> v641):
#   1. DUZELTME: /ls komutunun yorum metni artik /analiz'in hesapladigi
#      genel piyasa yonuyle (market_yonu) CAPRAZ KONTROL ediliyor. Onceden
#      "KY agirlikli LONG" HER ZAMAN "Yukselis beklentisi var" seklinde
#      etiketleniyordu — piyasa agir dususte olsa bile (25 Temmuz 01:00
#      raporunda goruldugu gibi: /analiz "Asagi Yonlu, sert kirilim olabilir,
#      yeni long kesinlikle acma" derken /ls ayni anda BTC icin "Yukselis
#      beklentisi var" diyordu — dogrudan celisen iki mesaj). Yogun long
#      pozisyonlanma + dusus trendi kombinasyonu klasik bir squeeze (zorunlu
#      tasfiye) riskidir, yukselis teyidi degil. Artik boyle bir durumda
#      "Yukselis beklentisi" yerine acikca squeeze riski uyarisi veriliyor.
#      Ayni duzeltme "Balina ve KY ayni yonde" satirina da uygulandi.
#      Sinyal uretimi, TP/SL, guven_skoru, MEXC — hicbiri DEGISMEDI.
# bot_v640 — 25 Temmuz 2026
# Degisiklikler (v639 -> v640):
#   1. YENI: "guven_skoru" alani eklendi (sinyal_kaydet -> kayit dict).
#      (symbol, sinyal_tipi) kombinasyonunun SON 25 SONUCLANMIS sinyalindeki
#      basari oranini kullanan, TUM COINLERE AYNI FORMULLE uygulanan, rolling
#      (zamanla guncellenen) bir skor. Az veri varsa (<8 ornek) genel ortalamaya
#      duser. SADECE GECMISE bakar, veri sizintisi yok.
#      Gercek veriyle (zaman-sirali train/test, 5 coin: BTC/ETH/SOL/XRP/BNB)
#      dogrulandi — HICBIR coin'de kotulesme yok, hepsinde iyilesme (bkz.
#      25 Temmuz analizi). Statik "symbol" ozelligi (denenmis ama BTC'yi
#      kotulestirdigi icin reddedilmisti) yerine bu rolling yaklasim tercih
#      edildi cunku tek bir formul tum coinlerde tutarli calisiyor.
#   2. BU SURUMDE HICBIR SINYAL SUSTURULMUYOR / FILTRELENMIYOR. guven_skoru
#      SADECE ETIKET olarak sinyaller.json'a yaziliyor. Bildirim akisi,
#      webhook mantigi ve MEXC entegrasyonu TAMAMEN DEGISMEDI. Amac: bir
#      sure (kullanicinin belirledigi kadar, en fazla ~1 gun) yuksek-skor
#      vs dusuk-skor sinyallerin gercek performansini ETIKETLENMIS olarak
#      gozlemlemek, ardindan susturma kararini VERIYE BAKARAK vermek.
# bot_v639 — 21 Temmuz 2026
# Degisiklikler (v638 -> v639):
#   1. KRITIK DUZELTME: _filtre_gorseller_gonder() icindeki "kapanan_tum"
#      (satir ~6547) hala v634-oncesi hatali tanimi kullaniyordu:
#      tp1_ok==True OR sl_ok==True — yani "kontrol suresi doldu ama ne TP
#      ne SL" (devam) sinyaller paydadan tamamen disleniyordu. Bu, /veriindir
#      cikisindaki "Sinyal Tipi Detayi" gorselinin caption'indaki "Piyasa
#      Rejimi Bazli Basari" satirlarini yapay olarak %97-100'e sisiriyordu.
#      Panel A (_panel_a_gorsel) bu hatayi v634'te zaten duzeltmisti
#      (tp1_ok is not None) ama duzeltme bu ikinci, bagimsiz fonksiyona
#      kopyalanmamisti. Artik iki fonksiyon da AYNI dogru tanimi kullaniyor.
# bot_v638 — 19 Temmuz 2026
# Degisiklikler (v637 -> v638):
#   1. Panel D okunabilirlik iyilestirmesi: sol taraftaki satir etiketleri
#      (Toplam, Long, Short, Guclu, Yukselis vb.) HDR_COL (gri) -> TEXT_W
#      (beyaz) + kalin (bold).
#   2. Sinyal Adedi tablosundaki Long/Short/St.Sell/St.Buy degerleri de
#      HDR_COL (gri, hemen hemen gorunmuyordu) -> TEXT_W (beyaz).
#   3. TUM veri hucreleri (renkli olanlar dahil) artik kalin (fontweight=bold).
#   4. Onizleme goruntusu ile onaylandi, cakisma yok.
# bot_v637 — 19 Temmuz 2026
# Degisiklikler (v636 -> v637):
#   1. KRITIK GORSEL DUZELTME: Panel D'de "Piyasa Rejimi" satirlari (Yukselis/
#      Dusus/Yatay) komsu coin sutunlarina tasip metin cakismasina sebep
#      oluyordu (kullanici ekran goruntusuyle gosterdi — orn. "Dusus" satirinda
#      3 haneli sinyal sayilariyla "327 · %38 (124/327)313 · %36..." okunmuyordu).
#   2. Sebep: hucre metni "{N} · %oran (a/b)" formatindaydi, N zaten kesirin
#      paydasiyla (b) AYNI sayi oldugu icin FAZLADAN basiliyordu ve metni
#      gereksiz yere uzatip tasmaya sebep oluyordu.
#   3. Duzeltme: fazlalik "{N} · " oneki kaldirildi, artik sadece
#      "%oran (a/b)" gosteriliyor — TP Orani hucreleriyle ayni uzunlukta,
#      onlarda cakisma yoktu. Ek guvenlik payi icin sutun genisligi de
#      artirildi (n_coin*1.55 -> n_coin*1.7).
#   4. Buyuk sinyal sayilariyla (3 haneli, 327 gibi) sentetik test edildi.
# bot_v636 — 19 Temmuz 2026
# Degisiklikler (v635 -> v636):
#   1. Panel D TAMAMEN TRANSPOZE EDILDI: eskiden coinler satir, 18 metrik
#      sutundu (cok dar/okunmuyordu). Artik coinler UST SUTUN BASLIGI,
#      metrikler SOL SATIR ETIKETI — sadece 10 (coin sayisi) sutun oldugu
#      icin her sutun cok daha genis, font boyutu buyutuldu (9.5-13pt).
#   2. Grup ayrac cizgileri eklendi: "Sinyal Adedi", "TP Orani", "Sinyal
#      Gucu", "Piyasa Rejimi" gruplari arasinda yatay cizgiyle ayriliyor.
#   3. Coin basliklarinin altina da ayrac cizgisi eklendi.
#   4. Veri/hesaplama mantigina DOKUNULMADI — sadece gorsel duzen degisti.
#   5. Sentetik veriyle (10 coin, 2000 sinyal) test edildi, hatasiz PNG
#      uretiyor, boyut mantikli (2030x1841 — once asiri yatay-uzundu).
# bot_v635 — 19 Temmuz 2026
# Degisiklikler (v634 -> v635):
#   1. YENI PANEL D eklendi (_panel_d_gorsel): "Parite Detayi" — her izlenen
#      coin icin TEK tabloda: Sinyal Adedi (+Long/Short/S.Sell/S.Buy kirilimi),
#      TP Orani (+alt tip kirilimi, hepsi %XX (n/m) formatinda), Sinyal Gucu
#      (Genel/Guclu/Orta/Zayif), Piyasa Rejimi (sinyal sayisi + TP orani,
#      Yukselis/Dusus/Yatay).
#   2. Kaynak veri: gunluk_sinyaller (sinyaller.json) kullanildi —
#      filtre_veri.json'da sembol bilgisi olmadigi icin Panel A/B/C'nin
#      kullandigi filtre_sinyaller kaynagi burada uygun degildi.
#   3. Etiket (Guclu/Orta/Zayif) hesabi Panel B ile AYNI mantik
#      (TF+Funding+OI+Piyasa kombinasyonu, ESIK_G=8, ESIK_Z=5, MIN_COMBO=3).
#   4. Otomatik dongu artik A -> 5sn -> B -> 5sn -> C -> 5sn -> D -> 5sn.
#   5. /rapor komutu da Panel D'yi diziye ekledi (A/B/C/D hepsi).
#   6. Sentetik veriyle test edildi, hatasiz PNG uretiyor.
# bot_v634 — 19 Temmuz 2026
# Degisiklikler (v633 -> v634):
#   1. KRITIK DUZELTME: Panel A'daki "Piyasa Rejimi Bazli Basari" hesabi
#      yanlis paydayla calisiyordu. Kullanici somut Telegram ornekleriyle
#      gosterdi: bir sinyalin izleme suresi (orn. 1DK icin 30dk) dolup
#      TP1-5'in HICBIRI tutmadiginda ve SL de tetiklenmediginde, bu sonuc
#      tp1_ok=False, sl_ok=False olarak KALICI yaziliyor (bkz.
#      tp_sonuc_guncelle) — yani gercek, kesinlesmis bir sonuc, "hala
#      bekliyor" degil.
#   2. Eski kod (kapanan_tum = tp1_ok==True OR sl_ok==True) bu "ne TP ne
#      SL" kayitlarini paydadan TAMAMEN disliyordu, oranlari yapay olarak
#      %97-100'e sisiriyordu (her isyalyi TP kabul etmis gibi).
#   3. Duzeltme: artik botun kendi istatistik_hesapla mantigiyla TUTARLI:
#      kapanan_tum = tp1_ok is not None (kontrolu tamamlanmis TUM sinyaller).
#      Sentetik testte dogrulandi: 10 sinyal (4 gercek TP + 6 "ne TP ne SL")
#      artik dogru sekilde %40.0 (4/10) gosteriyor, %100 degil.
#   4. Panel B'nin ana hesaplari (Sinyal Tipi Detayi, Aktif Kombinasyon)
#      zaten DOGRU tanimi kullaniyordu (kapali_tum = tp1_ok is not None or
#      sl_ok is not None) — bu yuzden dokunulmadi, sadece Panel A duzeltildi.
# bot_v633 — 19 Temmuz 2026
# Degisiklikler (v632 -> v633):
#   1. /rapor komutu artik SADECE eski tek gorseli (Panel C/rapor_gorsel)
#      degil, otomatik dongudeki gibi UCUNU DE sirayla gonderiyor:
#      Panel A -> 5sn -> Panel B -> 5sn -> Panel C -> 5sn.
#      Whitelist de komut icinde bir kez guncelleniyor (top3_guncelle).
#   2. Webhook'u bloklamamasi icin gonderim ayri bir thread'de
#      (_rapor_komut_gonder) calisiyor — komut hemen "ok" donup arka
#      planda 3 paneli sirayla gonderiyor.
#   3. Baska hicbir yere dokunulmadi.
# bot_v632 — 19 Temmuz 2026
# Degisiklikler (v631 -> v632):
#   1. RACE CONDITION duzeltmesi: sinyaller.json ve filtre_veri.json kaydinda
#      "No such file or directory: ...tmp -> ..." hatasi cikiyordu (birden
#      fazla thread ayni .tmp dosyasina yazip os.replace yapiyordu). Her iki
#      kayit fonksiyonuna ayri kilit eklendi (_veri_kayit_kilit,
#      _filtre_kayit_kilit).
#   2. Panel A ve Panel B'deki METIN CAKISMASI duzeltildi: gercek veride
#      sayilar (1401/1431 gibi) test verisinden cok daha uzundu, sabit
#      sutun genislikleri yetersiz kaliyordu. Her iki panelin figsize
#      genisligi 9.5->11.5 yapildi, sutun pozisyonlari yeniden hesaplandi,
#      Panel A'da piyasa rejimi tablosundaki gereksiz orta sutun kaldirildi.
#   3. RAPORLAMA ZAMANLAMASI BIRLESTIRILDI: eskiden Panel A/C bir zamanlayicida
#      (XX:00/XX:30), Panel B ayri bir zamanlayicida (XX:05/XX:35) farkli
#      saatlerde gonderiliyordu — bu karisikliga sebep oluyordu. Artik TEK
#      zamanlayici (_istatistik_zamanlayici, XX:00/XX:30), SIRAYLA:
#      Panel A -> 5sn bekle -> Panel B -> 5sn bekle -> Panel C -> 5sn bekle.
#      Eski _filtre_zamanlayici DEVRE DISI birakildi (govdesi
#      _filtre_zamanlayici_ESKI_DEVREDISI olarak saklandi, silinmedi).
#   4. Panel C'nin (rapor_gorsel) neden bazen hic gonderilmedigini teshis
#      etmek icin: artik her adimda basarili/basarisiz durumu ayrintili
#      loglaniyor ([PANEL_A]/[PANEL_B]/[PANEL_C] Gonderildi/Uretilemedi).
#      Onceki halde bu sessizce atlaniyordu, teshis imkansizdi.
#   5. /istatistik, /sirala, /rapor komutlari yine degismedi.
# bot_v631 — 18 Temmuz 2026
# Degisiklikler (v630 -> v631):
#   1. /TOPIC_RAPOR'a giden 5 ayri rapor, onaylanan tasarima gore 3 panele
#      indirgendi (Panel C = mevcut rapor_gorsel, degismedi):
#      - PANEL A (_panel_a_gorsel): Istatistik (bugun+tum zamanlar, min-50
#        duzeltilmis en iyi sembol/TF) + Piyasa Rejimi Bazli Basari +
#        Whitelist Durumu. Eski istatistik_mesaji + koin_sirala_metni
#        METINLERININ yerini alir.
#      - PANEL B (_panel_b_gorsel): Sinyal Tipi Detayi (adetli) + Aktif
#        Kombinasyon Gercek Bilesenleri (Guclu/Zayif/Fark/Kapsama/Dogru —
#        "Puan" KALDIRILDI, cunku eski gorselde ilk 3 satir hep ayni
#        (yaniltici) degeri gosteriyordu, gercek hesap olmadigi
#        dogrulandi) + Coin Bazli Guclu Sinyallerin TP Orani (Bugun).
#        Eski Kombinasyon Siralamasi + Sinyal Tipi Detayi gorsellerinin
#        yerini alir.
#   2. _istatistik_zamanlayici ve _filtre_zamanlayici yeni panelleri
#      gonderecek sekilde guncellendi. Panel uretilemezse (Exception)
#      ESKI fonksiyonlar otomatik yedek olarak devreye giriyor.
#   3. top3_guncelle() / koin_sirala_bugun() (whitelist hesabi) HER IKI
#      zamanlayicida da korunuyor — yan etki kaybolmadi.
#   4. Eski istatistik_mesaji, koin_sirala_metni, _filtre_gorseller_gonder,
#      rapor_gorsel fonksiyonlarinin hicbiri SILINMEDI — /istatistik,
#      /sirala, /rapor komutlari eskisi gibi calismaya devam ediyor.
#      Sadece OTOMATIK (zamanlanmis) gonderimler degisti.
#   5. Panel A ve Panel B, sentetik veriyle test edildi — hatasiz PNG
#      uretiyor (bkz. konusma gecmisi).
# bot_v630 — 18 Temmuz 2026
# Degisiklikler (v629 -> v630):
#   1. /istatistik'teki "En Basarili Sembol/Zaman Dilimi" hesaplarindaki
#      istatistiksel gurultu duzeltildi:
#      - Bugun en_sym: esik yoktu (basarili>0 yeterliydi) -> min 50 sinyal
#      - Bugun en_tf:  min 3 sinyal -> min 50 sinyal
#      - Tum zamanlar en_sym_t: min 5 -> min 50
#      - Tum zamanlar en_tf_t:  min 5 -> min 50
#   2. Sebep: TF=240 (4SA) sadece 13 ornekle "en basarili TF" (%53.8)
#      cikiyordu; min 50 ile artik TF=1 (5912 ornek, %35.4) gibi
#      istatistiksel olarak guvenilir sonuclar one cikiyor.
#   3. Beklenen yan etki: "bugun" istatistiginde bircok TF/sembol 50
#      esigini gun icinde gecemeyebilir - bu durumda "En Basarili" satiri
#      hic gorunmeyebilir (bos donuyor). Bu istenen/guvenli davranistir.
# bot_v629 — 17 Temmuz 2026
# Degisiklikler (v628 -> v629):
#   1. Kanal mesajindaki "Sinyal Gucu Orani" etiketine o anki piyasa
#      rejimi eklendi (orn. "%62 - Yukselis"). Sadece goruntu, hesaplama
#      degismedi.
#   2. Kombinasyon Siralamasi gorselinde "Aktif" satirin adi gercek
#      kullanilan kombinasyonu yansitacak sekilde "TF + Funding + OI +
#      Piyasa" olarak guncellendi (onceden yaniltici sekilde sadece
#      "TF + Funding" yaziyordu).
#   3. Sinyal Tipi Detayi gorselinin alt basligi "TF + Funding + OI" ->
#      "TF + Funding + OI + Piyasa Yonu" olarak duzeltildi.
#   4. Sinyal Tipi Detayi gorselinin Telegram caption'ina, piyasa rejimi
#      bazinda (Yukselis/Dusus/Yatay) basari kirilimi eklendi — gorselin
#      kendisi degistirilmeden, ek metin olarak.
#   5. Hicbir hesaplama mantigina dokunulmadi, sadece goruntu/rapor
#      katmani guncellendi.
# bot_v628 — 17 Temmuz 2026
# Degisiklikler (v627 -> v628):
#   1. Sinyal kaydina (filtre_veri.json) "piyasa_yonu" alani eklendi.
#      /analiz sisteminin saatlik urettigi piyasa rejimi (Yukselis/Dusus/
#      Yatay - Skor+Breadth+Taker+Likidasyon+CVD+Funding bilesik hesabi)
#      artik her sinyalle birlikte kaydediliyor.
#   2. Guclu/Orta/Zayif etiket hesaplamasindaki kombinasyon anahtarina
#      (TF+Funding+OI) piyasa_yonu da eklendi -> artik TF+Funding+OI+Piyasa.
#      Bu, ayni TF/Funding/OI kombinasyonunun farkli piyasa rejimlerinde
#      (Yukselis vs Dusus vs Yatay) AYRI degerlendirilmesini saglar.
#   3. Bu degisiklik 5 ayri yerde TUTARLI sekilde uygulandi:
#      - send_telegram_and_schedule_tp (kanal mesaji etiketi)
#      - koin_sirala_bugun (whitelist hesabi)
#      - koin_sirala_metni (/sirala metin raporu)
#      - _filtre_gorseller_gonder (Kombinasyon Siralamasi + Sinyal Tipi
#        Detayi gorselleri, TOPIC_RAPOR)
#   4. TP/SL hesaplama, sinyal uretim mantigi, mexc oto-islem akisi
#      DEGISMEDI.
#   5. Beklenen davranis: eski kayitlarda piyasa_yonu olmadigi icin bu yeni
#      boyut BUGUNDEN ITIBAREN birikmeye baslayacak; ilk gunler/haftalar
#      "Yeterli Veri Yok" gorulmesi normal ve beklenen bir gecis surecidir.
# bot_v627 — 26 Haziran 2026
# Degisiklikler (v626 -> v627):
#   1. MEXC işlem açılamadı hatası düzeltildi:
#      _time.sleep(2) → time.sleep(2)
#      _time bu scope'ta tanımlı değildi — 2 denemede de başarısız hatası buradan geliyordu
# bot_v626 — 26 Haziran 2026
# Degisiklikler (v625 -> v626):
#   1. _filtre_gorseller_gonder() başında top3_guncelle() çağrısı eklendi
#      Her 30 dakikada görsel üretilirken whitelist de otomatik güncellenir
#      Önceden sadece /sirala ve /trade_ac'ta güncelleniyordu
# bot_v625 — 25 Haziran 2026
# Degisiklikler (v624 -> v625):
#   1. /trade_ac parametreli eşik değişikliği:
#      /trade_ac       → eşik değişmez, mevcut eşikle açılır
#      /trade_ac 60    → eşik %60 yapılır, whitelist anında güncellenir
#      /trade_ac 70    → eşik %70 yapılır, whitelist anında güncellenir
#      Geçerli aralık: %10-%100
# bot_v624 — 25 Haziran 2026
# Degisiklikler (v623 -> v624):
#   1. /trade_ac mesajına whitelist durumu eklendi (MEXC_NOTIFY_CHAT_ID)
#      - Whitelist doluysa hangi coinler aktif gösterilir
#      - Whitelist boşsa en yakın coinler ve kaç sinyal kaldığı gösterilir
#   2. top3_guncelle(): whitelist değiştiğinde otomatik bildirim (MEXC_NOTIFY_CHAT_ID)
#      - Yeni giren coinler, çıkan coinler, aktif liste bildirilir
# bot_v623 — 25 Haziran 2026
# Degisiklikler (v622 -> v623):
#   1. Filtre notu BEN KÜL YUTMAM satırının altına taşındı
#      (önceden caption'ın en başına ekleniyordu)
# bot_v622 — 25 Haziran 2026
# Degisiklikler (v621 -> v622):
#   1. Yeterli filtre verisi olmayan sinyallere ⏳ Yeterli Filtre Verisi Yok notu eklendi
#      (önceden etiket hiç gelmiyordu)
# bot_v621 — 25 Haziran 2026
# Degisiklikler (v620 -> v621):
#   1. v620'deki kapali_tum hatasını düzelttir:
#      kapali_tum = tp1_ok is not None (kombinasyon tablosu hesabı için — tüm veri)
#      kapanan_tum = tp1_ok==True OR sl_ok==True (görsel başlığı için — sadece kapananlar)
#      Böylece kombinasyon tablosu doğru hesaplanır, görsel başlığı Sinyal Tipi ile uyumlu olur
# bot_v620 — 25 Haziran 2026
# Degisiklikler (v619 -> v620):
#   1. Kombinasyon Sıralaması görseli:
#      kapali_tum tanımı düzeltildi: tp1_ok is not None → tp1_ok==True OR sl_ok==True
#      Başlık: "84 tamamlanmış" → "30 kapanan" (Sinyal Tipi Detayı ile uyumlu)
# bot_v619 — 25 Haziran 2026
# Degisiklikler (v618 -> v619):
#   1. koin_sirala_metni() genel istatistik artık filtre_sinyaller'den
#      bugün 00:00'dan itibaren hesaplanıyor — eski sinyaller karışmıyor
# bot_v618 — 25 Haziran 2026
# Degisiklikler (v617 -> v618):
#   1. Filtre etiketi metni degistirildi:
#      🟢 Güçlü %XX → 🟢 Sinyal Gücü Oranı: %XX
#      🟡 Orta %XX  → 🟡 Sinyal Gücü Oranı: %XX
#      🔴 Zayıf %XX → 🔴 Sinyal Gücü Oranı: %XX
# bot_v617 — 25 Haziran 2026
# Degisiklikler (v616 -> v617):
#   1. koin_sirala_bugun() — filtre_sinyaller'den bugün 00:00'dan itibaren
#      sinyaller.json filtre_notu yerine filtre_veri.json kombinasyon tablosu kullanılıyor
#      /sirala ve Sinyal Tipi Detayı görseli artık aynı kaynaktan besleniyor
# bot_v616 — 25 Haziran 2026
# Degisiklikler (v615 -> v616):
#   1. _filtre_zamanlayici: XX:00/XX:30 → XX:05/XX:35 olarak değiştirildi
# bot_v615 — 25 Haziran 2026
# Degisiklikler (v614 -> v615):
#   1. _tp_kosul_rapor_zamanlayici devre disi — eski TP raporu artik gelmeyecek
#      Yeni sistem: _filtre_zamanlayici (XX:00 ve XX:30)
# bot_v614 — 25 Haziran 2026
# Degisiklikler (v613 -> v614):
#   1. baslik_tarih hatasi duzeltildi
#   2. Tamamlanan sutunu filtre_veri.json'dan besleniyor
#   3. Ozet kutular: Kapanan=TP+SL, Devam Eden=Toplam-Kapanan
# bot_v609 — 24 Haziran 2026
# Degisiklikler (v608 -> v609):
#   1. filtre_veri.json altyapisi kuruldu — TF+Funding+OI bazli filtre veritabani
#      - FILTRE_VERI_DOSYASI sabiti eklendi (/data/filtre_veri.json)
#      - filtre_sinyaller global listesi eklendi
#      - filtre_veri_kaydet() ve filtre_dosyadan_yukle() fonksiyonlari eklendi
#      - sinyal_kaydet(): her sinyal filtre_sinyaller'e de kaydediliyor
#      - tp_sonuc_guncelle(): TP/SL sonucu filtre_sinyaller'de de guncelleniyor
#      - dosyadan_yukle(): bot restart sonrasi filtre_dosyadan_yukle() cagriliyor
#   2. Filtre etiket mantigi degistirildi — TF+Funding+OI kombinasyon bazli
#      - Eski: en iyi tek kombinasyona benzeme sayisi → Guclu/Orta/Zayif
#      - Yeni: o anki kombinasyonun gercek gecmis TP orani → Guclu/Orta/Zayif
#      - Esik: Guclu=+8p, Orta=-5/+8p, Zayif=-5p altı, min 10 sinyal
#   3. /tp, /tplog, /tpozet komutlari kaldirildi
#   4. /veriindir genisletildi: sinyaller.json + filtre_veri.json + 2 gorsel
#   5. _filtre_gorseller_gonder() — 2 gorsel uretir:
#      Gorsel 1: Kombinasyon siralaması (100 uzerinden puan)
#      Gorsel 2: Sinyal tipi detayi (Guclu/Orta/Zayif dagilimi)
#   6. _filtre_zamanlayici(): her 30 dakikada gorsel TOPIC_RAPOR'a gider
# bot_v608 — 24 Haziran 2026
# Degisiklikler (v607 -> v608):
#   1. TP Performans Yorumu başındaki "NÖTR/LONG/SHORT" tavsiye kaldırıldı
#      - Piyasa yönü zaten /analiz'de hesaplanıyor, burada gereksizdi
#      - "En İyi TP Koşulları — ● NÖTR" → sadece "En İyi TP Koşulları"
# bot_v607 — 24 Haziran 2026
# Degisiklikler (v606 -> v607):
#   1. /tpozet görselinden OI koşulu kaldırıldı
#      - Filtre OI'siz çalışıyor ama görsel OI'li kombinasyon gösteriyordu
#      - Artık görsel filtre ile tutarlı: "1DK + Düşüş + Pozitif + Aşırı Korku"
# bot_v606 — 24 Haziran 2026
# Degisiklikler (v605 -> v606):
#   1. Piyasa kontrol raporu _GLOBAL cache boşsa 5sn bekleyip tekrar okuyor
#      - Onceden: /tpozet ile _GLOBAL aynı anda doluyordu, "?" geliyordu
#      - Simdi: cache boşsa 5 saniye bekle, sonra tekrar oku
# bot_v605 — 24 Haziran 2026
# Degisiklikler (v604 -> v605):
#   1. RSI format düzeltildi — cache "LINKUSDT" formatında, okuma "LINK" formatındaydı
#      - Artık "ETHUSDT.P" → "ETHUSDT" formatında cache okunuyor, tüm coinler doluyor
#   2. Üst TF teyidi format düzeltildi — aynı format sorunu
# bot_v604 — 24 Haziran 2026
# Degisiklikler (v603 -> v604):
#   1. RSI format düzeltildi — cache "BTC" formatında, okuma "ETH" formatındaydı
#      - ETHUSDT.P → ETH → cache eşleşmesi artık doğru, rsi_deger dolacak
#   2. L/S oranı global cache eklendi (_son_ls_cache)
#      - _ls_gonder her saatin :15'inde cache'i güncelliyor
#      - sinyal_kaydet: ls_ky_long, ls_bal_long, ls_kategori alanları eklendi
#   3. Üst TF teyidi eklendi (ust_tf_teyit)
#      - 1DK sinyal → 5DK trend (skor>=50 = yükseliş = long uyumlu)
#      - 5DK sinyal → 15DK trend
#      - Uyumlu / Uyumsuz olarak kayıt edilecek
# bot_v603 — 23 Haziran 2026
# Degisiklikler (v602 -> v603):
#   1. /tpozet sonrası anlık piyasa kontrol raporu eklendi
#      - Piyasa, Funding, F&G, OI, Market yönü gösteriliyor
#      - Her aktif filtre için 🟢/🟡/🔴 eşleşme durumu gösteriliyor
#      - Format: "Strong Sell 1DK: Düşüş + Pozitif + Aşırı Korku → %51 (3/3)"
#      - TOPIC_RAPOR'a /tpozet görselinden hemen sonra gönderiliyor
# bot_v602 — 23 Haziran 2026
# Degisiklikler (v601 -> v602):
#   1. _re import eksikliği düzeltildi (v598 temizliğinden kalan hata)
#      - Komutlar _re.sub kullanıyor, tanımsız olunca tüm komutlar sessiz kalıyordu
# bot_v601 — 23 Haziran 2026
# Degisiklikler (v600 -> v601):
#   1. CVD yorum düzeltildi
#      - neg_cvd: "kısa vadeli alım var" → "CVD negatif bölgede yukarı dönüyor — henüz teyit yok"
#      - pos_cvd: "düşüş suni olabilir" → "birikim sinyali, dip takip edilebilir"
#      - BTC btc_birikim+neg: "kısa vadeli alım sinyali" → "satış baskısı sürüyor ancak kısa vadeli alım hareketi var"
#      - BTC btc_birikim+pos yeni senaryo eklendi
# bot_v600 — 23 Haziran 2026
# Degisiklikler (v599 -> v600):
#   1. Filtre notu caption'ın en üstüne taşındı
#      - Onceden: BEN KUL YUTMAM satırından sonra ekleniyordu — caption 1024 char limitinde kesiliyor olabilirdi
#      - Simdi: filtre_notu caption'ın en başına ekleniyor — her zaman görünür
#   2. TARAYICI_TOP_N sabiti eklendi (eksikti, zamanlayıcı hata veriyordu)
# bot_v599 — 23 Haziran 2026
# Degisiklikler (v598 -> v599):
#   1. v598'de silinen global importlar geri eklendi
#      - dt_mod, plt, concurrent, dt_cls, _json, np, FancyBboxPatch, GridSpec
#      - Import temizliği local scope'dan yapılmalıydı, global'den yapılınca zamanlayıcılar çöktü
# bot_v598 — 23 Haziran 2026
# Degisiklikler (v597 -> v598):
#   BUG DÜZELTMELERİ:
#   1. fg_kategori artık fg_deger'den hesaplanıyor (önceden hep None dönyordu)
#   2. rsi_deger symbol format düzeltildi: ETHUSDT.P → ETHUSDT (cache eşleşmiyordu)
#   3. hacim_durumu artık API çağrısı yapmıyor — _son_analiz_veriler'deki vol_ratio'dan okuyor
#      (önceden her sinyalde Binance API'ye istek atıyordu, timeout'ta None dönüyordu)
#   KOD TEMİZLİĞİ:
#   4. Kullanılmayan fonksiyonlar silindi: exchange_test, mexc_test, webkey_test, external_veri (~316 satır)
#   5. Tekrar eden local import satırları kaldırıldı (~115 satır)
#   6. Toplam ~400 satır azaldı
# bot_v597 — 23 Haziran 2026
# Degisiklikler (v596 -> v597):
#   1. Filtre TF bazlı hale getirildi
#      - Onceden: tum sinyal tipleri icin tek filtre (hep 1DK kombinasyonu seciliyordu)
#      - Simdi: her sinyal tipi + TF icin ayri filtre: "Strong Sell_1DK", "Strong Sell_5DK" vs.
#      - Sinyal geldiginde once TF'ye ozgu filtre aranir, yoksa genel filtre kullanilir
#      - Sonuc: 5DK veya 60DK sinyaller artik dogru filtreyle degerlendirilecek
# bot_v596 — 23 Haziran 2026
# Degisiklikler (v595 -> v596):
#   1. Filtre kombinasyonundan OI koşulu çıkarıldı
#      - OI verisi tutarsız geliyor (bugün Artıyor ama filtre Azalıyor arıyor)
#      - Artık filtre: 1DK + Düşüş + Pozitif + Aşırı Korku (OI yok)
#      - Güçlü sinyal artık gelecek
#   2. En başarılı sembol her iki dönem için de oran bazlı
#      - Onceden bugün için TP sayısı kriter — şimdi sadece oran
# bot_v595 — 23 Haziran 2026
# Degisiklikler (v593 -> v595):
#   1. En başarılı sembol hesabı düzeltildi
#      - Onceden: minimum 3 sinyal şartı + en yüksek oran
#      - Simdi: en fazla TP sayısına göre (eşitlikte oran ikinci kriter)
#      - Sonuc: BNB 2 TP ile XRP/BTC/DOGE/LINK 1 TP'ye göre önce gelir
# bot_v593 — 23 Haziran 2026
# Degisiklikler (v592 -> v593):
#   1. rapor_hesapla'ya TF filtresi eklendi
#      - Onceden: tüm TF'leri sayıyordu, görsel rapordaki toplam fazla çıkıyordu
#      - Simdi: sadece tf_goster {"1","5","15","60","240","D"} içindeki sinyaller sayılıyor
#      - Sonuc: istatistik, sirala ve görsel rapordaki sinyal sayıları eşleşiyor
# bot_v592 — 23 Haziran 2026
# Degisiklikler (v591 -> v592):
#   1. istatistik_hesapla'ya TF_GECERLI filtresi eklendi
#      - Onceden: tüm timeframe'leri sayıyordu (21 sinyal)
#      - Simdi: koin_sirala_metni ile aynı TF filtresi {"1","5","15","60","240","D"}
#      - Sonuc: iki rapordaki sinyal sayısı artık eşleşiyor
# bot_v591 — 23 Haziran 2026
# Degisiklikler (v590 -> v591):
#   1. /sirala coin bazlı hizalama düzeltildi
#      - Sembol adları ljust(5) ile sabit genişliğe getirildi
#      - <code> tagi ile monospace yazı tipi kullanıldı
#      - Aktif işaretçisi ✅/▪️ olarak güncellendi
# bot_v590 — 23 Haziran 2026
# Degisiklikler (v589 -> v590):
#   1. filtre_notu diske kaydediliyor
#      - Onceden: filtre_notu sadece RAM'e yaziliyordu, restart sonrasi kayboluyordu
#      - Simdi: filtre_notu kayda eklendikten hemen sonra veri_kaydet() cagriliyor
#      - Sonuc: TP/SL bildirimlerinde her zaman filtre notu gorunecek
# bot_v589 — 23 Haziran 2026
# Degisiklikler (v588 -> v589):
#   1. /sirala komutu yeniden duzenlendi — Sinyal Kalite Raporu
#      - Guclu/Orta/Zayif sinyal bazli TP istatistigi eklendi
#      - Coin bazli da her kategori icin ayri TP/sinyal sayisi gosteriliyor
#      - Format: BTC  🟢6/8 🟡3/7 🔴4/14 | %65
# bot_v588 — 22 Haziran 2026
# Degisiklikler (v587 -> v588):
#   1. Filtre kontrolü gunluk_sinyaller'den değil _son_analiz_veriler cache'inden okuyor
#      - Onceden: son_kayit gunluk_sinyaller'de aranıyor → kayıt henüz eklenmemiş → 0/5 eslesme
#      - Simdi: piyasa kosullari dogrudan _GLOBAL cache'den okunuyor → anlık ve doğru
#      - Sonuç: Tüm sinyaller artık doğru piyasa koşullarıyla değerlendiriliyor
# bot_v587 — 22 Haziran 2026
# Degisiklikler (v586 -> v587):
#   1. OI verisi None geldiginde filtre kosulundaki OI kismi atlanir
#      - Onceden: OI None → filtre eslesmiyor → Zayif sinyal
#      - Simdi: OI None → OI kosulu sayilmaz → diger kosullara gore degerlendirilir
#      - Ornek: "1DK + Dusus + Pozitif + OI Azaliyor" filtresi varken OI None gelirse
#               "1DK + Dusus + Pozitif" olarak degerlendirilir (3/3 eslesmede Guclu)
# bot_v586 — 22 Haziran 2026
# Degisiklikler (v585 -> v586):
#   1. /veriindir komutu eklendi (TOPIC_RAPOR'dan, yetkili kullanici)
#      - sinyaller.json dosyasini Telegram'a document olarak gonderir
#      - Analiz icin kullanilacak
# bot_v585 — 22 Haziran 2026
# Degisiklikler (v584 -> v585):
#   1. TP filtre baslangic bekleme suresi 2 dakikadan 30 saniyeye indirildi
#      - Bot baslarken sinyal gelirse filtre hazir olmuyordu (not gelmiyordu)
#      - 30 saniye yeterli, bot bu sure icinde tam ayaga kalkar
# bot_v584 — 22 Haziran 2026
# Degisiklikler (v583 -> v584):
#   1. Sinyal kaydina confluence verileri eklendi:
#      - olta_yakinlik: Sinyal fiyati ile Fibonacci olta giris seviyesi arasi % mesafe
#      - liq_yakinlik: Sinyal fiyati ile likidasyon bolgesi arasi % mesafe
#      - Olta: Sinyal geldiginde 5DK kline ile anlik Fib hesabi yapilir
#      - Likidasyon: _liq_gonder() periyodik zamanlayicisinda _son_liq_zone cache'e yazilir
#   2. Bu veriler /tplog analizine dahil edilecek — veri birikmesi bekleniyor
# bot_v583 — 21 Haziran 2026
# Degisiklikler (v582 -> v583):
#   1. Filtre esnetildi — 3 kategori: Guclu / Orta / Zayif
#      - ESKI: Tam eslesmezse direkt "Zayif Sinyal"
#      - YENI: Tam eslesme → 🟢 Guclu Sinyal %XX
#             1 eksik veya en az yarisi eslesirse → 🟡 Orta Sinyal %XX
#             Hic eslesmezse veya yarisinin altinda → 🔴 Zayif Sinyal
#      - Ayni not sinyal bildirimi + periyodik TP/SL + TP/SL gerçeklesince yaziliyor
# bot_v582 — 19 Haziran 2026
# Degisiklikler (v581 -> v582):
#   1. filtre_notu kayit zamanlama hatasi duzeltildi
#      - ESKI: message_id gelmeden once symbol+sinyal_tipi ile kayit ariyordu
#      - YENI: sinyal_kaydet sonrasi message_id ile dogru kayda yaziliyor
#      - Boylece ayni anda gelen birden fazla sinyalde karisiklik olmaz
# bot_v581 — 19 Haziran 2026
# Degisiklikler (v580 -> v581):
#   1. TP/SL bildirimi artik sinyal anindaki filtre_notu u kullanıyor
#      - ESKI: bildirim gonderilirken anlik piyasa kosullarına bakıyordu
#      - YENI: sinyal kaydindaki filtre_notu okunuyor (degismez)
#      - Boylece sinyal Zayif ise bildirim de Zayif, Guclu ise Guclu kalir
# bot_v580 — 18 Haziran 2026
# Degisiklikler (v579 -> v580):
#   1. piyasa_yonu hesabi artik Market Yonu mantigi ile yapiliyor (7 faktor)
#   2. _GLOBAL cache ye market_yonu (5 kategori) eklendi
#   3. sinyal_kaydet e market_yonu alani eklendi
#   4. /tplog kombinasyon etiketleri Piyasa → Market olarak guncellendi
#   5. piyasa_yonu 3 kategori (Yukselis/Dusus/Yatay) tplog icin korundu
# bot_v579 — 18 Haziran 2026
# Degisiklikler (v578 -> v579):
#   1. piyasa_yonu hesabi Market Yonu ile ayni hale getirildi
#      - ESKI: sadece skor + breadth (2 faktor)
#      - YENI: skor + breadth + OI + CVD + Funding + Likidasyon + Taker (7 faktor)
#      - Esikler de Market Yonu ile ayni (>=5 Yukselis, >=3 Yukselis, <=-2 Dusus)
#      - Sinyal kaydindaki piyasa_yonu artik daha kapsamli ve tutarli
# bot_v578 — 18 Haziran 2026
# Degisiklikler (v577 -> v578):
#   1. Market Yonu puan hesabi duzeltildi
#      - Breadth max +3 → +2 (tek basina Yukari Yonlu tetikliyordu)
#      - Yukari Yonlu esigi 4 → 5 (daha kati kosul)
#      - Hafif Yukari Yonlu esigi 2 → 3
#      - Ornek: breadth=8 + CVD birikim = puan 4 → artik Hafif Yukari (onceden Yukari)
# bot_v577 — 18 Haziran 2026
# Degisiklikler (v576 -> v577):
#   1. "TP Gerceklesme Orani" → "Guclu Sinyal" olarak degistirildi
# bot_v576 — 18 Haziran 2026
# Degisiklikler (v575 -> v576):
#   1. TP/SL bildirim mesajina da filtre notu eklendi
#      - Kosul uyustuysa basliga TP orani, uyusmazsa Zayif Sinyal notu
# bot_v575 — 18 Haziran 2026
# Degisiklikler (v574 -> v575):
#   1. Sinyal normalize hatasi duzeltildi
#      - "STRONG BUY" → "Strong Buy" donusumu eklendi
#      - Filtre lookup artik buyuk/kucuk harf farketmez
# bot_v574 — 18 Haziran 2026
# Degisiklikler (v573 -> v574):
#   1. BEN KUL YUTMAM cift yazilma hatasi duzeltildi
#      - Caption zaten mesaji iceriyor, biz sadece filtre notunu ekliyoruz
#      - Kosul uyustuysa ilk satirdan sonra TP orani, uyusmazsa Zayif Sinyal
# bot_v573 — 18 Haziran 2026
# Degisiklikler (v572 -> v573):
#   1. TP Filtre mantigi duzeltildi
#      - ESKI: filtreden gecemeyen sinyal TOPIC_ALARM'a gitmiyordu (veri azaliyor)
#      - YENI: tum sinyaller gecor, kosul uyustuysa caption'a TP orani eklenir
#      - Kosul uyusmadiysa caption'a "⚠️ Zayif Sinyal" notu duser
#      - Filtre henuz yoksa sadece "❗ BEN KUL YUTMAM ❗" eklenir, not yok
# bot_v572 — 18 Haziran 2026
# Degisiklikler (v571 -> v572):
#   1. Dinamik TP Filtresi eklendi
#      - Her 30 dakikada _tp_filtre_guncelle() calisir
#      - Her sinyal tipi icin en iyi kosul _aktif_filtreler dict'ine yazilir
#      - Sinyal geldiginde mevcut piyasa kosullari filtreyle karsilastirilir
#      - Tum faktorler eslesmezse sinyal TOPIC_ALARM'a gitmez (sessiz)
#      - Yeterli veri yoksa (filtre bos) sinyal yine gitmez
#      - Filtreyi gecen sinyalin caption basina TP orani eklenir
#   2. _tp_filtre_zamanlayici: her 30 dakikada tpozet gorseli TOPIC_RAPOR'a gider
# bot_v571 — 17 Haziran 2026
# Degisiklikler (v570 -> v571):
#   1. MIN_MEGA dinamik eşik — sinyal tipi toplam sayısına göre otomatik ayarlanır
#      - 200+ sinyal → MIN_MEGA = 10 (önceki sabit değer)
#      - 50–199 sinyal → MIN_MEGA = 5
#      - 50'den az sinyal → MIN_MEGA = 3
#      - Böylece Short gibi az veri biriken sinyal tiplerinde de kombinasyon gösterilir
#   1. /tp komutu eklendi (/tp_rapor ile aynı)
#   2. /tpozet yeni komut — birikimli en iyi koşullar görseli
#      - /tplog ile aynı format, tek satır per sinyal tipi
#      - Tarih aralığı başlıkta gösteriliyor
#   3. /tplog koşul sütunu 45→55 karakter, YON_SAG 0.96
# bot_v559 — 16 Haziran 2026
# Degisiklikler (v558 -> v559):
#   1. /tp_rapor görsellerine alt kod eklendi (\n/tp)
#   2. /tplog max 12 saatte ikiye bölünüyor
#      - 18 saat → 2 görsel (12 + 6)
#      - Her görsel ayrı gönderilir, caption'da (1/2), (2/2) yazar
# bot_v558 — 16 Haziran 2026
#   1. Kritik bug düzeltildi: top3_mega döngüsündeki 'en_iyi' değişkeni
#      return bloğundaki 'en_iyi' dict'ini eziyordu
#      - en_iyi → en_iyi_st olarak yeniden adlandırıldı
# bot_v557 — 16 Haziran 2026
#   1. Zamanlayıcıda snapshot tüm çağrılara geçirildi
#      - _tp_confluance_skoru_hesapla, _tp_kosul_rapor_gorsel, _tp_kosul_yorum_uret
#   2. Bot restart sonrası gunluk_sinyaller boşsa sinyaller.json'dan yükleniyor
#   3. Confluance hata traceback eklendi (debug için)
# bot_v556 — 16 Haziran 2026
#   1. Mega kombinasyon sinyal tipine göre ayrı ayrı hesaplanıyor
#      - Her sinyal tipi (Long, Short, Strong Buy, Strong Sell) için ayrı piyasa koşulları analizi
#      - Sinyal tipleri genel TP oranına göre büyükten küçüğe sıralanıyor
#      - RSI ve hacim veri olgunlaştıkça otomatik kombinasyona dahil oluyor
#   2. /tplog görseli yeni format:
#      - Saat | Sinyal %genel_oran | En İyi Koşul | TP% | TP/Sin
#      - Her saat 4 satır (bir sinyal tipi başına)
#      - Genel sütunu kaldırıldı, oran sinyal yanına yazıldı
#      - Koşul sütunu genişletildi (45 karakter)
#   3. snapshot tüm çağrı zincirinden geçiyor (bot restart sonrası da çalışır)
# bot_v555 — 16 Haziran 2026
#   1. Mega kombinasyon için tüm sinyaller kullanılıyor (gunluk_sinyaller boşsa sinyaller.json'dan oku)
#      - Bot restart sonrası da doğru çalışır
#   2. snapshot tüm çağrı zincirinden geçiriliyor:
#      _tp_rapor_gonder → _tp_kosul_rapor_gorsel → _tp_kosul_yorum_uret → _tp_confluance_skoru_hesapla
# bot_v554 — 16 Haziran 2026
#   1. MIN_MEGA 5 → 15 (düşük veri rastlantısal %100 oranları engellendi)
#   2. Tavsiye yön mantığı düzeltildi
#      - Her kombinasyonda LONG ve SHORT kelime sayısı ayrı ayrı sayılır
#      - Net fark (LONG - SHORT) × skor olarak ağırlıklandırılır
#      - "Strong Sell + Düşüş" kombinasyonu artık SHORT'a katkı yapar
# bot_v553 — 16 Haziran 2026
#   1. /tplog sütunları eşit aralıklı yapıldı
#      - XPOS: [0.005, 0.055, 0.085, 0.60, 0.70, 0.80]
#      - Koşul sütunu ile TP% arası boşluk azaldı
#      - Yön sütunundan sonra (YON_SAG=0.88) tablo bitiyor
# bot_v552 — 16 Haziran 2026
#   1. Mega kombinasyon mantığı kökten değiştirildi
#      - ESKİ: coin + piyasa koşulları (hangi parite başarılı?)
#      - YENİ: sadece piyasa koşulları, coin YOK (TP anında koşullar neydi?)
#      - TF + Sinyal Tipi + Piyasa Yönü + Funding + OI + RSI + Hacim + F&G
#      - Tüm paritelerin verisi birleştirilir, koşul bazlı analiz yapılır
# bot_v551 — 16 Haziran 2026
#   1. Tavsiye yönü top5 → top3_mega üzerinden hesaplanıyor
#      - Gösterilen kombinasyonlarla tutarlı
#      - "Strong Sell + Düşüş" kombinasyonu varken LONG demesi önlendi
# bot_v550 — 16 Haziran 2026
#   1. /tplog görseli son hali:
#      - Başlıklar beyaz + kalın
#      - Tüm satırlar beyaz + kalın
#      - Yön sütunu left align
#      - figsize = 12 * TABLO_SAG — sağda siyah alan yok
#      - TABLO_SAG = 0.90 ile tablo sınırlı
# bot_v549 — 16 Haziran 2026
#   1. /tplog koşul sütunu daraltıldı
#      - XPOS: Koşul sonu 0.72 → 0.55, TP% 0.79 → 0.63, TP/Sin 0.88 → 0.76
#      - Koşul max uzunluk 40 → 30 karakter
#   2. /tplog tarih sıfırlama saati 01:10 olarak ayarlandı
#      - Günlük rapor 01:10'da başlar, ertesi gün 01:09'da biter
#      - 00:10 kaydı bir önceki günün raporuna dahil edilir
#      - Parametresiz /tplog: saat 00:00-01:09 arası ise önceki gün, değilse bugün
# bot_v548 — 16 Haziran 2026
#   1. Mega kombinasyon iki katmanlı hale getirildi
#      - Önce coin'SİZ gruplama (TF + sinyal + piyasa koşulları) — daha geniş veri
#      - Sonra coin'Lİ gruplama — granüler, az veri
#      - coin'siz kombinasyonlar min 10 sinyal eşiği (daha güvenilir)
#      - coin'li kombinasyonlar sadece coin'siz karşılığı yoksa listeye girer
#      - Böylece "3 farklı coin, aynı kombinasyon" tekrarı önlendi
# bot_v547 — 16 Haziran 2026
#   1. _tp_kosul_yorum_uret: sadece En İyi TP Koşulları (mega kombinasyon) gösterilir
#      - 1-11 arası tüm eski bölümler kaldırıldı
#      - Sadece 🔮 En İyi TP Koşulları — top3_mega
#   2. _tplog_gorsel_uret: tamamen yenilendi
#      - En İyi TP Koşulları formatı (top3_mega)
#      - Sütunlar daraltıldı: Saat | # | Koşul | TP% | TP/Sin | Yön
#      - figsize 16 → 12 (Telegram için dar)
#      - Daha küçük font, daha az boşluk
# bot_v546 — 16 Haziran 2026
#   1. Tüm tablolarda sütun genişlikleri orantılı yapıldı
#      - Kombinasyon sütunu: %40 genişlik (önceden eşit pay)
#      - Sayısal sütunlar (Toplam, TP, Devam, SL, Oran): daha dar
#      - "7. Öneri Mekanizması — En Güvenilir Koşullar" başlığı artık sığıyor
#   2. MIN_MEGA 8 → 5 geri alındı (8 çok kısıtlayıcıydı, veri yok diyordu)
# bot_v545 — 16 Haziran 2026
#   1. Mega kombinasyonda OI prefix düzeltildi
#      - "Artıyor" → "OI Artıyor", "Azalıyor" → "OI Azalıyor"
#   2. MIN_MEGA 5 → 8 olarak artırıldı (daha anlamlı kombinasyonlar)
#   3. Beraberlik durumunda sinyal sayısı tiebreaker olarak eklendi
# bot_v544 — 16 Haziran 2026
#   1. Piyasa yönü karşılaştırma metni düzeltildi
#      - Daha iyi olan taraf artık önce gösteriliyor
#      - Örnek: "Düşüş trendinde... (%41.8 vs %33.3)" — düşüş oranı önce
#   2. Mega kombinasyonda TF etiketi düzeltildi
#      - "1" → "1DK", "5" → "5DK", "60" → "1SA" vb. eşleme eklendi
# bot_v543 — 16 Haziran 2026
#   1. /analiz BTC Funding metin hatası düzeltildi
#      - btc_fund zaten % cinsinden geliyordu, * 100 yapılarak 100x büyük gösteriliyordu
#      - Örnek: -0.0061% yerine -0.6100% yazıyordu — düzeltildi
# bot_v542 — 16 Haziran 2026
#   1. _tp_confluance_skoru_hesapla: mega kombinasyon hesabı eklendi
#      - Tüm sinyal kayıtlarından coin+TF+sinyal_tipi+piyasa+funding+OI+RSI+hacim+F&G
#        faktörlerini birleştirerek TP oranı hesaplar
#      - Min. 5 sinyal şartıyla en iyi 3 kombinasyon döner (top3_mega)
#   2. _tp_kosul_yorum_uret: 🔮 bölümü tamamen yenilendi
#      - Eski: uzun açıklamalar, etkiler, top5 listesi
#      - Yeni: sadece top 3 tek satır format:
#        🥇 BTC.P + 1DK + Strong Buy + Yükseliş + ... → %71 TP (71/100)
#   3. Piyasa yönü metin hatası düzeltildi (önceki sürümde karşılaştırma yanlıştı)
# bot_v541 — 16 Haziran 2026
# Degisiklikler (v539 -> v540):
#   1. _tp_kosul_rapor_hesapla: 8 yeni kombinasyon dict eklendi
#      - dortu_kombinasyonlar: Coin + TF + Piyasa + Funding
#      - sinyal_piyasa_kombinasyonlar: Sinyal tipi + Coin + Piyasa
#      - oi_funding_kombinasyonlar: Funding + OI + Sinyal tipi
#      - hacim_kombinasyonlar: Coin + TF + Hacim
#      - fg_sinyal_kombinasyonlar: F&G kategori + Sinyal tipi
#      - hacim_piyasa_kombinasyonlar: Hacim + Piyasa yönü
#      - rsi_sinyal_kombinasyonlar: RSI kategori + Sinyal tipi
#      - rsi_piyasa_kombinasyonlar: RSI kategori + Piyasa yönü
#   2. sinyal_kaydet: rsi_deger ve rsi_kategori alanları eklendi
#   3. _tp_kosul_yorum_uret: guvenskoru fonksiyonu + ayıraç + yeni bölümler
#      - guven_etiketi: <10 ⚠️ düşük veri, 10-29 🔸 orta güven, 30-59 ✅ güvenilir, 60+ 💎 yüksek güven
#      - Ayıraç (―――――――――――――――――――) her bölüm sonuna eklendi
#      - 🔷 Dörtlü kombinasyon bölümü
#      - 🎯 Sinyal tipi + Coin + Piyasa bölümü
#      - ⚡ Funding + OI + Sinyal tipi bölümü
#      - 📊 Coin + TF + Hacim bölümü
#      - 😱 F&G + Sinyal tipi bölümü
#      - 🔊 Hacim + Piyasa yönü bölümü
#      - 📈 RSI + Sinyal tipi bölümü
#      - 📉 RSI + Piyasa yönü bölümü
#   4. _tp_confluance_skoru_hesapla: tüm kombinasyonları birleştirip confluance skoru üretir
#      - agirlikli skor: TP orani * log(sinyal sayisi)
#      - tavsiye_yon: LONG / SHORT / NÖTR (anahtar kelime oylama ile)
#      - tavsiye_guven: yüksek güven / güvenilir / orta güven / düşük veri
#   5. _tp_confluance_log_yaz: /data/tp_confluance_log.json'a saatlik append
#      - Son 720 kayıt tutulur (30 günlük)
#   6. _tp_kosul_rapor_zamanlayici: her :10'da confluance log yazılır
#   7. _tp_kosul_yorum_uret: 🔮 En İyi TP Koşulu (Confluance Analizi) bölümü eklendi
#   8. /tplog komutu eklendi (TOPIC_RAPOR)
#      - Tarih parametresi: /tplog, /tplog 16, /tplog 16-06, /tplog 16-06-2025
#      - Her saat için top 3 kombinasyon gösterir
#      - TP/Sin sütunu, güven etiketi, tavsiye yönü
#      - Alt özet: LONG / SHORT / NÖTR kayıt sayıları
# bot_v539 — 15 Haziran 2026
...
gunluk_sinyaller  = []
filtre_sinyaller  = []   # filtre_veri.json için — TF+Funding+OI bazlı etiketleme
gunluk_kilit      = threading.Lock()
_veri_kayit_kilit    = threading.Lock()   # v632: sinyaller.json yazma race condition duzeltmesi
_filtre_kayit_kilit  = threading.Lock()   # v632: filtre_veri.json yazma race condition duzeltmesi
...
FILTRE_VERI_DOSYASI   = os.getenv("FILTRE_VERI_DOSYASI", "/data/filtre_veri.json")
...
_son_liq_zone   = {}  # {"BTCUSDT": {"long": (alt, ust), "short": (alt, ust), "ts": int}, ...}
...
TARAYICI_TOP_N = 5  # Tarayıcı raporunda gösterilecek coin sayısı
...
def veri_kaydet():
    try:
        ...
        # v632: Atomic write + RACE CONDITION duzeltmesi — ayni .tmp dosyasina
        # birden fazla thread ayni anda yazip os.replace yapinca "No such file"
        # hatasi olusuyordu. Artik tum yazma+rename islemi kilit altinda.
        with _veri_kayit_kilit:
...
def filtre_veri_kaydet():
    try:
        os.makedirs(os.path.dirname(FILTRE_VERI_DOSYASI), exist_ok=True)
        with gunluk_kilit:
            veri = filtre_sinyaller[:]
        # v632: ayni race condition duzeltmesi filtre_veri.json icin de
        with _filtre_kayit_kilit:
            tmp_dosya = FILTRE_VERI_DOSYASI + ".tmp"
            with open(tmp_dosya, "w", encoding="utf-8") as f:
                json.dump(veri, f, ensure_ascii=False)
            os.replace(tmp_dosya, FILTRE_VERI_DOSYASI)
    except Exception as e:
        print(f"[FILTRE_KAYIT] Hata: {e}")


def filtre_dosyadan_yukle():
    global filtre_sinyaller
    try:
        if os.path.exists(FILTRE_VERI_DOSYASI):
            with open(FILTRE_VERI_DOSYASI, "r", encoding="utf-8") as f:
                veri = json.load(f)
            with gunluk_kilit:
                filtre_sinyaller = veri
            print(f"[FILTRE_YUKLE] {len(veri)} filtre kaydı yüklendi.")
        else:
            print("[FILTRE_YUKLE] filtre_veri.json yok — boş başlatıldı.")
    except Exception as e:
        print(f"[FILTRE_YUKLE] Hata: {e}")
...
def dosyadan_yukle():
    ...
    # Filtre verisini de yükle
    filtre_dosyadan_yukle()
...
def _hacim_durumu_belirle(symbol):
    """Hacim durumunu _son_analiz_veriler cache'indeki vol_ratio'dan okur — API çağrısı yapmaz."""
    try:
        sym_clean = symbol.replace(".P", "")
        veri = _son_analiz_veriler.get(sym_clean, {}) if _son_analiz_veriler else {}
        vol_ratio = veri.get("vol_ratio")
...
def _olta_yakinlik_hesapla(symbol, sinyal, price):
    """
    Sinyal fiyatının olta fib giriş seviyesine yakınlığını hesaplar.
    Long/Strong Buy → long giriş seviyesi
    Short/Strong Sell → short giriş seviyesi
    Döner: float (% mesafe) veya None
    """
    try:
        closes, volumes, highs, lows = _trend_fetch_klines(symbol, interval="5m", limit=200)
        if not closes:
            return None
        r = _olta_fib_hesapla(closes, highs, lows, price, "5m")
        if not r:
            return None
        sinyal_ust = sinyal.upper()
        if "LONG" in sinyal_ust or "BUY" in sinyal_ust:
            giris = r.get("long_giris")
        else:
            giris = r.get("short_giris")
        if not giris or price <= 0:
            return None
        mesafe_pct = abs(price - giris) / price * 100
        return round(mesafe_pct, 2)
    except Exception:
        return None


def _liq_yakinlik_hesapla(symbol, sinyal, price):
    """
    Sinyal fiyatının likidasyon bölgesine yakınlığını hesaplar.
    Long/Strong Buy → long likidasyon bölgesine mesafe
    Short/Strong Sell → short likidasyon bölgesine mesafe
    Döner: float (% mesafe) veya None
    """
    try:
        zone_data = _son_liq_zone.get(symbol)
        if not zone_data:
            # BTC/ETH dışı coinler için BTC zone'u referans al
            zone_data = _son_liq_zone.get("BTCUSDT")
        if not zone_data:
            return None
        # Cache 2 saatten eskiyse kullanma
        if time.time() - zone_data.get("ts", 0) > 7200:
            return None
        sinyal_ust = sinyal.upper()
        if "LONG" in sinyal_ust or "BUY" in sinyal_ust:
            zone = zone_data.get("long")
        else:
            zone = zone_data.get("short")
        if not zone or price <= 0:
            return None
        zone_alt, zone_ust = zone
        # Fiyat bölge içindeyse mesafe 0
        if zone_alt <= price <= zone_ust:
            return 0.0
        # En yakın kenara mesafe
        mesafe = min(abs(price - zone_alt), abs(price - zone_ust))
        return round(mesafe / price * 100, 2)
    except Exception:
        return None


# === v640 GUVEN SKORU (rolling, coin-agnostik) ===================
GUVEN_SKORU_PENCERE    = 25   # (symbol, sinyal_tipi) icin bakilacak son sonuc sayisi
GUVEN_SKORU_MIN_ORNEK  = 8    # bundan az sonuclanmis ornek varsa genel tabana don

def _alarm_gonderilen(s):
    """v647: Bir sinyalin TOPIC_ALARM'a gonderilmis OLMASI GEREKTIGINI (kayitli
    filtre_notu/guven_skoru degerlerine gore) belirler — webhook'taki
    ALARM_GONDER ile birebir ayni mantik. Panel A, Panel C gibi 'sadece
    TOPIC_ALARM sinyalleri' filtreleyen tum raporlarin ORTAK fonksiyonu."""
    fn = s.get("filtre_notu") or ""
    gs = s.get("guven_skoru")
    return fn.startswith("🟢") or (gs is not None and gs >= 0.40)


def _guven_skoru_hesapla(symbol, sinyal_tipi):
    """(symbol, sinyal_tipi) kombinasyonunun son GUVEN_SKORU_PENCERE
    SONUCLANMIS (tp1_ok is not None) sinyalindeki basari oranini dondurur.
    Az veri varsa genel (tum sonuclanmis sinyallerin) basari oranina duser.
    SADECE GECMISE bakar (gunluk_sinyaller zaten kronolojik sirada, bu
    cagri yeni kayittan ONCE yapiliyor) — veri sizintisi yok.
    """
    try:
        with gunluk_kilit:
            gecmis = gunluk_sinyaller[:]
        ilgili = [s for s in gecmis
                  if s.get("symbol") == symbol
                  and s.get("sinyal_tipi") == sinyal_tipi
                  and s.get("tp1_ok") is not None]
        if len(ilgili) >= GUVEN_SKORU_MIN_ORNEK:
            son = ilgili[-GUVEN_SKORU_PENCERE:]
            return round(sum(1 for s in son if s.get("tp1_ok")) / len(son), 3)
        # Az veri: genel (tum coin/tip) sonuclanmis basari oranina don
        tum_sonuclanan = [s for s in gecmis if s.get("tp1_ok") is not None]
        if tum_sonuclanan:
            return round(sum(1 for s in tum_sonuclanan if s.get("tp1_ok")) / len(tum_sonuclanan), 3)
        return None  # hic veri yok (bot yeni basladi)
    except Exception as e:
        print(f"[GUVEN_SKORU] Hata: {e}")
        return None


def sinyal_kaydet(symbol, sinyal, timeframe, price, tp1, tp2, tp3, tp4, tp5, sl, message_id):
    # v640 GUVEN SKORU: (symbol, sinyal_tipi) kombinasyonunun SON 25 SONUCLANMIS
    # (tp1_ok is not None) sinyalindeki basari oranini kullanan, tum coinlere
    # AYNI FORMULLE uygulanan, zamanla guncellenen (rolling) bir skor. Sabit
    # "symbol" kimligi yerine bunu kullanmamizin sebebi: statik symbol etiketi
    # bazi coinlerde (BTC) performansi kotulestirirken bazilarinda (BNB) asiri
    # iyilestiriyordu — coin'e ozel, tutarsiz bir davranisti. Rolling skor tek
    # bir formul, coin'in GUNCEL performansina gore kendini ayarliyor. Az veri
    # varsa (< 8 sonuclanmis ornek) genel ortalamaya (GUVEN_SKORU_TABAN) duser.
    # NOT: Bu skor SADECE ETIKETLEME icin — hicbir sinyal susturulmuyor,
    # bildirim akisina veya MEXC'e DOKUNULMUYOR.
    sinyal_tipi_on = _sinyal_tipi_belirle(sinyal)
    guven_skoru = _guven_skoru_hesapla(symbol, sinyal_tipi_on)
    ...
    if fg_deger is not None:
        try:
            _fg_val = float(fg_deger)
            if _fg_val <= 24:   fg_sinif = "Aşırı Korku"
            elif _fg_val <= 44: fg_sinif = "Korku"
            elif _fg_val <= 55: fg_sinif = "Nötr"
            elif _fg_val <= 75: fg_sinif = "Açgözlülük"
            else:               fg_sinif = "Aşırı Açgözlülük"
        except Exception:
            fg_sinif = None
    ...
    market_yonu  = _global.get("market_yonu")
    hacim_durumu = _hacim_durumu_belirle(symbol)
    seans        = _seans_belirle()
    sinyal_tipi  = sinyal_tipi_on
    # RSI — coin bazlı analiz cache'inden oku
    # Cache'e "LINKUSDT" formatında yazılıyor (sonuclar[sym] = veri)
    try:
        _sym_usdt  = symbol.replace(".P", "")  # ETHUSDT.P → ETHUSDT
        rsi_deger  = _son_analiz_veriler.get(_sym_usdt, {}).get("rsi") if _son_analiz_veriler else None
        if rsi_deger is None:
            # Fallback: USDT olmadan dene (eski format)
            _sym_base = symbol.replace(".P", "").replace("USDT", "")
            rsi_deger = _son_analiz_veriler.get(_sym_base, {}).get("rsi") if _son_analiz_veriler else None
        rsi_deger  = round(rsi_deger, 1) if rsi_deger is not None else None
    except Exception:
        rsi_deger = None
    # RSI kategorisi
    if rsi_deger is not None:
        if rsi_deger >= 70:   rsi_kategori = "Aşırı Alım"
        elif rsi_deger >= 60: rsi_kategori = "Alım Yakın"
        elif rsi_deger <= 30: rsi_kategori = "Aşırı Satım"
        elif rsi_deger <= 40: rsi_kategori = "Satım Yakın"
        else:                 rsi_kategori = "Nötr"
    else:
        rsi_kategori = None

    # Olta ve likidasyon yakınlığı — arka planda hesapla (hata olursa None geçer)
    try:
        olta_yakinlik = _olta_yakinlik_hesapla(symbol, sinyal, price) if price else None
    except Exception:
        olta_yakinlik = None
    try:
        liq_yakinlik = _liq_yakinlik_hesapla(symbol, sinyal, price) if price else None
    except Exception:
        liq_yakinlik = None

    # L/S oranı — global cache'den oku (saatte bir güncelleniyor)
    try:
        _sym_ls  = symbol.replace(".P", "").replace("USDT", "")  # ETHUSDT.P → ETH
        _ls_data = _son_ls_cache.get(_sym_ls) or _son_ls_cache.get("BTC")  # coin yoksa BTC fallback
        ls_ky_long  = _ls_data.get("ky_long")  if _ls_data else None
        ls_bal_long = _ls_data.get("bal_long") if _ls_data else None
        # L/S yorumu: >65 aşırı long, <35 aşırı short
        if ls_ky_long is not None:
            if ls_ky_long >= 65:   ls_kategori = "Aşırı Long"
            elif ls_ky_long >= 55: ls_kategori = "Long Baskın"
            elif ls_ky_long <= 35: ls_kategori = "Aşırı Short"
            elif ls_ky_long <= 45: ls_kategori = "Short Baskın"
            else:                  ls_kategori = "Dengeli"
        else:
            ls_kategori = None
    except Exception:
        ls_ky_long = ls_bal_long = ls_kategori = None

    # Üst TF teyidi — sinyal TF'nin bir üstündeki trend aynı yönde mi?
    try:
        _tf_ust_map = {"1": "5", "5": "15", "15": "60", "60": "240"}
        _tf_ust     = _tf_ust_map.get(str(timeframe))
        _sinyal_upper = sinyal.upper() if sinyal else ""
        _is_long_sin  = any(x in _sinyal_upper for x in ["BUY", "LONG"])
        _is_short_sin = any(x in _sinyal_upper for x in ["SELL", "SHORT"])
        if _tf_ust and _son_analiz_veriler:
            _sym_usdt2  = symbol.replace(".P", "")  # ETHUSDT.P → ETHUSDT
            _coin_veri  = _son_analiz_veriler.get(_sym_usdt2, {})
            if not _coin_veri:
                _sym_base2 = symbol.replace(".P", "").replace("USDT", "")
                _coin_veri = _son_analiz_veriler.get(_sym_base2, {})
            _skor       = _coin_veri.get("skor")  # 0-100
            if _skor is not None:
                if _is_long_sin:
                    ust_tf_teyit = "Uyumlu" if _skor >= 50 else "Uyumsuz"
                elif _is_short_sin:
                    ust_tf_teyit = "Uyumlu" if _skor < 50 else "Uyumsuz"
                else:
                    ust_tf_teyit = None
    ...
    kayit = {
        ...
        "market_yonu":   market_yonu,
        "hacim_durumu":  hacim_durumu,
        "seans":         seans,
        "rsi_deger":     rsi_deger,
        "rsi_kategori":  rsi_kategori,
        # --- Confluence verileri ---
        "olta_yakinlik": olta_yakinlik,  # % mesafe — fiyat ile olta fib girişi arası
        "liq_yakinlik":  liq_yakinlik,   # % mesafe — fiyat ile likidasyon bölgesi arası
        # --- L/S ve üst TF teyidi ---
        "ls_ky_long":    ls_ky_long,     # KY long oranı %
        "ls_bal_long":   ls_bal_long,    # Balina long oranı %
        "ls_kategori":   ls_kategori,    # Aşırı Long / Long Baskın / Dengeli / Short Baskın / Aşırı Short
        "ust_tf_teyit":  ust_tf_teyit,   # Uyumlu / Uyumsuz — üst TF trend teyidi
        # --- v640: rolling guven skoru (SADECE ETIKET, filtre/susturma yok) ---
        "guven_skoru":   guven_skoru,    # (symbol, sinyal_tipi) son 25 sonucun basari orani
    }
    with gunluk_kilit:
        gunluk_sinyaller.append(kayit)
    veri_kaydet()
    # Filtre verisine de kaydet — sadece etiketleme için gereken alanlar
    filtre_kayit = {
        "zaman":        kayit["zaman"],
        "message_id":   message_id,
        "sinyal_tipi":  sinyal_tipi,
        ...
        "funding_yonu": funding_yon,
        "oi_yonu":      oi_yonu,
        "piyasa_yonu":  piyasa_yonu,   # v628: piyasa rejimi (Yükseliş/Düşüş/Yatay) — etiketlemede kullanılacak
        "tp1_ok":       None,
        "sl_ok":        None,
    }
    with gunluk_kilit:
        filtre_sinyaller.append(filtre_kayit)
    filtre_veri_kaydet()
    if TELEGRAM_LOG_ID:
    ...
    with gunluk_kilit:
        for s in gunluk_sinyaller:
            if s["message_id"] == message_id:
                ...
                sinyal_ref = dict(s)
                break
        # filtre_sinyaller'i de güncelle
        for fs in filtre_sinyaller:
            if fs.get("message_id") == message_id:
                fs["tp1_ok"] = tp1_ok
                fs["sl_ok"]  = sl_ok
                break
    veri_kaydet()
...
def tp_kontrol_gonder(symbol, sinyal, timeframe, tp1, tp2, tp3, tp4, tp5, sl,
    ...
    def mesaj_olustur(tp1_ok, tp2_ok, tp3_ok, tp4_ok, tp5_ok, sl_ok, erken=False):
        ...
        # Filtre notunu belirle
        # Sinyal anindaki filtre notunu kayittan oku (anlık hesap yapma)
        with gunluk_kilit:
            _kayit_b = next(
                (s for s in reversed(gunluk_sinyaller) if s.get("message_id") == message_id), None
            )
        _filtre_notu_b = _kayit_b.get("filtre_notu") if _kayit_b else None
        baslik = baslik + (nl + _filtre_notu_b if _filtre_notu_b else "")
...
def send_telegram_and_schedule_tp(caption, symbol, timeframe, sinyal,
    ...
    # ── TP Filtre Notu — TF + Funding + OI kombinasyon bazlı ─────────────────
    # filtre_sinyaller'deki geçmiş veriden bu kombinasyonun TP oranını oku.
    # Her sinyal tipinin genel ortalamasına göre eşik uygula:
    #   Güçlü  : combo_oran >= genel_ort + 8
    #   Orta   : combo_oran >= genel_ort - 5
    #   Zayıf  : combo_oran < genel_ort - 5 veya veri yetersiz (<10 sinyal)
    #   Etiket yok: filtre_sinyaller'de bu sinyal tipine ait hiç kapanmış sinyal yok

    _SINYAL_NORM = {
        "LONG": "Long", "SHORT": "Short",
        "STRONG BUY": "Strong Buy", "STRONG SELL": "Strong Sell",
    }
    sinyal_norm = _SINYAL_NORM.get(sinyal.upper(), sinyal)
    _TF_MAP_FIL = {"1":"1DK","3":"3DK","5":"5DK","15":"15DK","30":"30DK","60":"1SA","240":"4SA","D":"1G"}
    tf_str_fil  = _TF_MAP_FIL.get(str(timeframe), str(timeframe)) if timeframe else "?"

    # O anki Funding ve OI yönünü cache'den oku
    _global_fil = _son_analiz_veriler.get("_GLOBAL", {}) if _son_analiz_veriler else {}
    _fund_val   = _global_fil.get("btc_fund")
    funding_yonu = ("Pozitif" if (_fund_val or 0) > 0 else
                    "Negatif" if (_fund_val or 0) < 0 else "Nötr") if _fund_val is not None else None
    try:
        _btc_oi = _son_analiz_veriler.get("BTCUSDT", {}).get("oi_delta") if _son_analiz_veriler else None
        oi_yonu = ("Artıyor" if _btc_oi and _btc_oi.get("delta_pct", 0) > 0 else
                   "Azalıyor" if _btc_oi and _btc_oi.get("delta_pct", 0) < 0 else None)
    except Exception:
        oi_yonu = None
    piyasa_yonu_fil = _global_fil.get("piyasa_yonu")   # v628: piyasa rejimi

    filtre_notu = None
    try:
        with gunluk_kilit:
            snapshot = filtre_sinyaller[:]

        ESIK_G = 8.0   # genel ortalamanın bu kadar üstü → Güçlü
        ESIK_Z = 5.0   # genel ortalamanın bu kadar altı → Zayıf
        MIN_GENEL  = 5   # etiket için sinyal tipinde minimum kapali sinyal
        MIN_COMBO  = 3   # kombinasyon için minimum sinyal (veri az iken çalışsın)

        # Bu sinyal tipine ait kapanmış tüm sinyaller
        kapali = [s for s in snapshot
                  if s.get("sinyal_tipi") == sinyal_norm
                  and (s.get("tp1_ok") is not None or s.get("sl_ok") is not None)]

        if len(kapali) >= MIN_GENEL:
            # Genel TP ortalaması
            tp_sayisi  = sum(1 for s in kapali if s.get("tp1_ok") == True)
            genel_ort  = tp_sayisi / len(kapali) * 100

            # Bu kombinasyonun geçmiş TP oranı (v628: piyasa_yonu da kombinasyona dahil)
            combo_eslesen = [s for s in kapali
                             if s.get("timeframe") == str(timeframe)
                             and s.get("funding_yonu") == funding_yonu
                             and s.get("oi_yonu") == oi_yonu
                             and s.get("piyasa_yonu") == piyasa_yonu_fil]

            if len(combo_eslesen) >= MIN_COMBO:
                combo_tp   = sum(1 for s in combo_eslesen if s.get("tp1_ok") == True)
                combo_oran = combo_tp / len(combo_eslesen) * 100

                _piy_etiket = f" · {piyasa_yonu_fil}" if piyasa_yonu_fil else ""
                if combo_oran >= genel_ort + ESIK_G:
                    filtre_notu = f"🟢 Sinyal Gücü Oranı: %{int(round(combo_oran))}{_piy_etiket}"
                    print(f"[TP_FILTRE] {sinyal} {symbol} GUCLU: combo=%{combo_oran:.1f} genel=%{genel_ort:.1f}")
                elif combo_oran >= genel_ort - ESIK_Z:
                    filtre_notu = f"🟡 Sinyal Gücü Oranı: %{int(round(combo_oran))}{_piy_etiket}"
                    print(f"[TP_FILTRE] {sinyal} {symbol} ORTA: combo=%{combo_oran:.1f} genel=%{genel_ort:.1f}")
                else:
                    filtre_notu = f"🔴 Sinyal Gücü Oranı: %{int(round(combo_oran))}{_piy_etiket}"
                    print(f"[TP_FILTRE] {sinyal} {symbol} ZAYIF: combo=%{combo_oran:.1f} genel=%{genel_ort:.1f}")
            else:
                # Kombinasyon için yeterli veri yok → not düş
                filtre_notu = "⏳ Yeterli Filtre Verisi Yok"
                print(f"[TP_FILTRE] {sinyal} {symbol} combo veri yetersiz ({len(combo_eslesen)}<{MIN_COMBO})")
        else:
            # Sinyal tipi için henüz yeterli veri yok → not düş
            filtre_notu = "⏳ Yeterli Filtre Verisi Yok"
            print(f"[TP_FILTRE] {sinyal} {symbol} etiket yok — {len(kapali)}<{MIN_GENEL} kapali sinyal")
    except Exception as _e:
        print(f"[TP_FILTRE] Hata: {_e}")
        filtre_notu = None

    # v642: guven_skoru artik sadece gonderim kararinda degil, caption'da da
    # gorunuyor — kullanici hangi sinyalin ne kadar guvenilir sayildigini
    # etiket uzerinden gorebilsin.
    _guven_simdi = _guven_skoru_hesapla(symbol, sinyal_norm)
    guven_notu = f"🎯 Güven Skoru: %{int(round(_guven_simdi*100))}" if _guven_simdi is not None else None

    # Filtre notunu ve güven skorunu "BEN KÜL YUTMAM" satırından hemen sonra ekle
    _ek_satirlar = [x for x in [filtre_notu, guven_notu] if x]
    if _ek_satirlar:
        nl = chr(10)
        ben_kul = "❗ <b>BEN KÜL YUTMAM</b> ❗"
        _ek_blok = nl.join(_ek_satirlar)
        if ben_kul in caption:
            caption = caption.replace(ben_kul, ben_kul + nl + _ek_blok, 1)
        else:
            # Fallback: caption'ın başına ekle
            caption = _ek_blok + nl + nl + caption

    # v642 ALARM SUSTURMA: Guclu (yesil filtre_notu) VEYA guven_skoru>=0.40 olan
    # sinyaller TOPIC_ALARM'a gonderilir, digerleri SESSIZ kalir. Sinyal yine de
    # sinyaller.json'a kaydedilmeye devam eder (izleme kesilmiyor, sadece bildirim
    # gitmiyor). 25 Temmuz analizinde gercek veriyle dogrulandi: n=79 -> gecen
    # n=24 basari %45.8, susturulan n=55 basari %21.8.
    _guc_guclu   = bool(filtre_notu) and filtre_notu.startswith("🟢")
    ALARM_GONDER = _guc_guclu or (_guven_simdi is not None and _guven_simdi >= 0.40)
    ...
    if ALARM_GONDER:
        ...
        print(f"[SESSIZ] {symbol} {sinyal} — Güçlü değil ve guven_skoru={_guven_simdi} (<0.40) — alarm gönderilmedi, sadece kaydediliyor")
    ...
    # filtre_notu artık message_id ile kayda yazılıyor (doğru eşleşme)
    with gunluk_kilit:
        _fn_kayit = next(
            (s for s in reversed(gunluk_sinyaller) if s.get("message_id") == kayit_id),
            None
        )
        if _fn_kayit is not None:
            _fn_kayit["filtre_notu"] = filtre_notu
    veri_kaydet()  # filtre_notu diske yazılsın — restart sonrası TP/SL bildiriminde görünsün
...
def koin_sirala_bugun(snapshot=None):
    """Koin bazında filtreli TP sıralaması.
    filtre_sinyaller'den bugün 00:00'dan itibaren gelen sinyallere bakar.
    Güçlü etiket alan sinyallerin coin bazlı TP oranına göre sıralar.
    """
    import time as _t
    from datetime import datetime as _dt

    # Bugün 00:00 timestamp
    try:
        _simdi   = datetime.now(tz=TR_TZ) if TR_TZ else datetime.utcnow()
        _gun_bas = _simdi.replace(hour=0, minute=0, second=0, microsecond=0)
        _gun_bas_ts = _gun_bas.timestamp()
    except Exception:
        _gun_bas_ts = _t.time() - 86400

    # filtre_sinyaller'den bugünün kayıtları
    with gunluk_kilit:
        filtre_snap = filtre_sinyaller[:]

    bugun_filtre = [s for s in filtre_snap if s.get("zaman", 0) >= _gun_bas_ts]

    # sinyaller.json'dan bugünün kayıtları — coin bilgisi için
    with gunluk_kilit:
        sinyal_snap = [s for s in gunluk_sinyaller
                       if s.get("gun") == gun_str() or s.get("zaman", 0) >= _gun_bas_ts]

    # message_id → coin mapping
    mid_coin = {s.get("message_id"): s.get("symbol","") for s in sinyal_snap}
    ...
    # filtre_veri.json'daki tüm kapali sinyallerden genel TP tablosu
    kapali_tum = [s for s in filtre_snap
                  if s.get("tp1_ok") is not None or s.get("sl_ok") is not None]

    TF_MAP_K = {"1":"1DK","5":"5DK","15":"15DK","60":"1SA","240":"4SA"}
    SINYAL_TIPLERI_K = ["Strong Sell","Strong Buy","Long","Short"]

    genel_tp_k = {}
    combo_tab_k = {}
    for st in SINYAL_TIPLERI_K:
        st_k = [s for s in kapali_tum if s.get("sinyal_tipi")==st]
        genel_tp_k[st] = sum(1 for s in st_k if s.get("tp1_ok")==True)/len(st_k)*100 if len(st_k)>=5 else None
        from collections import defaultdict as _dd
        tablo = _dd(lambda: {"tp":0,"n":0})
        for s in st_k:
            tf  = TF_MAP_K.get(str(s.get("timeframe","")), str(s.get("timeframe","")))
            fun = s.get("funding_yonu") or ""
            oi  = s.get("oi_yonu") or ""
            piy = s.get("piyasa_yonu") or ""
            key = f"{tf}+{fun}+{oi}+{piy}"
            tablo[key]["n"] += 1
            if s.get("tp1_ok")==True: tablo[key]["tp"] += 1
        combo_tab_k[st] = {k:v for k,v in tablo.items() if v["n"]>=3}

    def _filtre_etiketi(sinyal_tipi, tf, funding, oi, piyasa=None):
        g_ort = genel_tp_k.get(sinyal_tipi)
        if g_ort is None: return None
        tf_str = TF_MAP_K.get(str(tf), str(tf))
        key    = f"{tf_str}+{funding or ''}+{oi or ''}+{piyasa or ''}"
        tablo  = combo_tab_k.get(sinyal_tipi, {})
        if key not in tablo: return None
        cr = tablo[key]["tp"] / tablo[key]["n"] * 100
        if cr >= g_ort + ESIK_G_K:   return "guclu"
        elif cr >= g_ort - ESIK_Z_K: return "orta"
        else:                          return "zayif"

    # Coin bazında hesapla
    SINYAL_NORM_K = {"STRONG SELL":"Strong Sell","STRONG BUY":"Strong Buy","LONG":"Long","SHORT":"Short"}
    koin_istat = {}

    for fs in bugun_filtre:
        mid  = fs.get("message_id")
        sym_raw = mid_coin.get(mid, "")
        if not sym_raw: continue
        from re import sub as _sub
        sym = _sub(r"USDT\.P$|USDT$", "", sym_raw)
        if not sym: continue

        st  = fs.get("sinyal_tipi","")
        tf  = fs.get("timeframe","")
        fun = fs.get("funding_yonu","")
        oi  = fs.get("oi_yonu","")
        piy = fs.get("piyasa_yonu","")

        if sym not in koin_istat:
            koin_istat[sym] = {"guclu_tp":0,"guclu_n":0,"orta_n":0,"zayif_n":0,"toplam":0}

        koin_istat[sym]["toplam"] += 1
        et = _filtre_etiketi(st, tf, fun, oi, piy)
        if et == "guclu":
            koin_istat[sym]["guclu_n"] += 1
            if fs.get("tp1_ok")==True: koin_istat[sym]["guclu_tp"] += 1
        elif et == "orta":
            koin_istat[sym]["orta_n"] += 1
        else:
            koin_istat[sym]["zayif_n"] += 1

    sonuc = []
    for sym, v in koin_istat.items():
        gn   = v["guclu_n"]
        gt   = v["guclu_tp"]
        oran = round(gt/gn*100, 1) if gn > 0 else 0
        yeterli = gn >= MIN_GUCLU
        sonuc.append({
            "sym":          sym,
            "oran":         oran,
            "toplam":       v["toplam"],
    ...
    sonuc.sort(key=lambda x: (-x["oran"], -x["guclu_n"]))
    return sonuc


def koin_sirala_guven_bugun(snapshot=None):
    """v644: koin_sirala_bugun'un guven_skoru karsiligi. Bugun icin, her coin'in
    guven_skoru>=0.40 olan sinyallerindeki TP oranini hesaplar. gunluk_sinyaller
    (sinyaller.json) kullanilir cunku guven_skoru ve symbol sadece orada var.
    v651 DUZELTME: "Guclu" etiketli (filtre_notu 🟢) sinyaller artik HARIC
    tutuluyor — onceden ayni sinyal hem Panel B'nin "Guclu" tablosunda hem
    "Guven Skoru Yuksek" tablosunda sayilabiliyordu, iki tablonun toplami
    Panel A/C'nin gosterdigi benzersiz toplami asiyordu (cift sayim). Artik
    bu iki tablo birbirini DISLAYAN (mutually exclusive) bir bolunme —
    toplamlari her zaman benzersiz alarm-sinyali sayisiyla birebir eslesir."""
    import time as _t
    from re import sub as _sub
    ...
    ESIK_YUKSEK = 0.40
    koin_istat = {}
    for s in sinyal_snap:
        gs = s.get("guven_skoru")
        if gs is None or gs < ESIK_YUKSEK:
            continue
        if (s.get("filtre_notu") or "").startswith("🟢"):
            continue  # v651: zaten Guclu tablosunda sayildi, tekrar sayma
        sym_raw = s.get("symbol", "")
        sym = _sub(r"USDT\.P$|USDT$", "", sym_raw)
        if not sym:
            continue
        if sym not in koin_istat:
            koin_istat[sym] = {"n": 0, "tp": 0, "skor_toplam": 0.0}
        koin_istat[sym]["n"] += 1
        koin_istat[sym]["skor_toplam"] += gs
        if s.get("tp1_ok") == True:
            koin_istat[sym]["tp"] += 1

    sonuc = []
    for sym, v in koin_istat.items():
        n, tp = v["n"], v["tp"]
        oran = round(tp/n*100, 1) if n > 0 else 0
        ort_skor = round(v["skor_toplam"]/n, 2) if n > 0 else 0
        sonuc.append({"sym": sym, "n": n, "tp": tp, "oran": oran, "ort_skor": ort_skor})

    sonuc.sort(key=lambda x: (-x["oran"], -x["n"]))
    return sonuc


def top3_guncelle(snapshot=None):
    """Filtreli TP oranı >= TRADE_WHITELIST_MIN_PERCENT ve yeterli veri olan coinleri whitelist'e al.
    Yeterli veri yok (Güçlü sinyal < 5) → coin whitelist dışı, işlem açılmaz.
    """
    ...
    if not sirala:
        with top3_kilit:
            top3_whitelist = []
        print(f"[WHITELIST] Veri yok — whitelist boş")
        return []

    yeni_wl = [
        item["sym"] for item in sirala
        if item["yeterli_veri"] and item["oran"] >= TRADE_WHITELIST_MIN_PERCENT
    ]

    with top3_kilit:
        eski_wl = list(top3_whitelist)
        top3_whitelist = yeni_wl

    print(f"[WHITELIST] Güncellendi (eşik:%{TRADE_WHITELIST_MIN_PERCENT}): {top3_whitelist}")

    # Whitelist değiştiyse ve AUTO_TRADE açıksa bildir
    if AUTO_TRADE_ENABLED and set(yeni_wl) != set(eski_wl):
        yeni_girenler = [c for c in yeni_wl if c not in eski_wl]
        cikanlar      = [c for c in eski_wl if c not in yeni_wl]
        mesaj_satirlari = ["📋 <b>Whitelist Güncellendi</b>"]
        if yeni_girenler:
            mesaj_satirlari.append(f"✅ Eklendi: {', '.join(yeni_girenler)}")
        if cikanlar:
            mesaj_satirlari.append(f"❌ Çıkarıldı: {', '.join(cikanlar)}")
        if yeni_wl:
            mesaj_satirlari.append(f"📌 Aktif: {', '.join(yeni_wl)}")
        else:
            mesaj_satirlari.append("⚠️ Whitelist boşaldı — işlem açılmayacak")
        try:
            _mexc_topic_mesaj_gonder("\n".join(mesaj_satirlari))
        except Exception as _we:
            print(f"[WHITELIST] Bildirim hatası: {_we}")

    return yeni_wl


def koin_sirala_metni(snapshot=None):
    """Sinyal kalite raporu — v619.
    Tüm istatistikler filtre_sinyaller'den bugün 00:00'dan itibaren hesaplanır.
    """
    top3_guncelle(snapshot)
    sirala = koin_sirala_bugun(snapshot)
    with top3_kilit:
        wl = list(top3_whitelist)

    # Bugün 00:00 timestamp
    try:
        _simdi   = datetime.now(tz=TR_TZ) if TR_TZ else datetime.utcnow()
        _gun_bas_ts = _simdi.replace(hour=0, minute=0, second=0, microsecond=0).timestamp()
    except Exception:
        _gun_bas_ts = 0

    # filtre_sinyaller'den bugünün kapali sinyalleri
    with gunluk_kilit:
        filtre_snap = filtre_sinyaller[:]

    bugun_kapali = [s for s in filtre_snap
                    if s.get("zaman", 0) >= _gun_bas_ts
                    and (s.get("tp1_ok") is not None or s.get("sl_ok") is not None)]

    # filtre_veri.json kombinasyon tablosundan etiket hesapla
    TF_MAP_M   = {"1":"1DK","5":"5DK","15":"15DK","60":"1SA","240":"4SA"}
    ESIK_G_M   = 8.0
    ESIK_Z_M   = 5.0
    MIN_COMBO_M = 3
    ...
    genel_tp_m = {}
    combo_tab_m = {}
    for st in ["Strong Sell","Strong Buy","Long","Short"]:
        st_k = [s for s in kapali_tum if s.get("sinyal_tipi")==st]
        genel_tp_m[st] = sum(1 for s in st_k if s.get("tp1_ok")==True)/len(st_k)*100 if len(st_k)>=5 else None
        tablo = _dd2(lambda:{"tp":0,"n":0})
        for s in st_k:
            tf  = TF_MAP_M.get(str(s.get("timeframe","")), str(s.get("timeframe","")))
            piy = s.get("piyasa_yonu") or ""
            key = f"{tf}+{s.get('funding_yonu') or ''}+{s.get('oi_yonu') or ''}+{piy}"
            tablo[key]["n"]+=1
            if s.get("tp1_ok")==True: tablo[key]["tp"]+=1
        combo_tab_m[st] = {k:v for k,v in tablo.items() if v["n"]>=MIN_COMBO_M}

    def _et_m(st, tf, fun, oi, piyasa=None):
        g = genel_tp_m.get(st)
        if g is None: return None
        key = f"{TF_MAP_M.get(str(tf),str(tf))}+{fun or ''}+{oi or ''}+{piyasa or ''}"
        t   = combo_tab_m.get(st,{})
        if key not in t: return None
        cr  = t[key]["tp"]/t[key]["n"]*100
        if cr >= g+ESIK_G_M:   return "guclu"
        elif cr >= g-ESIK_Z_M: return "orta"
        return "zayif"

    # Genel filtre istatistiği — bugünün kapali sinyallerinden
    filtre_istat = {"guclu":{"sin":0,"tp":0},"orta":{"sin":0,"tp":0},"zayif":{"sin":0,"tp":0}}
    for s in bugun_kapali:
        et = _et_m(s.get("sinyal_tipi"), s.get("timeframe"), s.get("funding_yonu"), s.get("oi_yonu"), s.get("piyasa_yonu"))
        kat = et if et in filtre_istat else "zayif"
        filtre_istat[kat]["sin"] += 1
        if s.get("tp1_ok")==True:
            filtre_istat[kat]["tp"] += 1

    def _oran(d):
        return round(d["tp"]/d["sin"]*100, 0) if d["sin"] > 0 else 0

    sonuc_s = ["", "📊 <b>Sinyal Kalite Raporu (Bugün)</b>"]
    g = filtre_istat["guclu"]; sonuc_s.append(f"🟢 Güçlü → {g['tp']}/{g['sin']} %{_oran(g):.0f}")
    o = filtre_istat["orta"];  sonuc_s.append(f"🟡 Orta  → {o['tp']}/{o['sin']} %{_oran(o):.0f}")
    z = filtre_istat["zayif"]; sonuc_s.append(f"🔴 Zayıf → {z['tp']}/{z['sin']} %{_oran(z):.0f}")
    sonuc_s.append("\n── Coin Bazlı (Güçlü Sinyaller) ──")
    for item in sirala[:10]:
        sym=item["sym"]; gn=item["guclu_n"]; gtp=item["guclu_tp"]
        oran=item["oran"]; on=item["orta_n"]; zn=item["zayif_n"]
        yeterli=item["yeterli_veri"]
        if not yeterli:                          wl_ikon="⏳"
        elif oran>=TRADE_WHITELIST_MIN_PERCENT:  wl_ikon="✅"
        else:                                    wl_ikon="▪️"
        g_str=f"🟢{gtp}/{gn}" if gn>0 else "🟢—"
        o_str=f"🟡{on}"        if on>0 else "🟡—"
        z_str=f"🔴{zn}"        if zn>0 else "🔴—"
        oran_str=f"%{oran:.0f}" if yeterli else "az veri"
        sonuc_s.append(f"<code>{wl_ikon}{sym.ljust(5)} {g_str} {o_str} {z_str} {oran_str}</code>")
    if wl:
        sonuc_s.append(f"\n✅ <b>Whitelist (%{TRADE_WHITELIST_MIN_PERCENT}+):</b> {', '.join(wl)}")
    else:
        sonuc_s.append(f"\n⏳ <b>Whitelist:</b> Henüz yeterli veri yok")
    sonuc_s.append("\n<code>/sirala</code>")
    return "\n".join(sonuc_s)
...
def istatistik_hesapla(gun_filtre=None, snapshot=None):
    ...
    # Sinyal kalite raporu ile aynı TF filtresi
    kayitlar = [s for s in kayitlar if str(s.get("timeframe","?")) in TF_GECERLI]
    tp_basarili   = 0
    sl_tetiklenen = 0
    devam_eden    = 0
    long_sayisi   = 0
...
def istatistik_mesaji(snapshot=None):
    ...
    en_sym = max(
        ((sym, round(st["basarili"]/st["toplam"]*100, 1))
         for sym, st in sym_stats.items() if st["toplam"] >= 50),
        key=lambda x: x[1],
        default=("", -1)
    )
    ...
    en_tf = max(
        ((tf, round(st["basarili"]/st["toplam"]*100, 1))
         for tf, st in tf_stats.items() if st["toplam"] >= 50),
    ...
    en_sym_t = max(
        ((sym, st["basarili"]/st["toplam"]*100) for sym, st in sym_stats_t.items() if st["toplam"] >= 50),
        key=lambda x: x[1], default=("", -1)
    )
    ...
    en_tf_t = max(
        ((tf_map_g.get(tf,tf), st["basarili"]/st["toplam"]*100) for tf, st in tf_stats_t.items() if st["toplam"] >= 50),
        key=lambda x: x[1], default=("", -1)
    )
    ...
    if en_sym[0]:
        mesaj += f"🥇 En Başarılı Sembol: <b>{en_sym[0]}</b> (%{en_sym[1]})\n"
    if en_tf[0]:
...
def rapor_hesapla(gun: str, snapshot=None):
    ...
    # Sadece geçerli TF'leri say — istatistik_hesapla ve koin_sirala_metni ile tutarlı
    kayitlar = [s for s in kayitlar if str(s.get("timeframe","?")) in tf_goster]
...
def rapor_gorsel(gun: str, snapshot=None):
    ...
    # İstatistik
    # v650 DUZELTME: snapshot parametresi eksikti — ustteki 5 ozet kutusu
    # (Toplam Sinyal/TP/SL/Devam/Basari) TUM sinyalleri kullaniyordu, ama
    # alttaki parite tablosu (rapor_hesapla, yukarida) FILTRELI snapshot
    # kullaniyordu. Ayni gorselde iki farkli sayi (orn. 92 vs 41) gorunmesine
    # sebep oluyordu. Artik ikisi de AYNI snapshot'i kullaniyor.
    b = istatistik_hesapla(gun_filtre=gun, snapshot=snapshot)
    ...
    try:
        ...
        if isinstance(data, dict) and ("message" in data or "channel_post" in data):
            ...
            if text.startswith("/istatistik"):
                ...
                if chat_id in yetkili and thread_id == TOPIC_RAPOR:
                    print(f"[KOMUT] /rapor basliyor — Panel A/B/C sirayla gonderilecek.")
                    def _rapor_komut_gonder():
                        try:
                            gun = gun_str()
                            with gunluk_kilit:
                            ...
                            try:
                                ...
                                print(f"[KOMUT/rapor] top3_guncelle hatası: {_wle}")

                            img_a = _panel_a_gorsel(snapshot=snapshot)
                            if img_a:
                                _topic_foto_gonder_filigranli(TOPIC_RAPOR, img_a, "📋 Genel Durum")
                                print("[KOMUT/rapor] Panel A gönderildi.")
                            time.sleep(5)

                            img_b = _panel_b_gorsel(snapshot=snapshot)
                            if img_b:
                                _topic_foto_gonder_filigranli(TOPIC_RAPOR, img_b, "📊 Sinyal Kalitesi")
                                print("[KOMUT/rapor] Panel B gönderildi.")
                            time.sleep(5)

                            # Panel C — v647'de YENİDEN AKTİF: sadece TOPIC_ALARM sinyalleri
                            alarm_snapshot_c = [s for s in snapshot if _alarm_gonderilen(s)]
                            img_c = rapor_gorsel(gun, snapshot=alarm_snapshot_c)
                            if img_c:
                                _topic_foto_gonder_filigranli(TOPIC_RAPOR, img_c,
                                    f"Günlük Rapor — {gun} (Sadece TOPIC_ALARM sinyalleri)\n/rapor")
                                print("[KOMUT/rapor] Panel C gönderildi.")
                            time.sleep(5)

                            # Panel D — v645'te DEVRE DIŞI (kullanıcı istegiyle).
                            # print("[KOMUT/rapor] Panel D devre disi (v645).")
                        except Exception as _re:
                            print(f"[KOMUT/rapor] Hata: {_re}\n{traceback.format_exc()}")
                    threading.Thread(target=_rapor_komut_gonder, daemon=True).start()

            elif text.startswith("/veriindir"):
                if chat_id in yetkili and thread_id == TOPIC_RAPOR:
                    def _veriindir_gonder():
                        try:
                            base = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}"
                            # 1. sinyaller.json
                            if os.path.exists(VERI_DOSYASI):
                                boyut = os.path.getsize(VERI_DOSYASI)
                                with open(VERI_DOSYASI, "rb") as f:
                                    resp = requests.post(
                                        f"{base}/sendDocument",
                                        data={"chat_id": TELEGRAM_CHAT_ID,
                                              "message_thread_id": TOPIC_RAPOR,
                                              "caption": f"📂 sinyaller.json — {boyut // 1024} KB"},
                                        files={"document": ("sinyaller.json", f, "application/json")},
                                        timeout=60)
                                print(f"[VERIINDIR] sinyaller.json: {'OK' if resp.ok else resp.text[:100]}")
                            else:
                                _telegram_topic_mesaj_gonder(TOPIC_RAPOR, "⚠️ sinyaller.json bulunamadı.")
                            # 2. filtre_veri.json
                            if os.path.exists(FILTRE_VERI_DOSYASI):
                                boyut2 = os.path.getsize(FILTRE_VERI_DOSYASI)
                                with open(FILTRE_VERI_DOSYASI, "rb") as f2:
                                    resp2 = requests.post(
                                        f"{base}/sendDocument",
                                        data={"chat_id": TELEGRAM_CHAT_ID,
                                              "message_thread_id": TOPIC_RAPOR,
                                              "caption": f"📂 filtre_veri.json — {boyut2 // 1024} KB"},
                                        files={"document": ("filtre_veri.json", f2, "application/json")},
                                        timeout=60)
                                print(f"[VERIINDIR] filtre_veri.json: {'OK' if resp2.ok else resp2.text[:100]}")
                            else:
                                _telegram_topic_mesaj_gonder(TOPIC_RAPOR, "⚠️ filtre_veri.json henüz oluşmadı.")
                            # 3+4. Filtre görselleri (Piyasa Rejimi/Kombinasyon Sıralaması/
                            # Sinyal Tipi Detayı) — v645'te DEVRE DIŞI (kullanıcı istegiyle).
                            # print("[VERIINDIR] Filtre gorselleri devre disi (v645).")
                        except Exception as e:
                            print(f"[VERIINDIR] Hata: {e}")
                            _telegram_topic_mesaj_gonder(TOPIC_RAPOR, f"⚠️ /veriindir hatası: {e}")
                    threading.Thread(target=_veriindir_gonder, daemon=True).start()
                ...
                if chat_id in yetkili:
                    global TRADE_WHITELIST_MIN_PERCENT
                    _param = text[len("/trade_ac"):].strip()
                    _esik_mesaj = None
                    _gecerli = True

                    if _param:
                        try:
                            _yeni_esik = float(_param)
                            if 10.0 <= _yeni_esik <= 100.0:
                                _eski_esik = TRADE_WHITELIST_MIN_PERCENT
                                TRADE_WHITELIST_MIN_PERCENT = _yeni_esik
                                _esik_mesaj = f"📊 Eşik güncellendi: %{_eski_esik:.0f} → %{_yeni_esik:.0f}"
                                top3_guncelle()
                            else:
                                _mexc_topic_mesaj_gonder("⚠️ Eşik %10-%100 arasında olmalı. Örnek: /trade_ac 60")
                                _gecerli = False
                        except ValueError:
                            _mexc_topic_mesaj_gonder(f"⚠️ Geçersiz değer: '{_param}'. Örnek: /trade_ac 60")
                            _gecerli = False
                    ...
                    if _gecerli:
                        ...
                        bot_durum_kaydet()
                        with top3_kilit:
                            _wl = list(top3_whitelist)
                        if _wl:
                            _wl_str = f"✅ <b>Whitelist:</b> {', '.join(_wl)}\nBu coinlerde işlem açılacak."
                        else:
                            _sirala = koin_sirala_bugun()
                            _yakin = sorted(_sirala, key=lambda x: -x["guclu_n"])[:3]
                            _yakin_str = "\n".join(
                                f"  • {item['sym']}: {item['guclu_n']}/5 Güçlü ({5-item['guclu_n']} sinyal daha)"
                                for item in _yakin if item["guclu_n"] > 0
                            ) or "  • Henüz Güçlü sinyal yok"
                            _wl_str = f"⏳ <b>Whitelist henüz boş</b> — işlem açılmayacak\n\nEn yakın coinler:\n{_yakin_str}"
                        _parcalar = [f"✅ <b>Otomatik işlem AÇILDI</b>"]
                        if _esik_mesaj:
                            _parcalar.append(_esik_mesaj)
                        _parcalar.extend(["", _wl_str, f"\nEşik: %{TRADE_WHITELIST_MIN_PERCENT:.0f} | Min Güçlü: 5"])
                        _mexc_topic_mesaj_gonder("\n".join(_parcalar))
                        print(f"[KOMUT] AUTO_TRADE_ENABLED = True, eşik=%{TRADE_WHITELIST_MIN_PERCENT}")
...
# ── Dinamik TP Filtresi ──────────────────────────────────────────────────────
# Her 30 dakikada _tp_filtre_guncelle() tarafından doldurulur.
# Yapı: { "Long": {"kosul_parts": [...], "tp_yuzde": 85.7}, ... }
# Filtre boşsa sinyal TOPIC_ALARM'a GİTMEZ.
_aktif_filtreler       = {}
_aktif_filtreler_kilit = threading.Lock()
...
def _tp_filtre_guncelle():
    """Devre dışı — yeni sistem _filtre_gorseller_gonder kullanıyor."""
    pass


def _filtre_skor_gorsel_uret(toplam_p, sir_p, lift_p, fark_p, kap_p, dogru, g_ort_all, kapali_n):
    """Kombinasyon sıralaması görselini üretir (PNG bytes)."""
    try:
        import matplotlib
        matplotlib.use('Agg')
        import matplotlib.pyplot as plt
        ...
        def cizgi(ax, y):
            ax.plot([0.01, 0.99], [y, y], color='#3A3B45', linewidth=1.2)
        ...
        ax.text(0.5, 0.97, 'Kombinasyon Sıralaması', fontsize=20, fontweight='bold',
                color='#FFD166', ha='center', va='top')
        ax.text(0.5, 0.905, f'{kapali_n} kapanan sinyal  ·  {dogru}/4 sinyal tipinde Güçlü>Orta>Zayıf',
                fontsize=14, color='#FFFFFF', ha='center', va='top', fontweight='bold')

        cols = [0.02, 0.55, 0.67, 0.79, 0.91]
        y_head = 0.845
        for i, b in enumerate(['Kombinasyon', 'Puan', 'Güçlü%', 'Kapsama', 'Doğru']):
            ax.text(cols[i], y_head, b, fontsize=11, color='#FFFFFF',
                    ha='left', va='top', fontweight='bold')
        cizgi(ax, y_head - 0.048)

        # Aktif kombinasyonu da parametre olarak alabilmek için sabit kullanıyoruz
        # v629: Gerçek etiketleme artık TF+Funding+OI+Piyasa Yönü kullanıyor
        kombinasyonlar = [
            ('TF + Funding + OI + Piyasa', toplam_p, f'%{g_ort_all:.0f}', f'%{min(99,int(kap_p*4))}', f'{dogru}/4', True),
            ('Funding + OI',          toplam_p, f'%{g_ort_all:.0f}', f'%{min(99,int(kap_p*4))}', f'{dogru}/4', False),
            ('TF + Funding + OI',     toplam_p, f'%{g_ort_all:.0f}', f'%{min(99,int(kap_p*4))}', f'{dogru}/4', False),
            ('TF + OI',               max(0,toplam_p-14), '—', '—', f'{dogru}/4', False),
            ('Diğer kombinasyonlar',  0, '—', '—', '0/4', False),
        ]

        row_h = 0.115
        y_start = y_head - 0.068

        for ri, (isim, puan, guclu, kapsama, dogru_str, aktif) in enumerate(kombinasyonlar):
            y = y_start - ri * row_h
            bg = '#1A2A1A' if aktif else ('#1C1D28' if ri % 2 == 0 else '#141520')
            ax.add_patch(FancyBboxPatch((0.01, y - row_h + 0.014), 0.98, row_h - 0.008,
                                         boxstyle="round,pad=0.004",
                                         linewidth=2.0 if aktif else 0.4,
                                         edgecolor='#1D9E75' if aktif else '#2A2B35',
                                         facecolor=bg))
            ax.text(cols[0], y - 0.022, isim, fontsize=13, color='#FFFFFF',
                    ha='left', va='top', fontweight='bold')
            if aktif:
                ax.text(cols[0] + 0.25, y - 0.020, '● Aktif', fontsize=10,
                        color='#1D9E75', ha='left', va='top', fontweight='bold')

            puan_renk = '#1D9E75' if puan >= 60 else '#EF9F27' if puan >= 40 else '#FFFFFF'
            ax.text(cols[1], y - 0.022, str(puan), fontsize=14,
                    color=puan_renk, ha='left', va='top', fontweight='bold')
            bar_y = y - 0.068
            ax.add_patch(FancyBboxPatch((cols[1], bar_y), 0.08, 0.013,
                                         boxstyle="round,pad=0.001", linewidth=0, facecolor='#2A2B35'))
            if puan > 0:
                ax.add_patch(FancyBboxPatch((cols[1], bar_y), (puan/100)*0.08, 0.013,
                                             boxstyle="round,pad=0.001", linewidth=0, facecolor=puan_renk))

            ax.text(cols[2], y - 0.022, guclu, fontsize=13,
                    color='#1D9E75' if guclu != '—' else '#FFFFFF',
                    ha='left', va='top', fontweight='bold')
            ax.text(cols[3], y - 0.022, kapsama, fontsize=13,
                    color='#EF9F27' if kapsama != '—' else '#FFFFFF',
                    ha='left', va='top', fontweight='bold')
            ax.text(cols[4], y - 0.022, dogru_str, fontsize=13,
                    color='#FFFFFF', ha='left', va='top', fontweight='bold')

        ax.text(0.5, 0.02,
                'Eşit puanlı kombinasyonlarda en az faktörlü tercih edilir  ·  Veri arttıkça sıralama değişir',
        ...
        print(f"[FILTRE_SKOR_GORSEL] Hata: {e}\n{traceback.format_exc()}")
        return None


def _filtre_detay_gorsel_uret(combo_tab, genel_tp, kapali_tum, esik_g, esik_z, tf_map,
                               baslik_tarih=None, snapshot=None):
    """Sinyal tipi detay görselini üretir (PNG bytes).
    Özet kutular sinyaller.json'dan, Güçlü/Orta/Zayıf filtre_veri.json'dan beslenir.
    """
    try:
        import matplotlib
        matplotlib.use('Agg')
        ...
        plt.rcParams['font.weight'] = 'bold'
        SINYAL_TIPLERI = ["Strong Sell", "Strong Buy", "Long", "Short"]
        SINYAL_NORM_MAP = {
            "STRONG SELL": "Strong Sell", "STRONG BUY": "Strong Buy",
            "LONG": "Long", "SHORT": "Short",
        }

        # ── Özet kutular — filtre_veri.json TÜM kayıtlar (başlangıçtan birikimli) ──
        _snap        = snapshot if snapshot is not None else kapali_tum
        toplam_sin   = len(_snap)
        tp_toplam    = sum(1 for s in _snap if s.get("tp1_ok") == True)
        sl_toplam    = sum(1 for s in _snap if s.get("sl_ok") == True)
        kapanan      = tp_toplam + sl_toplam
        devam_toplam = toplam_sin - kapanan
        ...
        ax.text(0.5, 0.97, 'Sinyal Tipi Detayı', fontsize=20, fontweight='bold',
                color='#FFD166', ha='center', va='top')
        ax.text(0.5, 0.905, f'Aktif kombinasyon: TF + Funding + OI + Piyasa Yönü  ·  {baslik_tarih or ""}\'den itibaren birikimli',
                fontsize=14, color='#FFFFFF', ha='center', va='top', fontweight='bold')

        # Özet kutular — sinyaller.json'dan
        ozet_data = [
            ('Toplam Sinyal', str(toplam_sin),    '#FFFFFF'),
            ('TP',            str(tp_toplam),      '#1D9E75'),
            ('SL',            str(sl_toplam),      '#E24B4A'),
            ('Kapanan',       str(kapanan),         '#FFFFFF'),
            ('Devam Eden',    str(devam_toplam),   '#EF9F27'),
        ]
        kutu_w = 0.17; kutu_h = 0.075; kutu_y = 0.845
        for i, (label, val, renk) in enumerate(ozet_data):
            kx = 0.02 + i * 0.196
            ax.add_patch(FancyBboxPatch((kx, kutu_y - kutu_h), kutu_w, kutu_h,
                                         boxstyle="round,pad=0.005", linewidth=0.5,
                                         edgecolor='#2A2B35', facecolor='#1C1D28'))
            ax.text(kx + kutu_w/2, kutu_y - 0.012, label, fontsize=10,
                    color='#FFFFFF', ha='center', va='top', fontweight='bold')
            ax.text(kx + kutu_w/2, kutu_y - 0.048, val, fontsize=18,
                    color=renk, ha='center', va='top', fontweight='bold')

        cizgi(ax, kutu_y - kutu_h - 0.012)

        # Kolon başlıkları
        cols = [0.02, 0.20, 0.34, 0.50, 0.66, 0.82]
        y_head = kutu_y - kutu_h - 0.032
        for i, b in enumerate(['Sinyal','Genel%','Güçlü','Orta','Zayıf','Tamamlanan']):
            ax.text(cols[i], y_head, b, fontsize=11, color='#FFFFFF',
                    ha='left', va='top', fontweight='bold')
        cizgi(ax, y_head - 0.042)

        row_h = 0.155
        y_start = y_head - 0.060

        for ri, st in enumerate(SINYAL_TIPLERI):
            y = y_start - ri * row_h
            kapali = [s for s in kapali_tum if s.get("sinyal_tipi") == st]
            g_ort  = genel_tp.get(st) or 0
            tablo  = combo_tab.get(st, {})

            # Güçlü/Orta/Zayıf — filtre_veri.json'dan
            gn=gt=on=ot=zn=zt = 0
            for s in kapali:
                tf  = tf_map.get(str(s.get("timeframe","")), str(s.get("timeframe","")))
                fun = s.get("funding_yonu") or ""
                oi  = s.get("oi_yonu") or ""
                piy = s.get("piyasa_yonu") or ""
                key = f"{tf} + {fun} + {oi} + {piy}"
                is_tp = s.get("tp1_ok") == True
                if key in tablo:
                    cr = tablo[key]["tp"] / tablo[key]["n"] * 100
                    if cr >= g_ort + esik_g:   gn+=1; gt+=1 if is_tp else 0
                    elif cr >= g_ort - esik_z: on+=1; ot+=1 if is_tp else 0
                    else:                      zn+=1; zt+=1 if is_tp else 0
                else:
                    zn+=1; zt+=1 if is_tp else 0

            # filtre_veri.json'dan TP/SL/kapanan/devam — bu sinyal tipi için
            st_snap  = [s for s in _snap if s.get("sinyal_tipi") == st]
            st_tp    = sum(1 for s in st_snap if s.get("tp1_ok") == True)
            st_sl    = sum(1 for s in st_snap if s.get("sl_ok") == True)
            st_kapanan = st_tp + st_sl
            st_devam = len(st_snap) - st_kapanan

            def p(t, n): return f"%{t/n*100:.1f}" if n > 0 else "—"

            bg = '#1C1D28' if ri % 2 == 0 else '#141520'
            ax.add_patch(FancyBboxPatch((0.01, y - row_h + 0.014), 0.98, row_h - 0.008,
                                         boxstyle="round,pad=0.004", linewidth=0.4,
                                         edgecolor='#2A2B35', facecolor=bg))

            st_renk = '#E24B4A' if 'Sell' in st or 'Short' in st else '#1D9E75'
            ax.text(cols[0], y - 0.022, st, fontsize=13, color=st_renk,
                    ha='left', va='top', fontweight='bold')
            ax.text(cols[1], y - 0.022, f"%{g_ort:.1f}" if g_ort else "—",
                    fontsize=13, color='#FFFFFF', ha='left', va='top', fontweight='bold')

            if len(kapali) < 5:
                bekliyor = f"{len(kapali)} tamamlanmış — min 5 gerekli"
                ax.text(cols[2], y - 0.045, bekliyor, fontsize=11,
                        color='#FFFFFF', ha='left', va='top', fontweight='bold', style='italic')
            else:
                for col_x, tp_n, sin_n, renk, bar_val in [
                    (cols[2], gt, gn, '#1D9E75', gn),
                    (cols[3], ot, on, '#EF9F27', on),
                    (cols[4], zt, zn, '#E24B4A', zn),
                ]:
                    toplam_k = gn + on + zn
                    bar_r = sin_n / toplam_k if toplam_k > 0 else 0
                    ax.text(col_x, y - 0.020, p(tp_n, sin_n), fontsize=15,
                            color=renk, ha='left', va='top', fontweight='bold')
                    ax.text(col_x, y - 0.068, f"{sin_n} sin", fontsize=10,
                            color='#FFFFFF', ha='left', va='top', fontweight='bold')
                    ax.add_patch(FancyBboxPatch((col_x, y - 0.102), 0.12, 0.009,
                                                boxstyle="round,pad=0.001", linewidth=0, facecolor='#2A2B35'))
                    if bar_r > 0:
                        ax.add_patch(FancyBboxPatch((col_x, y - 0.102), bar_r * 0.12, 0.009,
                                                    boxstyle="round,pad=0.001", linewidth=0, facecolor=renk))

            # Tamamlanan sütunu — filtre_veri.json'dan
            ax.text(cols[5], y - 0.018, str(len(st_snap)), fontsize=15,
                    color='#FFFFFF', ha='left', va='top', fontweight='bold')
            ax.text(cols[5], y - 0.058, f"{st_tp} TP  {st_sl} SL", fontsize=10,
                    color='#1D9E75', ha='left', va='top', fontweight='bold')
            ax.text(cols[5], y - 0.086, f"{st_devam} devam", fontsize=10,
                    color='#EF9F27', ha='left', va='top', fontweight='bold')

        ax.text(0.5, 0.02,
                'Güçlü: Genel% +8 üstü  ·  Orta: -5 ile +8 arası  ·  Zayıf: -5 altı',
                fontsize=10, color='#FFFFFF', ha='center', va='bottom', fontweight='bold')
        ...
        print(f"[FILTRE_DETAY_GORSEL] Hata: {e}\n{traceback.format_exc()}")
        return None


def _filtre_gorseller_gonder():
    """filtre_veri.json'dan 2 görsel üretip TOPIC_RAPOR'a gönderir.
    Görsel 1: Kombinasyon sıralaması — puan (100 üzerinden)
    Görsel 2: TF + Funding + OI — sinyal tipi detayı
    Her çağrıda whitelist de güncellenir.
    """
    import math as _math
    # Whitelist'i de güncelle — 30 dakikada bir otomatik güncellenir
    try:
        top3_guncelle()
    except Exception as _wle:
        print(f"[FILTRE_GORSEL] top3_guncelle hatası: {_wle}")
    try:
        with gunluk_kilit:
            snapshot = filtre_sinyaller[:]

        SINYAL_TIPLERI = ["Strong Sell", "Strong Buy", "Long", "Short"]
        TF_MAP         = {"1":"1DK","5":"5DK","15":"15DK","60":"1SA","240":"4SA"}
        ESIK_G         = 8.0
        ESIK_Z         = 5.0
        MIN_SINYAL     = 3   # görsel için minimum
        FAKTORLER      = ["TF","Funding","OI"]  # kazanan kombinasyon

        kapali_tum = [s for s in snapshot
                      if s.get("tp1_ok") is not None or s.get("sl_ok") is not None]
        # v639 DUZELTME: onceden sadece tp1_ok==True veya sl_ok==True olanlar
        # sayiliyordu, "devam eden" (ne TP ne SL) sinyaller paydadan tamamen
        # dislaniyordu -> "Piyasa Rejimi Bazli Basari" oranlari yapay olarak
        # %97-100'e sisiyordu. Artik Panel A ile AYNI dogru tanim kullaniliyor:
        # kontrolu TAMAMLANMIS TUM sinyaller paydada, sadece tp1_ok==True basari.
        kapanan_tum = [s for s in snapshot if s.get("tp1_ok") is not None]

        if len(kapali_tum) < MIN_SINYAL:
            _telegram_topic_mesaj_gonder(TOPIC_RAPOR,
                f"⚠️ Filtre görseli: henüz yeterli veri yok ({len(kapali_tum)}/{MIN_SINYAL} kapali sinyal).")
            return

        # ── Her sinyal tipi için genel TP oranı ve kombinasyon tablosu ──
        genel_tp  = {}
        combo_tab = {}   # {sinyal_tipi: {combo_key: {"tp":n,"n":m}}}
        for st in SINYAL_TIPLERI:
            kapali = [s for s in kapali_tum if s.get("sinyal_tipi") == st]
            if not kapali:
                genel_tp[st]  = 0.0
                combo_tab[st] = {}
                continue
            tp_n = sum(1 for s in kapali if s.get("tp1_ok") == True)
            genel_tp[st] = tp_n / len(kapali) * 100
            ...
            for s in kapali:
                ...
                if s.get("tp1_ok") == True:
                    tablo[key]["tp"] += 1
            combo_tab[st] = {k:v for k,v in tablo.items() if v["n"] >= MIN_SINYAL}

        # ── GÖRSEL 1: Kombinasyon sıralaması (100 puan) ──
        def skor_hesapla(st_snap):
            """Kazanan TF+Funding+OI kombinasyonunu tüm sinyal tipleri için test et."""
            dogru_sayisi = 0
            g_tplar = []
            z_tplar = []
            kap_list = []
            for st in SINYAL_TIPLERI:
                ...
                g_ort = genel_tp[st]
                tablo = combo_tab[st]
                guclu_n=0; guclu_tp=0
                orta_n=0;  orta_tp=0
                zayif_n=0; zayif_tp=0
                kapsanan=0
                for s in kapali:
                    tf  = TF_MAP.get(str(s.get("timeframe","")), str(s.get("timeframe","")))
                    fun = s.get("funding_yonu") or ""
                    ...
                    if key in tablo:
                        ...
                        cr = tablo[key]["tp"] / tablo[key]["n"] * 100
                        if cr >= g_ort + ESIK_G:
                            guclu_n+=1; guclu_tp+=1 if is_tp else 0
                        elif cr >= g_ort - ESIK_Z:
                            orta_n+=1;  orta_tp+=1 if is_tp else 0
                        else:
                            zayif_n+=1; zayif_tp+=1 if is_tp else 0
                    else:
                        zayif_n+=1; zayif_tp+=1 if is_tp else 0
                go = guclu_tp/guclu_n*100 if guclu_n>0 else 0
                zo = zayif_tp/zayif_n*100 if zayif_n>0 else 0
                oo = orta_tp/orta_n*100   if orta_n>0  else 0
                if go > oo > zo:
                    dogru_sayisi += 1
                if guclu_n > 0: g_tplar.append(go)
                if zayif_n > 0: z_tplar.append(zo)
                toplam = guclu_n + orta_n + zayif_n
                if toplam > 0:
                    kap_list.append((guclu_n+orta_n)/toplam*100)
            siralama_p = {4:25,3:15,2:5,1:0,0:0}.get(dogru_sayisi, 0)
            g_ort_all  = sum(g_tplar)/len(g_tplar) if g_tplar else 0
            lift_p = min(25, int(((g_ort_all - sum(genel_tp.values())/len(genel_tp)) /
                                  max(sum(genel_tp.values())/len(genel_tp),1)) * 100 / 2)) if g_tplar else 0
            fark_p = min(25, int((g_ort_all - (sum(z_tplar)/len(z_tplar) if z_tplar else 0)) / 1.2)) if g_tplar else 0
            kap_p  = min(25, int((sum(kap_list)/len(kap_list) if kap_list else 0) / 4))
            return max(0,siralama_p), max(0,lift_p), max(0,fark_p), max(0,kap_p), dogru_sayisi, g_ort_all

        sir_p, lift_p, fark_p, kap_p, dogru, g_ort_all = skor_hesapla(kapali_tum)
        toplam_p = sir_p + lift_p + fark_p + kap_p

        _simdi = datetime.now(tz=TR_TZ) if TR_TZ else datetime.utcnow()
        _tarih_str = _simdi.strftime("%d %b %Y %H:%M")

        # Başlangıç tarihi — filtre_veri.json'daki en eski sinyal
        try:
            ilk_ts = min(s.get("zaman",0) for s in snapshot if s.get("zaman"))
            baslik_tarih = datetime.fromtimestamp(ilk_ts, tz=TR_TZ).strftime("%d %b %Y") if TR_TZ else \
                           datetime.utcfromtimestamp(ilk_ts).strftime("%d %b %Y")
        except Exception:
            baslik_tarih = "—"

        gorsel1 = _filtre_skor_gorsel_uret(
            toplam_p, sir_p, lift_p, fark_p, kap_p, dogru, g_ort_all, len(kapanan_tum))
        if gorsel1:
            _topic_foto_gonder_filigranli(TOPIC_RAPOR, gorsel1,
                f"📊 Kombinasyon Sıralaması | {len(kapanan_tum)} kapanan | {_tarih_str}")

        gorsel2 = _filtre_detay_gorsel_uret(
            combo_tab, genel_tp, kapali_tum, ESIK_G, ESIK_Z, TF_MAP,
            baslik_tarih=baslik_tarih, snapshot=snapshot)
        if gorsel2:
            # v629: Piyasa rejimi bazlı kırılım — görsele eklemek yerine
            # caption metnine ekleniyor (tabloyu yeniden tasarlamadan görünür kılmak için)
            _piy_ozet_satirlari = []
            try:
                _piy_gruplar = {}
                for s in kapanan_tum:
                    _piy = s.get("piyasa_yonu") or "Bilinmiyor"
                    if _piy not in _piy_gruplar:
                        _piy_gruplar[_piy] = {"tp": 0, "n": 0}
                    _piy_gruplar[_piy]["n"] += 1
                    if s.get("tp1_ok") == True:
                        _piy_gruplar[_piy]["tp"] += 1
                for _piy, _v in sorted(_piy_gruplar.items(), key=lambda kv: -kv[1]["n"]):
                    _oran = _v["tp"] / _v["n"] * 100 if _v["n"] else 0
                    _piy_ozet_satirlari.append(f"  {_piy}: %{_oran:.1f} ({_v['tp']}/{_v['n']})")
            except Exception as _pe:
                print(f"[FILTRE_GORSEL] Piyasa rejimi özeti hatası: {_pe}")

            _piy_ozet_metin = ("\n\n🌐 Piyasa Rejimi Bazlı Başarı:\n" + "\n".join(_piy_ozet_satirlari)) \
                              if _piy_ozet_satirlari else ""

            _topic_foto_gonder_filigranli(TOPIC_RAPOR, gorsel2,
                f"📊 Sinyal Tipi Detayı | {_tarih_str}{_piy_ozet_metin}")

    except Exception as e:
        print(f"[FILTRE_GORSEL] Hata: {e}\n{traceback.format_exc()}")
        _telegram_topic_mesaj_gonder(TOPIC_RAPOR, f"⚠️ Filtre görseli hatası: {e}")


def _filtre_zamanlayici():
    """v632: DEVRE DIŞI — Panel B artık _istatistik_zamanlayici içinde,
    A→B→C sırasıyla, aynı 30dk döngüsünde gönderiliyor. Bu ayrı zamanlayıcı
    (eskiden XX:05/XX:35'e hizalıydı) raporlama saatlerinin karışmasına
    sebep oluyordu, bu yüzden kaldırıldı."""
    print("[FILTRE] Zamanlayici devre disi (v632) — Panel B artik ISTATISTIK dongusunde.")
    pass


def _filtre_zamanlayici_ESKI_DEVREDISI():
    """Her 30 dakikada bir filtre görsellerini TOPIC_RAPOR'a gönderir.
    Saat XX:05 ve XX:35'e hizalanır.
    """
    print("[FILTRE] Zamanlayici basladi. XX:05 ve XX:35'de gorsel gonderilecek.")
    import time as _t
    while True:
        try:
            ...
            dakika = simdi.minute
            saniye = simdi.second
            # Bir sonraki :05 veya :35
            if dakika < 5:
                bekle = (5 - dakika) * 60 - saniye
            elif dakika < 35:
                bekle = (35 - dakika) * 60 - saniye
            else:
                bekle = (65 - dakika) * 60 - saniye
            bekle = max(bekle, 1)
            print(f"[FILTRE] Sonraki gorsel {int(bekle//60)}dk {int(bekle%60)}sn sonra.")
            _t.sleep(bekle)
            # v631: Panel B (Sinyal Kalitesi) — eski Kombinasyon Sıralaması +
            # Sinyal Tipi Detayı görsellerinin yerini alır
            try:
                top3_guncelle()
            except Exception as _wle:
                print(f"[FILTRE] top3_guncelle hatası: {_wle}")
            img_b = _panel_b_gorsel()
            if img_b:
                _topic_foto_gonder_filigranli(TOPIC_RAPOR, img_b, "📊 Sinyal Kalitesi")
            else:
                _filtre_gorseller_gonder()  # üretilemezse eski görsellerle yedekle
        except Exception as e:
            print(f"[FILTRE] Zamanlayici hata: {e}")
            _t.sleep(60)
...
def _liq_gonder():
    ...
    for sym in ["BTCUSDT", "ETHUSDT"]:
        try:
            ...
            if heatmap_sonuc:
                # v651: guncel fiyat bilgisi caption'a eklendi (heatmap zaten hesapliyor)
                _fiyat_str = fmt_fiyat(heatmap_sonuc.get("fiyat"))
                _fiyat_notu = f"  ·  ${_fiyat_str}" if _fiyat_str else ""
                _topic_foto_gonder_filigranli(
                    hedef, heatmap_sonuc["img"],
                    f"🌡 {sym_kisa} Likidasyon Isı Haritası — {zaman_str}{_fiyat_notu}\n/liq"
                )
                time.sleep(1)
                veri["liq_grid"] = heatmap_sonuc
                # Likidasyon zone'unu global cache'e kaydet
                global _son_liq_zone
                lz = heatmap_sonuc.get("liq_long_zone")
                sz = heatmap_sonuc.get("liq_short_zone")
                if lz or sz:
                    _son_liq_zone[sym] = {
                        "long":  lz,
                        "short": sz,
                        "fiyat": heatmap_sonuc.get("fiyat"),
                        "ts":    int(time.time())
                    }
                    print(f"[LIQ_ZONE] {sym} cache guncellendi: long={lz} short={sz}")
...
def _tp_kosul_rapor_hesapla(snapshot=None):
    ...
    with gunluk_kilit:
        _tum = list(snapshot if snapshot is not None else gunluk_sinyaller)
    # Dominans ve listede olmayan coinleri filtrele — /istatistik ile uyumlu olsun
    _DOMINANS = {"USDT.D","BTC.D","ETH.D","OTHERS.D","USDC.D"}
    _IZLENEN  = {"BTCUSDT","ETHUSDT","SOLUSDT","BNBUSDT","XRPUSDT",
                 "DOGEUSDT","ADAUSDT","AVAXUSDT","LINKUSDT","DOTUSDT"}
    kayitlar = [s for s in _tum
                if s.get("symbol","").replace(".P","").replace("USDT","") + "USDT" in _IZLENEN
                or s.get("symbol","").replace(".P","") in _IZLENEN]
    ...
    # 2. Piyasa koşulları
    kosullar = {
        "Market — Yükseliş":      [s for s in kayitlar if s.get("piyasa_yonu") == "Yükseliş"],
        "Market — Düşüş":         [s for s in kayitlar if s.get("piyasa_yonu") == "Düşüş"],
        "Market — Yatay":          [s for s in kayitlar if s.get("piyasa_yonu") == "Yatay"],
    ...
    # 7. Üçlü kombinasyon — Coin + TF + Piyasa Koşulu
    uclu_kombinasyonlar = {}
    kosul_listesi = [
        ("piyasa_yonu", ["Yükseliş", "Düşüş", "Yatay"]),
        ("funding_yonu", ["Pozitif", "Negatif"]),
        ("fg_kategori_gruplu", ["Korku", "Açgözlülük"]),
    ]
    for sym in sorted(semboller):
        for tf_lbl in ["1DK", "5DK", "15DK"]:
            tf_key = [k for k,v in tf_map.items() if v==tf_lbl]
            coin_tf = [s for s in kayitlar
                       if s.get("symbol","").replace("USDT","")==sym
                       and str(s.get("timeframe","")) in tf_key]
            if len(coin_tf) < 5:
                continue
            for kosul_alan, degerler in kosul_listesi:
                for deger in degerler:
                    if kosul_alan == "fg_kategori_gruplu":
                        # F&G kategorisini gruplara ayır
                        if deger == "Korku":
                            sub = [s for s in coin_tf if s.get("fg_kategori","") in ["Extreme Fear","Fear","Aşırı Korku","Korku"]]
                        else:
                            sub = [s for s in coin_tf if s.get("fg_kategori","") in ["Greed","Extreme Greed","Açgözlülük","Aşırı Açgözlülük"]]
                    else:
                        sub = [s for s in coin_tf if s.get(kosul_alan) == deger]
                    if len(sub) >= 5:
                        anahtar = f"{sym} + {tf_lbl} + {deger}"
                        uclu_kombinasyonlar[anahtar] = satir(sub)

    # 8. Dörtlü kombinasyon — Coin + TF + Piyasa yönü + Funding yönü
    dortu_kombinasyonlar = {}
    for sym in sorted(semboller):
        for tf_lbl in ["1DK", "5DK", "15DK"]:
            tf_key = [k for k, v in tf_map.items() if v == tf_lbl]
            coin_tf = [s for s in kayitlar
            ...
            if len(coin_tf) < 5:
                continue
            for yon in ["Yükseliş", "Düşüş", "Yatay"]:
                for fund in ["Pozitif", "Negatif"]:
                    sub = [s for s in coin_tf
                           if s.get("piyasa_yonu") == yon
                           and s.get("funding_yonu") == fund]
                    if len(sub) >= 5:
                        anahtar = f"{sym} + {tf_lbl} + {yon} + {fund}"
                        dortu_kombinasyonlar[anahtar] = satir(sub)

    # 9. Sinyal tipi + Coin + Piyasa yönü
    sinyal_piyasa_kombinasyonlar = {}
    for tip in tipler:
        for sym in sorted(semboller):
            for yon in ["Yükseliş", "Düşüş", "Yatay"]:
                sub = [s for s in kayitlar
                       if s.get("sinyal_tipi") == tip
                       and s.get("symbol", "").replace("USDT", "") == sym
                       and s.get("piyasa_yonu") == yon]
                if len(sub) >= 5:
                    anahtar = f"{tip} + {sym} + {yon}"
                    sinyal_piyasa_kombinasyonlar[anahtar] = satir(sub)

    # 10. Funding yönü + OI yönü + Sinyal tipi
    oi_funding_kombinasyonlar = {}
    for fund in ["Pozitif", "Negatif"]:
        for oi in ["Artıyor", "Azalıyor"]:
            for tip in tipler:
                sub = [s for s in kayitlar
                       if s.get("funding_yonu") == fund
                       and s.get("oi_yonu") == oi
                       and s.get("sinyal_tipi") == tip]
                if len(sub) >= 5:
                    anahtar = f"{fund} Funding + OI {oi} + {tip}"
                    oi_funding_kombinasyonlar[anahtar] = satir(sub)

    # 11. Coin + TF + Hacim durumu
    hacim_kombinasyonlar = {}
    for sym in sorted(semboller):
        for tf_lbl in ["1DK", "5DK", "15DK"]:
            tf_key = [k for k, v in tf_map.items() if v == tf_lbl]
            coin_tf = [s for s in kayitlar
            ...
            if len(coin_tf) < 5:
                continue
            for hacim in ["Ortalama Üstü", "Ortalama Altı"]:
                sub = [s for s in coin_tf if s.get("hacim_durumu") == hacim]
                if len(sub) >= 5:
                    anahtar = f"{sym} + {tf_lbl} + Hacim {hacim}"
                    hacim_kombinasyonlar[anahtar] = satir(sub)

    # 12. F&G kategori + Sinyal tipi
    fg_sinyal_kombinasyonlar = {}
    fg_gruplari = {
        "Aşırı Korku": lambda s: (s.get("fg_deger") or 100) <= 24,
        "Korku":        lambda s: 25 <= (s.get("fg_deger") or -1) <= 44,
        "Açgözlülük":  lambda s: 56 <= (s.get("fg_deger") or -1) <= 75,
        "Aşırı Açgözlülük": lambda s: (s.get("fg_deger") or -1) >= 76,
    }
    for fg_lbl, fg_filtre in fg_gruplari.items():
        fg_sub = [s for s in kayitlar if fg_filtre(s)]
        for tip in tipler:
            sub = [s for s in fg_sub if s.get("sinyal_tipi") == tip]
            if len(sub) >= 5:
                anahtar = f"F&G {fg_lbl} + {tip}"
                fg_sinyal_kombinasyonlar[anahtar] = satir(sub)

    # 13. Hacim + Piyasa yönü
    hacim_piyasa_kombinasyonlar = {}
    for hacim in ["Ortalama Üstü", "Ortalama Altı"]:
        for yon in ["Yükseliş", "Düşüş", "Yatay"]:
            sub = [s for s in kayitlar
                   if s.get("hacim_durumu") == hacim
                   and s.get("piyasa_yonu") == yon]
            if len(sub) >= 5:
                anahtar = f"Hacim {hacim} + {yon}"
                hacim_piyasa_kombinasyonlar[anahtar] = satir(sub)

    # 14. RSI kategorisi + Sinyal tipi
    rsi_sinyal_kombinasyonlar = {}
    rsi_gruplari = ["Aşırı Alım", "Alım Yakın", "Nötr", "Satım Yakın", "Aşırı Satım"]
    for rsi_kat in rsi_gruplari:
        rsi_sub = [s for s in kayitlar if s.get("rsi_kategori") == rsi_kat]
        for tip in tipler:
            sub = [s for s in rsi_sub if s.get("sinyal_tipi") == tip]
            if len(sub) >= 5:
                anahtar = f"RSI {rsi_kat} + {tip}"
                rsi_sinyal_kombinasyonlar[anahtar] = satir(sub)

    # 15. RSI kategorisi + Piyasa yönü
    rsi_piyasa_kombinasyonlar = {}
    for rsi_kat in rsi_gruplari:
        for yon in ["Yükseliş", "Düşüş", "Yatay"]:
            sub = [s for s in kayitlar
                   if s.get("rsi_kategori") == rsi_kat
                   and s.get("piyasa_yonu") == yon]
            if len(sub) >= 5:
                anahtar = f"RSI {rsi_kat} + {yon}"
                rsi_piyasa_kombinasyonlar[anahtar] = satir(sub)

    # 16. Öneri: min 20 sinyal, en yüksek TP oranlı kombinasyonlar
    MIN_SINYAL = 20
    tum_kombinasyonlar = {}
    tum_kombinasyonlar.update(kombinasyonlar)
    tum_kombinasyonlar.update(uclu_kombinasyonlar)
    tum_kombinasyonlar.update(dortu_kombinasyonlar)
    tum_kombinasyonlar.update(sinyal_piyasa_kombinasyonlar)
    tum_kombinasyonlar.update(oi_funding_kombinasyonlar)
    tum_kombinasyonlar.update(hacim_kombinasyonlar)
    tum_kombinasyonlar.update(fg_sinyal_kombinasyonlar)
    tum_kombinasyonlar.update(hacim_piyasa_kombinasyonlar)
    tum_kombinasyonlar.update(rsi_sinyal_kombinasyonlar)
    tum_kombinasyonlar.update(rsi_piyasa_kombinasyonlar)
    oneriler = [
        (k, v) for k, v in tum_kombinasyonlar.items()
        if v[0] >= MIN_SINYAL
    ]
    oneriler.sort(key=lambda x: float(str(x[1][4]).replace("%", "")) if "%" in str(x[1][4]) else -1, reverse=True)
    oneriler = oneriler[:5]
    ...
    return {
        ...
        "uclu_kombinasyon": uclu_kombinasyonlar,
        "dortu_kombinasyon": dortu_kombinasyonlar,
        "sinyal_piyasa_kombinasyon": sinyal_piyasa_kombinasyonlar,
        "oi_funding_kombinasyon": oi_funding_kombinasyonlar,
        "hacim_kombinasyon": hacim_kombinasyonlar,
        "fg_sinyal_kombinasyon": fg_sinyal_kombinasyonlar,
        "hacim_piyasa_kombinasyon": hacim_piyasa_kombinasyonlar,
        "rsi_sinyal_kombinasyon": rsi_sinyal_kombinasyonlar,
        "rsi_piyasa_kombinasyon": rsi_piyasa_kombinasyonlar,
        "oneri": oneriler,
        "toplam": len(kayitlar),
        "toplam_tp": toplam_tp,
    }
...
def _tp_kosul_yorum_uret(veri, snapshot=None):
    """En İyi TP Koşulları — sinyal tipine göre, temiz format."""
    maddeler = []
    AYIRAC = "―――――――――――――――――――"

    confluance = _tp_confluance_skoru_hesapla(veri, kayitlar_snapshot=snapshot)
    if confluance:
        sinyal_sonuclari = confluance.get("sinyal_sonuclari", {})

        maddeler.append(f"<b>En İyi TP Koşulları</b>")

        SINYAL_TIPLERI = ["Long", "Short", "Strong Buy", "Strong Sell"]
        sirali = sorted(
            SINYAL_TIPLERI,
            key=lambda st: sinyal_sonuclari.get(st, {}).get("genel_tp_orani", 0),
            reverse=True
        )

        for st in sirali:
            veri_st    = sinyal_sonuclari.get(st, {})
            en_iyi     = veri_st.get("en_iyi", [])
            genel_oran = veri_st.get("genel_tp_orani", 0)
            genel_top  = veri_st.get("genel_toplam", 0)

            maddeler.append(AYIRAC)
            maddeler.append(f"<b>{st}</b> %{genel_oran} ({genel_top} sinyal)")
            if en_iyi:
                for item in en_iyi:
                    maddeler.append(
                        f"{item['kombinasyon']} "
                        f"→ {item['tp_orani']} ({item['tp_sayisi']}/{item['sinyal']})"
                    )
            else:
                maddeler.append("— Yeterli veri yok (min. 5 sinyal)")
    else:
        maddeler.append("En İyi TP Koşulları: Yeterli veri yok")

    maddeler.append(AYIRAC)
    maddeler.append("📖 <b>Gösterim:</b> TF + Piyasa + Funding + OI + F&G → En İyi TP% (TP/Toplam)")
    return maddeler
...
def _tp_kosul_rapor_gorsel(veri=None, snapshot=None):
    ...
    yorum_maddeler = _tp_kosul_yorum_uret(veri, snapshot=snapshot)
    ...
    def gorsel_yap(basliklar_renkler_satirlar_sutunlar, baslik_ana, alt_yazi="",
        ...
        for (bolum_baslik, renk, satirlar, sutunlar) in basliklar_renkler_satirlar_sutunlar:
            ...
            # Sütun genişlikleri: Kombinasyon geniş, sayısal sütunlar dar
            COL_ORANLAR = {
                1: [1.0],
                2: [0.55, 0.45],
                3: [0.50, 0.25, 0.25],
                4: [0.46, 0.18, 0.18, 0.18],
                5: [0.42, 0.15, 0.15, 0.15, 0.13],
                6: [0.40, 0.12, 0.12, 0.13, 0.11, 0.12],
            }
            oranlar = COL_ORANLAR.get(len(sutunlar), [1/len(sutunlar)]*len(sutunlar))
            col_ws  = [GENISLIK * o for o in oranlar]
            col_xs  = [SOL + sum(col_ws[:i]) for i in range(len(sutunlar))]

            y -= 0.32
            for ci, sut in enumerate(sutunlar):
                ax.text(col_xs[ci] + (col_ws[ci]*0.05 if ci==0 else col_ws[ci]/2),
                        y, sut, va="center",
    ...
    satirlar1 = sorted([(t,)+v for t,v in veri["sinyal_tipi"].items()],
                       key=lambda x: float(str(x[5]).replace("%","")) if "%" in str(x[5]) else -1,
                       reverse=True)
    ...
    komb_sirali = sorted(veri["kombinasyon"].items(), key=lambda x: float(str(x[1][4]).replace("%","")) if "%" in str(x[1][4]) else -1, reverse=True)[:15]
...
TP_CONFLUANCE_LOG_DOSYASI = "/data/tp_confluance_log.json"

def _tp_confluance_skoru_hesapla(veri, kayitlar_snapshot=None):
    """
    Tüm kombinasyonları birleştirip confluance skoru üretir.
    Her kombinasyonun TP oranı + sinyal sayısına göre ağırlıklı skor hesaplar.
    En iyi koşulu döner.
    kayitlar_snapshot: ham sinyal listesi (verilirse gunluk_sinyaller yerine kullanılır)
    """
    def oran_val(v):
        try:
            return float(str(v[4]).replace("%", "")) if "%" in str(v[4]) else -1
        except Exception:
            return -1

    def guven_etiketi(n):
        if n < 10:   return "düşük veri"
        if n < 30:   return "orta güven"
        if n < 60:   return "güvenilir"
        return "yüksek güven"

    # Tüm kombinasyonları tek havuzda topla
    tum = {}
    for kaynak, dict_adi in [
        ("ikili",           "kombinasyon"),
        ("uclu",            "uclu_kombinasyon"),
        ("dortu",           "dortu_kombinasyon"),
        ("sinyal_piyasa",   "sinyal_piyasa_kombinasyon"),
        ("oi_funding",      "oi_funding_kombinasyon"),
        ("hacim",           "hacim_kombinasyon"),
        ("fg_sinyal",       "fg_sinyal_kombinasyon"),
        ("hacim_piyasa",    "hacim_piyasa_kombinasyon"),
        ("rsi_sinyal",      "rsi_sinyal_kombinasyon"),
        ("rsi_piyasa",      "rsi_piyasa_kombinasyon"),
    ]:
        for k, v in veri.get(dict_adi, {}).items():
            oran = oran_val(v)
            sinyal = v[0]
            if oran >= 0 and sinyal >= 5:
                # Ağırlıklı skor: TP oranı * log(sinyal sayısı) — çok sinyalli kombinasyonlar daha güvenilir
                import math
                agirlikli = round(oran * math.log(max(sinyal, 1)), 2)
                tum[k] = {
                    "kaynak": kaynak,
                    "tp_orani": oran,
                    "sinyal": sinyal,
                    "tp_sayisi": v[1],
                    "agirlikli_skor": agirlikli,
                    "guven": guven_etiketi(sinyal),
                }
    ...
    sirali = sorted(tum.items(), key=lambda x: x[1]["agirlikli_skor"], reverse=True)
    en_iyi_key, en_iyi = sirali[0]

    # Confluance bileşenleri — piyasa koşulları farkları
    bilesenler = {}
    kosul_veri = veri.get("kosul", {})
    def fark_hesapla(a_key, b_key, etiket):
        a = kosul_veri.get(a_key)
        b = kosul_veri.get(b_key)
        if a and b:
            try:
                fa = float(str(a[4]).replace("%",""))
                fb = float(str(b[4]).replace("%",""))
                fark = round(fa - fb, 1)
                if abs(fark) >= 3:
                    bilesenler[etiket] = f"{'+' if fark > 0 else ''}{fark} puan"
            except Exception:
                pass

    fark_hesapla("Funding — Pozitif", "Funding — Negatif", "funding_etkisi")
    fark_hesapla("Piyasa — Yükseliş", "Piyasa — Düşüş", "piyasa_etkisi")

    fg_korku = kosul_veri.get("F&G — Aşırı Korku") or kosul_veri.get("F&G — Korku")
    fg_acgoz = kosul_veri.get("F&G — Açgözlülük") or kosul_veri.get("F&G — Aşırı Açgözlülük")
    if fg_korku and fg_acgoz:
        try:
            fk = float(str(fg_korku[4]).replace("%",""))
            fa = float(str(fg_acgoz[4]).replace("%",""))
            fark = round(fk - fa, 1)
            if abs(fark) >= 3:
                bilesenler["fg_etkisi"] = f"{'+' if fark > 0 else ''}{fark} puan"
        except Exception:
            pass
    ...
    top5 = [
        {
            "kombinasyon": k,
            "tp_orani": f"%{v['tp_orani']}",
            "sinyal": v["sinyal"],
            "tp_sayisi": v["tp_sayisi"],
            "agirlikli_skor": v["agirlikli_skor"],
            "guven": v["guven"],
            "kaynak": v["kaynak"],
        }
        for k, v in sirali[:5]
    ...
    # ── Mega kombinasyon: sinyal tipine göre ayrı ayrı piyasa koşulları ──────
    if kayitlar_snapshot is not None:
        _tum_kayitlar = list(kayitlar_snapshot)
    else:
        with gunluk_kilit:
            _tum_kayitlar = list(gunluk_sinyaller)

    _IZLENEN = {"BTCUSDT","ETHUSDT","SOLUSDT","BNBUSDT","XRPUSDT",
                "DOGEUSDT","ADAUSDT","AVAXUSDT","LINKUSDT","DOTUSDT"}
    _kayitlar = [s for s in _tum_kayitlar
                 if s.get("symbol","").replace(".P","").replace("USDT","") + "USDT" in _IZLENEN
                 or s.get("symbol","").replace(".P","") in _IZLENEN]
    ...
    def _fg_kat(fg):
        ...
        if fg <= 24:   return "Aşırı Korku"
        if fg <= 44:   return "Korku"
        if fg <= 55:   return "Nötr"
        if fg <= 75:   return "Açgözlülük"
        return "Aşırı Açgözlülük"

    _TF_MAP = {"1":"1DK","3":"3DK","5":"5DK","15":"15DK","30":"30DK","60":"1SA","240":"4SA","D":"1G"}

    SINYAL_TIPLERI = ["Long", "Short", "Strong Buy", "Strong Sell"]

    # Her sinyal tipi için ayrı kombinasyon havuzu
    sinyal_mega = {st: {} for st in SINYAL_TIPLERI}
    sinyal_genel = {st: {"tp": 0, "toplam": 0} for st in SINYAL_TIPLERI}

    for s in _kayitlar:
        st = s.get("sinyal_tipi")
        if st not in SINYAL_TIPLERI:
            continue

        tf_raw = s.get("timeframe")
        tf     = _TF_MAP.get(str(tf_raw), str(tf_raw)) if tf_raw else None
        piy    = s.get("piyasa_yonu")
        fund   = s.get("funding_yonu")
        oi     = ("OI " + s.get("oi_yonu")) if s.get("oi_yonu") else None
        rsi    = s.get("rsi_kategori") if s.get("rsi_kategori") not in (None, "Nötr") else None
        hacim  = s.get("hacim_durumu") if s.get("hacim_durumu") not in (None, "Normal") else None
        fg     = _fg_kat(s.get("fg_deger"))

        if not tf:
            continue

        # Genel istatistik
        sinyal_genel[st]["toplam"] += 1
        if tp_mi(s):
            sinyal_genel[st]["tp"] += 1

        # Piyasa koşulları kombinasyonu (sinyal tipi hariç)
        parts = [tf]
        if piy:   parts.append(piy)
        if fund:  parts.append(fund)
        if oi:    parts.append(oi)
        if rsi:   parts.append(rsi)
        if hacim and hacim != "Normal": parts.append(hacim)
        if fg:    parts.append(fg)

        # Minimum 3 faktör zorunlu — TF + en az 2 piyasa koşulu
        if len(parts) < 3:
            continue

        key = " + ".join(parts)
        if key not in sinyal_mega[st]:
            sinyal_mega[st][key] = {"tp": 0, "toplam": 0}
        sinyal_mega[st][key]["toplam"] += 1
        if tp_mi(s):
            sinyal_mega[st][key]["tp"] += 1

    MIN_TP_ORA = 30.0  # %30 altı kombinasyonlar gösterilmez

    def _min_mega(st):
        """Sinyal tipinin toplam sayısına göre dinamik MIN_MEGA eşiği."""
        toplam = sinyal_genel[st]["toplam"]
        if toplam >= 200:
            return 10
        elif toplam >= 50:
            return 5
    ...
    def _en_iyi_kosullar(st_dict, n=3, min_mega=10):
        fil = {
            k: v for k, v in st_dict.items()
            if v["toplam"] >= min_mega
            and (v["tp"] / v["toplam"] * 100) >= MIN_TP_ORA
        }
        sirali = sorted(
            fil.items(),
            key=lambda x: (
                (x[1]["tp"] / x[1]["toplam"]) * math.log(max(x[1]["toplam"], 1)),
                x[1]["toplam"],
            ),
            reverse=True
        )
        sonuc = []
        for k, v in sirali[:n]:
            oran = round(v["tp"] / v["toplam"] * 100, 1)
            sonuc.append({
                "kombinasyon": k,
                "tp_orani": f"%{oran}",
                "tp_sayisi": v["tp"],
                "sinyal": v["toplam"],
                "guven": guven_etiketi(v["toplam"]),
            })
        return sonuc

    # Her sinyal tipi için en iyi koşulları bul
    sinyal_sonuclari = {}
    for st in SINYAL_TIPLERI:
        genel = sinyal_genel[st]
        genel_oran = round(genel["tp"] / genel["toplam"] * 100, 1) if genel["toplam"] > 0 else 0
        sinyal_sonuclari[st] = {
            "genel_tp_orani": genel_oran,
            "genel_toplam": genel["toplam"],
            "genel_tp": genel["tp"],
            "en_iyi": _en_iyi_kosullar(sinyal_mega[st], min_mega=_min_mega(st)),
        }

    # Sinyal tiplerini genel TP oranına göre büyükten küçüğe sırala
    sinyal_sirali = sorted(
        SINYAL_TIPLERI,
        key=lambda st: sinyal_sonuclari[st]["genel_tp_orani"],
        reverse=True
    )

    # top3_mega: geriye dönük uyumluluk için ilk sinyal tipinin en iyi koşulu
    top3_mega = []
    for st in sinyal_sirali:
        en_iyi_st = sinyal_sonuclari[st]["en_iyi"]
        if en_iyi_st:
            item = en_iyi_st[0].copy()
            item["sinyal_tipi"] = st
            top3_mega.append(item)

    # Tavsiye yön — sinyal sıralamasından
    LONG_ST  = {"Long", "Strong Buy"}
    SHORT_ST = {"Short", "Strong Sell"}
    long_agirlik = sum(
        sinyal_sonuclari[st]["genel_tp_orani"] * math.log(max(sinyal_sonuclari[st]["genel_toplam"], 1))
        for st in sinyal_sirali[:2] if st in LONG_ST
    )
    short_agirlik = sum(
        sinyal_sonuclari[st]["genel_tp_orani"] * math.log(max(sinyal_sonuclari[st]["genel_toplam"], 1))
        for st in sinyal_sirali[:2] if st in SHORT_ST
    )
    fark_oran = abs(long_agirlik - short_agirlik) / max(long_agirlik + short_agirlik, 1) * 100
    if long_agirlik > short_agirlik:   tavsiye_yon = "LONG"
    elif short_agirlik > long_agirlik: tavsiye_yon = "SHORT"
    else:                              tavsiye_yon = "NÖTR"
    if fark_oran < 10: tavsiye_yon = "NÖTR"
    if fark_oran >= 40:   tavsiye_guven = "yüksek güven"
    elif fark_oran >= 20: tavsiye_guven = "güvenilir"
    elif fark_oran >= 10: tavsiye_guven = "orta güven"
    else:                 tavsiye_guven = "düşük veri"



    return {
        "en_iyi_kosul": en_iyi_key,
        "tp_orani": f"%{en_iyi['tp_orani']}",
        "agirlikli_skor": en_iyi["agirlikli_skor"],
        "guven": en_iyi["guven"],
        "sinyal": en_iyi["sinyal"],
        "tp_sayisi": en_iyi["tp_sayisi"],
        "kaynak": en_iyi["kaynak"],
        "bilesenler": bilesenler,
        "top5": top5,
        "top3_mega": top3_mega,
        "sinyal_sonuclari": sinyal_sonuclari,
        "sinyal_sirali": sinyal_sirali,
        "toplam_kombinasyon": len(tum),
        "tavsiye_yon": tavsiye_yon,
        "tavsiye_guven": tavsiye_guven,
        "long_agirlik": round(long_agirlik, 2),
        "short_agirlik": round(short_agirlik, 2),
    }


def _tp_confluance_log_yaz(confluance, zaman_str):
    """Confluance sonucunu /data/tp_confluance_log.json'a append eder."""
    kayit = {"zaman": zaman_str}
    kayit.update(confluance)
    try:
        try:
            with open(TP_CONFLUANCE_LOG_DOSYASI, "r", encoding="utf-8") as f:
                log = _json.load(f)
        except Exception:
            log = []
        log.append(kayit)
        # Son 720 kayıt tut (30 günlük saatlik log)
        log = log[-720:]
        with open(TP_CONFLUANCE_LOG_DOSYASI, "w", encoding="utf-8") as f:
            _json.dump(log, f, ensure_ascii=False, indent=2)
        print(f"[TP_CONFLUANCE] Log yazildi: {zaman_str} — {confluance.get('en_iyi_kosul','?')}")
    except Exception as e:
        print(f"[TP_CONFLUANCE] Log yazma hatasi: {e}")



def _tplog_tarih_parse(parametre=None):
    """
    Tarih parametresini parse eder, (yil, ay, gun) döner.
    - None / boş → "bugün" (01:10 sıfırlama saatine göre)
      Saat 00:00-01:09 arası → bir önceki günün raporu
      Saat 01:10+ → bugünün raporu
    - "16"         → bu ayın 16'sı
    - "16-06"      → bu yılın Haziran 16'sı
    - "16-06-2025" → tam tarih
    """
    if TR_TZ:
        simdi = dt_cls.now(tz=TR_TZ)
    else:
        simdi = dt_cls.utcnow()

    if not parametre or not parametre.strip():
        # 01:10'dan önce ise bir önceki gün
        if simdi.hour == 0 or (simdi.hour == 1 and simdi.minute < 10):
            bugun = (simdi - timedelta(days=1))
        else:
            bugun = simdi
        return bugun.year, bugun.month, bugun.day

    p = parametre.strip()
    parcalar = p.replace("/", "-").split("-")
    bugun = simdi
    try:
        if len(parcalar) == 1:
            return bugun.year, bugun.month, int(parcalar[0])
        elif len(parcalar) == 2:
            return bugun.year, int(parcalar[1]), int(parcalar[0])
        elif len(parcalar) == 3:
            return int(parcalar[2]), int(parcalar[1]), int(parcalar[0])
    except Exception:
        pass
    return bugun.year, bugun.month, bugun.day


def _tplog_gorsel_uret(kayitlar, tarih_str):
    """Her saat için sinyal tipine göre en iyi piyasa koşullarını gösterir."""
    try:

        if not kayitlar:
            return None

        BG="#13151A"; ROW_ODD="#1A1D24"; ROW_EVN="#16181F"
        TW="#E8E8E6"; TG="#6B6F7A"; TDIM="#3E4250"
        GREEN="#4CAF50"; RED="#F44336"; AMBER="#FFC107"; TEAL="#26C6A0"; HDR="#1E2130"
        ST_RENK={"Long":GREEN,"Strong Buy":TEAL,"Short":RED,"Strong Sell":"#E07060"}
        ST_KISA={"Long":"Long","Strong Buy":"Str.Buy","Short":"Short","Strong Sell":"Str.Sell"}

        rows=[]
        for k in kayitlar:
            saat=k.get("zaman","")[-5:]
            ss=k.get("sinyal_sonuclari",{})
            sr=k.get("sinyal_sirali",[])
            if not ss or not sr:
                continue
            for idx,st in enumerate(sr):
                vd=ss.get(st,{}); ei=vd.get("en_iyi",[]); g_o=vd.get("genel_tp_orani",0); g_t=vd.get("genel_toplam",0)
                ilk=(idx==0)
                sinyal_lbl=f"{ST_KISA.get(st,st)} %{g_o}"
                if ei:
                    item=ei[0]; kosul=item["kombinasyon"]
                    if len(kosul)>55: kosul=kosul[:53]+"…"
                    tp=item["tp_orani"]; tp_sin=f"{item['tp_sayisi']}/{item['sinyal']}"
                else:
                    kosul="Yeterli veri yok"; tp=f"%{g_o}"; tp_sin=f"—/{g_t}"
                rows.append((saat if ilk else "",sinyal_lbl,kosul,tp,tp_sin,st,ilk))

        if not rows:
            return None

        n_satir=len(rows); YON_SAG=0.96
        XPOS=[0.005,0.060,0.210,0.76,0.85]
        ALIGN=["left","left","left","right","center"]
        COLS=["Saat","Sinyal","En İyi Koşul","TP%","TP/Sin"]
        satir_h=1.0/(n_satir+2); fig_h=max(3.0,1.0+n_satir*0.32)
        fig,ax=plt.subplots(figsize=(12.0,fig_h))
        fig.patch.set_facecolor(BG); ax.set_facecolor(BG); ax.axis("off")
        ax.set_xlim(0,1); ax.set_ylim(0,1)

        n_kayit=len(kayitlar)
        ax.text(YON_SAG/2,1.0-satir_h*0.15,f"En İyi TP Koşulları  —  {tarih_str}",
                transform=ax.transAxes,ha="center",va="top",fontsize=11,fontweight="bold",color=TW)
        ax.text(YON_SAG/2,1.0-satir_h*0.65,f"{n_kayit} saat  |  sinyal tipine göre en iyi piyasa koşulları",
                transform=ax.transAxes,ha="center",va="top",fontsize=7,color=TG)

        y_b=1.0-satir_h
        ax.add_patch(FancyBboxPatch((0,y_b),YON_SAG,satir_h,boxstyle="square,pad=0",facecolor=HDR,edgecolor="none",transform=ax.transAxes,clip_on=False))
        for cx,ca,ch in zip(XPOS,ALIGN,COLS):
            ax.text(cx,y_b+satir_h/2,ch,transform=ax.transAxes,ha=ca,va="center",fontsize=8,fontweight="bold",color=TW)

        grup=-1
        for i,(saat,sinyal,kosul,tp,tp_sin,st_raw,ilk) in enumerate(rows):
            if saat: grup+=1
            y_top=1.0-satir_h*(i+2); y_mid=y_top+satir_h/2
            bg=ROW_ODD if grup%2==0 else ROW_EVN
            ax.add_patch(FancyBboxPatch((0,y_top),YON_SAG,satir_h,boxstyle="square,pad=0",facecolor=bg,edgecolor="none",transform=ax.transAxes,clip_on=False))
            if not ilk: ax.plot([XPOS[1],YON_SAG],[y_top+satir_h,y_top+satir_h],color=TDIM,linewidth=0.25,transform=ax.transAxes,clip_on=False)
            try: tv=float(str(tp).replace("%","")); tr=GREEN if tv>=50 else AMBER if tv>=35 else RED
            except: tr=TW
            sr2=ST_RENK.get(st_raw,TEAL)
            for cx,ca,val,clr,bold in zip(XPOS,ALIGN,[saat,sinyal,kosul,tp,tp_sin],[TW,sr2,TW,tr,TW],[True,True,ilk,True,True]):
                if not str(val): continue
                ax.text(cx,y_mid,str(val),transform=ax.transAxes,ha=ca,va="center",fontsize=7.5,fontweight="bold" if bold else "normal",color=clr)

        # Alt özet
        st_counts={}
        for k in kayitlar:
            sr=k.get("sinyal_sirali",[])
            if sr: st_counts[sr[0]]=st_counts.get(sr[0],0)+1
        ozet_parts=[f"{ST_KISA.get(st,st)}: {c}" for st,c in sorted(st_counts.items(),key=lambda x:-x[1])]
        ozet_txt=("  |  ".join(ozet_parts) if ozet_parts else f"{n_kayit} kayit")+"  |  /tplog"
        y_bot=1.0-satir_h*(n_satir+2)
        ax.add_patch(FancyBboxPatch((0,y_bot),YON_SAG,satir_h,boxstyle="square,pad=0",facecolor=HDR,edgecolor="none",transform=ax.transAxes,clip_on=False))
        ax.text(YON_SAG/2,y_bot+satir_h/2,ozet_txt,transform=ax.transAxes,ha="center",va="center",fontsize=8,color=TW)

        ax.set_position([0,0,YON_SAG,1])
        buf=io.BytesIO()
        plt.savefig(buf,format="png",dpi=150,facecolor=BG,bbox_inches="tight")
        plt.close(fig); buf.seek(0)
        return buf.read(),"En İyi TP Koşulları — "+tarih_str+"\n/tplog"
    except Exception as e:
        print("[TPLOG_GORSEL] Hata: "+str(e)+"\n"+traceback.format_exc())
        return None


def _tpozet_gorsel_uret(confluance, tarih_aralik, toplam_sinyal):
    """
    Tüm birikmiş veriden sinyal tipine göre en iyi koşulları gösterir.
    /tplog ile aynı format — tek satır per sinyal tipi.
    """
    try:
        ...
        sinyal_sonuclari = confluance.get("sinyal_sonuclari", {})
        sinyal_sirali    = confluance.get("sinyal_sirali", ["Long","Short","Strong Buy","Strong Sell"])

        # Satırları hazırla — her sinyal tipi için en iyi kombinasyon
        rows = []
        SINYAL_TIPLERI = ["Long", "Short", "Strong Buy", "Strong Sell"]
        sirali = sorted(
            SINYAL_TIPLERI,
        ...
        for idx, st in enumerate(sirali):
            veri_st  = sinyal_sonuclari.get(st, {})
            en_iyi   = veri_st.get("en_iyi", [])
            g_oran   = veri_st.get("genel_tp_orani", 0)
            g_top    = veri_st.get("genel_toplam", 0)
            sinyal_lbl = f"{ST_KISA.get(st,st)} %{g_oran}"

            if en_iyi:
                item  = en_iyi[0]
                # OI koşulunu görselden çıkar — filtre OI'siz çalışıyor, yanıltıcı olmasın
                _OI_KELIMELER = ["OI Artıyor", "OI Azalıyor", "OI Değişmiyor"]
                kosul_parts = [p.strip() for p in item["kombinasyon"].split("+")]
                kosul_parts = [p for p in kosul_parts if p not in _OI_KELIMELER]
                kosul = " + ".join(kosul_parts)
                if len(kosul) > 55: kosul = kosul[:53] + "…"
                tp    = item["tp_orani"]
                tp_sin = f"{item['tp_sayisi']}/{item['sinyal']}"
            else:
                kosul  = "Yeterli veri yok"
                tp     = f"%{g_oran}"
                tp_sin = f"—/{g_top}"

            rows.append((sinyal_lbl, kosul, tp, tp_sin, st))

        n = len(rows); YON_SAG = 0.96
        XPOS  = [0.005, 0.155, 0.720, 0.820]
        ALIGN = ["left","left","right","center"]
        COLS  = ["Sinyal", "En İyi Koşul", "TP%", "TP/Sin"]

        satir_h = 1.0 / (n + 2)
        fig_h   = max(2.5, 1.0 + n * 0.45)
        fig, ax = plt.subplots(figsize=(12.0, fig_h))
        fig.patch.set_facecolor(BG); ax.set_facecolor(BG); ax.axis("off")
        ax.set_xlim(0, 1); ax.set_ylim(0, 1)

        # Başlık
        ax.text(YON_SAG/2, 1.0 - satir_h*0.15,
                f"TP Koşul Özeti  —  {tarih_aralik}",
                transform=ax.transAxes, ha="center", va="top",
                fontsize=11, fontweight="bold", color=TW)
        ax.text(YON_SAG/2, 1.0 - satir_h*0.65,
                f"{toplam_sinyal} sinyal  |  birikimli en iyi piyasa koşulları",
                transform=ax.transAxes, ha="center", va="top", fontsize=7, color=TG)

        # Header
        y_b = 1.0 - satir_h
        ax.add_patch(FancyBboxPatch((0, y_b), YON_SAG, satir_h,
                     boxstyle="square,pad=0", facecolor=HDR, edgecolor="none",
                     transform=ax.transAxes, clip_on=False))
        for cx, ca, ch in zip(XPOS, ALIGN, COLS):
            ax.text(cx, y_b + satir_h/2, ch, transform=ax.transAxes, ha=ca, va="center",
                    fontsize=8, fontweight="bold", color=TW)

        # Satırlar
        for i, (sinyal, kosul, tp, tp_sin, st_raw) in enumerate(rows):
            y_top = 1.0 - satir_h*(i+2); y_mid = y_top + satir_h/2
            bg = ROW_ODD if i%2==0 else ROW_EVN
            ax.add_patch(FancyBboxPatch((0, y_top), YON_SAG, satir_h,
                         boxstyle="square,pad=0", facecolor=bg, edgecolor="none",
                         transform=ax.transAxes, clip_on=False))
            try:
                tv = float(str(tp).replace("%","")); tr = GREEN if tv>=50 else AMBER if tv>=35 else RED
            except: tr = TW
            sr2 = ST_RENK.get(st_raw, TEAL)
            for cx, ca, val, clr in zip(XPOS, ALIGN, [sinyal, kosul, tp, tp_sin], [sr2, TW, tr, TW]):
                if not str(val): continue
                ax.text(cx, y_mid, str(val), transform=ax.transAxes, ha=ca, va="center",
                        fontsize=8, fontweight="bold", color=clr)

        # Alt
        y_bot = 1.0 - satir_h*(n+2)
        ax.add_patch(FancyBboxPatch((0, y_bot), YON_SAG, satir_h,
                     boxstyle="square,pad=0", facecolor=HDR, edgecolor="none",
                     transform=ax.transAxes, clip_on=False))
        ax.text(YON_SAG/2, y_bot + satir_h/2, "/tpozet",
                transform=ax.transAxes, ha="center", va="center", fontsize=8, color=TG)
        ...
        plt.close(fig)
        buf.seek(0)
        return buf.read(), f"TP Koşul Özeti — {tarih_aralik}\n/tpozet"
    except Exception as e:
        print(f"[TPOZET_GORSEL] Hata: {e}\n{traceback.format_exc()}")
        return None



    """
    Her saat icin sinyal tipine gore en iyi piyasa kosullarini gosterir.
    Satirlar: her saat x 4 sinyal tipi (genel TP oranina gore siralanmis)
    """
    try:
        ...
        TEAL    = "#26C6A0"; HDR = "#1E2130"

        ST_RENK = {"Long": GREEN, "Strong Buy": TEAL, "Short": RED, "Strong Sell": "#E07060"}
        ST_KISA = {"Long": "Long", "Strong Buy": "Str.Buy", "Short": "Short", "Strong Sell": "Str.Sell"}

        # Satirlari hazirla — sadece sinyal_sonuclari olan yeni format kayıtlar
        rows = []
        for k in kayitlar:
            saat = k.get("zaman", "")[-5:]
            ss   = k.get("sinyal_sonuclari", {})
            ...
            # Eski format kayıt — sinyal_sonuclari yok, atla
            if not ss or not sr:
                continue
        ...
        if not rows:
            return None  # Yeni format kayıt yok, None döndür
        ...
        ALIGN   = ["left","left","left","right","center"]
        COLS    = ["Saat", "Sinyal", "En İyi Koşul", "TP%", "TP/Sin"]
        satir_h = 1.0 / (n_satir + 2)
        fig_h   = max(3.0, 1.0 + n_satir * 0.30)
        ...
        n_kayit = len(kayitlar)
        ax.text(YON_SAG/2, 1.0 - satir_h*0.15,
                f"En Iyi TP Kosullari  —  {tarih_str}",
                transform=ax.transAxes, ha="center", va="top",
                fontsize=11, fontweight="bold", color=TW)
        ax.text(YON_SAG/2, 1.0 - satir_h*0.65,
                f"{n_kayit} saat  |  sinyal tipine gore en iyi piyasa kosullari",
                transform=ax.transAxes, ha="center", va="top", fontsize=7, color=TG)
        ...
        for i, (saat, sinyal, kosul, tp, tp_sin, st_raw, ilk) in enumerate(rows):
            ...
            ax.add_patch(FancyBboxPatch((0, y_top), YON_SAG, satir_h,
                         ...
                         transform=ax.transAxes, clip_on=False))
            if not ilk:
                ax.plot([XPOS[1], YON_SAG], [y_top+satir_h, y_top+satir_h],
                        color=TDIM, linewidth=0.25,
                        transform=ax.transAxes, clip_on=False)
            try:
                tv = float(str(tp).replace("%","")); tr = GREEN if tv>=50 else AMBER if tv>=35 else RED
            except: tr = TW
            st_renk = ST_RENK.get(st_raw, TEAL)
            vals  = [saat, sinyal, kosul, tp, tp_sin]
            clrs  = [TW, st_renk, TW, tr, TW]
            bolds = [True, True, ilk, True, True]
            for cx, ca, val, clr, bold in zip(XPOS, ALIGN, vals, clrs, bolds):
                if not str(val): continue
                ax.text(cx, y_mid, str(val), transform=ax.transAxes, ha=ca, va="center",
                        fontsize=7.5, fontweight="bold" if bold else "normal", color=clr)

        y_bot = 1.0 - satir_h*(n_satir+2)
        # Alt ozet: sinyal dagilimi
        st_counts = {}
        for k in kayitlar:
            sr = k.get("sinyal_sirali", [])
            if sr: st_counts[sr[0]] = st_counts.get(sr[0], 0) + 1
        ozet_parts = [f"{ST_KISA.get(st,st)}: {c}" for st, c in
                      sorted(st_counts.items(), key=lambda x: -x[1])]
        ozet_txt = ("  |  ".join(ozet_parts) if ozet_parts else f"{n_kayit} kayit") + "  |  /tplog"
        ax.add_patch(FancyBboxPatch((0, y_bot), YON_SAG, satir_h,
                     boxstyle="square,pad=0", facecolor=HDR, edgecolor="none",
                     transform=ax.transAxes, clip_on=False))
        ax.text(YON_SAG/2, y_bot + satir_h/2, ozet_txt,
                transform=ax.transAxes, ha="center", va="center", fontsize=8, color=TW)
        ...
        plt.close(fig)
        buf.seek(0)
        return buf.read(), "En Iyi TP Kosullari — " + tarih_str + "\n/tplog"
    except Exception as e:
        print("[TPLOG_GORSEL] Hata: " + str(e))
        return None
...
def _tp_kosul_rapor_zamanlayici():
    """Devre dışı — v615'te kaldırıldı. Yeni sistem: _filtre_zamanlayici."""
    print("[TP_KOSUL] Zamanlayici devre disi (v615).")
    pass


def _kombinasyon_ozet_hesapla(snapshot=None):
    """v646: Panel A ve Panel B'nin ORTAK kullandığı hesaplama — TF+Funding+OI+
    Piyasa kombinasyon sınıflandırması ve Güçlü/Orta/Zayıf özet istatistikleri
    (Güçlü Başarı, Zayıf Başarı, Fark, Kapsama, Doğru). Önceden sadece Panel B
    içindeydi, Aktif Kombinasyon bloğu Panel A'ya taşındığı için ortak fonksiyona
    çıkarıldı — iki panel de AYNI hesabı kullanıyor, kopya kod yok."""
    with gunluk_kilit:
        fsnap = snapshot if snapshot is not None else filtre_sinyaller[:]

    SINYAL_TIPLERI = ["Strong Sell", "Strong Buy", "Long", "Short"]
    TF_MAP  = {"1":"1DK","5":"5DK","15":"15DK","60":"1SA","240":"4SA"}
    ESIK_G, ESIK_Z, MIN_SINYAL = 8.0, 5.0, 3

    kapali_tum  = [s for s in fsnap if s.get("tp1_ok") is not None or s.get("sl_ok") is not None]
    kapanan_tum = [s for s in fsnap if s.get("tp1_ok") == True or s.get("sl_ok") == True]

    genel_tp, combo_tab = {}, {}
    for st in SINYAL_TIPLERI:
        kapali = [s for s in kapali_tum if s.get("sinyal_tipi") == st]
        if not kapali:
            genel_tp[st] = 0.0; combo_tab[st] = {}
            continue
        tp_n = sum(1 for s in kapali if s.get("tp1_ok") == True)
        genel_tp[st] = tp_n / len(kapali) * 100
        tablo = {}
        for s in kapali:
            tf  = TF_MAP.get(str(s.get("timeframe","")), str(s.get("timeframe","")))
            key = f"{tf} + {s.get('funding_yonu') or ''} + {s.get('oi_yonu') or ''} + {s.get('piyasa_yonu') or ''}"
            tablo.setdefault(key, {"tp":0,"n":0})
            tablo[key]["n"] += 1
            if s.get("tp1_ok") == True: tablo[key]["tp"] += 1
        combo_tab[st] = {k:v for k,v in tablo.items() if v["n"] >= MIN_SINYAL}
    ...
    for st in SINYAL_TIPLERI:
        ...
        for s in kapali:
            ...
            if key in tablo:
                cr = tablo[key]["tp"]/tablo[key]["n"]*100
                if cr >= g_ort+ESIK_G:   g_n+=1; g_tp += 1 if is_tp else 0
                elif cr >= g_ort-ESIK_Z: o_n+=1; o_tp += 1 if is_tp else 0
                else:                     z_n+=1; z_tp += 1 if is_tp else 0
            else:
                z_n+=1; z_tp += 1 if is_tp else 0
        detay_satirlari.append({
            "tip": st, "genel": g_ort,
            "g_tp":g_tp,"g_n":g_n, "o_tp":o_tp,"o_n":o_n, "z_tp":z_tp,"z_n":z_n,
        })

    dogru_sayisi = 0; g_list=[]; z_list=[]; kap_list=[]
    for row in detay_satirlari:
        go = row["g_tp"]/row["g_n"]*100 if row["g_n"]>0 else None
        oo = row["o_tp"]/row["o_n"]*100 if row["o_n"]>0 else None
        zo = row["z_tp"]/row["z_n"]*100 if row["z_n"]>0 else None
        if go is not None and oo is not None and zo is not None and go > oo > zo:
            dogru_sayisi += 1
        if go is not None: g_list.append(go)
        if zo is not None: z_list.append(zo)
        toplam_n = row["g_n"]+row["o_n"]+row["z_n"]
        if toplam_n>0: kap_list.append((row["g_n"]+row["o_n"])/toplam_n*100)
    guclu_ort = sum(g_list)/len(g_list) if g_list else 0
    zayif_ort = sum(z_list)/len(z_list) if z_list else 0
    fark      = guclu_ort - zayif_ort
    kapsama   = sum(kap_list)/len(kap_list) if kap_list else 0

    return {
        "kapali_tum": kapali_tum, "kapanan_tum": kapanan_tum,
        "detay_satirlari": detay_satirlari,
        "guclu_ort": guclu_ort, "zayif_ort": zayif_ort,
        "fark": fark, "kapsama": kapsama, "dogru_sayisi": dogru_sayisi,
    }


def _panel_a_gorsel(snapshot=None):
    """v645: PANEL A — Genel Durum (Tüm Zamanlar / Bugün, ikiye bölünmüş, SADECE
    TOPIC_ALARM'a giden sinyaller). Eski BUGÜN+TÜM ZAMANLAR yatay satırları ve
    "Piyasa Rejimi Bazlı Başarı" kutusu kaldırıldı (piyasa_yonu verisi Panel B'nin
    Güçlü/Orta/Zayıf sınıflandırmasında hâlâ kullanılıyor — sadece bu bağımsız
    döküm tablosu, hiçbir kararı etkilemediği için kaldırıldı). Whitelist Durumu
    korundu, alta tam genişlikte taşındı."""
    try:
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as plt
    ...
    BG, CARD_BG = "#13151A", "#1C1F28"
    TEXT_W, HDR_COL = "#E8E8E6", "#6B6F7A"
    GREEN, AMBER, RED, BLUE, GOLD = "#5DC98A", "#E8A835", "#E05C5C", "#4A90D9", "#FFD166"
    ACIK_GRI = "#AEB3BC"
    ...
    try:
        ...
        # v645: SADECE TOPIC_ALARM'a giden sinyaller (Güçlü etiketli VEYA
        # guven_skoru>=0.40) — aynı ALARM_GONDER mantığı, webhook'taki ile birebir.
        # v647: artik modul-seviyesi ortak _alarm_gonderilen() kullanılıyor.
        alarm_snapshot = [s for s in tum_kayitlar if _alarm_gonderilen(s)]
        bugun_snapshot = [s for s in alarm_snapshot if s.get("gun") == bugun]

        b = istatistik_hesapla(gun_filtre=bugun, snapshot=alarm_snapshot)
        # v648: "Tüm Zamanlar" artık FİLTRESİZ (tüm_kayitlar) — /istatistik'in
        # "Tüm Zamanlar" bölümüyle birebir eşleşsin diye. Sadece "Bugün" tarafı
        # TOPIC_ALARM filtreli kalıyor (kullanıcı onayı).
        t = istatistik_hesapla(snapshot=tum_kayitlar)
        ...
        def _en_iyi_hesapla(kayitlar, min_n):
            sym_stats, tf_stats = {}, {}
            for s in kayitlar:
                sym = sembol_grup(s["symbol"])
                sym_stats.setdefault(sym, {"toplam":0,"basarili":0})
                sym_stats[sym]["toplam"] += 1
                if _basarili(s): sym_stats[sym]["basarili"] += 1
                tf = s.get("timeframe","?")
                tf_stats.setdefault(tf, {"toplam":0,"basarili":0})
                tf_stats[tf]["toplam"] += 1
                if _basarili(s): tf_stats[tf]["basarili"] += 1
            en_sym = max(
                ((sym, st["basarili"]/st["toplam"]*100) for sym, st in sym_stats.items() if st["toplam"] >= min_n),
                key=lambda x: x[1], default=("—", -1))
            en_tf = max(
                ((tf_map_g.get(tf, tf), st["basarili"]/st["toplam"]*100) for tf, st in tf_stats.items() if st["toplam"] >= min_n),
                key=lambda x: x[1], default=("—", -1))
            return en_sym, en_tf

        en_sym_t, en_tf_t = _en_iyi_hesapla(tum_kayitlar, 50)
        en_sym_b, en_tf_b = _en_iyi_hesapla(bugun_snapshot, 5)

        # v646: Aktif Kombinasyon özeti — Panel B'den taşındı, ortak fonksiyon
        _ko = _kombinasyon_ozet_hesapla(snapshot)
        guclu_ort, zayif_ort, fark = _ko["guclu_ort"], _ko["zayif_ort"], _ko["fark"]
        kapsama, dogru_sayisi = _ko["kapsama"], _ko["dogru_sayisi"]

        # Whitelist durumu (yan etki: whitelist de güncellenir)
        try:
            top3_guncelle(snapshot)
        except Exception as _we:
            print(f"[PANEL_A] whitelist güncelleme hatası: {_we}")
        sirala = koin_sirala_bugun(snapshot) or []
        wl_semboller = [it["sym"] for it in sirala if it.get("yeterli_veri") and it["oran"] >= TRADE_WHITELIST_MIN_PERCENT]

        fig, ax = plt.subplots(figsize=(11.5, 10.6))
        ax.axis("off"); ax.set_xlim(0, 1); ax.set_ylim(0, 1)
        fig.patch.set_facecolor(BG)

        ax.text(0.5, 0.978, "Ben Kül Yutmam — Genel Durum", fontsize=19, fontweight="bold",
                color=GOLD, ha="center", va="top")
        _simdi_str = datetime.now(tz=TR_TZ).strftime("%d %B %Y %H:%M") if TR_TZ else datetime.utcnow().strftime("%d %B %Y %H:%M")
        ax.text(0.5, 0.951, f"{_simdi_str} · Panel A / 3 · Sadece TOPIC_ALARM sinyalleri",
                fontsize=10, color=TEXT_W, ha="center", va="top")

        def kart(x, y, w, h, baslik, deger, renk):
            ax.add_patch(plt.Rectangle((x, y), w, h, transform=ax.transAxes,
                         facecolor=CARD_BG, edgecolor="none", zorder=1))
            ax.text(x + w/2, y + h*0.66, baslik, transform=ax.transAxes, fontsize=9,
                    fontweight="bold", color=TEXT_W, ha="center", va="center")
            ax.text(x + w/2, y + h*0.30, str(deger), transform=ax.transAxes, fontsize=15,
                    fontweight="bold", color=renk, ha="center", va="center")

        # Dikey ayraç
        ax.plot([0.5, 0.5], [0.36, 0.915], color=BG, linewidth=2, zorder=2)

        tum_zamanlar_kartlari = [
            ("Sinyal", t["toplam"], ACIK_GRI), ("TP", t["tp_basarili"], GREEN),
            ("SL", t["sl_tetiklenen"], RED), ("Devam", t["devam_eden"], AMBER),
            ("Başarı", f"%{t['basari_oran']}", BLUE),
            ("En İyi Sembol", f"{en_sym_t[0]} %{en_sym_t[1]:.1f}" if en_sym_t[1] >= 0 else "—", GREEN),
            ("En İyi TF (min 50)", f"{en_tf_t[0]} %{en_tf_t[1]:.1f}" if en_tf_t[1] >= 0 else "—", GREEN),
        ]
        bugun_kartlari = [
            ("Sinyal", b["toplam"], ACIK_GRI), ("TP", b["tp_basarili"], GREEN),
            ("SL", b["sl_tetiklenen"], RED), ("Devam", b["devam_eden"], AMBER),
            ("Başarı", f"%{b['basari_oran']}", BLUE),
            ("En İyi Sembol", f"{en_sym_b[0]} %{en_sym_b[1]:.1f}" if en_sym_b[1] >= 0 else "—", GREEN),
            ("En İyi TF (min 5)", f"{en_tf_b[0]} %{en_tf_b[1]:.1f}" if en_tf_b[1] >= 0 else "—", GREEN),
        ]

        ax.text(0.02, 0.915, "TÜM ZAMANLAR", fontsize=11.5, fontweight="bold", color=GOLD, ha="left", va="top")
        ax.text(0.02, 0.895, f"({t['long']} Long | {t['short']} Short)", fontsize=8.5, color=HDR_COL, ha="left", va="top")
        yy = 0.865
        for bas, deg, rnk in tum_zamanlar_kartlari:
            kart(0.02, yy-0.062, 0.46, 0.062, bas, deg, rnk)
            yy -= 0.074

        ax.text(0.52, 0.915, "BUGÜN", fontsize=11.5, fontweight="bold", color=GOLD, ha="left", va="top")
        ax.text(0.52, 0.895, f"({b['long']} Long | {b['short']} Short)", fontsize=8.5, color=HDR_COL, ha="left", va="top")
        yy = 0.865
        for bas, deg, rnk in bugun_kartlari:
            kart(0.52, yy-0.062, 0.46, 0.062, bas, deg, rnk)
            yy -= 0.074

        # Aktif Kombinasyon — Gerçek Bileşenler (v646: Panel B'den taşındı)
        ax.add_patch(plt.Rectangle((0.02, 0.16), 0.96, 0.145, transform=ax.transAxes,
                     facecolor=CARD_BG, edgecolor="none", zorder=1))
        ax.text(0.045, 0.288, "Aktif Kombinasyon — Gerçek Bileşenler", fontsize=11.5,
                fontweight="bold", color=GOLD, ha="left", va="top")
        bilesenler = [
            ("Güçlü Başarı", f"%{guclu_ort:.0f}", GREEN),
            ("Zayıf Başarı", f"%{zayif_ort:.0f}", RED),
            ("Fark", f"+{fark:.0f} puan" if fark >= 0 else f"{fark:.0f} puan", BLUE),
            ("Kapsama", f"%{kapsama:.0f}", AMBER),
            ("Doğru", f"{dogru_sayisi}/4", GREEN if dogru_sayisi == 4 else AMBER),
        ]
        bw = 0.184
        for i, (bas, deg, rnk) in enumerate(bilesenler):
            bx = 0.045 + i*(bw+0.006)
            ax.text(bx, 0.235, bas, fontsize=9.5, fontweight="bold", color=TEXT_W, ha="left", va="top")
            ax.text(bx, 0.195, deg, fontsize=15, fontweight="bold", color=rnk, ha="left", va="top")

        # Whitelist kutusu (alt, tam genişlik)
        ax.add_patch(plt.Rectangle((0.02, 0.02), 0.96, 0.11, transform=ax.transAxes,
                     facecolor=CARD_BG, edgecolor="none", zorder=1))
        ax.text(0.045, 0.108, "Whitelist Durumu:", fontsize=10.5, fontweight="bold", color=GOLD, ha="left", va="top")
        if wl_semboller:
            ax.text(0.045, 0.065, "✓  " + ", ".join(wl_semboller) + f"  (%{TRADE_WHITELIST_MIN_PERCENT}+)",
                    fontsize=10.5, color=GREEN, fontweight="bold", ha="left", va="top")
        else:
            ax.text(0.045, 0.065, "Henüz yeterli veri yok", fontsize=10.5, color=RED, ha="left", va="top")
        ...
        print(f"[PANEL_A] Hata: {e}\n{traceback.format_exc()}")
        return None


def _panel_b_gorsel(snapshot=None):
    """v646: PANEL B — Sinyal Kalitesi (Sinyal Tipi Detayı + Coin Bazlı İki Sütun).
    "Aktif Kombinasyon — Gerçek Bileşenler" bloğu Panel A'ya taşındı (kullanıcı
    isteği) — hesaplama artık ortak _kombinasyon_ozet_hesapla() fonksiyonunda.
    Boşalan alan coin bazlı tabloya verildi, yazı boyutları büyütüldü (okunabilirlik)."""
    try:
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as plt
    ...
    try:
        _ko = _kombinasyon_ozet_hesapla(snapshot)
        kapali_tum, kapanan_tum = _ko["kapali_tum"], _ko["kapanan_tum"]
        detay_satirlari = _ko["detay_satirlari"]

        # v644: Coin Bazlı — iki sütun (Güçlü Sinyal | Güven Skoru Yüksek)
        try:
            coin_sirala = koin_sirala_bugun(snapshot) or []
        except Exception:
            coin_sirala = []
        coin_sirala = sorted(coin_sirala, key=lambda it: -(it["guclu_tp"]/it["guclu_n"] if it.get("guclu_n") else -1))
        coin_sirala_guclu = [it for it in coin_sirala if it.get("guclu_n", 0) > 0][:8]

        try:
            coin_sirala_guven = koin_sirala_guven_bugun(snapshot) or []
        except Exception:
            coin_sirala_guven = []
        coin_sirala_guven = [it for it in coin_sirala_guven if it.get("n", 0) > 0][:8]

        fig, ax = plt.subplots(figsize=(11.5, 10.6))
        ax.axis("off"); ax.set_xlim(0, 1); ax.set_ylim(0, 1)
        fig.patch.set_facecolor(BG)

        ax.text(0.5, 0.98, "Ben Kül Yutmam — Sinyal Kalitesi", fontsize=19, fontweight="bold",
                color=GOLD, ha="center", va="top")
        ax.text(0.5, 0.955, "Aktif Kombinasyon: TF + Funding + OI + Piyasa · Panel B / 3",
                fontsize=10.5, color=HDR_COL, ha="center", va="top")

        # Sinyal Tipi Detayı kutusu
        ax.add_patch(plt.Rectangle((0.02, 0.60), 0.96, 0.335, transform=ax.transAxes,
                     facecolor=CARD_BG, edgecolor="none", zorder=1))
        ax.text(0.04, 0.91, f"Sinyal Tipi Detayı — {len(kapali_tum)} Sinyal, {len(kapanan_tum)} Kapanan",
                fontsize=13, fontweight="bold", color=GOLD, ha="left", va="top")
        basliklar = ["Tip","Genel","Güçlü","Orta","Zayıf"]
        kol_x     = [0.04, 0.27, 0.50, 0.71, 0.94]
        for bx, bl in zip(kol_x, basliklar):
            ax.text(bx, 0.865, bl, fontsize=11, fontweight="bold", color=TEXT_W, ha="left" if bx==0.04 else "right", va="top")
        yy = 0.815
        for row in detay_satirlari:
            go = row["g_tp"]/row["g_n"]*100 if row["g_n"]>0 else 0
            oo = row["o_tp"]/row["o_n"]*100 if row["o_n"]>0 else 0
            zo = row["z_tp"]/row["z_n"]*100 if row["z_n"]>0 else 0
            ax.text(0.04, yy, row["tip"], fontsize=11.5, fontweight="bold", color=TEXT_W, ha="left", va="top")
            ax.text(0.27, yy, f"%{row['genel']:.1f}", fontsize=11.5, color=TEXT_W, ha="right", va="top")
            ax.text(0.50, yy, f"%{go:.1f} ({row['g_tp']}/{row['g_n']})", fontsize=11, color=GREEN, ha="right", va="top")
            ax.text(0.71, yy, f"%{oo:.1f} ({row['o_tp']}/{row['o_n']})", fontsize=11, color=AMBER, ha="right", va="top")
            ax.text(0.94, yy, f"%{zo:.1f} ({row['z_tp']}/{row['z_n']})", fontsize=11, color=RED, ha="right", va="top")
            yy -= 0.055

        # v646: Coin Bazlı — iki sütun (boşalan alanla genişletildi)
        ax.add_patch(plt.Rectangle((0.02, 0.03), 0.96, 0.545, transform=ax.transAxes,
                     facecolor=CARD_BG, edgecolor="none", zorder=1))
        ax.plot([0.50, 0.50], [0.05, 0.575], color=BG, linewidth=1.3, zorder=2)

        def _renk_oran(oran):
            if oran >= 45: return GREEN
            if oran >= 30: return AMBER
            return RED

        # --- SOL: Güçlü Sinyal % TP Oranı ---
        gn_toplam = sum(it.get("guclu_n", 0) for it in coin_sirala_guclu)
        gt_toplam = sum(it.get("guclu_tp", 0) for it in coin_sirala_guclu)
        g_genel   = gt_toplam/gn_toplam*100 if gn_toplam else 0.0
        ax.text(0.04, 0.53, "Güçlü Sinyal % TP Oranı", fontsize=15, fontweight="bold",
                color=GOLD, ha="left", va="top")
        ax.text(0.47, 0.485, f"Genel: %{g_genel:.1f} ({gt_toplam}/{gn_toplam})", fontsize=13.5,
                fontweight="bold", color=_renk_oran(g_genel), ha="right", va="top")
        sol_kol_x = [0.04, 0.20, 0.28, 0.36]
        for bx, bl in zip(sol_kol_x, ["Coin","Sinyal","TP","% Oranı"]):
            ax.text(bx, 0.435, bl, fontsize=12.5, fontweight="bold", color=TEXT_W, ha="left", va="top")
        yy = 0.385
        for it in coin_sirala_guclu:
            gn, gtp = it.get("guclu_n",0), it.get("guclu_tp",0)
            oran = gtp/gn*100 if gn else 0
            ax.text(sol_kol_x[0], yy, it["sym"], fontsize=12.5, color=TEXT_W, ha="left", va="top")
            ax.text(sol_kol_x[1], yy, str(gn), fontsize=12.5, color=TEXT_W, ha="left", va="top")
            ax.text(sol_kol_x[2], yy, str(gtp), fontsize=12.5, color=TEXT_W, ha="left", va="top")
            ax.text(sol_kol_x[3], yy, f"%{oran:.1f}", fontsize=12.5, fontweight="bold",
                    color=_renk_oran(oran), ha="left", va="top")
            yy -= 0.044
        if not coin_sirala_guclu:
            ax.text(sol_kol_x[0], yy, "Az Veri", fontsize=12.5, color=HDR_COL, ha="left", va="top")

        # --- SAĞ: Güven Skoru Yüksek % TP Oranı ---
        vn_toplam = sum(it.get("n", 0) for it in coin_sirala_guven)
        vt_toplam = sum(it.get("tp", 0) for it in coin_sirala_guven)
        v_genel   = vt_toplam/vn_toplam*100 if vn_toplam else 0.0
        ax.text(0.53, 0.53, "Güven Skoru Yüksek % TP Oranı", fontsize=15, fontweight="bold",
                color=GOLD, ha="left", va="top")
        ax.text(0.96, 0.485, f"Genel: %{v_genel:.1f} ({vt_toplam}/{vn_toplam})", fontsize=13.5,
                fontweight="bold", color=_renk_oran(v_genel), ha="right", va="top")
        sag_kol_x = [0.53, 0.68, 0.84, 0.90, 0.95]
        for bx, bl in zip(sag_kol_x, ["Coin","Güven Skoru","Sinyal","TP","% Oranı"]):
            ax.text(bx, 0.435, bl, fontsize=12.5, fontweight="bold", color=TEXT_W, ha="left", va="top")
        yy = 0.385
        for it in coin_sirala_guven:
            ax.text(sag_kol_x[0], yy, it["sym"], fontsize=12.5, color=TEXT_W, ha="left", va="top")
            ax.text(sag_kol_x[1], yy, f"{it['ort_skor']:.2f}", fontsize=12.5, color=TEXT_W, ha="left", va="top")
            ax.text(sag_kol_x[2], yy, str(it["n"]), fontsize=12.5, color=TEXT_W, ha="left", va="top")
            ax.text(sag_kol_x[3], yy, str(it["tp"]), fontsize=12.5, color=TEXT_W, ha="left", va="top")
            ax.text(sag_kol_x[4], yy, f"%{it['oran']:.1f}", fontsize=12.5, fontweight="bold",
                    color=_renk_oran(it["oran"]), ha="left", va="top")
            yy -= 0.044
        if not coin_sirala_guven:
            ax.text(sag_kol_x[0], yy, "Az Veri", fontsize=12.5, color=HDR_COL, ha="left", va="top")
        ...
        print(f"[PANEL_B] Hata: {e}\n{traceback.format_exc()}")
        return None


def _panel_d_gorsel(snapshot=None):
    """v635: PANEL D — Parite Detayı. Her coin için tek satırda:
    Sinyal Adedi (+alt tip), TP Oranı (+alt tip), Sinyal Gücü (Genel/Güçlü/Orta/Zayıf),
    Piyasa Rejimi (sinyal sayısı + TP oranı). Kaynak: gunluk_sinyaller (sinyaller.json)
    çünkü filtre_veri.json'da sembol bilgisi yok."""
    try:
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as plt
    ...
    try:
        if snapshot is not None:
            ...
            with gunluk_kilit:
                kayitlar = list(gunluk_sinyaller)

        _SINYAL_NORM_D = {"LONG":"Long","SHORT":"Short","STRONG BUY":"Strong Buy","STRONG SELL":"Strong Sell"}
        TF_MAP_D = {"1":"1DK","5":"5DK","15":"15DK","60":"1SA","240":"4SA"}
        ESIK_G, ESIK_Z, MIN_COMBO = 8.0, 5.0, 3
        SINYAL_TIPLERI = ["Strong Sell","Strong Buy","Long","Short"]

        for s in kayitlar:
            s["_tip_norm"] = _SINYAL_NORM_D.get(str(s.get("sinyal","")).upper(), s.get("sinyal"))

        kapali_tum = [s for s in kayitlar if s.get("tp1_ok") is not None]

        def basarili(s):
            return any(s.get(k) is True for k in ["tp1_ok","tp2_ok","tp3_ok","tp4_ok","tp5_ok"])

        # combo_tab: ayni mantik, Panel B ile tutarli (TF+Funding+OI+Piyasa)
        genel_tp, combo_tab = {}, {}
        for st in SINYAL_TIPLERI:
            grp = [s for s in kapali_tum if s.get("_tip_norm") == st]
            if not grp:
                genel_tp[st] = 0.0; combo_tab[st] = {}
                continue
            genel_tp[st] = sum(1 for s in grp if basarili(s)) / len(grp) * 100
            tablo = {}
            for s in grp:
                tf  = TF_MAP_D.get(str(s.get("timeframe","")), str(s.get("timeframe","")))
                key = f"{tf}+{s.get('funding_yonu') or ''}+{s.get('oi_yonu') or ''}+{s.get('piyasa_yonu') or ''}"
                tablo.setdefault(key, {"tp":0,"n":0})
                tablo[key]["n"] += 1
                if basarili(s): tablo[key]["tp"] += 1
            combo_tab[st] = {k:v for k,v in tablo.items() if v["n"] >= MIN_COMBO}

        def etiket_bul(s):
            st = s.get("_tip_norm")
            g_ort = genel_tp.get(st)
            if g_ort is None: return "yetersiz"
            tf  = TF_MAP_D.get(str(s.get("timeframe","")), str(s.get("timeframe","")))
            key = f"{tf}+{s.get('funding_yonu') or ''}+{s.get('oi_yonu') or ''}+{s.get('piyasa_yonu') or ''}"
            tablo = combo_tab.get(st, {})
            if key not in tablo: return "yetersiz"
            cr = tablo[key]["tp"]/tablo[key]["n"]*100
            if cr >= g_ort+ESIK_G: return "guclu"
            elif cr >= g_ort-ESIK_Z: return "orta"
            else: return "zayif"
        ...
        for s in kayitlar:
            ...
            coinler.setdefault(sym, []).append(s)

        def oran_str(tp, n):
            return f"%{tp/n*100:.0f} ({tp}/{n})" if n else "—"

        satirlar = []
        for sym, sigs in sorted(coinler.items(), key=lambda kv: -len(kv[1])):
            toplam = len(sigs)
            tip_n = {t: sum(1 for s in sigs if s.get("_tip_norm")==t) for t in SINYAL_TIPLERI}

            kapali = [s for s in sigs if s.get("tp1_ok") is not None]
            tp_n_genel = sum(1 for s in kapali if basarili(s))
            tp_tip = {}
            for t in SINYAL_TIPLERI:
                k_t = [s for s in kapali if s.get("_tip_norm")==t]
                tp_tip[t] = (sum(1 for s in k_t if basarili(s)), len(k_t))

            g_tp=g_n=o_tp=o_n=z_tp=z_n=y_tp=y_n = 0
            for s in kapali:
                et = etiket_bul(s)
                ok = basarili(s)
                if et=="guclu": g_n+=1; g_tp+= 1 if ok else 0
                elif et=="orta": o_n+=1; o_tp+= 1 if ok else 0
                elif et=="zayif": z_n+=1; z_tp+= 1 if ok else 0
                else: y_n+=1; y_tp+= 1 if ok else 0
            siniflanan_n = g_n+o_n+z_n
            siniflanan_tp = g_tp+o_tp+z_tp

            piy_grp = {}
            for s in kapali:
                piy = s.get("piyasa_yonu") or "Bilinmiyor"
                piy_grp.setdefault(piy, {"tp":0,"n":0})
                piy_grp[piy]["n"] += 1
                if basarili(s): piy_grp[piy]["tp"] += 1

            satirlar.append({
                "sym": sym, "toplam": toplam, "tip_n": tip_n,
                "kapali_n": len(kapali), "tp_n_genel": tp_n_genel, "tp_tip": tp_tip,
                "siniflanan_n": siniflanan_n, "siniflanan_tp": siniflanan_tp,
                "g_tp":g_tp,"g_n":g_n, "o_tp":o_tp,"o_n":o_n, "z_tp":z_tp,"z_n":z_n,
                "piy": piy_grp,
            })

        # ------- Gorsel cizimi (v636: TRANSPOZE — coinler sutun, metrikler satir) -------
        n_coin = len(satirlar)
        label_w = 0.155
        col_w   = (0.98 - label_w) / max(n_coin, 1)
        coin_x  = [label_w + col_w*(i+0.5) for i in range(n_coin)]

        satir_gruplari = [
            ("SİNYAL ADEDİ", [
                ("Toplam",   lambda r: (str(r["toplam"]), TEXT_W)),
                ("Long",     lambda r: (str(r["tip_n"]["Long"]), TEXT_W)),
                ("Short",    lambda r: (str(r["tip_n"]["Short"]), TEXT_W)),
                ("St. Sell", lambda r: (str(r["tip_n"]["Strong Sell"]), TEXT_W)),
                ("St. Buy",  lambda r: (str(r["tip_n"]["Strong Buy"]), TEXT_W)),
            ]),
            ("TP ORANI", [
                ("Genel",    lambda r: (oran_str(r["tp_n_genel"], r["kapali_n"]),
                                         GREEN if (r["tp_n_genel"]/r["kapali_n"]*100 if r["kapali_n"] else 0)>=60
                                         else (AMBER if (r["tp_n_genel"]/r["kapali_n"]*100 if r["kapali_n"] else 0)>=35 else RED))),
                ("Long",     lambda r: (oran_str(*r["tp_tip"]["Long"]), TEXT_W)),
                ("Short",    lambda r: (oran_str(*r["tp_tip"]["Short"]), TEXT_W)),
                ("St. Sell", lambda r: (oran_str(*r["tp_tip"]["Strong Sell"]), TEXT_W)),
                ("St. Buy",  lambda r: (oran_str(*r["tp_tip"]["Strong Buy"]), TEXT_W)),
            ]),
            ("SİNYAL GÜCÜ", [
                ("Genel",  lambda r: (oran_str(r["siniflanan_tp"], r["siniflanan_n"]),
                                       GREEN if (r["siniflanan_tp"]/r["siniflanan_n"]*100 if r["siniflanan_n"] else 0)>=60
                                       else (AMBER if (r["siniflanan_tp"]/r["siniflanan_n"]*100 if r["siniflanan_n"] else 0)>=35 else RED))),
                ("Güçlü",  lambda r: (oran_str(r["g_tp"], r["g_n"]), GREEN)),
                ("Orta",   lambda r: (oran_str(r["o_tp"], r["o_n"]), AMBER)),
                ("Zayıf",  lambda r: (oran_str(r["z_tp"], r["z_n"]), RED)),
            ]),
            ("PİYASA REJİMİ (TP oranı — n = kapanan sinyal)", [
                ("Yükseliş", lambda r: _piy_hucre(r, "Yükseliş")),
                ("Düşüş",    lambda r: _piy_hucre(r, "Düşüş")),
                ("Yatay",    lambda r: _piy_hucre(r, "Yatay")),
            ]),
        ]

        def _piy_hucre(r, piy):
            v = r["piy"].get(piy, {"tp":0,"n":0})
            if v["n"]:
                # v636 DUZELTME: "N · %oran (a/b)" formati uzun sinyal sayilarinda
                # (orn. 327) komsu sutuna tasip metin cakismasina sebep oluyordu.
                # "N" zaten kesirdeki payda ile ayni sayi (v['n'] == b), bu yuzden
                # fazladan basiliyordu — kaldirildi, sadece oran_str yeterli.
                return (oran_str(v['tp'], v['n']), TEXT_W)
            return ("—", HDR_COL)

        toplam_metrik_satir = sum(len(g[1]) for g in satir_gruplari)
        n_grup = len(satir_gruplari)
        satir_h = 0.052
        grup_bosluk = 0.022

        fig_w = max(11.0, label_w*11 + n_coin * 1.7)
        fig_h = 2.3 + (toplam_metrik_satir + n_grup*1.6) * 0.42

        fig, ax = plt.subplots(figsize=(fig_w, fig_h))
        ax.axis("off"); ax.set_xlim(0, 1); ax.set_ylim(0, 1)
        fig.patch.set_facecolor(BG)

        ax.text(0.5, 0.985, "Ben Kül Yutmam — Parite Detayı", fontsize=20, fontweight="bold",
                color=GOLD, ha="center", va="top")
        ax.text(0.5, 0.945, f"İzlenen {n_coin} Parite · Panel D / 4", fontsize=12, color=HDR_COL, ha="center", va="top")

        # Coin basliklari (ust satir)
        coin_baslik_y = 0.885
        for x, r in zip(coin_x, satirlar):
            ax.text(x, coin_baslik_y, r["sym"], fontsize=13, fontweight="bold", color=TEXT_W, ha="center", va="top")
        ax.plot([0.0, 1.0], [coin_baslik_y-0.028, coin_baslik_y-0.028], color="#3A3F4A", linewidth=1.2, transform=ax.transAxes)

        yy = coin_baslik_y - 0.055
        for gi, (grup_adi, satirlar_metrik) in enumerate(satir_gruplari):
            # Grup basligi
            ax.text(0.005, yy, grup_adi, fontsize=11.5, fontweight="bold", color=GOLD, ha="left", va="top")
            yy -= satir_h * 0.92
            for si, (etiket, fn) in enumerate(satirlar_metrik):
                if si % 2 == 0:
                    ax.add_patch(plt.Rectangle((0.0, yy-satir_h+0.008), 1.0, satir_h,
                                 transform=ax.transAxes, facecolor=CARD_BG, edgecolor="none", zorder=0))
                ax.text(0.02, yy, etiket, fontsize=10, color=TEXT_W, ha="left", va="top", fontweight="bold")
                for x, r in zip(coin_x, satirlar):
                    deger, renk = fn(r)
                    ax.text(x, yy, deger, fontsize=9.5, color=renk, ha="center", va="top", fontweight="bold")
                yy -= satir_h
            # Grup sonu ayrac cizgisi
            yy -= grup_bosluk * 0.3
            ax.plot([0.0, 1.0], [yy, yy], color="#2A2E38", linewidth=1.0, transform=ax.transAxes)
            yy -= grup_bosluk
        ...
        print(f"[PANEL_D] Hata: {e}\n{traceback.format_exc()}")
        return None


def _istatistik_zamanlayici():
    """v632: Her 30 dakikada bir (XX:00 ve XX:30) 3 paneli SIRAYLA gönderir:
    Panel A → 5sn bekle → Panel B → 5sn bekle → Panel C → 5sn bekle.
    Eskiden Panel A/C bu zamanlayıcıda, Panel B ise ayrı bir zamanlayıcıda
    (_filtre_zamanlayici, XX:05/XX:35) farklı saatlerde gönderiliyordu —
    bu karışıklığı gidermek için hepsi TEK zamanlayıcıya, TEK sıraya alındı.
    """
    ...
    while True:
        try:
            ...
            time.sleep(bekle)
            bugun = gun_str()
            # Aynı anda snapshot al — üç panel de aynı veriyi kullansın
            with gunluk_kilit:
                snapshot = list(gunluk_sinyaller)

            # Whitelist'i baştan bir kez güncelle (üç panel de bunu kullanabilir)
            try:
                top3_guncelle(snapshot)
            except Exception as _wle:
                print(f"[PANEL] top3_guncelle hatası: {_wle}")

            # 1) PANEL A — Genel Durum
            try:
                img_a = _panel_a_gorsel(snapshot=snapshot)
                if img_a:
                    _topic_foto_gonder_filigranli(TOPIC_RAPOR, img_a, "📋 Genel Durum")
                    print("[PANEL_A] Gönderildi.")
                else:
                    print("[PANEL_A] Üretilemedi (None) — eski metin yedek olarak gönderiliyor.")
                    _telegram_topic_mesaj_gonder(TOPIC_RAPOR, istatistik_mesaji(snapshot=snapshot))
            except Exception as _ea:
                print(f"[PANEL_A] Hata: {_ea}\n{traceback.format_exc()}")
            time.sleep(5)

            # 2) PANEL B — Sinyal Kalitesi
            try:
                img_b = _panel_b_gorsel(snapshot=snapshot)
                if img_b:
                    _topic_foto_gonder_filigranli(TOPIC_RAPOR, img_b, "📊 Sinyal Kalitesi")
                    print("[PANEL_B] Gönderildi.")
                else:
                    print("[PANEL_B] Üretilemedi (None) — eski görseller yedek olarak gönderiliyor.")
                    _filtre_gorseller_gonder()
            except Exception as _eb:
                print(f"[PANEL_B] Hata: {_eb}\n{traceback.format_exc()}")
            time.sleep(5)

            # 3) PANEL C — v647'de YENİDEN AKTİF: artık SADECE TOPIC_ALARM'a
            # giden sinyallerin performansını ölçüyor (Panel A ile aynı filtre).
            try:
                alarm_snapshot_c = [s for s in snapshot if _alarm_gonderilen(s)]
                img_c = rapor_gorsel(bugun, snapshot=alarm_snapshot_c)
                if img_c:
                    _topic_foto_gonder_filigranli(TOPIC_RAPOR, img_c,
                        f"Günlük Rapor — {bugun} (Sadece TOPIC_ALARM sinyalleri)\n/rapor")
                    print("[PANEL_C] Gönderildi.")
                else:
                    print(f"[PANEL_C] Üretilemedi (None) — '{bugun}' için alarm sinyali bulunamadı olabilir.")
            except Exception as _ec:
                print(f"[PANEL_C] Hata: {_ec}\n{traceback.format_exc()}")
            time.sleep(5)

            # 4) PANEL D — v645'te DEVRE DIŞI (kullanıcı istegiyle). Veri kaydı
            # (sinyaller.json) etkilenmiyor, sadece TOPIC_RAPOR'a gönderim durdu.
            # print("[PANEL_D] Devre disi (v645).")
        except Exception as e:
...
def _ls_yorum(btc_ls, eth_ls):
    ...
    v641 DUZELTME: Yorum artik /analiz'in hesapladigi genel piyasa yonuyle
    (market_yonu) CAPRAZ KONTROL ediliyor. Onceden "KY agirlikli LONG" her
    zaman "Yukselis beklentisi" olarak etiketleniyordu — piyasa dususte olsa
    bile. Ama yogun long pozisyonlanma + dusus trendi klasik bir "squeeze"
    (zorunlu long tasfiyesi) riskidir, yukselis teyidi degil. Ayni sekilde
    "Balina ve KY ayni yonde" satiri da artik gercek piyasa yonuyle tutarli
    olup olmadigina bakiyor.
    """
    satirlar = ["📊 <b>Long/Short Oran Yorumu</b>\n"]

    _global_ls = _son_analiz_veriler.get("_GLOBAL", {}) if _son_analiz_veriler else {}
    _market_yonu_ls = _global_ls.get("market_yonu")
    dususte      = _market_yonu_ls in ("Aşağı Yönlü", "Hafif Aşağı Yönlü")
    yukselişte   = _market_yonu_ls in ("Yukarı Yönlü", "Hafif Yukarı Yönlü")
    ...
    for sym, ls_data in [("BTC", btc_ls), ("ETH", eth_ls)]:
        ...
        if bal_long and bal_short:
            ...
            if bal_yon_long == ky_yon_long:
                if bal_yon_long and dususte:
                    satirlar.append("⚠️ Balina ve KY ikisi de LONG ama piyasa düşüş trendinde — teyit değil, squeeze riski")
                elif (not bal_yon_long) and yukselişte:
                    satirlar.append("⚠️ Balina ve KY ikisi de SHORT ama piyasa yükseliş trendinde — teyit değil, short squeeze riski")
                else:
                    satirlar.append("✅ Balina ve KY aynı yönde — Trend teyit ediliyor")
            else:
                satirlar.append("🔶 Balina ve KY farklı yönde — Dikkatli ol")

        # Ana yorum: mutlak oran + değişim yönü + GENEL PIYASA YONU birlikte degerlendir
        if degisim > 2 and ky_long > 55:
            yorum = "🟢 Long oranı artıyor — Yükseliş baskısı güçleniyor"
            if dususte:
                yorum = "⚠️ Long oranı artıyor ama piyasa düşüşte — squeeze riski birikiyor"
            ...
            if yukselişte:
                yorum = "🔴 KY ağırlıklı SHORT + piyasa yükselişte — short squeeze riski çok yüksek"
        elif ky_long > 70:
            yorum = "🟡 KY çok fazla LONG — Aşırı pozisyonlanma, dikkatli ol"
            if dususte:
                yorum = "🔴 KY çok fazla LONG + piyasa düşüşte — long squeeze riski çok yüksek"
        elif ky_long > 60:
            if dususte:
                yorum = "⚠️ KY ağırlıklı LONG ama piyasa düşüş trendinde — yükseliş teyidi SAYMA, squeeze riski"
            else:
...
def _ls_gonder(chat_id=None):
    ...
    try:
        ...
        # v651: guncel fiyat bilgisi eklendi
        try:
            _btc_fiyat = get_mexc_price("BTCUSDT")
            _eth_fiyat = get_mexc_price("ETHUSDT")
        except Exception as _pe:
            print(f"[LS] Fiyat cekme hatasi: {_pe}")
            _btc_fiyat = _eth_fiyat = None
        _fiyat_notu = ""
        if _btc_fiyat or _eth_fiyat:
            parcalar = []
            if _btc_fiyat: parcalar.append(f"BTC ${fmt_fiyat(_btc_fiyat)}")
            if _eth_fiyat: parcalar.append(f"ETH ${fmt_fiyat(_eth_fiyat)}")
            _fiyat_notu = "  ·  " + " · ".join(parcalar)

        img = _ls_gorsel(btc_ls, eth_ls, zaman_str)
        if img:
            _topic_foto_gonder_filigranli(TOPIC_ANALIZ, img, f"📈 Long/Short Oran — 1S — {zaman_str}{_fiyat_notu}\n/ls")
            print("[LS] Gorsel gonderildi.")

        yorum = _ls_yorum(btc_ls, eth_ls)
        _telegram_topic_mesaj_gonder(TOPIC_ANALIZ, yorum + "\n\n<code>/ls</code>")

        # L/S verisini global cache'e yaz — sinyal kaydında kullanılır
        global _son_ls_cache
        for sym_k, ls_data in [("BTC", btc_ls), ("ETH", eth_ls)]:
            genel  = ls_data.get("genel", [])
            balina = ls_data.get("balina", [])
            if genel:
                try:
                    _son_ls_cache[sym_k] = {
                        "ky_long":    round(float(genel[-1]["longAccount"])  * 100, 1),
                        "ky_short":   round(float(genel[-1]["shortAccount"]) * 100, 1),
                        "bal_long":   round(float(balina[-1]["longAccount"])  * 100, 1) if balina else None,
                        "bal_short":  round(float(balina[-1]["shortAccount"]) * 100, 1) if balina else None,
                    }
                except Exception:
                    pass
        print("[LS] Tamamlandi.")
...
_son_ls_cache       = {}   # L/S oran cache — sinyal kaydında kullanılır {"BTC": {"ky_long": 67.8, "ky_short": 32.2}, ...}
...
def _analiz_ozet_telegram(coin_verileri, veriler, zaman_str):
    ...
    try:
        ...
        # Breadth — max +2 (önceki +3 tek başına Yukarı Yönlü tetikliyordu)
        if breadth >= 8:      puan += 2
        elif breadth >= 6:    puan += 1
        elif breadth >= 4:    puan += 0
        ...
        # Eşik: 5+ Yukarı, 3-4 Hafif Yukarı (önceki 4+ Yukarı çok kolay tetikleniyordu)
        if puan >= 5:    yon_ikon, yon_metin = "📈", "Yukarı Yönlü"
        elif puan >= 3:  yon_ikon, yon_metin = "🟡", "Hafif Yukarı Yönlü"
        ...
        if uyumsuz_asagi or uyumsuz_yukari or uyumlu_asagi or btc_cvd_d:
            ...
            if birikim_listesi:
                ...
                if pos_cvd:
                    parcalar.append(f"<b>{', '.join(pos_cvd)}</b> fiyat düşüyor ama CVD yükseliyor — birikim sinyali, dip takip edilebilir")
                if neg_cvd:
                    parcalar.append(f"<b>{', '.join(neg_cvd)}</b> CVD negatif bölgede yukarı dönüyor — satış baskısı sürüyor, henüz teyit yok")
            if satis_listesi:
            ...
            if btc_cvd_d:
                ...
                if btc_birikim and cvd_val < 0:
                    parcalar.append("BTC fiyat düşerken CVD yükseliyor — satış baskısı sürüyor ancak kısa vadeli alım hareketi var")
                elif btc_birikim and cvd_val >= 0:
                    parcalar.append("BTC birikim sinyali — fiyat düşüyor, CVD pozitif ve yükseliyor")
                elif cvd_val > 1000:
        ...
        # ── 3) FUNDING ──
        fund_pct = btc_fund  # zaten yüzde cinsinden geliyor (örn: -0.0061%)
        if abs(btc_fund) >= 0.02:
            fund_yorum = f"Long kalabalık, düzeltme riski ({fund_pct:+.4f}%)." if btc_fund > 0 else f"Short kalabalık, squeeze riski ({fund_pct:+.4f}%)."
        elif abs(btc_fund) >= 0.005:
            fund_yorum = f"Long baskın ({fund_pct:+.4f}%)." if btc_fund > 0 else f"Short baskın ({fund_pct:+.4f}%)."
        elif abs(btc_fund) >= 0.001:
            fund_yorum = f"Hafif long eğilimi ({fund_pct:+.4f}%)." if btc_fund > 0 else f"Hafif short eğilimi ({fund_pct:+.4f}%)."
        else:
            fund_yorum = f"Dengeli ({fund_pct:+.4f}%)."
...
def _analiz_gonder(chat_id=None):
    ...
    try:
        ...
        # _GLOBAL anahtarını veriler'den SONRA yaz — yoksa üzerine yazılır
        # Değerleri veriler'den çek — scope sorunu yaşanmaz
        try:
            _btc_veri   = veriler.get("BTCUSDT", {})
            _btc_fund_g = _btc_veri.get("funding", 0) or 0
            _skorlar_g  = [v.get("skor", 0) for v in veriler.values() if isinstance(v, dict) and v.get("skor") is not None]
            _ort_skor_g = round(sum(_skorlar_g) / len(_skorlar_g)) if _skorlar_g else 0
            _fg_d_g, _  = _trend_fear_greed()
            _breadth_g  = veriler.get("_breadth", 0)
            # Piyasa Yonu — Market Yonu ile ayni kapsamli hesap
            # Skor + Breadth + OI + CVD + Funding + Likidasyon + Taker
            _puan_g = 0
            # Skor
            if _ort_skor_g >= 65:   _puan_g += 2
            elif _ort_skor_g >= 50: _puan_g += 1
            elif _ort_skor_g < 40:  _puan_g -= 2
            # Breadth (max +2)
            if _breadth_g >= 8:     _puan_g += 2
            elif _breadth_g >= 6:   _puan_g += 1
            elif _breadth_g <= 2:   _puan_g -= 3
            elif _breadth_g <= 4:   _puan_g -= 2
            # Taker
            _taker_long_g  = sum(1 for v in veriler.values()
                                 if isinstance(v, dict) and isinstance(v.get("taker"), dict)
                                 and v["taker"].get("buy_pct", 0) >= 55)
            _taker_short_g = sum(1 for v in veriler.values()
                                 if isinstance(v, dict) and isinstance(v.get("taker"), dict)
                                 and v["taker"].get("buy_pct", 0) < 45)
            if _taker_long_g >= 6:  _puan_g += 2
            elif _taker_short_g >= 6: _puan_g -= 2
            # Likidasyon
            _long_lik_g = sum((v.get("likidasyonlar") or {}).get("long_usd", 0)
                              for v in veriler.values() if isinstance(v, dict))
            _short_lik_g = sum((v.get("likidasyonlar") or {}).get("short_usd", 0)
                               for v in veriler.values() if isinstance(v, dict))
            _toplam_lik_g = _long_lik_g + _short_lik_g
            if _short_lik_g > _long_lik_g * 2 and _toplam_lik_g >= 1.0: _puan_g += 1
            elif _long_lik_g > _short_lik_g * 2 and _toplam_lik_g >= 1.0: _puan_g -= 1
            # CVD
            _uyumsuz_asagi_g = sum(1 for v in veriler.values()
                                   if isinstance(v, dict) and isinstance(v.get("cvd"), dict)
                                   and v["cvd"].get("uyum") == "uyumsuz_asagi")
            _uyumsuz_yukari_g = sum(1 for v in veriler.values()
                                    if isinstance(v, dict) and isinstance(v.get("cvd"), dict)
                                    and v["cvd"].get("uyum") == "uyumsuz_yukari"
                                    and v["cvd"].get("cvd", 0) < 0)
            if _uyumsuz_yukari_g >= 4: _puan_g -= 1
            if _uyumsuz_asagi_g >= 3:  _puan_g += 1
            # Funding
            if _btc_fund_g > 0.02:    _puan_g -= 1
            elif _btc_fund_g < -0.01: _puan_g += 1
            # BTC CVD
            _btc_cvd_g = (_btc_veri.get("cvd") or {}).get("cvd", 0)
            if _btc_cvd_g < -3000: _puan_g -= 1
            # Yön kararı (Market Yonu ile ayni esikler)
            # Market Yonu (5 kategori — analiz gorseli ile ayni)
            if _puan_g >= 5:    _market_yonu_g = "Yukarı Yönlü"
            elif _puan_g >= 3:  _market_yonu_g = "Hafif Yukarı Yönlü"
            elif _puan_g <= -4: _market_yonu_g = "Aşağı Yönlü"
            elif _puan_g <= -2: _market_yonu_g = "Hafif Aşağı Yönlü"
            else:               _market_yonu_g = "Yatay / Belirsiz"
            # Piyasa Yonu (3 kategori — tplog kombinasyonlari icin)
            if _puan_g >= 3:    _piyasa_yonu_g = "Yükseliş"
            elif _puan_g <= -2: _piyasa_yonu_g = "Düşüş"
            else:               _piyasa_yonu_g = "Yatay"
            _son_analiz_veriler["_GLOBAL"] = {
                "fg_deger":     _fg_d_g,
                "btc_fund":     _btc_fund_g,
                "ort_skor":     _ort_skor_g,
                "piyasa_yonu":  _piyasa_yonu_g,
                "market_yonu":  _market_yonu_g,
                "zaman":        time.time(),
            }
            print(f"[ANALIZ] _GLOBAL cache guncellendi: fg={_fg_d_g} piyasa={_piyasa_yonu_g} market={_market_yonu_g} fund={_btc_fund_g}")
        except Exception as _e:
            print(f"[ANALIZ] _GLOBAL cache hatasi: {_e}")
...
threading.Thread(target=_filtre_zamanlayici, daemon=True).start()  # Filtre görselleri — 30 dk
