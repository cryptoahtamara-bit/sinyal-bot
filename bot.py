...
def mexc_private_post(url, body, timeout=15):
    """Private POST — body_json imzasi + json=body + browser headers (WAF bypass C yontemi)"""
    ts = str(int(time.time() * 1000))
    sorted_body = dict(sorted(body.items()))
    body_json = json.dumps(sorted_body, separators=(",", ":"), sort_keys=True, ensure_ascii=False)
    ...
    print(f"[MEXC IMZA] body_json: {body_json}")
...
def mexc_test():
    ...
    sonuc = {"version": "v203", "proxy": bool(MEXC_PRIVATE_PROXY)}
...
print(f"[BASLANGIC] ========== BOT VERSIYON: v203 ==========")
