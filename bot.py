...
# Degisiklikler (v540 -> v541):
#   1. _tp_kosul_yorum_uret: çift ayıraç sorunu düzeltildi
#      - Piyasa koşulları bölümü piyasa_maddeler geçici listesine yazar
#      - Sadece madde eklendiyse AYIRAC koyar
#      - Hiç madde yoksa "Anlamlı fark bulunamadı" mesajı gösterir
# bot_v540 — 16 Haziran 2026
...
def _tp_kosul_yorum_uret(veri):
    ...
    if confluance:
        ...
        maddeler.append("🔮 <b>En İyi TP Koşulu (Confluance Analizi):</b>")
        maddeler.append(
            f"  🥇 <b>{confluance['en_iyi_kosul']}</b>\n"
            f"     TP Oranı: {confluance['tp_orani']} | "
            f"Sinyal: {confluance['sinyal']} | "
            f"Güven: {confluance['guven']}"
        )
        maddeler.append(
            f"  {yon_emoji} <b>Tavsiye Yön: {confluance.get('tavsiye_yon','NÖTR')}</b> "
            f"— {confluance.get('tavsiye_guven','?')} "
            f"(Long ağırlık: {confluance.get('long_agirlik',0)} / "
            f"Short ağırlık: {confluance.get('short_agirlik',0)})"
        )
        if confluance.get("bilesenler"):
            bil = confluance["bilesenler"]
            bil_str = "  📌 Etkiler: " + " | ".join(
                f"{k.replace('_etkisi','').capitalize()}: {v}"
                for k, v in bil.items()
            )
            maddeler.append(bil_str)
        if len(confluance.get("top5", [])) > 1:
            maddeler.append("  📋 Diğer güçlü koşullar:")
            for item in confluance["top5"][1:]:
                maddeler.append(
                    f"    • <b>{item['kombinasyon']}</b> → {item['tp_orani']} TP "
                    f"({item['sinyal']} sinyal) {item['guven']}"
                )
        maddeler.append(
            f"  📊 Toplam {confluance['toplam_kombinasyon']} kombinasyon analiz edildi"
        )
    else:
