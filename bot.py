...
def mexc_private_post(url, body, timeout=15):
    """Private POST — JSON imzasi + query string gonderim (WAF bypass)"""
    hdrs, ts = mexc_headers(body)  # imza JSON body uzerinden
...
def mexc_test():
    ...
    sonuc = {"version": "v197", "proxy": bool(MEXC_PRIVATE_PROXY)}
    ...
    # 4. Imza debug — JSON imzasi ile test
    if MEXC_API_KEY and MEXC_API_SECRET:
        ...
        body_json = json.dumps(sorted_body, separators=(",", ":"), ensure_ascii=False, sort_keys=True)
        sign_str = MEXC_API_KEY + ts_dbg + body_json
        ...
        # Gercek istek — JSON imzasi + query string gonderim
        try:
            hdrs_dbg = {
                "ApiKey": MEXC_API_KEY,
                "Request-Time": ts_dbg,
            ...
            r_dbg = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
                                  headers=hdrs_dbg, json={}, params=sorted_body, timeout=8)
...
print(f"[BASLANGIC] ========== BOT VERSIYON: v197 ==========")
