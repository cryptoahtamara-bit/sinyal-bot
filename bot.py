...
def _panel_a_gorsel(snapshot=None):
    ...
    try:
        ...
        t = istatistik_hesapla(snapshot=alarm_snapshot)
        ...
        en_sym_t, en_tf_t = _en_iyi_hesapla(alarm_snapshot, 50)
...
def _panel_b_gorsel(snapshot=None):
    ...
    try:
        ...
        ax.add_patch(plt.Rectangle((0.02, 0.68), 0.96, 0.255, transform=ax.transAxes,
                     facecolor=CARD_BG, edgecolor="none", zorder=1))
        ax.text(0.04, 0.912, f"Sinyal Tipi Detayı — {len(kapali_tum)} Sinyal, {len(kapanan_tum)} Kapanan",
        ...
        ax.add_patch(plt.Rectangle((0.02, 0.03), 0.96, 0.62, transform=ax.transAxes,
                     facecolor=CARD_BG, edgecolor="none", zorder=1))
        ax.plot([0.50, 0.50], [0.05, 0.62], color=BG, linewidth=1.3, zorder=2)
        ...
        ax.text(0.04, 0.62, "Güçlü Sinyal % TP Oranı", fontsize=15, fontweight="bold",
                color=GOLD, ha="left", va="top")
        ax.text(0.47, 0.575, f"Genel: %{g_genel:.1f} ({gt_toplam}/{gn_toplam})", fontsize=13.5,
        ...
        for bx, bl in zip(sol_kol_x, ["Coin","Sinyal","TP","% Oranı"]):
            ax.text(bx, 0.525, bl, fontsize=12.5, fontweight="bold", color=TEXT_W, ha="left", va="top")
        yy = 0.475
        ...
        ax.text(0.53, 0.62, "Güven Skoru Yüksek % TP Oranı", fontsize=15, fontweight="bold",
                color=GOLD, ha="left", va="top")
        ax.text(0.96, 0.575, f"Genel: %{v_genel:.1f} ({vt_toplam}/{vn_toplam})", fontsize=13.5,
                fontweight="bold", color=_renk_oran(v_genel), ha="right", va="top")
        sag_kol_x = [0.53, 0.66, 0.78, 0.85, 0.91]
