    ...
    try:
        ...
        for i, (lbl, val, col, sub) in enumerate(kartlar):
            ...
            ax.text(kx + kart_w/2, y - 0.14, lbl,
                    ha="center", va="top", fontsize=7, color=DGRAY, fontweight="bold")
            ax.text(kx + kart_w/2, y - 0.44, val,
                    ha="center", va="top", fontsize=14, color=col, fontweight="bold")
            ax.text(kx + kart_w/2, y - 0.74, sub,
        ...
        for cx, lbl, ha in cols:
            ax.text(cx, y, lbl, ha=ha, va="top", fontsize=7.5, color=DGRAY, fontweight="bold")
        y -= 0.12
        ...
        for i, sym in enumerate(SEMBOLLER):
            ...
            # Fiyat
            ax.text(1.28, my, fmt_fiyat(veri.get("fiyat")), ha="right", va="center",
                    fontsize=8.5, color=GRAY)

            # 24s%
            d24 = veri.get("degisim_24h")
            ax.text(2.10, my, f"{d24:+.1f}%" if d24 is not None else "—",
                    ha="right", va="center", fontsize=9, color=renk_pct(d24))
            ...
            if skor is not None:
                ...
                ax.add_patch(FancyBboxPatch(
                    (2.52, my - 0.13), 0.74, 0.26,
                    boxstyle="round,pad=0.03", linewidth=0.5,
                    edgecolor=s_col, facecolor=BG))
                ax.text(2.89, my, str(skor), ha="center", va="center",
                        fontsize=9.5, color=s_col, fontweight="bold")

            # RSI
            rsi = veri.get("rsi")
            r_col = ORANGE if rsi and rsi >= 70 else (ORANGE if rsi and rsi <= 30 else GRAY)
            ax.text(3.76, my, f"{rsi:.1f}" if rsi else "—",
                    ha="right", va="center", fontsize=9, color=r_col)

            # Funding
            fund = veri.get("funding", 0)
            ax.text(4.56, my, f"{fund:+.4f}" if fund != 0 else "0",
            ...
            if oi_d:
                dp = oi_d["delta_pct"]
                oi_str = f"{dp:+.1f}%"
                ax.text(5.56, my, oi_str, ha="right", va="center", fontsize=8.5,
                        color=renk_pct(dp))
            else:
                ax.text(5.56, my, "—", ha="right", va="center", fontsize=8.5, color=GRAY)
            ...
            if taker:
                ...
                ax.text(6.56, my, f"{bp:.0f}%B", ha="right", va="center",
                        fontsize=8.5, color=t_col)
            else:
                ax.text(6.56, my, "—", ha="right", va="center", fontsize=8.5, color=GRAY)
            ...
            if cvd:
                ...
                ax.text(7.56, my, cvd_str, ha="right", va="center",
                        fontsize=8.5, color=c_col)
            else:
                ax.text(7.56, my, "—", ha="right", va="center", fontsize=8.5, color=GRAY)

            # Likidasyon
            lik = veri.get("likidasyonlar")
            if lik and lik["toplam_usd"] > 0:
                l_col = RED if lik["agirlik"] == "long_baskin" else (GREEN if lik["agirlik"] == "short_baskin" else GRAY)
                ax.text(9.10, my, f"${lik['toplam_usd']:.1f}M", ha="right", va="center",
                        fontsize=8.5, color=l_col)
            else:
                ax.text(9.10, my, "—", ha="right", va="center", fontsize=8.5, color=DGRAY)

            # Hacim çarpanı
            hc = veri.get("hac_carpan", 0)
            hc_col = ORANGE if hc > 5 else (GRAY)
            hc_str = f"{hc:.1f}x⚠" if hc > 5 else (f"{hc:.1f}x" if hc > 0 else "—")
            ax.text(10.50, my, hc_str, ha="right", va="center",
                    fontsize=8.5, color=hc_col)
        ...
        for pi, sym in enumerate(["BTCUSDT", "ETHUSDT", "SOLUSDT"]):
            ...
            if nota:
                ax.text(px + panel_w/2, y - 0.30, nota[:55],
                        ha="center", va="top", fontsize=7, color=ORANGE)

            ax.axhline(y - 0.38, xmin=px/fig_w + 0.01, xmax=(px+panel_w)/fig_w - 0.01,
            ...
            def satir(label, val1, val2=None, renk=WHITE, indent=0):
                nonlocal ly
                lx = px + 0.16 + indent
                ax.text(lx, ly, label, ha="left", va="top", fontsize=8, color=DGRAY)
                vstr = fmt_fiyat(val1)
                if val2:
                    vstr += f" – {fmt_fiyat(val2)}"
                ax.text(px + panel_w - 0.10, ly, vstr, ha="right", va="top",
        ...
        ax.add_patch(FancyBboxPatch(
            (0.10, y - 1.20), fig_w - 0.20, 1.14,
            boxstyle="round,pad=0.05", linewidth=0.6,
            edgecolor="#1a3a1a", facecolor="#0f1a0f"))
        ax.text(0.22, y - 0.10, "ÖZET", ha="left", va="top",
                fontsize=8, color=GREEN, fontweight="bold")

        # Özeti satırlara böl (fig genişliğine sığacak şekilde)
        import textwrap
        satirlar_ozet = textwrap.wrap(ozet, width=145)
        for si, sat in enumerate(satirlar_ozet[:4]):
            ax.text(0.22, y - 0.28 - si * 0.22, sat,
                    ha="left", va="top", fontsize=8.5, color="#E8E8E6")
        ...
        for sym2, col, lbl in legend_items:
            ...
            ax.text(lx + 0.16, y - 0.14, lbl, ha="left", va="top", fontsize=7.5, color=DGRAY)
            lx += 1.80
        ax.text(fig_w - 0.20, y - 0.14,
                "CVD=Kümülatif Alış-Satış Farkı  ·  LİK=Son 30dk Likidasyon  ·  HAC.Ç=Futures/Spot",
                ha="right", va="top", fontsize=7, color=DGRAY)
