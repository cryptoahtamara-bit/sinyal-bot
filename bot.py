...
def _filtre_detay_gorsel_uret(combo_tab, genel_tp, kapali_tum, esik_g, esik_z, tf_map,
    ...
    try:
        ...
        # ── Özet kutular — filtre_veri.json'daki TÜM kayıtlar (başlangıçtan itibaren birikir) ──
        toplam_sin   = len(snapshot)
        tp_toplam    = sum(1 for s in snapshot if s.get("tp1_ok") == True)
        sl_toplam    = sum(1 for s in snapshot if s.get("sl_ok") == True)
        tam_toplam   = sum(1 for s in snapshot
                           if s.get("tp1_ok") is not None or s.get("sl_ok") is not None)
        devam_toplam = toplam_sin - tam_toplam
        # Başlangıç tarihini bul — en eski sinyal
        try:
            ilk_ts = min(s.get("zaman",0) for s in snapshot if s.get("zaman"))
            ilk_tarih = datetime.fromtimestamp(ilk_ts, tz=TR_TZ).strftime("%d %b %Y") if TR_TZ else \
                        datetime.utcfromtimestamp(ilk_ts).strftime("%d %b %Y")
        except Exception:
            ilk_tarih = "—"
        baslik_tarih = ilk_tarih
