...
def koin_sirala_metni(snapshot=None):
    """Sinyal kalite raporu — yeni filtre mantigi (v612).
    Guclu etiket alan sinyallerin coin bazli TP oranina gore sirala.
    """
    ...
    with gunluk_kilit:
        kayitlar = list(gunluk_sinyaller) if snapshot is None else list(snapshot)
    TF_GECERLI = {"1","5","15","60","240","D"}
    filtre_istat = {"guclu":{"sin":0,"tp":0},"orta":{"sin":0,"tp":0},"zayif":{"sin":0,"tp":0}}
    for s in kayitlar:
        if str(s.get("timeframe","?")) not in TF_GECERLI: continue
        fn = (s.get("filtre_notu") or "").lower()
        if "güçlü" in fn or "guclu" in fn: kat = "guclu"
        elif "orta" in fn:                                    kat = "orta"
        else:                                                  kat = "zayif"
    ...
    sonuc_s = ["", "📊 <b>Sinyal Kalite Raporu</b>"]
