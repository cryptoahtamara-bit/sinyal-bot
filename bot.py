...
def mexc_update_tpsl(symbol, is_long, tp1, sl):
    ...
    try:
        ...
        print(f"[MEXC TPSL] pid={pid} oim={oim:.4f}")
                                ...
                                with gunluk_kilit:
                                    for s in reversed(gunluk_sinyaller):
                                        if s.get("symbol") == symbol and not s.get("mexc_acildi"):
                                            ...
                                            s["mexc_oim"]       = tpsl_oim or sonuc.get("oim", 0)
                                            s["vol"]            = sonuc.get("vol", 0)
                                            s["contract_size"]  = sonuc.get("contract_size", 1.0)
                                            print(f"[MEXC] mexc_acildi=True set edildi: {symbol} lev={sonuc.get('leverage',1)} oim={tpsl_oim:.4f}")
                                            break
                            elif sonuc:
                                mexc_notify(symbol, sinyal, hata_msg=sonuc.get("msg", "Hata"))
        except Exception as e:
...
BOT_VERSIYON = "v284"
print(f"[BASLANGIC] ========== BOT VERSIYON: v334 ==========")
