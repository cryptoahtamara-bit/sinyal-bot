...
def webhook():
    ...
    try:
        ...
        if isinstance(data, dict) and ("message" in data or "channel_post" in data):
            ...
            reply_text    = reply_msg.get("text", "") if reply_msg else ""
...
def _olta_skor_parse(metin):
    """
    Olta mesajinin <pre> blogundan seviyeleri parse eder.
    Ornek satir: "5dk   69.890 69.699 70.334 70.506  ⭐85"
    Donus: {"sym": "BTCUSDT", "yon": "long"/"short",
            "satirlar": [{"tf": "5dk", "iv": "5m", "giris": 69890, "sl": 69699, "tp1": 70334, "tp2": 70506}]}
    """
    import re
    ...
    sym   = None
    yon   = None
    m_sym = re.search(r'(\w+USDT)\s*[\u2014-]\s*(LONG|SHORT)', metin, re.IGNORECASE)
    if m_sym:
        sym = m_sym.group(1).upper()
        yon = m_sym.group(2).lower()

    if not sym or not yon:
        return None

    # TF satırlarını parse et
    # Format: "5dk   69.890 69.699 70.334 70.506  ⭐85"
    # Sayılar nokta ile ayrılmış (Türk formatı, virgül yok)
    tf_iv_map = {"5dk": "5m", "15dk": "15m", "1s": "1h", "4s": "4h", "gun": "1d", "gün": "1d"}
    satirlar  = []

    for line in metin.split("\n"):
        line = line.strip()
        # TF etiketi ile başlamalı
        parcalar = line.split()
        if not parcalar:
            continue
        ...
        if tf_raw not in tf_iv_map:
            continue
        # Sayıları çek — nokta ondalık, nokta binlik de olabilir (69.890)
        sayilar = re.findall(r"\d+[\.,]\d+|\d{4,}", line)
        sayilar = [float(s.replace(",", ".")) for s in sayilar]
        if len(sayilar) < 3:
            continue
        satirlar.append({
            "tf":    parcalar[0],
    ...
    return {"sym": sym, "yon": yon, "satirlar": satirlar}


def _olta_skor_kontrol(metin):
    """
    Alintilanan olta mesajini parse edip son 24 saatlik mumlarla
    her TF'in seviyelerini kontrol eder.
    """
    parsed = _olta_skor_parse(metin)
    if not parsed:
        return "⚠️ Olta mesajı okunamadı. Lütfen doğru mesajı alıntılayın."

    sym     = parsed["sym"]
    yon     = parsed["yon"]
    satirlar = parsed["satirlar"]

    # Anlık fiyat
    fiyat = mexc_get_mark_price(sym)
    if not fiyat:
        return f"⚠️ {sym} fiyatı alınamadı."

    dec = 6 if fiyat < 0.01 else (5 if fiyat < 0.1 else (4 if fiyat < 1 else (2 if fiyat < 10 else (1 if fiyat < 100 else 0))))
    def p(v): return f"{v:,.{dec}f}"

    # Sonuç satırları
    baslik_emoji = "🟢" if yon == "long" else "🔴"
    yon_str      = "LONG" if yon == "long" else "SHORT"
    ...
    for s in satirlar:
        ...
        # Son 24 saatlik mum sayısı
        limit_map = {"5m": 288, "15m": 96, "1h": 24, "4h": 6, "1d": 1}
        limit = limit_map.get(iv, 96)

        try:
            closes, volumes, highs, lows = _trend_fetch_klines(sym, interval=iv, limit=limit)
        except Exception as e:
            satirlar_cikti.append(f"  {tf:<5} ⚠️ veri alınamadı")
            continue

        if not closes:
            satirlar_cikti.append(f"  {tf:<5} ⚠️ veri yok")
            continue

        min_low  = min(lows)
        max_high = max(highs)

        # Giriş bölgesine ulaşıldı mı?
        if yon == "long":
            giris_ulasildi = min_low <= giris
        else:
            giris_ulasildi = max_high >= giris

        if not giris_ulasildi:
            # Fiyat giriş bölgesine henüz ulaşmadı
            uzaklik = abs(fiyat - giris) / fiyat * 100
            if yon == "long":
                uzak_str = f"-%{uzaklik:.1f} uzakta" if fiyat > giris else f"+%{uzaklik:.1f} geçildi"
            else:
                uzak_str = f"+%{uzaklik:.1f} uzakta" if fiyat < giris else f"-%{uzaklik:.1f} geçildi"
            satirlar_cikti.append(f"  {tf:<5} ⏳ Bekleniyor ({uzak_str})")
            continue

        # Giriş oldu — TP mi SL mi önce geldi?
        if yon == "long":
            tp1_hit = max_high >= tp1
            tp2_hit = tp2 and max_high >= tp2
            sl_hit  = min_low  <= sl
        else:
            tp1_hit = min_low  <= tp1
            tp2_hit = tp2 and min_low <= tp2
            sl_hit  = max_high >= sl

        if sl_hit and not tp1_hit:
            # SL vurdu
            kayip_pct = abs(giris - sl) / giris * 100
            satirlar_cikti.append(f"  {tf:<5} ❌ SL Hit  (-%{kayip_pct:.1f})")
        elif tp2_hit:
            kazanc_pct = abs(tp2 - giris) / giris * 100
            satirlar_cikti.append(f"  {tf:<5} ⭐ TP2 Hit (+%{kazanc_pct:.1f})")
        elif tp1_hit:
            kazanc_pct = abs(tp1 - giris) / giris * 100
            satirlar_cikti.append(f"  {tf:<5} ✅ TP1 Hit (+%{kazanc_pct:.1f})")
        else:
            # Giriş oldu ama TP/SL henüz gelmedi
            if yon == "long":
                anlik_pct = (fiyat - giris) / giris * 100
            else:
                anlik_pct = (giris - fiyat) / giris * 100
            isaret = "+" if anlik_pct >= 0 else ""
            satirlar_cikti.append(f"  {tf:<5} 📍 Açık    ({isaret}%{anlik_pct:.1f} anlık)")

    nl = "\n"
    sonuc = (
        "📊 <b>" + sym + " " + yon_str + " — Skor Kontrolü</b>\n"
        "💰 Anlık Fiyat: <b>" + p(fiyat) + "</b>\n"
        "🕐 Son 24 saatlik mumlar\n\n"
        + nl.join(satirlar_cikti)
    )
    return sonuc
...
BOT_VERSIYON = "v418"
print(f"[BASLANGIC] ========== BOT VERSIYON: v418 ==========")
