...
def _liq_veri_cek(symbol):
    ...
    try:
        ...
        # 8) Likidasyon bölgesi — kaldıraç bazlı, 4h/30 mum (geniş giriş aralığı)
        liq_long_baskisi  = None
        liq_short_baskisi = None
        try:
            if len(klines_liq) >= 10 and fiyat > 0:
                # Sadece yüksek kaldıraç — 10x çıkarıldı (haritada görünmüyor)
                kaldıraçlar = [(25, 0.40), (50, 0.35), (100, 0.25)]
                long_seviyeler  = []
                short_seviyeler = []
                ...
                # En ağır top 8 seviyenin içinde fiyata en yakın yoğun kümeyi bul
                def _kume_bul(seviyeler, fiyat_ref, taraf, top_n=8, kume_esik_pct=0.06):
                    if not seviyeler:
                        return None
                    # Hacim × fiyata yakınlık skoru
                    skorlu = []
                    for w, s in seviyeler:
                        uzaklik_pct = abs(s - fiyat_ref) / fiyat_ref
                        # Fiyata yakın olanlar daha yüksek skor alsın
                        yakinlik_bonus = max(0, 1 - uzaklik_pct / 0.10)
                        skor = w * (1 + yakinlik_bonus)
                        skorlu.append((skor, s))
                    skorlu.sort(reverse=True)
                    adaylar = sorted([s for _, s in skorlu[:top_n]])
                    if not adaylar:
                        return None
                    # En yoğun ve birbirine yakın küme
                    en_iyi_kume = adaylar[:2] if len(adaylar) >= 2 else adaylar
                    en_kucuk_aralik = adaylar[-1] - adaylar[0]
                    for i in range(len(adaylar)):
                        for j in range(i+2, len(adaylar)+1):
                            k = adaylar[i:j]
                            aralik = k[-1] - k[0]
                            pct = aralik / k[0]
                ...
                liq_long_baskisi  = _kume_bul(long_seviyeler,  fiyat, "long",  top_n=8, kume_esik_pct=0.06)
                liq_short_baskisi = _kume_bul(short_seviyeler, fiyat, "short", top_n=8, kume_esik_pct=0.06)
...
BOT_VERSIYON = "v497"
print(f"[BASLANGIC] ========== BOT VERSIYON: v497 ==========")
