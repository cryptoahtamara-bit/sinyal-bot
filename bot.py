import os, time, json, re, threading, requests, base64, io, hmac, hashlib
...
# IPRoyal Residential Proxy — sadece MEXC istekleri için
_proxy_user = os.getenv("IPROYAL_USER", "")
_proxy_pass = os.getenv("IPROYAL_PASS", "")
MEXC_PROXY = {
    "http":  f"http://{_proxy_user}:{_proxy_pass}_session-mexcbot_lifetime-60m@geo.iproyal.com:12321",
    "https": f"http://{_proxy_user}:{_proxy_pass}_session-mexcbot_lifetime-60m@geo.iproyal.com:12321",
...
def mexc_get_mark_price(symbol):
    ...
    try:
        r = requests.get(f"{MEXC_BASE_URL}/api/v1/contract/detail",
                         params={"symbol": sym}, timeout=10, proxies=MEXC_PROXY)
...
def mexc_sign(body_dict, ts):
    sign_str = MEXC_API_KEY + ts + json.dumps(body_dict, separators=(",", ":"), ensure_ascii=False)
    return hmac.new(
...
def mexc_place_order(symbol, sinyal, tp1=None, sl=None):
    ...
    ts = str(int(time.time() * 1000))
    sign_str = MEXC_API_KEY + ts + json.dumps(body, separators=(",", ":"))
    sign = hmac.new(
        MEXC_API_SECRET.encode("utf-8"),
        sign_str.encode("utf-8"),
        hashlib.sha256
    ...
    try:
        r = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
                          headers=hdrs, json=body, timeout=15, proxies=MEXC_PROXY)
...
def mexc_update_tpsl(symbol, is_long, tp1, sl):
    sym = mexc_format_symbol(symbol)
    ts  = str(int(time.time() * 1000))
    sign = hmac.new(MEXC_API_SECRET.encode(),
                    (MEXC_API_KEY + ts).encode(), hashlib.sha256).hexdigest()
    hdrs = {"ApiKey": MEXC_API_KEY, "Request-Time": ts,
            "Signature": sign, "Content-Type": "application/json"}
    try:
        r = requests.get(f"{MEXC_BASE_URL}/api/v1/private/position/open_positions",
                         headers=hdrs, params={"symbol": sym}, timeout=10, proxies=MEXC_PROXY)
        ...
        r2 = requests.post(f"{MEXC_BASE_URL}/api/v1/private/position/change_tpsl",
                           headers=hdrs2, json=body, timeout=10, proxies=MEXC_PROXY)
    ...
    for deneme in range(3):
        try:
            ...
            r = requests.get(f"{MEXC_BASE_URL}/api/v1/private/account/assets",
                             headers=hdrs, timeout=10, proxies=MEXC_PROXY)
            res = r.json()
...
def get_mexc_price(symbol: str) -> float:
    ...
    for api_url in [
        ...
        try:
            proxies = MEXC_PROXY if "mexc.com" in api_url else None
    ...
    if AUTO_TRADE_ENABLED:
        try:
            ...
            if is_trade:
                ...
                # SENARYO D: TP1 zaten gecildi mi?
                tp1_gecildi = False
                if tp1 and mp:
                    try:
                        tp1_f = float(str(tp1).replace(",","."))
                        if is_long and mp >= tp1_f:     tp1_gecildi = True
                        elif not is_long and mp <= tp1_f: tp1_gecildi = True
                    except: pass
...
def _tarayici_veri_cek():
    ...
    try:
        r = requests.get("https://contract.mexc.com/api/v1/contract/ticker", timeout=10, proxies=MEXC_PROXY)
        if r.status_code == 200:
            data = r.json()
            if data.get("success") and data.get("data"):
                return data["data"]
