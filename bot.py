...
def _tp_kosul_rapor_hesapla(snapshot=None):
    ...
    # 8. Öneri: min 20 sinyal, en yüksek TP oranlı kombinasyonlar
    MIN_SINYAL = 20
    oneriler = [
...
def _tp_kosul_yorum_uret(veri):
    """TP olan işlemlerde piyasa koşulları nasıldı — confluance + üçlü kombinasyon yorumu."""
    MIN_KOMB  = 20
    MIN_KOSUL = 5
    MIN_UCLU  = 5
    maddeler = []
    ...
    if komb_veri:
        ...
        for i, (k, v) in enumerate(sirali[:3]):
            emoji = ["  1️⃣", "  2️⃣", "  3️⃣"][i]
            maddeler.append(f"{emoji} <b>{k}</b> → {v[4]} TP ({v[1]} TP / {v[0]} sinyal)")
    else:
    ...
    # 3. Piyasa koşulları confluance analizi
            ...
            if abs(fark) >= 3:
                if fark > 0:
                    maddeler.append(
                        ...
                        f"(Pozitif {fund_pos[4]} vs Negatif {fund_neg[4]})"
                    )
                else:
                    maddeler.append(
                        f"💰 Funding <b>Negatif</b> ortamda TP oranı <b>{abs(fark)} puan</b> daha yüksek "
                        f"(Negatif {fund_neg[4]} vs Pozitif {fund_pos[4]})"
                    )
