...
def _liq_heatmap_gercek(symbol, saat=6):
    ...
    try:
        ...
        try:
            def _bolge_hesapla_dis(band_arr, fiyat_ref, taraf, n_pb_, price_min_, price_max_):
                esik = band_arr.max() * 0.30
                ...
                while i < n_pb_:
                    if band_arr[i] >= esik:
                        j = i
                        while j < n_pb_ and band_arr[j] >= esik:
                            j += 1
                        fiyat_alt = price_min_ + (i / n_pb_) * (price_max_ - price_min_)
                        fiyat_ust = price_min_ + (j / n_pb_) * (price_max_ - price_min_)
            ...
            if band_long_usd is not None and band_long_usd.max() > 0:
                liq_long_zone  = _bolge_hesapla_dis(band_long_usd,  fiyat, "long",  n_pb, price_min, price_max)
            if band_short_usd is not None and band_short_usd.max() > 0:
                liq_short_zone = _bolge_hesapla_dis(band_short_usd, fiyat, "short", n_pb, price_min, price_max)
...
BOT_VERSIYON = "v509"
print(f"[BASLANGIC] ========== BOT VERSIYON: v509 ==========")
