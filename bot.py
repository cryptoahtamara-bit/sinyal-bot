    ...
    if AUTO_TRADE_ENABLED:
        try:
            ...
            if is_trade:
                ...
                # Önce hedge muafiyetini kontrol et — ters yön açık pozisyon varsa whitelist atlanır
                _wl_mevcut = pozisyon_al(sym_key, is_long) or pozisyon_al(sym_key, not is_long)
                _wl_hedge  = _wl_mevcut and (_wl_mevcut["is_long"] != is_long)

                if not is_futures:
                    print("[MEXC] " + symbol + " futures degil, atlandi.")
                elif not top3_whitelist and not _wl_hedge:
                    # Whitelist boş, hedge muafiyeti de yok — sessizce atla
                    ...
                    # Whitelist'te yok, hedge muafiyeti de yok — sessizce atla
                    print(f"[MEXC] {timeframe}dk sinyal, {sym_key} %%50+ whitelist'te degil — islem atlandı. Whitelist: {top3_whitelist}")
                elif tp1_gecildi:
                    # SENARYO 3: TP1 geçildi — işlem geçildi, sadece log
                    print(f"[MEXC] SENARYO 3 — TP1 gecildi, islem gecildi (bildirim yok)")
                else:
...
BOT_VERSIYON = "v403"
print(f"[BASLANGIC] ========== BOT VERSIYON: v403 ==========")
...
def _pnl_rapor_gorsel(gun, bakiye, kullanilan, unrealized):
    ...
    try:
        ...
        # Açık poz PNL — sıfırsa "Veri Yok" göster
        unreal_str = fmt_usdt(unrealized) if unrealized != 0 else "—"
        kart(fig.add_subplot(gs[0,3]), "Acik Poz. PNL",
             unreal_str, "#26a69a" if unrealized >= 0 else "#ef5350")
