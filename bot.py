...
def mexc_place_order(symbol, sinyal, tp1=None, sl=None, hedge=False):
    ...
    # Pre-Set TP/SL — emir body'sine ekle (Position TP/SL ekranında görünür, Batches değil)
    # trend: 1=Fair Price, 2=Last Price
    if tp1:
        try:
            body["takeProfitPrice"] = float(str(tp1).replace(",", "."))
            body["takeProfitTrend"] = 2  # Last Price
        except Exception:
            pass
    if sl:
        try:
            body["stopLossPrice"] = float(str(sl).replace(",", "."))
            body["stopLossTrend"] = 2  # Last Price
        except Exception:
            pass
...
def mexc_update_tpsl(symbol, is_long, tp1, sl):
    """
    Position TP/SL — MEXC resmi dokümantasyonundaki
    'Place TP/SL Order by Position' endpoint'ini kullanır.
    Endpoint: /api/v1/private/position/stop_loss_plan
    Bu UI'daki 'Position TP/SL' ekranına karşılık gelir (Batches değil).
    Trigger: 2=Last Price
    """
    sym = mexc_format_symbol(symbol)
    try:
