...
def mexc_place_order(symbol, sinyal, tp1=None, sl=None):
    ...
    # TP/SL order body'sine eklenmiyor — order acildiktan sonra change_tpsl ile set edilecek
    ...
    try:
        ...
        if res.get("success"):
            # Order acildi, simdi TP/SL set et
            if tp1 or sl:
...
def mexc_update_tpsl(symbol, is_long, tp1, sl):
    ...
    try:
        ...
        if tp1:
            ...
            body["takeProfitPriceType"]= 1
        if sl:
            body["stopLossPrice"]     = float(str(sl).replace(",", "."))
            body["stopLossPriceType"] = 1
        print(f"[MEXC TPSL] change_tpsl body: {body}")
        r2 = mexc_private_post(f"{MEXC_BASE_URL}/api/v1/private/planorder/place/tpsl", body)
        print(f"[MEXC TPSL] change_tpsl yanit: {r2.text}")
...
def mexc_pozisyon_kapat(symbol, is_long):
    ...
    try:
        ...
        # Kapat: LONG kapat = SELL (side=2, closePosition=true), SHORT kapat = BUY (side=1)
        side = 2 if is_long else 1
        body = {"symbol": sym, "price": 0, "vol": vol, "side": side,
                "type": 5, "openType": 2}
        r2 = mexc_private_post(f"{MEXC_BASE_URL}/api/v1/private/order/submit", body)
        res = r2.json()
...
def _liq_gonder():
    """BTC ve ETH için likidisyon baskı haritası gönder. Veri GitHub Actions'dan /external_veri ile geliyor."""
    print("[LIQ] Zamanlayici tetiklendi — veri GitHub Actions'dan bekleniyor.")
...
def _ls_gonder(chat_id=None):  # chat_id artık kullanılmıyor
    """L/S grafiği + yorum gönder. Veri GitHub Actions'dan /external_veri ile geliyor."""
    print(f"[LS] Zamanlayici tetiklendi — veri GitHub Actions'dan bekleniyor.")
    # Veri artık /external_veri endpoint'inden geliyor, burada bir şey yapılmıyor.
...
print(f"[BASLANGIC] ========== BOT VERSIYON: v236 ==========")
