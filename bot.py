...
def _analiz_gorsel(veriler, zaman_str):
    ...
    try:
        ...
        def fmt_fiyat(f):
            if f is None: return "—"
            S = "\u0024"  # dollar sign — $ latex'i tetiklememesi için
            if f >= 10000: return f"{S}{f/1000:.1f}k"
            if f >= 100:   return f"{S}{f:,.0f}"
            if f >= 1:     return f"{S}{f:.2f}"
            return f"{S}{f:.4f}"

        def fmt_para(v):
            if v is None: return "—"
            S = "\u0024"
            if abs(v) >= 1000: return f"{S}{v/1000:.1f}B"
            return f"{S}{v:.1f}M"
        ...
        for cx, lbl, ha in cols:
            ax.text(cx, y, lbl, ha=ha, va="top", fontsize=9, color=WHITE, fontweight="bold")
        y -= 0.14
        ...
        for i, sym in enumerate(SEMBOLLER):
            ...
            ax.text(0.18, my, sym_kisa, ha="left", va="center",
                    fontsize=11, color=sym_renk, fontweight="bold")
            ...
            if skor is not None:
                ...
                ax.add_patch(FancyBboxPatch(
                    (3.10, my - 0.15), 0.88, 0.30,
                    boxstyle="round,pad=0.03", linewidth=0.5,
                    edgecolor=s_col, facecolor=BG))
                ax.text(3.54, my, str(skor), ha="center", va="center",
                        fontsize=10.5, color=s_col, fontweight="bold")
            ...
            if oi_d:
                dp = oi_d["delta_pct"]
                oi_str = f"{dp:+.1f}%"
                ax.text(6.70, my, oi_str, ha="right", va="center", fontsize=9.5,
                        color=renk_pct(dp))
            else:
                ax.text(6.70, my, "—", ha="right", va="center", fontsize=9.5, color=GRAY)
            ...
            if taker:
                ...
                ax.text(7.90, my, f"{bp:.0f}%B", ha="right", va="center",
                        fontsize=9.5, color=t_col)
            else:
                ax.text(7.90, my, "—", ha="right", va="center", fontsize=9.5, color=GRAY)
            ...
            if cvd:
                ...
                ax.text(9.10, my, cvd_str, ha="right", va="center",
                        fontsize=9.5, color=c_col)
            else:
                ax.text(9.10, my, "—", ha="right", va="center", fontsize=9.5, color=GRAY)

            # Likidasyon
            lik = veri.get("likidasyonlar")
            if lik and lik["toplam_usd"] > 0:
                l_col = RED if lik["agirlik"] == "long_baskin" else (GREEN if lik["agirlik"] == "short_baskin" else GRAY)
                ax.text(10.90, my, f"${lik['toplam_usd']:.1f}M", ha="right", va="center",
                        fontsize=9.5, color=l_col)
            else:
                ax.text(10.90, my, "—", ha="right", va="center", fontsize=9.5, color=DGRAY)

            # Hacim çarpanı
            hc = veri.get("hac_carpan", 0)
            hc_col = ORANGE if hc > 5 else (GRAY)
            hc_str = f"{hc:.1f}x⚠" if hc > 5 else (f"{hc:.1f}x" if hc > 0 else "—")
            ax.text(12.55, my, hc_str, ha="right", va="center",
        ...
        for pi, sym in enumerate(["BTCUSDT", "ETHUSDT", "SOLUSDT"]):
            ...
            s_renk   = renk_skor(skor)
            fiyat    = veri.get("fiyat", 0)
            ax.text(px + panel_w/2, y - 0.16,
            ...
            if nota:
                ax.text(px + panel_w/2, y - 0.36, nota[:60],
                        ha="center", va="top", fontsize=8, color=ORANGE)

            ax.axhline(y - 0.46, xmin=px/fig_w + 0.01, xmax=(px+panel_w)/fig_w - 0.01,
                       color=BORDER, linewidth=0.5)

            ly = y - 0.60

            def satir(label, val1, val2=None, renk=WHITE, indent=0):
                nonlocal ly
                lx = px + 0.16 + indent
                ax.text(lx, ly, label, ha="left", va="top", fontsize=9, color=WHITE)
                vstr = fmt_fiyat(val1)
                if val2:
                    vstr += f" – {fmt_fiyat(val2)}"
                ax.text(px + panel_w - 0.12, ly, vstr, ha="right", va="top",
                        fontsize=10, color=renk, fontweight="bold")
                ly -= 0.33

            # LONG
            ax.text(px + 0.14, ly, "LONG OLTA", ha="left", va="top",
                    fontsize=10, color=GREEN, fontweight="bold")
            ly -= 0.28
            satir("Giriş:", olta["long_giris_alt"], olta["long_giris_ust"], GREEN)
            satir("Stop:",  olta["long_stop"],  renk=RED)
            satir("TP1:",   olta["long_tp1"],   renk=LGREEN)
            satir("TP2:",   olta["long_tp2"],   renk=LGREEN)

            ax.axhline(ly + 0.06, xmin=px/fig_w + 0.02, xmax=(px+panel_w)/fig_w - 0.02,
                       color=BORDER, linewidth=0.4)
            ly -= 0.05

            # SHORT
            ax.text(px + 0.14, ly, "SHORT OLTA", ha="left", va="top",
        ...
        ax.add_patch(FancyBboxPatch(
            (0.10, y - 1.50), fig_w - 0.20, 1.44,
            boxstyle="round,pad=0.05", linewidth=0.6,
            edgecolor="#1a3a1a", facecolor="#0f1a0f"))
        ax.text(0.22, y - 0.12, "ÖZET", ha="left", va="top",
                fontsize=10, color=GREEN, fontweight="bold")

        # Özeti satırlara böl (fig genişliğine sığacak şekilde)
        import textwrap
        ozet_safe = ozet.replace("$", "\u0024")  # $ -> unicode dollar
        satirlar_ozet = textwrap.wrap(ozet_safe, width=165)
        for si, sat in enumerate(satirlar_ozet[:4]):
            ax.text(0.22, y - 0.34 - si * 0.26, sat,
                    ha="left", va="top", fontsize=9.5, color="#E8E8E6",
                    usetex=False)
