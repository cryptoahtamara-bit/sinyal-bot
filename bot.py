...
def mexc_place_order(symbol, sinyal, tp1=None, sl=None):
    ...
    # Vol hesabi — sembol hassasiyetine gore yuvarlama
    vol_raw = (MEXC_MARGIN_USDT * lev) / mp
    vol = round(vol_raw, vol_decimals)
    if vol <= 0:
        return {"success": False, "msg": f"Lot sifir: {vol}"}

    print(f"[MEXC] Vol hesabi: {vol_raw} -> {vol} (decimals={vol_decimals})")
    ...
    if tp1:
        try:
            ...
            if tp_gecerli:
                ...
                print(f"[MEXC] TP1 gecersiz (is_long={is_long}, mp={mp}, tp1={tp1_f}) — atlanıyor")
        except:
            pass
    if sl:
        try:
            sl_f = float(str(sl).replace(",", "."))
            sl_gecerli = (is_long and sl_f < mp) or (not is_long and sl_f > mp)
            if sl_gecerli:
                body["stopLossPrice"] = sl_f
                body["stopLossPriceType"] = 2
            else:
                print(f"[MEXC] SL gecersiz (is_long={is_long}, mp={mp}, sl={sl_f}) — atlanıyor")
        except:
            pass
...
def mexc_test():
    ...
    sonuc = {"version": "v207", "proxy": bool(MEXC_PRIVATE_PROXY)}
...
print(f"[BASLANGIC] ========== BOT VERSIYON: v207 ==========")
