...
def send_telegram_and_schedule_tp(caption, symbol, timeframe, sinyal,
    ...
    if filtre:
        ...
        tum_eslesme = all(p in mevcut_kosullar for p in kosul_parts)

        if tum_eslesme:
            tp_yuzde_int = int(round(tp_yuzde))
            filtre_notu = f"Güçlü Sinyal %{tp_yuzde_int} ✅"
            print(f"[TP_FILTRE] {sinyal} {symbol} kosul uyustu (%{tp_yuzde}).")
        else:
            filtre_notu = "⚠️ Zayıf Sinyal"
            print(f"[TP_FILTRE] {sinyal} {symbol} kosul uyusmadi.")
    else:
