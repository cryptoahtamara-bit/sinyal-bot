...
def _panel_a_gorsel(snapshot=None):
    ...
    try:
        ...
        for piy, v in piy_sirali:
            ...
            ax.text(0.06, yy, piy, fontsize=10.5, color=TEXT_W, ha="left", va="top")
            ax.text(0.30, yy, str(v["n"]), fontsize=10.5, color=HDR_COL, ha="left", va="top")
            ax.text(0.46, yy, f"%{oran:.1f} ({v['tp']}/{v['n']})", fontsize=10.5, color=renk, ha="right", va="top")
            yy -= 0.055
...
def _panel_b_gorsel(snapshot=None):
    ...
    try:
        ...
        kol_x     = [0.05, 0.34, 0.50, 0.68, 0.86]
        for bx, bl in zip(kol_x, basliklar):
            ax.text(bx, 0.845, bl, fontsize=9, color=HDR_COL, ha="left" if bx==0.05 else "right", va="top")
        yy = 0.80
        for row in detay_satirlari:
            ...
            ax.text(0.05, yy, row["tip"], fontsize=10, color=TEXT_W, ha="left", va="top")
            ax.text(0.34, yy, f"%{row['genel']:.1f}", fontsize=10, color=TEXT_W, ha="right", va="top")
            ax.text(0.50, yy, f"%{go:.1f} ({row['g_tp']}/{row['g_n']})", fontsize=10, color=GREEN, ha="right", va="top")
            ax.text(0.68, yy, f"%{oo:.1f} ({row['o_tp']}/{row['o_n']})", fontsize=10, color=AMBER, ha="right", va="top")
            ax.text(0.86, yy, f"%{zo:.1f} ({row['z_tp']}/{row['z_n']})", fontsize=10, color=RED, ha="right", va="top")
            yy -= 0.052
...
def _istatistik_zamanlayici():
    ...
    while True:
        try:
            ...
            # 1) PANEL A — Genel Durum (eski istatistik metni + koin sıralaması metni yerine)
            img_a = _panel_a_gorsel(snapshot=snapshot)
            if img_a:
                _topic_foto_gonder_filigranli(TOPIC_RAPOR, img_a, "📋 Genel Durum")
            else:
                # Panel üretilemezse eski metin yedek olarak gönderilsin
                _telegram_topic_mesaj_gonder(TOPIC_RAPOR, istatistik_mesaji(snapshot=snapshot))
            # 2) Günlük rapor görseli (Panel C — parite × TF ızgarası, değişmedi)
