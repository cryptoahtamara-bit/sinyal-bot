...
def send_telegram_and_schedule_tp(caption, symbol, timeframe, sinyal,
    ...
    # Filtre sonucunu sinyal kaydina yaz (TP/SL bildirimi ayni notu kullansin)
    with gunluk_kilit:
        _kayit_guncelle = next(
            (s for s in reversed(gunluk_sinyaller)
             if s.get("symbol") == symbol and s.get("sinyal_tipi") == sinyal_norm),
            None
        )
        if _kayit_guncelle is not None:
            _kayit_guncelle["filtre_notu"] = filtre_notu
