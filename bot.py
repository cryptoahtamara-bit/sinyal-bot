...
def mexc_test():
    ...
    sonuc = {"version": "v187", "proxy": bool(MEXC_PRIVATE_PROXY)}
...
def webkey_test():
    ...
    # Test 4: Body boyutunu kademeli artir
    if MEXC_API_KEY and MEXC_API_SECRET:
        test_bodies = [
            ("bos",        {}),
            ("bir_int",    {"a": 1}),
            ...
            ("side_1",     {"side": 1}),
            ("vol",        {"vol": 1}),
            ("sym_side",   {"symbol": "BTC_USDT", "side": 1}),
            ("sym_vol",    {"symbol": "BTC_USDT", "vol": 1}),
            ("uclu",       {"symbol": "BTC_USDT", "side": 1, "vol": 1}),
            ("dortlu",     {"symbol": "BTC_USDT", "side": 1, "vol": 1, "type": 5}),
            ("tam",        {"symbol": "BTC_USDT", "price": 0, "vol": 0.001, "side": 1, "type": 5, "openType": 2, "leverage": 20}),
        ]
        for isim, body in test_bodies:
            try:
                hdrs, _ = mexc_headers(body)
                r = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
                                  headers=hdrs, json=body, timeout=8)
                body_size = len(json.dumps(body))
                sonuc[f"size_{isim}_{body_size}b"] = {"status": r.status_code}
            except Exception as e:
                sonuc[f"size_{isim}"] = {"error": str(e)[:30]}
...
print(f"[BASLANGIC] ========== BOT VERSIYON: v187 ==========")
