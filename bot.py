...
def _marketyonu_gonder():
    """OI + Funding Rate + Spot/Futures hacim analizi gönder. (v281 — :45'de çalışır)"""
    print("[MARKETYONU] Zamanlayici tetiklendi — veri çekiliyor.")
    try:
    ...
    try:
        ...
        for i, sym in enumerate(SEMBOLLER):
            ...
            if lik and lik["toplam_usd"] > 0:
                ...
                ax.text(COL["lik"], my, f"USD {lik['toplam_usd']:.1f}M", ha="right", va="center",
                        fontsize=11, color=l_col)
            else:
                ax.text(COL["lik"], my, "—", ha="right", va="center", fontsize=11, color=DGRAY)

            # Hacim çarpanı
            hc = veri.get("hac_carpan", 0)
            hc_col = ORANGE if hc > 5 else GRAY
            hc_str = f"{hc:.1f}x⚠" if hc > 5 else (f"{hc:.1f}x" if hc > 0 else "—")
            ax.text(COL["hac"], my, hc_str, ha="right", va="center",
                    fontsize=11, color=hc_col)
        ...
        ax.text(fig_w - 0.20, y - 0.16,
                "CVD=Kümülatif Alış-Satış Farkı  ·  LİK=Son 30dk Likidasyon  ·  HAC.Ç=Futures/Spot",
