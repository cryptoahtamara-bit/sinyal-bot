...
def _gunluk_pnl_gonder(hedef_chat_id=None):
    ...
    try:
        ...
        # Unrealized PNL — önce account assets'ten dene, olmadıysa open_positions'tan topla
        _unreal_fields = ["unrealisedPnl","unrealised","unrealizedPnl","unRealised","frozenBalance"]
        unrealized = 0.0
        for _f in _unreal_fields:
            _v = usdt.get(_f)
            if _v is not None:
                try:
                    unrealized = float(_v)
        ...
        # Eğer hâlâ 0 ise open_positions'tan topla (MEXC UI ile aynı)
        if unrealized == 0:
            try:
                _r_pos = mexc_private_get(f"{MEXC_BASE_URL}/api/v1/private/position/open_positions")
                _pos_data = _r_pos.json().get("data", [])
                unrealized = sum(float(p.get("unrealised", 0) or p.get("unrealisedPnl", 0) or 0)
                                 for p in (_pos_data if isinstance(_pos_data, list) else []))
                print(f"[PNL DEBUG] unrealized open_positions toplamı: {unrealized} ({len(_pos_data)} poz)")
        ...
        try:
            ...
            ax_pnl.text(0.5, 0.1, f"{len(gun_kayitlar)} işlem", transform=ax_pnl.transAxes,
                        ha="center", fontsize=8, color="#484f58")
            kart(fig.add_subplot(gs[0,3]), "Acik Poz. PNL", fmt_usdt2(unrealized),
        ...
        caption = (
            ...
            f"📈 Açık Poz. PNL: <b>{unrealized:+.4f} USDT</b>\n\n"
        )
        if gun_kayitlar2:
    ...
    try:
        ...
        if leverage <= 0:
            ...
            if _vol_l > 0 and _oim_l > 0 and giris > 0:
                ...
                leverage = 1
                print(f"[PNL] leverage bulunamadi, 1x kullanildi")
        if is_long:
...
BOT_VERSIYON = "v384"
print(f"[BASLANGIC] ========== BOT VERSIYON: v384 ==========")
...
def _pnl_rapor_gorsel(gun, bakiye, kullanilan, unrealized):
    ...
    try:
        ...
        kart(fig.add_subplot(gs[0,3]), "Acik Poz. PNL",
             fmt_usdt(unrealized), "#26a69a" if unrealized >= 0 else "#ef5350")
