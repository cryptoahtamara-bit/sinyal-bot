...
def _panel_d_gorsel(snapshot=None):
    ...
    try:
        ...
        fig_w = 24.0
        fig_h = 2.2 + n_satir * 0.62
        fig, ax = plt.subplots(figsize=(fig_w, fig_h))
        ax.axis("off"); ax.set_xlim(0, 1); ax.set_ylim(0, 1)
        fig.patch.set_facecolor(BG)

        ax.text(0.5, 0.99, "Ben Kül Yutmam — Parite Detayı", fontsize=19, fontweight="bold",
                color=GOLD, ha="center", va="top")
        ax.text(0.5, 0.955, f"İzlenen {n_satir} Parite · Panel D / 4", fontsize=10.5, color=HDR_COL, ha="center", va="top")

        # Sutun x-pozisyonlari (0-1 araligi)
        cols = {
            "coin":0.008,
            "top":0.055,"L":0.078,"S":0.101,"SS":0.124,"SB":0.150,
            "tpg":0.215,"tpL":0.278,"tpS":0.338,"tpSS":0.398,"tpSB":0.458,
            "gucg":0.525,"guclu":0.590,"orta":0.655,"zayif":0.720,
            "yuk":0.815,"dus":0.900,"yat":0.985,
        }
        baslik_y = 0.90
        grup_y   = 0.925

        ax.text((cols["top"]+cols["SB"])/2, grup_y, "SİNYAL ADEDİ", fontsize=8.5, color=GOLD, ha="center", va="top", fontweight="bold")
        ax.text((cols["tpg"]+cols["tpSB"])/2, grup_y, "TP ORANI", fontsize=8.5, color=GOLD, ha="center", va="top", fontweight="bold")
        ax.text((cols["gucg"]+cols["zayif"])/2, grup_y, "SİNYAL GÜCÜ", fontsize=8.5, color=GOLD, ha="center", va="top", fontweight="bold")
        ax.text((cols["yuk"]+cols["yat"])/2, grup_y, "PİYASA REJİMİ (sinyal · TP oranı)", fontsize=8.5, color=GOLD, ha="center", va="top", fontweight="bold")

        basliklar = [("coin","Coin","left"),("top","Top.","right"),("L","Long","right"),("S","Short","right"),
                     ("SS","S.Sell","right"),("SB","S.Buy","right"),
                     ("tpg","Genel","right"),("tpL","Long","right"),("tpS","Short","right"),
                     ("tpSS","S.Sell","right"),("tpSB","S.Buy","right"),
                     ("gucg","Genel","right"),("guclu","Güçlü","right"),("orta","Orta","right"),("zayif","Zayıf","right"),
                     ("yuk","Yükseliş","right"),("dus","Düşüş","right"),("yat","Yatay","right")]
        for key, lbl, ha in basliklar:
            ax.text(cols[key], baslik_y, lbl, fontsize=8, color=HDR_COL, ha=ha, va="top")

        yy0 = 0.86
        satir_h = 0.80 / max(n_satir, 1)
        for i, r in enumerate(satirlar):
            yy = yy0 - i * satir_h
            if i % 2 == 0:
                ax.add_patch(plt.Rectangle((0.0, yy - satir_h + 0.01), 1.0, satir_h,
                             transform=ax.transAxes, facecolor=CARD_BG, edgecolor="none", zorder=0))
            ax.text(cols["coin"], yy, r["sym"], fontsize=9.5, color=TEXT_W, ha="left", va="top", fontweight="bold")
            ax.text(cols["top"], yy, str(r["toplam"]), fontsize=8.5, color=TEXT_W, ha="right", va="top")
            ax.text(cols["L"], yy, str(r["tip_n"]["Long"]), fontsize=8.5, color=HDR_COL, ha="right", va="top")
            ax.text(cols["S"], yy, str(r["tip_n"]["Short"]), fontsize=8.5, color=HDR_COL, ha="right", va="top")
            ax.text(cols["SS"], yy, str(r["tip_n"]["Strong Sell"]), fontsize=8.5, color=HDR_COL, ha="right", va="top")
            ax.text(cols["SB"], yy, str(r["tip_n"]["Strong Buy"]), fontsize=8.5, color=HDR_COL, ha="right", va="top")

            _oran_g = r["tp_n_genel"]/r["kapali_n"]*100 if r["kapali_n"] else 0
            _renk_g = GREEN if _oran_g>=60 else (AMBER if _oran_g>=35 else RED)
            ax.text(cols["tpg"], yy, oran_str(r["tp_n_genel"], r["kapali_n"]), fontsize=8.5, color=_renk_g, ha="right", va="top", fontweight="bold")
            for key, t in [("tpL","Long"),("tpS","Short"),("tpSS","Strong Sell"),("tpSB","Strong Buy")]:
                tp, n = r["tp_tip"][t]
                ax.text(cols[key], yy, oran_str(tp, n), fontsize=8, color=TEXT_W, ha="right", va="top")

            _oran_guc = r["siniflanan_tp"]/r["siniflanan_n"]*100 if r["siniflanan_n"] else 0
            _renk_guc = GREEN if _oran_guc>=60 else (AMBER if _oran_guc>=35 else RED)
            ax.text(cols["gucg"], yy, oran_str(r["siniflanan_tp"], r["siniflanan_n"]), fontsize=8.5, color=_renk_guc, ha="right", va="top", fontweight="bold")
            ax.text(cols["guclu"], yy, oran_str(r["g_tp"], r["g_n"]), fontsize=8, color=GREEN, ha="right", va="top")
            ax.text(cols["orta"], yy, oran_str(r["o_tp"], r["o_n"]), fontsize=8, color=AMBER, ha="right", va="top")
            ax.text(cols["zayif"], yy, oran_str(r["z_tp"], r["z_n"]), fontsize=8, color=RED, ha="right", va="top")

            for key, piy in [("yuk","Yükseliş"),("dus","Düşüş"),("yat","Yatay")]:
                v = r["piy"].get(piy, {"tp":0,"n":0})
                if v["n"]:
                    ax.text(cols[key], yy, f"{v['n']} · {oran_str(v['tp'], v['n'])}", fontsize=8, color=TEXT_W, ha="right", va="top")
                else:
                    ax.text(cols[key], yy, "—", fontsize=8, color=HDR_COL, ha="right", va="top")
