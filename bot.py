...
def _analiz_gorsel(veriler, zaman_str):
    ...
    try:
        ...
        def fmt_fiyat(f):
            if f is None: return "—"
            if f >= 10000: return f"USD {f/1000:.1f}k"
            if f >= 100:   return f"USD {f:,.0f}"
            if f >= 1:     return f"USD {f:.2f}"
            return f"USD {f:.4f}"
        ...
        for i, sym in enumerate(SEMBOLLER):
            ...
            if lik and lik["toplam_usd"] > 0:
                ...
                ax.text(10.90, my, f"USD {lik['toplam_usd']:.1f}M", ha="right", va="center",
        ...
        import textwrap
        # $ işaretini matplotlib math parser'ından koru
        ozet_safe = ozet.replace("$", "USD ")
