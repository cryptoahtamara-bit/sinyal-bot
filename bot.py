                    ...
                    def _tplog_gonder(parametre=parametre):
                        try:
                            ...
                            if not gonderildi:
                                _telegram_topic_mesaj_gonder(
                                    TOPIC_RAPOR,
                                    f"⚠️ TP Confluance Log görseli üretilemedi ({tarih_str})."
                                )
                        except Exception as e:
...
def _tp_kosul_yorum_uret(veri, snapshot=None):
    """En İyi TP Koşulları — sadece mega kombinasyon (confluance) gösterilir."""
    maddeler = []
    ...
    def guven_etiketi(sinyal_sayisi):
        if sinyal_sayisi < 10:  return "⚠️ düşük veri"
        elif sinyal_sayisi < 30: return "🔸 orta güven"
        elif sinyal_sayisi < 60: return "✅ güvenilir"
        else:                    return "💎 yüksek güven"

    AYIRAC = "―――――――――――――――――――"

    # Tüm kombinasyonların confluance analizi — sinyal tipine göre
    confluance = _tp_confluance_skoru_hesapla(veri, kayitlar_snapshot=snapshot)
    if confluance:
        sinyal_sirali   = confluance.get("sinyal_sirali", [])
        sinyal_sonuclari = confluance.get("sinyal_sonuclari", {})
        tavsiye_yon     = confluance.get("tavsiye_yon", "NÖTR")
        yon_emoji       = {"LONG": "🟢", "SHORT": "🔴", "NÖTR": "⚪"}.get(tavsiye_yon, "⚪")

        maddeler.append(f"🔮 <b>En İyi TP Koşulları — {yon_emoji} {tavsiye_yon}</b>")

        ST_EMOJI = {"Long": "📈", "Strong Buy": "📈", "Short": "📉", "Strong Sell": "📉"}
        SIRA_E   = ["🥇", "🥈", "🥉"]
        ...
        for st in sinyal_sirali:
            ...
            maddeler.append(
                f"{emoji} <b>{st}</b> — genel %{genel_oran} TP ({genel_top} sinyal)"
            )
            if en_iyi:
                for i, item in enumerate(en_iyi):
                    maddeler.append(
                        f"  {SIRA_E[i]} {item['kombinasyon']} → "
                        f"{item['tp_orani']} TP ({item['tp_sayisi']}/{item['sinyal']})"
                    )
            else:
                maddeler.append("  ⏳ Yeterli veri yok (min. 10 sinyal)")
    else:
        maddeler.append("🔮 Confluance analizi: Yeterli kombinasyon verisi yok")
...
def _tplog_gorsel_uret(kayitlar, tarih_str):
    ...
    try:
        ...
        for k in kayitlar:
            ...
            sr   = k.get("sinyal_sirali", list(ss.keys()))
            if not sr: sr = ["Long", "Short", "Strong Buy", "Strong Sell"]
            ...
            for idx, st in enumerate(sr):
                ...
                if en_iyi:
                    ...
                    if len(kosul) > 45: kosul = kosul[:43] + "…"
                    tp     = item["tp_orani"]
        ...
        n_satir = len(rows)
        YON_SAG = 0.92
        XPOS    = [0.005, 0.060, 0.210, 0.73, 0.82]
