...
def _liq_veri_cek(symbol):
    ...
    try:
        ...
        # 8) Likidasyon bölgesi — kaldıraç bazlı (ısı haritasıyla aynı mantık)
        liq_long_baskisi  = None
        liq_short_baskisi = None
        try:
            if len(klines) >= 10 and fiyat > 0:
                # Yüksek kaldıraç seviyeleri daha gerçekçi — 25x/50x/100x ağırlıklı
                kaldıraçlar = [(25, 0.35), (50, 0.35), (100, 0.20), (10, 0.10)]
                long_seviyeler  = []
                short_seviyeler = []
                ...
                for k in klines:
                    ...
                    for lev, agirlik in kaldıraçlar:
                        ...
                        if long_lik < fiyat:
                            long_seviyeler.append((long_w, long_lik))
                        if short_lik > fiyat:
                            short_seviyeler.append((short_w, short_lik))

                # En ağır top 8 seviyenin içinde en yoğun kümeyi bul
                def _kume_bul(seviyeler, top_n=8, kume_esik_pct=0.08):
                    if not seviyeler:
                        return None
                    seviyeler.sort(reverse=True)
                    adaylar = sorted([s for _, s in seviyeler[:top_n]])
                    if not adaylar:
                        return None
                    # Ardışık en yakın 3 seviyeyi bul
                    en_iyi_kume = adaylar
                    en_kucuk_aralik = adaylar[-1] - adaylar[0]
                    for i in range(len(adaylar)):
                        for j in range(i+1, len(adaylar)+1):
                            alt_kume = adaylar[i:j]
                            if len(alt_kume) >= 2:
                                aralik = alt_kume[-1] - alt_kume[0]
                                pct = aralik / alt_kume[0]
                                if pct <= kume_esik_pct and aralik < en_kucuk_aralik:
                                    en_kucuk_aralik = aralik
                                    en_iyi_kume = alt_kume
                    return (round(min(en_iyi_kume), 2), round(max(en_iyi_kume), 2))

                liq_long_baskisi  = _kume_bul(long_seviyeler,  top_n=8, kume_esik_pct=0.06)
                liq_short_baskisi = _kume_bul(short_seviyeler, top_n=8, kume_esik_pct=0.06)
...
BOT_VERSIYON = "v494"
print(f"[BASLANGIC] ========== BOT VERSIYON: v494 ==========")
