...
# MEXC bildirimleri artik grubun TOPIC_MEXC topic'ine gidiyor
MEXC_NOTIFY_CHAT_ID = None  # Artik kullanilmiyor
...
def mexc_place_order(symbol, sinyal, tp1=None, sl=None):
    ...
    # Minimum mesafe: %0.1 (MEXC kucuk farklarda reddediyor)
    MIN_MESAFE = 0.001  # %0.1
    if tp1:
        try:
            tp1_f = round(float(str(tp1).replace(",", ".")), 8)
            tp_gecerli = (is_long and tp1_f > mp * (1 + MIN_MESAFE)) or \
                         (not is_long and tp1_f < mp * (1 - MIN_MESAFE))
            if tp_gecerli:
                body["takeProfitPrice"] = tp1_f
                body["takeProfitPriceType"] = 1
                print(f"[MEXC] TP1 eklendi: {tp1_f}")
            else:
                print(f"[MEXC] TP1 cok yakin, atlanıyor: mp={mp} tp1={tp1_f}")
        except Exception as e:
            print(f"[MEXC] TP1 hatasi: {e}")
    if sl:
        try:
            sl_f = round(float(str(sl).replace(",", ".")), 8)
            sl_gecerli = (is_long and sl_f < mp * (1 - MIN_MESAFE)) or \
                         (not is_long and sl_f > mp * (1 + MIN_MESAFE))
            if sl_gecerli:
                body["stopLossPrice"] = sl_f
                body["stopLossPriceType"] = 1
                print(f"[MEXC] SL eklendi: {sl_f}")
            else:
                print(f"[MEXC] SL cok yakin, atlanıyor: mp={mp} sl={sl_f}")
        except Exception as e:
            print(f"[MEXC] SL hatasi: {e}")
    ...
    try:
        ...
        if img_bytes:
            requests.post(f"{base}/sendPhoto",
                          data={"chat_id": TELEGRAM_GRUP_ID,
                                "message_thread_id": TOPIC_MEXC,
                                "caption": caption,
                                "parse_mode": "HTML"},
...
def _mexc_topic_mesaj_gonder(metin, parse_mode="HTML"):
    """MEXC islem bildirimlerini grubun TOPIC_MEXC topic'ine gonder."""
    return _telegram_topic_mesaj_gonder(TOPIC_MEXC, metin, parse_mode=parse_mode)


def _mexc_topic_foto_gonder(img_data, caption, parse_mode="HTML"):
    """MEXC islem fotografini grubun TOPIC_MEXC topic'ine gonder."""
    if not TELEGRAM_GRUP_ID:
        return None
    base = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}"
    try:
        ...
        print(f"[MEXC TOPIC] Foto gonderme hatasi: {e}")
        return None
...
def webhook():
    ...
    try:
        ...
        if isinstance(data, dict) and ("message" in data or "channel_post" in data):
            ...
            yetkili   = [x for x in [TELEGRAM_CHAT_ID, TELEGRAM_LOG_ID, TELEGRAM_GRUP_ID] if x]
...
print(f"[BASLANGIC] ========== BOT VERSIYON: v218 ==========")
