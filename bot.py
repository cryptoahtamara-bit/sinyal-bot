...
def _tp_kosul_yorum_uret(veri):
    ...
    def gercek_piyasa_verisi_var_mi(v):
        """Sadece seans değil, gerçek piyasa koşulu verisi mi?"""
        return not any(x in v[0] for x in ["Seans"])
    ...
    # 3. Seans ayrı satır — yeterli veri varsa
    seans_veri = {k: v for k, v in veri["kosul"].items()
                  if "Seans" in k and v[0] >= MIN_GENEL and oran_val(v) >= 0}
    if seans_veri:
        en_iyi_s = max(seans_veri.items(), key=lambda x: oran_val(x[1]))
        maddeler.append(f"🕐 En başarılı seans: <b>{en_iyi_s[0].replace('Seans —','').strip()}</b> → <b>{en_iyi_s[1][3]}</b> TP ({en_iyi_s[1][0]} sinyal)")
