...
def _tp_confluance_skoru_hesapla(veri):
    ...
    mega = {}  # coin hariç — piyasa koşulları (TF + sinyal + koşullar)
    ...
    for s in _kayitlar:
        ...
        # Sadece piyasa koşulları — coin yok
        parts = [tf, st]
    ...
    # Tavsiye yön — top3_mega üzerinden (tanımlandıktan sonra)
    LONG_KELIMELER  = ["Yükseliş", "Pozitif", "Strong Buy", "Long", "OI Artıyor",
                       "Aşırı Korku", "Korku", "Ortalama Üstü", "Aşırı Satım", "Satım Yakın"]
    SHORT_KELIMELER = ["Düşüş", "Negatif", "Strong Sell", "Short", "OI Azalıyor",
                       "Açgözlülük", "Aşırı Açgözlülük", "Ortalama Altı", "Aşırı Alım", "Alım Yakın"]

    long_agirlik  = 0.0
    short_agirlik = 0.0
    for item in top3_mega:
        komb = item["kombinasyon"]
        skor = (item["tp_sayisi"] / item["sinyal"]) * math.log(max(item["sinyal"], 1))
        # Her kombinasyon için net oy: LONG sayısı - SHORT sayısı
        l_say = sum(1 for kw in LONG_KELIMELER  if kw in komb)
        s_say = sum(1 for kw in SHORT_KELIMELER if kw in komb)
        if l_say > s_say:
            long_agirlik  += skor * (l_say - s_say)
        elif s_say > l_say:
            short_agirlik += skor * (s_say - l_say)
        # Eşitse nötr — hiçbir tarafa ekleme
...
def _tplog_gorsel_uret(kayitlar, tarih_str):
    """
    Her saat icin top3_mega kombinasyonlarini gosterir.
    Axes transform - yatay format, tight layout ile sag bosluğu keser.
    """
    try:
        import matplotlib
        matplotlib.use("Agg")
        ...
        YON_C = {"LONG": GREEN, "SHORT": RED, "NÖTR": AMBER}
        YON_L = {"LONG": "▲ LONG", "SHORT": "▼ SHORT", "NÖTR": "● NÖTR"}
        SR_C  = [GREEN, TEAL, TW]
        ...
        for k in kayitlar:
            ...
            if not top3:
                top5 = k.get("top5", [])
                item = top5[0] if top5 else {}
                kosul = item.get("kombinasyon", "—")
                if len(kosul) > 32: kosul = kosul[:30] + "…"
                rows.append((saat, "1.", kosul, item.get("tp_orani","—"),
                             str(item.get("tp_sayisi","—")) + "/" + str(item.get("sinyal","—")),
                             yon, True))
            else:
                for idx, item in enumerate(top3[:3]):
                    ...
                    if len(kosul) > 32: kosul = kosul[:30] + "…"
                    ilk = (idx == 0)
                    tp_sin = str(item.get("tp_sayisi","—")) + "/" + str(item.get("sinyal","—"))
                    rows.append((
                        saat if ilk else "",
                        str(idx+1) + ".",
                        kosul,
        ...
        n_satir = len(rows)
        # Sutun x pozisyonlari (axes koordinati 0-1)
        # Saat | # | Kosul | TP% | TP/Sin | Yon
        # Yon sutununun sag siniri = YON_SAG
        YON_SAG = 0.88
        XPOS  = [0.005, 0.055, 0.085, 0.60, 0.70, 0.80]
        ALIGN = ["left","left","left","right","center","left"]
        COLS  = ["Saat", "#", "Koşul", "TP%", "TP/Sin", "Yön"]

        satir_h = 1.0 / (n_satir + 2)
        fig_h = max(3.0, 1.0 + n_satir * 0.34)
        # figsize genisligi: YON_SAG oraninda — sag bosluğu onlemek icin
        fig_w = 11.0
        fig, ax = plt.subplots(figsize=(fig_w, fig_h))
        fig.patch.set_facecolor(BG)
        ax.set_facecolor(BG)
        ...
        ax.text(YON_SAG/2, 1.0 - satir_h*0.65,
                f"{n_kayit} saat  |  her saat icin top 3 mega kombinasyon",
                transform=ax.transAxes, ha="center", va="top",
                fontsize=7, color=TG)
        ...
        for cx, ca, ch in zip(XPOS, ALIGN, COLS):
            ax.text(cx, y_b + satir_h/2, ch,
                    transform=ax.transAxes, ha=ca, va="center",
                    fontsize=8, fontweight="bold", color=TW)

        # Veri satirlari
        grup = -1
        for i, (saat, sira, kosul, tp, tp_sin, yon, ilk) in enumerate(rows):
            if saat: grup += 1
            y_top = 1.0 - satir_h * (i + 2)
            y_mid = y_top + satir_h / 2
            ...
            yr = YON_C.get(yon, AMBER) if yon else TW
            yl = YON_L.get(yon, yon) if yon else ""
            sc = SR_C[int(sira[0])-1] if sira and sira[0].isdigit() else TW
            vals  = [saat, sira, kosul, tp, tp_sin, yl]
            clrs  = [TW, sc, TW, tr, TW, yr]
            bolds = [True, True, ilk, True, True, True]
            for cx, ca, val, clr, bold in zip(XPOS, ALIGN, vals, clrs, bolds):
                if not str(val): continue
                ax.text(cx, y_mid, str(val),
                        transform=ax.transAxes, ha=ca, va="center",
                        fontsize=7.5, fontweight="bold" if bold else "normal", color=clr)

        # Alt ozet
        ls = sum(1 for k in kayitlar if k.get("tavsiye_yon")=="LONG")
        ss = sum(1 for k in kayitlar if k.get("tavsiye_yon")=="SHORT")
        ns = sum(1 for k in kayitlar if k.get("tavsiye_yon")=="NÖTR")
        y_bot = 1.0 - satir_h * (n_satir + 2)
        ax.add_patch(FancyBboxPatch((0, y_bot), YON_SAG, satir_h,
                     boxstyle="square,pad=0", facecolor=HDR, edgecolor="none",
                     transform=ax.transAxes, clip_on=False))
        ax.text(YON_SAG/2, y_bot + satir_h/2,
                f"▲ LONG: {ls}    ▼ SHORT: {ss}    ● NÖTR: {ns}",
                transform=ax.transAxes, ha="center", va="center",
                fontsize=8, color=TW)

        # Sag taraf boslugunu kes: sadece 0..YON_SAG araligini kaydet
        plt.tight_layout(pad=0)
        buf = io.BytesIO()
        # ax boyutunu YON_SAG ile kisalt
