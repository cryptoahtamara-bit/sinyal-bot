...
def mexc_sign(body_dict, ts):
    # MEXC Futures Contract API imza:
    # Query string gonderim icin: APIKey + ts + sorted_query_string
    # POST JSON gonderim icin: APIKey + ts + body_json
    if body_dict:
        # Query string formati — alfabetik sirali
...
def mexc_private_get(url, params=None, timeout=10):
    """Private GET — query string imzasi"""
    hdrs, ts = mexc_headers(params)
...
def mexc_private_post(url, body, timeout=15):
    """Private POST — json={} + params=body ile WAF bypass, query string imzasi"""
    ...
    print(f"[MEXC POST] Status: {r.status_code} Body: {r.text[:100]}")
    return r
...
def mexc_test():
    ...
    sonuc = {"version": "v196", "proxy": bool(MEXC_PRIVATE_PROXY)}
    ...
    # 4. Imza debug — tam olarak ne imzalanıyor göster
    if MEXC_API_KEY and MEXC_API_SECRET:
        ...
        sign_str = MEXC_API_KEY + ts_dbg + param_str
        import hashlib as hl
        sig = hmac.new(MEXC_API_SECRET.encode(), sign_str.encode(), hashlib.sha256).hexdigest()
        sonuc["imza_debug"] = {
            "ts": ts_dbg,
            "param_str": param_str,
            "sign_str_uzunluk": len(sign_str),
            "signature": sig[:20] + "...",
            "api_key_bas": MEXC_API_KEY[:8] + "..."
        }
        # Gercek istek at ve sonucu goster
        try:
            hdrs_dbg = {
                "ApiKey": MEXC_API_KEY,
                "Request-Time": ts_dbg,
            ...
            r_dbg = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
                                  headers=hdrs_dbg, json={}, params=sorted_params, timeout=8)
            sonuc["imza_test_sonuc"] = {"status": r_dbg.status_code, "body": r_dbg.text[:200]}
        except Exception as e:
            sonuc["imza_test_sonuc"] = {"error": str(e)}
...
print(f"[BASLANGIC] ========== BOT VERSIYON: v196 ==========")
