...
def _tp_kosul_yorum_uret(veri):
    ...
    if kosul_veri:
        ...
        if fund_pos and fund_neg and oran_val(fund_pos) >= 0 and oran_val(fund_neg) >= 0:
            ...
            if abs(fark) >= 3:
                daha_iyi = "Pozitif" if fark > 0 else "Negatif"
                maddeler.append(
                    f"💰 Funding <b>{daha_iyi}</b> ortamda TP oranı <b>{abs(fark)} puan</b> daha yüksek "
                    f"({fund_pos[4]} vs {fund_neg[4]})"
                )
