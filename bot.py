...
def mexc_test():
    ...
    sonuc = {"version": "v184", "proxy": bool(MEXC_PRIVATE_PROXY)}
...
def webkey_test():
    ...
    # Test 4: Farkli body encode yontemleri
    if MEXC_API_KEY and MEXC_API_SECRET:
        import gzip, base64 as b64
        test_body = {"symbol": "BTC_USDT", "price": 0, "vol": 0.001, "side": 1, "type": 5, "openType": 2, "leverage": 20}
        
        # Yontem 1: data= ile string olarak
        try:
            hdrs1, _ = mexc_headers(test_body)
            r1 = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
                               headers=hdrs1, data=json.dumps(test_body, separators=(",",":")), timeout=8)
            sonuc["encode_data_string"] = {"status": r1.status_code, "body": r1.text[:100]}
        except Exception as e:
            sonuc["encode_data_string"] = {"error": str(e)[:50]}

        # Yontem 2: Content-Type olmadan
        try:
            hdrs2, _ = mexc_headers(test_body)
            hdrs2.pop("Content-Type", None)
            r2 = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
                               headers=hdrs2, data=json.dumps(test_body), timeout=8)
            sonuc["encode_no_content_type"] = {"status": r2.status_code, "body": r2.text[:100]}
        except Exception as e:
            sonuc["encode_no_content_type"] = {"error": str(e)[:50]}

        # Yontem 3: Body gzip ile sikistir
        try:
            body_bytes = json.dumps(test_body, separators=(",",":")).encode("utf-8")
            body_gz = gzip.compress(body_bytes)
            hdrs3, _ = mexc_headers(test_body)
            hdrs3["Content-Encoding"] = "gzip"
            r3 = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
                               headers=hdrs3, data=body_gz, timeout=8)
            sonuc["encode_gzip"] = {"status": r3.status_code, "body": r3.text[:100]}
        except Exception as e:
            sonuc["encode_gzip"] = {"error": str(e)[:50]}

        # Yontem 4: Chunked transfer
        try:
            hdrs4, _ = mexc_headers(test_body)
            hdrs4["Transfer-Encoding"] = "chunked"
            r4 = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
                               headers=hdrs4, json=test_body, timeout=8)
            sonuc["encode_chunked"] = {"status": r4.status_code, "body": r4.text[:100]}
        except Exception as e:
            sonuc["encode_chunked"] = {"error": str(e)[:50]}
...
print(f"[BASLANGIC] ========== BOT VERSIYON: v184 ==========")
