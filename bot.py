...
MEXC_BASE_URL_ALT  = "https://futures.mexc.com"
...
# Proxy devre disi — direkt Railway IP ile baglanti (bot_v175)
_proxy_user = os.getenv("IPROYAL_USER", "")
_proxy_pass = os.getenv("IPROYAL_PASS", "")
MEXC_PRIVATE_PROXY = None
print(f"[PROXY] MEXC_PRIVATE_PROXY aktif: {bool(MEXC_PRIVATE_PROXY)}")
...
def mexc_get_contract_info(symbol):
    ...
    try:
        ...
        if d.get("success"):
            ...
            print(f"[MEXC] Contract fields: fairPrice={data.get('fairPrice')} lastPrice={data.get('lastPrice')} maxLev={lev} volDec={vol_decimals}")
            if mp > 0:
                return mp, lev, vol_decimals, contract_size
    except Exception as e:
        print(f"[MEXC] Contract info hatasi: {e}")
    # Fallback: eski mark price fonksiyonu
    mp_fallback = get_mexc_price(symbol)
    return mp_fallback, 20, 4, 1
...
def mexc_private_get(url, params=None, timeout=10):
    """Private GET — bos string imzasi"""
    hdrs, ts = mexc_headers(None)
...
def mexc_private_post(url, body, timeout=15):
    """Private POST — requests'in urettigi body ile ayni imza (WAF bypass)"""
    ts = str(int(time.time() * 1000))
    sorted_body = dict(sorted(body.items()))
    # requests json= ile gonderirken bosluklu format kullanir: {"key": value}
    # Imzayi ayni formatta hesaplamaliyiz
    body_json = json.dumps(sorted_body, sort_keys=True)  # varsayilan: ", " ve ": " ayraclari
    ...
    r = requests.post(url, headers=hdrs, json=sorted_body, timeout=timeout)
...
def mexc_place_order(symbol, sinyal, tp1=None, sl=None):
    ...
    side = 1 if is_long else 2

    mp, lev, vol_decimals, contract_size = mexc_get_contract_info(symbol)
    print(f"[MEXC] Mark Price ham: {mp}")
    print(f"[MEXC] Leverage: {lev} | Vol Decimals: {vol_decimals} | Contract Size: {contract_size}")
    ...
    # Vol hesabi — MEXC kontrat adedi (tam sayi) olarak hesapla
    # contractSize API'den gelemiyor (cs=1 default), bu yuzden direkt lot hesapla
    # Vol = marjin * leverage / mark_price → tam sayıya yuvarla, minimum 1
    notional = MEXC_MARGIN_USDT * lev
    vol_raw = notional / mp
    vol = max(1, int(vol_raw))  # tam sayi, minimum 1 kontrat

    print(f"[MEXC] Vol hesabi: notional={notional} mp={mp} vol_raw={vol_raw:.4f} -> vol={vol}")

    body = {"symbol": sym, "price": 0, "vol": vol, "side": side,
            "type": 5, "openType": 2, "leverage": lev}

    # TP/SL — MEXC kurallari:
    # LONG: takeProfitPrice > mp, stopLossPrice < mp
    # SHORT: takeProfitPrice < mp, stopLossPrice > mp
    # TP veya SL gecersizse islem ACILMAZ
    if not tp1 or not sl:
        return {"success": False, "msg": "TP1 veya SL eksik, islem acilmadi"}

    try:
        tp1_f = round(float(str(tp1).replace(",", ".")), 8)
        sl_f = round(float(str(sl).replace(",", ".")), 8)
    except Exception as e:
        return {"success": False, "msg": f"TP/SL format hatasi: {e}"}

    tp_gecerli = (is_long and tp1_f > mp) or (not is_long and tp1_f < mp)
    sl_gecerli = (is_long and sl_f < mp) or (not is_long and sl_f > mp)

    if not tp_gecerli:
        print(f"[MEXC] TP1 gecersiz yonde: mp={mp} tp1={tp1_f}")
        return {"success": False, "msg": f"TP1 gecersiz yonde (mp={mp} tp1={tp1_f}), islem atlandı"}
    if not sl_gecerli:
        print(f"[MEXC] SL gecersiz yonde: mp={mp} sl={sl_f}")
        return {"success": False, "msg": f"SL gecersiz yonde (mp={mp} sl={sl_f}), islem atlandı"}

    body["takeProfitPrice"] = tp1_f
    body["takeProfitPriceType"] = 2
    # takeProfitVol cross margin'de desteklenmiyor — kaldirildi
    body["stopLossPrice"] = sl_f
    body["stopLossPriceType"] = 2
    print(f"[MEXC] TP1 eklendi: {tp1_f} | SL eklendi: {sl_f}")

    try:
        r = mexc_private_post(f"{MEXC_BASE_URL}/api/v1/private/order/submit", body)
        ...
        print(f"[MEXC] Emir hatasi traceback: {traceback.format_exc()}")
...
def koin_sirala_bugun(snapshot=None):
    ...
    Her koin için: başarı oranı, toplam işlem, ağırlıklı yön (LONG/SHORT) döndür.
    """
    bugun = gun_str()
    with gunluk_kilit:
        kayitlar = [s for s in (snapshot or gunluk_sinyaller) if s.get("gun") == bugun]

    koin_istat = {}
    for s in kayitlar:
        sym = s.get("symbol", "").upper().replace("USDT.P", "").replace("USDT", "")
        if not sym:
            continue
    ...
    for sym, v in koin_istat.items():
        ...
        oran = round(v["tp"] / toplam * 100, 1) if toplam > 0 else 0
...
def mexc_test():
    ...
    sonuc = {"version": "v217", "proxy": bool(MEXC_PRIVATE_PROXY)}
    ...
    if MEXC_API_KEY and MEXC_API_SECRET:
        ...
        try:
            ...
            r_a = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
        ...
        try:
            ...
            hdrs_b = {"ApiKey": MEXC_API_KEY, "Request-Time": ts_dbg, "Signature": sig_b,
                      ...
                      "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
                      "Origin": "https://futures.mexc.com", "Referer": "https://futures.mexc.com/exchange/BTC_USDT"}
            r_b = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
        ...
        try:
            r_c = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
    ...
    for domain in ["contract.mexc.com", "futures.mexc.com"]:
        try:
            test_url = f"https://{domain}/api/v1/private/order/submit"
...
def webkey_test():
    ...
    try:
        ...
        r1 = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
    ...
    try:
        ...
        r2 = requests.post(f"{MEXC_BASE_URL_ALT}/api/v1/private/order/submit",
    ...
    if MEXC_API_KEY and MEXC_API_SECRET:
        try:
            ...
            r3 = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
    ...
    if MEXC_API_KEY and MEXC_API_SECRET:
        ...
        try:
            r = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
...
print(f"[BASLANGIC] ========== BOT VERSIYON: v226 ==========")
