...
def mexc_test():
    ...
    sonuc = {"version": "v189", "proxy": bool(MEXC_PRIVATE_PROXY)}
...
def webkey_test():
    ...
    # Test 4: Bos body + gercek parametreler query string'de
    if MEXC_API_KEY and MEXC_API_SECRET:
        test_body = {"symbol": "BTC_USDT", "price": 0, "vol": 0.001, "side": 1, "type": 5, "openType": 2, "leverage": 20}

        # Yontem 1: json={} + params=test_body (query string)
        try:
            hdrs1, _ = mexc_headers({})
            hdrs1["User-Agent"] = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
            hdrs1["Origin"] = "https://futures.mexc.com"
            hdrs1["Referer"] = "https://futures.mexc.com/exchange/BTC_USDT"
            r1 = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
                               headers=hdrs1, json={}, params=test_body, timeout=8)
            sonuc["bos_json_query_params"] = {"status": r1.status_code, "body": r1.text[:200]}
        except Exception as e:
            sonuc["bos_json_query_params"] = {"error": str(e)[:50]}

        # Yontem 2: Sadece params, json yok
        try:
            hdrs2, _ = mexc_headers(None)
            hdrs2["User-Agent"] = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
            hdrs2["Origin"] = "https://futures.mexc.com"
            hdrs2["Referer"] = "https://futures.mexc.com/exchange/BTC_USDT"
            r2 = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
                               headers=hdrs2, params=test_body, timeout=8)
            sonuc["sadece_query_params"] = {"status": r2.status_code, "body": r2.text[:200]}
        except Exception as e:
            sonuc["sadece_query_params"] = {"error": str(e)[:50]}

        # Yontem 3: URL'e direkt parametreleri ekle
        try:
            import urllib.parse
            query = urllib.parse.urlencode(test_body)
            url = f"{MEXC_BASE_URL}/api/v1/private/order/submit?{query}"
            hdrs3, _ = mexc_headers(None)
            hdrs3["User-Agent"] = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
            hdrs3["Origin"] = "https://futures.mexc.com"
            hdrs3["Referer"] = "https://futures.mexc.com/exchange/BTC_USDT"
            r3 = requests.post(url, headers=hdrs3, timeout=8)
            sonuc["params_in_url"] = {"status": r3.status_code, "body": r3.text[:200]}
        except Exception as e:
            sonuc["params_in_url"] = {"error": str(e)[:50]}
...
print(f"[BASLANGIC] ========== BOT VERSIYON: v189 ==========")
