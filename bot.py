                ...
                if chat_id in yetkili and thread_id == TOPIC_RAPOR:
                    ...
                    def _rapor_komut_gonder():
                        try:
                            ...
                            img_c = rapor_gorsel(gun, snapshot=snapshot)
                            if img_c:
                                _topic_foto_gonder_filigranli(TOPIC_RAPOR, img_c, f"Günlük Rapor — {gun}\n/rapor")
                                print("[KOMUT/rapor] Panel C gönderildi.")
                            else:
                                _telegram_topic_mesaj_gonder(TOPIC_RAPOR, rapor_mesaji(gun))
                            time.sleep(5)

                            img_d = _panel_d_gorsel(snapshot=snapshot)
                            if img_d:
                                _topic_foto_gonder_filigranli(TOPIC_RAPOR, img_d, "📋 Parite Detayı")
                                print("[KOMUT/rapor] Panel D gönderildi.")
                            time.sleep(5)
...
def _panel_a_gorsel(snapshot=None):
    """v631: PANEL A — Genel Durum (İstatistik + Piyasa Rejimi + Whitelist).
    Eski istatistik_mesaji + koin_sirala_metni metinlerinin yerini alır."""
    try:
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as plt
    ...
    try:
        ...
        for s in tum_kayitlar:
            sym = sembol_grup(s["symbol"])
            sym_stats_t.setdefault(sym, {"toplam": 0, "basarili": 0})
            sym_stats_t[sym]["toplam"] += 1
            if _basarili(s): sym_stats_t[sym]["basarili"] += 1
        en_sym_t = max(
            ((sym, st["basarili"]/st["toplam"]*100) for sym, st in sym_stats_t.items() if st["toplam"] >= 50),
            key=lambda x: x[1], default=("—", -1))
        ...
        for s in tum_kayitlar:
            tf = s.get("timeframe", "?")
            tf_stats_t.setdefault(tf, {"toplam": 0, "basarili": 0})
            tf_stats_t[tf]["toplam"] += 1
            if _basarili(s): tf_stats_t[tf]["basarili"] += 1
        en_tf_t = max(
            ((tf_map_g.get(tf, tf), st["basarili"]/st["toplam"]*100) for tf, st in tf_stats_t.items() if st["toplam"] >= 50),
            key=lambda x: x[1], default=("—", -1))

        # Piyasa rejimi kırılımı (kapanan sinyaller, filtre_veri kaynağı)
        # v634 DUZELTME: onceden sadece tp1_ok==True veya sl_ok==True olanlar
        # sayiliyordu — "kontrol suresi doldu ama ne TP ne SL" olan sinyaller
        # (filtre_veri.json'da tp1_ok=False, sl_ok=False olarak KALICI yazilan
        # kayitlar) paydadan tamamen dislaniyordu, bu da oranlari yapay olarak
        # %97-100'e sisiriyordu. Artik istatistik_hesapla ile AYNI tanim
        # kullaniliyor: kontrolu TAMAMLANMIS (tp1_ok is not None) TUM sinyaller
        # paydaya dahil, sadece gercekten TP olanlar basari sayiliyor.
        with gunluk_kilit:
            filtre_snap = filtre_sinyaller[:]
        kapanan_tum = [s for s in filtre_snap if s.get("tp1_ok") is not None]
        piy_gruplar = {}
        for s in kapanan_tum:
            piy = s.get("piyasa_yonu") or "Bilinmiyor"
            piy_gruplar.setdefault(piy, {"tp": 0, "n": 0})
            piy_gruplar[piy]["n"] += 1
            if s.get("tp1_ok") == True:
                piy_gruplar[piy]["tp"] += 1
        piy_sirali = sorted(piy_gruplar.items(), key=lambda kv: -kv[1]["n"])
        ...
        ax.text(0.5, 0.975, "Ben Kül Yutmam — Genel Durum", fontsize=19, fontweight="bold",
        ...
        ax.text(0.5, 0.925, f"{_simdi_str} · Panel A / 3", fontsize=11, color=HDR_COL, ha="center", va="top")
        ...
        def kart(x, y, w, h, baslik, deger, renk):
            ...
            ax.text(x + w/2, y + h*0.62, baslik, transform=ax.transAxes, fontsize=8.7,
                    color=HDR_COL, ha="center", va="center")
            ax.text(x + w/2, y + h*0.28, str(deger), transform=ax.transAxes, fontsize=14,
                    fontweight="bold", color=renk, ha="center", va="center")

        ax.text(0.02, 0.865, "BUGÜN", fontsize=10, fontweight="bold", color="#D0D3DB", ha="left", va="top")
        kart_w = 0.184; gap = 0.008; x0 = 0.02
        bugun_kartlar = [
            ("Sinyal", b["toplam"], TEXT_W), ("TP", b["tp_basarili"], GREEN),
            ("SL", b["sl_tetiklenen"], RED), ("Devam", b["devam_eden"], AMBER),
            ("Başarı", f"%{b['basari_oran']}", BLUE),
        ]
        for i, (bas, deg, rnk) in enumerate(bugun_kartlar):
            kart(x0 + i*(kart_w+gap), 0.735, kart_w, 0.10, bas, deg, rnk)

        ax.text(0.02, 0.685, "TÜM ZAMANLAR", fontsize=10, fontweight="bold", color="#D0D3DB", ha="left", va="top")
        tum_kartlar = [
            ("Sinyal", t["toplam"], TEXT_W), ("TP", t["tp_basarili"], GREEN),
            ("SL", t["sl_tetiklenen"], RED),
        ...
        for i, (bas, deg, rnk) in enumerate(tum_kartlar):
            kart(x0 + i*(kart_w+gap), 0.555, kart_w, 0.10, bas, deg, rnk)

        # Piyasa Rejimi kutusu (sol)
        ax.add_patch(plt.Rectangle((0.02, 0.06), 0.47, 0.42, transform=ax.transAxes,
                     facecolor=CARD_BG, edgecolor="none", zorder=1))
        ax.text(0.045, 0.45, "Piyasa Rejimi Bazlı Başarı", fontsize=11.5, fontweight="bold",
                color=GOLD, ha="left", va="top")
        yy = 0.385
        for piy, v in piy_sirali:
            oran = v["tp"]/v["n"]*100 if v["n"] else 0
            renk = GREEN if oran >= 60 else (AMBER if oran >= 35 else RED)
            ax.text(0.06, yy, f"{piy}  ({v['n']} sinyal)", fontsize=10, color=TEXT_W, ha="left", va="top")
            ax.text(0.465, yy, f"%{oran:.1f} ({v['tp']}/{v['n']})", fontsize=10, color=renk, ha="right", va="top")
            yy -= 0.058
        ax.text(0.045, 0.10, "Az örnek — henüz erken" if kapanan_tum and max(v["n"] for _,v in piy_sirali) < 50 else "",
                fontsize=8.5, color=HDR_COL, ha="left", va="top")

        # Whitelist kutusu (sağ)
        ax.add_patch(plt.Rectangle((0.51, 0.06), 0.47, 0.42, transform=ax.transAxes,
                     facecolor=CARD_BG, edgecolor="none", zorder=1))
        ax.text(0.535, 0.45, "Whitelist Durumu", fontsize=11.5, fontweight="bold",
                color=GOLD, ha="left", va="top")
        if wl_semboller:
            ax.text(0.55, 0.37, "✓  " + ", ".join(wl_semboller), fontsize=12, color=GREEN,
                    ha="left", va="top", fontweight="bold")
            ax.text(0.55, 0.30, f"(%{TRADE_WHITELIST_MIN_PERCENT}+)", fontsize=9.5, color=HDR_COL, ha="left", va="top")
        else:
            ax.text(0.55, 0.37, "Henüz yeterli veri yok", fontsize=11, color=HDR_COL, ha="left", va="top")
...
def _panel_b_gorsel(snapshot=None):
    """v631: PANEL B — Sinyal Kalitesi (Sinyal Tipi Detayı + Kombinasyon + Coin Bazlı Güçlü).
    Eski Kombinasyon Sıralaması + Sinyal Tipi Detayı görsellerinin yerini alır.
    Not: Eski görselde ilk 3 satır hep aynı (yanıltıcı) değerleri gösteriyordu —
    bu yüzden burada SADECE gerçekten hesaplanan aktif kombinasyon gösteriliyor."""
    try:
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as plt
    ...
    try:
        ...
        # Sinyal Tipi Detayı satırları (adetli)
        detay_satirlari = []
        for st in SINYAL_TIPLERI:
            kapali = [s for s in kapali_tum if s.get("sinyal_tipi") == st]
            g_ort = genel_tp[st]
        ...
        # Aktif kombinasyon özeti (Güçlü/Zayıf/Fark/Kapsama/Doğru) — GERÇEK hesap
        ...
        ax.text(0.5, 0.975, "Ben Kül Yutmam — Sinyal Kalitesi", fontsize=19, fontweight="bold",
                color=GOLD, ha="center", va="top")
        ax.text(0.5, 0.93, "Aktif Kombinasyon: TF + Funding + OI + Piyasa · Panel B / 3",
                fontsize=10.5, color=HDR_COL, ha="center", va="top")

        # Sinyal Tipi Detayı kutusu
        ax.add_patch(plt.Rectangle((0.02, 0.60), 0.96, 0.30, transform=ax.transAxes,
                     facecolor=CARD_BG, edgecolor="none", zorder=1))
        ax.text(0.04, 0.885, f"Sinyal Tipi Detayı — {len(kapali_tum)} Sinyal, {len(kapanan_tum)} Kapanan",
                fontsize=11.5, fontweight="bold", color=GOLD, ha="left", va="top")
        basliklar = ["Tip","Genel","Güçlü","Orta","Zayıf"]
        kol_x     = [0.04, 0.27, 0.50, 0.71, 0.94]
        for bx, bl in zip(kol_x, basliklar):
            ax.text(bx, 0.845, bl, fontsize=9, color=HDR_COL, ha="left" if bx==0.04 else "right", va="top")
        yy = 0.80
        for row in detay_satirlari:
            ...
            ax.text(0.04, yy, row["tip"], fontsize=9.5, color=TEXT_W, ha="left", va="top")
            ax.text(0.27, yy, f"%{row['genel']:.1f}", fontsize=9.5, color=TEXT_W, ha="right", va="top")
            ax.text(0.50, yy, f"%{go:.1f} ({row['g_tp']}/{row['g_n']})", fontsize=9.5, color=GREEN, ha="right", va="top")
            ax.text(0.71, yy, f"%{oo:.1f} ({row['o_tp']}/{row['o_n']})", fontsize=9.5, color=AMBER, ha="right", va="top")
            ax.text(0.94, yy, f"%{zo:.1f} ({row['z_tp']}/{row['z_n']})", fontsize=9.5, color=RED, ha="right", va="top")
            yy -= 0.052

        # Kombinasyon özeti kutusu (Puan yok — gerçek bileşenler)
        ax.add_patch(plt.Rectangle((0.02, 0.35), 0.96, 0.22, transform=ax.transAxes,
                     facecolor=CARD_BG, edgecolor="none", zorder=1))
        ax.text(0.04, 0.545, "Aktif Kombinasyon — Gerçek Bileşenler", fontsize=11.5, fontweight="bold",
                color=GOLD, ha="left", va="top")
        bilesenler = [
        ...
        for i,(bas,deg,rnk) in enumerate(bilesenler):
            bx = 0.04 + i*(bw+0.006)
            ax.text(bx, 0.48, bas, fontsize=9, color=HDR_COL, ha="left", va="top")
            ax.text(bx, 0.42, deg, fontsize=13, fontweight="bold", color=rnk, ha="left", va="top")

        # v644: Coin Bazlı — iki sütun (Güçlü Sinyal | Güven Skoru Yüksek)
        ax.add_patch(plt.Rectangle((0.02, 0.03), 0.96, 0.29, transform=ax.transAxes,
                     facecolor=CARD_BG, edgecolor="none", zorder=1))
        ax.plot([0.50, 0.50], [0.045, 0.305], color=BG, linewidth=1.3, zorder=2)
        ...
        ax.text(0.04, 0.305, "Güçlü Sinyal % TP Oranı", fontsize=11, fontweight="bold",
                color=GOLD, ha="left", va="top")
        ax.text(0.47, 0.268, f"Genel: %{g_genel:.1f} ({gt_toplam}/{gn_toplam})", fontsize=9.5,
        ...
        for bx, bl in zip(sol_kol_x, ["Coin","Sinyal","TP","% Oranı"]):
            ax.text(bx, 0.232, bl, fontsize=8.5, fontweight="bold", color=TEXT_W, ha="left", va="top")
        yy = 0.196
        for it in coin_sirala_guclu:
            gn, gtp = it.get("guclu_n",0), it.get("guclu_tp",0)
            oran = gtp/gn*100 if gn else 0
            ax.text(sol_kol_x[0], yy, it["sym"], fontsize=9, color=TEXT_W, ha="left", va="top")
            ax.text(sol_kol_x[1], yy, str(gn), fontsize=9, color=TEXT_W, ha="left", va="top")
            ax.text(sol_kol_x[2], yy, str(gtp), fontsize=9, color=TEXT_W, ha="left", va="top")
            ax.text(sol_kol_x[3], yy, f"%{oran:.1f}", fontsize=9, fontweight="bold",
                    color=_renk_oran(oran), ha="left", va="top")
            yy -= 0.030
        if not coin_sirala_guclu:
            ax.text(sol_kol_x[0], yy, "Az Veri", fontsize=9, color=HDR_COL, ha="left", va="top")
        ...
        ax.text(0.53, 0.305, "Güven Skoru Yüksek % TP Oranı", fontsize=11, fontweight="bold",
                color=GOLD, ha="left", va="top")
        ax.text(0.96, 0.268, f"Genel: %{v_genel:.1f} ({vt_toplam}/{vn_toplam})", fontsize=9.5,
        ...
        for it in coin_sirala_guven:
            ax.text(sag_kol_x[0], yy, it["sym"], fontsize=9, color=TEXT_W, ha="left", va="top")
            ax.text(sag_kol_x[1], yy, f"{it['ort_skor']:.2f}", fontsize=9, color=TEXT_W, ha="left", va="top")
            ax.text(sag_kol_x[2], yy, str(it["n"]), fontsize=9, color=TEXT_W, ha="left", va="top")
            ax.text(sag_kol_x[3], yy, str(it["tp"]), fontsize=9, color=TEXT_W, ha="left", va="top")
            ax.text(sag_kol_x[4], yy, f"%{it['oran']:.1f}", fontsize=9, fontweight="bold",
                    color=_renk_oran(it["oran"]), ha="left", va="top")
            yy -= 0.030
        if not coin_sirala_guven:
            ax.text(sag_kol_x[0], yy, "Az Veri", fontsize=9, color=HDR_COL, ha="left", va="top")
...
def _istatistik_zamanlayici():
    ...
    while True:
        try:
            ...
            # 3) PANEL C — Parite × TF (mevcut Günlük Rapor görseli)
            try:
                img_c = rapor_gorsel(bugun, snapshot=snapshot)
                if img_c:
                    _topic_foto_gonder_filigranli(TOPIC_RAPOR, img_c, f"Günlük Rapor — {bugun}\n/rapor")
                    print("[PANEL_C] Gönderildi.")
                else:
                    print(f"[PANEL_C] Üretilemedi (None) — '{bugun}' için kayıt bulunamadı olabilir.")
            except Exception as _ec:
                print(f"[PANEL_C] Hata: {_ec}\n{traceback.format_exc()}")
            time.sleep(5)

            # 4) PANEL D — Parite Detayı (coin bazlı tek tablo)
            try:
                img_d = _panel_d_gorsel(snapshot=snapshot)
                if img_d:
                    _topic_foto_gonder_filigranli(TOPIC_RAPOR, img_d, "📋 Parite Detayı")
                    print("[PANEL_D] Gönderildi.")
                else:
                    print("[PANEL_D] Üretilemedi (None).")
            except Exception as _ed:
                print(f"[PANEL_D] Hata: {_ed}\n{traceback.format_exc()}")
            time.sleep(5)
        except Exception as e:
