...
def sinyal_kaydet(symbol, sinyal, timeframe, price, tp1, tp2, tp3, tp4, tp5, sl, message_id):
    ...
    # RSI — coin bazlı analiz cache'inden oku (symbol format: ETHUSDT.P → ETHUSDT)
    try:
        _sym_clean = symbol.replace(".P", "").replace("USDT", "USDT")
        rsi_deger = _son_analiz_veriler.get(_sym_clean, {}).get("rsi")
        if rsi_deger is None:
            # Alternatif: USDT olmadan dene
            _sym_base = symbol.replace(".P", "")
            rsi_deger = _son_analiz_veriler.get(_sym_base, {}).get("rsi")
