...
def format_mesaj(symbol, price, timeframe, sinyal, tp1=None, tp2=None, tp3=None,
                 tp4=None, tp5=None, tp6=None, tp7=None, tp8=None, tp9=None, tp10=None, sl=None):
    tf_map = {
    ...
    tp_listesi = [tp1, tp2, tp3, tp4, tp5, tp6, tp7, tp8, tp9, tp10]
...
def tp_kontrol_gonder(symbol, sinyal, timeframe, tp1, tp2, tp3, tp4, tp5, tp6, tp7, tp8, tp9, tp10, sl,
    ...
    def mesaj_olustur(tp1_ok, tp2_ok, tp3_ok, tp4_ok, tp5_ok, tp6_ok, tp7_ok, tp8_ok, tp9_ok, tp10_ok, sl_ok, erken=False):
        ...
        tp_kontroller = [
            ...
            (tp6, tp6_ok, "TP6"), (tp7, tp7_ok, "TP7"), (tp8, tp8_ok, "TP8"),
            (tp9, tp9_ok, "TP9"), (tp10, tp10_ok, "TP10"),
        ]
    ...
    while gecen_sure < toplam_sure:
        ...
        tp6_ok = tp_ulasti(tp6, yuksek, dusuk)
        tp7_ok = tp_ulasti(tp7, yuksek, dusuk)
        tp8_ok = tp_ulasti(tp8, yuksek, dusuk)
        tp9_ok = tp_ulasti(tp9, yuksek, dusuk)
        tp10_ok = tp_ulasti(tp10, yuksek, dusuk)
        ...
        if (any([tp1_ok, tp2_ok, tp3_ok, tp4_ok, tp5_ok]) or sl_ok) and not erken_bildirim_gonderildi:
            msg = mesaj_olustur(tp1_ok, tp2_ok, tp3_ok, tp4_ok, tp5_ok, tp6_ok, tp7_ok, tp8_ok, tp9_ok, tp10_ok, sl_ok, erken=True)
            bildirim_gonder(msg)
    ...
    if not erken_bildirim_gonderildi:
        msg = mesaj_olustur(tp1_ok, tp2_ok, tp3_ok, tp4_ok, tp5_ok, tp6_ok, tp7_ok, tp8_ok, tp9_ok, tp10_ok, sl_ok, erken=False)
        bildirim_gonder(msg)
...
def send_telegram_and_schedule_tp(caption, symbol, timeframe, sinyal,
                                   tp1, tp2, tp3, tp4, tp5, tp6, tp7, tp8, tp9, tp10, sl,
    ...
    tp_var = any(x and str(x) != 'null' for x in [tp1, tp2, tp3, tp4, tp5, tp6, tp7, tp8, tp9, tp10])
    if message_id and tp_var:
        sinyal_ts  = int(time.time())
        kontrol_dk = tp_sure(timeframe)
        t = threading.Thread(
            target=tp_kontrol_gonder,
            args=(symbol, sinyal, timeframe, tp1, tp2, tp3, tp4, tp5, tp6, tp7, tp8, tp9, tp10, sl,
...
def parse_plain(raw: str):
    ...
    for line in lines:
        ...
        elif line.startswith("TP6 "): tp6 = line[4:].strip()
        elif line.startswith("TP7 "): tp7 = line[4:].strip()
        elif line.startswith("TP8 "): tp8 = line[4:].strip()
        elif line.startswith("TP9 "): tp9 = line[4:].strip()
        elif line.startswith("TP10 "): tp10 = line[5:].strip()
        elif line.lower().startswith("cikis:"):
            sl = line.split(":", 1)[1].strip()
        elif line.lower().startswith("giris:"):
            price = line.split(":", 1)[1].strip()
    return symbol, price, timeframe, sinyal if sinyal else "SINYAL", tp1, tp2, tp3, tp4, tp5, tp6, tp7, tp8, tp9, tp10, sl
...
def webhook():
    ...
    try:
        ...
        if isinstance(data, dict) and "symbol" in data:
            ...
            tp6 = _clean(data.get("tp6"))
            tp7 = _clean(data.get("tp7"))
            tp8 = _clean(data.get("tp8"))
            tp9 = _clean(data.get("tp9"))
            tp10 = _clean(data.get("tp10"))
            sl  = _clean(data.get("sl", data.get("cikis")))
        else:
            raise ValueError
    except Exception:
        symbol, price, timeframe, sinyal, tp1, tp2, tp3, tp4, tp5, tp6, tp7, tp8, tp9, tp10, sl = parse_plain(raw)
        imageurl = None
    ...
    print(f"[SINYAL] {symbol} {sinyal} @ {price} ({timeframe}) | TP1={tp1} TP10={tp10} SL={sl}")
    mesaj = format_mesaj(symbol, price, timeframe, sinyal, tp1, tp2, tp3, tp4, tp5, tp6, tp7, tp8, tp9, tp10, sl)

    t = threading.Thread(
        target=send_telegram_and_schedule_tp,
        args=(mesaj, symbol, timeframe, sinyal, tp1, tp2, tp3, tp4, tp5, tp6, tp7, tp8, tp9, tp10, sl, imageurl, price))
    t.daemon = True
    t.start()
