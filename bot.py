...
def _gunluk_pnl_gorsel_olustur(veri: dict) -> bytes:
    """PNL ozet kart gorselini PNG bytes olarak olusturur (matplotlib)."""
    try:
        import matplotlib
        matplotlib.use("Agg")
        ...
        bakiye       = veri.get("bakiye", 0)
        kullanilan   = veri.get("kullanilan", 0)
        realized     = veri.get("realized", 0)
        unrealized   = veri.get("unrealized", 0)
        saatlik_pnl  = veri.get("saatlik_pnl", [])  # list of (saat, pnl)
        zaman_str    = veri.get("zaman_str", "")

        fig = plt.figure(figsize=(10, 6), facecolor="#0d1117")
        gs  = GridSpec(2, 3, figure=fig, hspace=0.5, wspace=0.4)

        # --- Kart renkleri ---
        realized_renk   = "#26a69a" if realized   >= 0 else "#ef5350"
        unrealized_renk = "#26a69a" if unrealized >= 0 else "#ef5350"

        def kart(ax, baslik, deger, renk, birim="USDT"):
            ax.set_facecolor("#161b22")
            ax.set_xticks([]); ax.set_yticks([])
            for spine in ax.spines.values():
                spine.set_edgecolor("#30363d")
            ax.text(0.5, 0.72, baslik, transform=ax.transAxes,
                    ha="center", va="center", fontsize=9,
                    color="#8b949e", fontweight="bold")
            isaretli = f"{deger:+.2f}" if isinstance(deger, float) and "PNL" in baslik else f"{deger:.2f}"
            ax.text(0.5, 0.38, f"{isaretli}", transform=ax.transAxes,
                    ha="center", va="center", fontsize=14,
                    color=renk, fontweight="bold")
            ax.text(0.5, 0.12, birim, transform=ax.transAxes,
                    ha="center", va="center", fontsize=8, color="#484f58")

        # Ust 3 kart
        ax1 = fig.add_subplot(gs[0, 0]); kart(ax1, "Toplam Bakiye",    bakiye,     "#e6edf3")
        ax2 = fig.add_subplot(gs[0, 1]); kart(ax2, "Kullanilan Marjin", kullanilan, "#e6edf3")
        ax3 = fig.add_subplot(gs[0, 2]); kart(ax3, "Acik Poz. PNL",    unrealized, unrealized_renk)

        # Alt 3 kart: gerceklesen PNL + bos + bos
        ax4 = fig.add_subplot(gs[1, 0]); kart(ax4, "Gerceklesen PNL",  realized,   realized_renk)

        # Saatlik PNL grafigi (alt orta + sag)
        ...
        if saatlik_pnl:
            saatlar = [x[0] for x in saatlik_pnl]
            pnller  = [x[1] for x in saatlik_pnl]
            renkler = ["#26a69a" if p >= 0 else "#ef5350" for p in pnller]
            bars = ax5.bar(range(len(saatlar)), pnller, color=renkler, width=0.7)
            ax5.set_xticks(range(len(saatlar)))
            ax5.set_xticklabels(saatlar, fontsize=6, color="#8b949e", rotation=45)
            ax5.tick_params(colors="#8b949e", labelsize=6)
            ax5.axhline(0, color="#30363d", linewidth=0.8)
            ax5.set_title("Saatlik PNL", color="#8b949e", fontsize=8, pad=4)
        else:
            ax5.text(0.5, 0.5, "Veri yok", transform=ax5.transAxes,
                     ha="center", va="center", color="#484f58", fontsize=9)
            ax5.set_title("Saatlik PNL", color="#8b949e", fontsize=8, pad=4)
...
def _mexc_saatlik_pnl_cek(headers: dict) -> list:
    """Bugunun saatlik gerceklesen PNL listesini MEXC'ten ceker.
    Returns: [(saat_str, pnl_float), ...]
    """
    try:
        if TR_TZ:
            bugun_baslangic = datetime.now(tz=TR_TZ).replace(
                hour=0, minute=0, second=0, microsecond=0)
            import calendar
            start_ts = int(bugun_baslangic.timestamp()) * 1000
        else:
            import datetime as dt_mod
            now = dt_mod.datetime.utcnow()
            bugun_start = now.replace(hour=0, minute=0, second=0, microsecond=0)
            start_ts = int(bugun_start.timestamp()) * 1000

        end_ts = int(time.time() * 1000)

        r = requests.get(
            f"{MEXC_BASE_URL}/api/v1/private/account/profit_and_loss",
            headers=headers,
            params={"startTime": start_ts, "endTime": end_ts, "pageNum": 1, "pageSize": 50},
            timeout=10
        )
        result = r.json()
        ...
        kayitlar = result.get("data", {}).get("resultList", [])
        # saate gore grupla
        saatlik = {}
        for k in kayitlar:
            ts_ms = k.get("createTime", 0)
            if TR_TZ:
                dt = datetime.fromtimestamp(ts_ms / 1000, tz=TR_TZ)
            else:
                import datetime as dt_mod
                dt = dt_mod.datetime.utcfromtimestamp(ts_ms / 1000)
            saat = dt.strftime("%H:00")
            pnl  = float(k.get("realizedPnl", 0))
            saatlik[saat] = saatlik.get(saat, 0) + pnl

        # sirala
        sirali = sorted(saatlik.items(), key=lambda x: x[0])
        return sirali
    except Exception as e:
        print(f"[PNL SAATLIK] Hata: {e}")
        return []


def _gunluk_pnl_gonder(hedef_chat_id):
    """MEXC Futures gunluk PNL ozet kartini Telegram'a gonderir."""
    if not MEXC_API_KEY or not MEXC_API_SECRET:
        _telegram_mesaj_gonder(hedef_chat_id, "Marjin yok: MEXC API anahtari tanimli degil.")
        return
    try:
        ...
        # Bakiye bilgisi
        r = requests.get(
            f"{MEXC_BASE_URL}/api/v1/private/account/assets",
            headers=headers, timeout=10
        )
        result = r.json()
        if not result.get("success"):
            _telegram_mesaj_gonder(hedef_chat_id,
                f"Marjin yok: MEXC hesap sorgu hatasi: {result.get('message','')}")
            return
        ...
        if not usdt:
            _telegram_mesaj_gonder(hedef_chat_id, "Marjin yok: USDT bakiyesi bulunamadi.")
            return
        ...
        # Saatlik PNL
        saatlik_pnl = _mexc_saatlik_pnl_cek(headers)

        # Gorsel olustur
        veri = {
            "bakiye": bakiye, "kullanilan": kullanilan,
            "realized": realized, "unrealized": unrealized,
            "saatlik_pnl": saatlik_pnl, "zaman_str": zaman_str,
        }
        img_bytes = _gunluk_pnl_gorsel_olustur(veri)

        base = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}"
        caption = (
            f"📊 <b>Gunluk PNL — {zaman_str}</b>\n\n"
            f"💰 Bakiye: <b>${bakiye:.2f}</b>\n"
            f"📦 Kullanilan: <b>${kullanilan:.2f}</b>\n"
            f"✅ Gerceklesen: <b>${realized:+.2f}</b>\n"
            f"📈 Acik Poz.: <b>${unrealized:+.2f}</b>"
        )

        if img_bytes:
            requests.post(
                f"{base}/sendPhoto",
                data={"chat_id": hedef_chat_id, "caption": caption, "parse_mode": "HTML"},
                files={"photo": ("pnl.png", img_bytes, "image/png")},
                timeout=30
            )
        else:
            _telegram_mesaj_gonder(hedef_chat_id, caption)

    except Exception as e:
        print(f"[PNL] Hata: {e}")
        _telegram_mesaj_gonder(hedef_chat_id, f"PNL sorgu hatasi: {str(e)[:100]}")


def mexc_format_symbol(symbol: str) -> str:
    """BTCUSDT -> BTC_USDT formatına çevir."""
    sym = get_sym(symbol)
...
def mexc_get_max_leverage(symbol: str) -> int:
    """Sembol için maksimum kaldıracı döner."""
    sym = mexc_format_symbol(symbol)
    try:
        url = f"{MEXC_BASE_URL}/api/v1/contract/detail"
        r = requests.get(url, params={"symbol": sym}, timeout=10)
...
def mexc_set_leverage(symbol: str, leverage: int) -> bool:
    ...
    sym = mexc_format_symbol(symbol)
    ts = str(int(time.time() * 1000))
    body = {"symbol": sym, "leverage": leverage, "openType": 2}  # cross margin
...
def mexc_signature(payload_body: dict, ts: str) -> str:
    """MEXC Futures API imzası: ApiKey + timestamp + JSON body."""
    sign_str = MEXC_API_KEY + ts + json.dumps(payload_body, separators=(",", ":"))
    return hmac.new(
        MEXC_API_SECRET.encode("utf-8"),
        sign_str.encode("utf-8"),
        hashlib.sha256
...
def mexc_bildirim_gonder(symbol, sinyal, vol=0, leverage=0, margin=0,
                          order_id="", price=None, tp1=None, sl=None,
                          hata_msg=None, mark_price=None, bilgi_msg=None):
    """Islem sonucunu MEXC bildirim kanalina gonderir."""
    base = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}"
    ...
    yon_str = f"{yon_emoji} {yon}"

    if bilgi_msg:
        mesaj = f"⚡ <b>{symbol}</b> | {yon_str}\n\n{bilgi_msg}"
    elif hata_msg:
        parts = []
        parts.append("⚠️ <b>MEXC Islem ACILAMADI</b>")
        parts.append("")
        parts.append(f"⚡ {symbol} | {yon_str}")
        parts.append(f"Hata: {hata_msg}")
        mesaj = "\n".join(parts)
    else:
        parts = []
        parts.append("✅ <b>MEXC Islem ACILDI</b>")
        parts.append("")
        parts.append(f"⚡ {symbol} | {yon_str}")
        parts.append(f"Giris (Mark Price): {fmt_fiyat(mark_price or price)}")
        parts.append(f"Lot: {vol} kontrat")
        parts.append(f"Kaldirac: {leverage}x")
        parts.append(f"Marjin: ${margin} USDT")
        if tp1:
            parts.append(f"TP1: {fmt_fiyat(tp1)} (Mark Price - tum pozisyon kapanir)")
        if sl:
            parts.append(f"SL: {fmt_fiyat(sl)} (Mark Price tetikleyici)")
...
def mexc_place_order(symbol, side, sl=None, tp=None):
    """
    MEXC Futures market emri açar.
    - Fiyat: Mark Price (Fair Price)
    - Kaldıraç: maksimum (API'den otomatik)
    - TP1: Mark Price tetikleyici, tüm pozisyon kapanır
    - SL: Mark Price tetikleyici
    Returns: dict {"success": bool, "order_id": str, "vol": float,
                   "leverage": int, "mark_price": float, "msg": str}
             veya None (AUTO_TRADE_ENABLED=False)
    """
    ...
    mexc_side = 1 if is_long else 2  # 1=open long, 2=open short
    ...
    # Marjin kontrolu: hesapta yeterli USDT var mi?
    try:
        ts_chk = str(int(time.time() * 1000))
        sign_chk = hmac.new(
            MEXC_API_SECRET.encode("utf-8"),
            (MEXC_API_KEY + ts_chk).encode("utf-8"),
            hashlib.sha256
        ).hexdigest()
        hdr_chk = {"ApiKey": MEXC_API_KEY, "Request-Time": ts_chk,
                   "Signature": sign_chk, "Content-Type": "application/json"}
        r_chk = requests.get(f"{MEXC_BASE_URL}/api/v1/private/account/assets",
                              headers=hdr_chk, timeout=10)
        res_chk = r_chk.json()
        if res_chk.get("success"):
            assets = res_chk.get("data", {})
            usdt_asset = None
            ...
            if usdt_asset:
                ...
                if available < MEXC_MARGIN_USDT:
                    return {"success": False,
                            "msg": f"Marginsiz islem acilamadi: Bakiye ${available:.2f} USDT, "
                                   f"Gereken ${MEXC_MARGIN_USDT:.2f} USDT"}
    except Exception as e:
        print(f"[MEXC] Marjin kontrol hatasi (devam): {e}")

    # Lot hesabı
    notional = MEXC_MARGIN_USDT * max_lev
    vol = round(notional / mark_price, 4)
    if vol <= 0:
        return {"success": False, "msg": f"Hesaplanan lot sıfır: {vol}"}

    # TP/SL
    tp_price = None
    if tp:
        try:
            tp_price = float(str(tp).replace(",", "."))
        except:
            pass
    ...
    if tp_price:
        ...
        order_body["takeProfitPriceType"] = 2   # Mark Price
        order_body["takeProfitVol"] = vol        # tüm pozisyon
    if sl_price:
        order_body["stopLossPrice"] = sl_price
        order_body["stopLossPriceType"] = 2     # Mark Price

    ts = str(int(time.time() * 1000))
    signature = mexc_signature(order_body, ts)
    headers = {
        "ApiKey":       MEXC_API_KEY,
        "Request-Time": ts,
        "Signature":    signature,
...
def mexc_update_tpsl(symbol: str, is_long: bool, tp1=None, sl=None) -> bool:
    """
    MEXC Futures'da acik pozisyonun TP ve SL'ini gunceller.
    Mevcut pozisyonu kapatip yeniden acmadan sadece TP/SL degistirir.
    ...
    try:
        r = requests.get(
            ...
            headers=headers, params={"symbol": sym}, timeout=10
        )
        result = r.json()
        if not result.get("success"):
            print(f"[MEXC TPSL] Pozisyon sorgu hatasi: {result}")
            return False

        pozisyonlar = result.get("data", [])
        hedef_poz = None
        for poz in pozisyonlar:
            # side: 1=long, 2=short
            poz_side = poz.get("positionType", 0)
            if is_long and poz_side == 1:
                hedef_poz = poz
                break
            elif not is_long and poz_side == 2:
                hedef_poz = poz
                break

        if not hedef_poz:
            print(f"[MEXC TPSL] Acik pozisyon bulunamadi: {sym}")
            return False

        position_id = hedef_poz.get("positionId") or hedef_poz.get("id")
        ...
        if tp1:
            try:
                ...
                body["takeProfitPriceType"] = 2  # Mark Price
            except:
                pass
        if sl:
            try:
                body["stopLossPrice"] = float(str(sl).replace(",", "."))
                body["stopLossPriceType"] = 2  # Mark Price
            except:
                pass
        ...
        sig2 = hmac.new(
            MEXC_API_SECRET.encode("utf-8"),
            (MEXC_API_KEY + ts2 + json.dumps(body, separators=(",", ":"))).encode("utf-8"),
            hashlib.sha256
        ).hexdigest()
        headers2 = {
            "ApiKey": MEXC_API_KEY,
        ...
        r2 = requests.post(
            f"{MEXC_BASE_URL}/api/v1/private/position/change_tpsl",
            headers=headers2, json=body, timeout=10
        )
        result2 = r2.json()
        print(f"[MEXC TPSL] Guncelleme yaniti: {result2}")
...
def pozisyon_kaydet():
    """aktif_pozisyonlar dict'ini diske yazar."""
    try:
...
def pozisyon_yukle():
    """Disk'ten aktif_pozisyonlar'i yukler (deploy sonrasi)."""
...
def mexc_pozisyon_sorgula(symbol: str, is_long: bool) -> bool:
    """MEXC API'den bu sembolde acik pozisyon var mi diye sorar.
    Returns: True=acik pozisyon var, False=yok veya hata
    """
    if not MEXC_API_KEY or not MEXC_API_SECRET:
        return False
    sym = mexc_format_symbol(symbol)
    try:
        ...
        headers = {"ApiKey": MEXC_API_KEY, "Request-Time": ts,
        ...
        print(f"[MEXC SORGU] Hata: {e}")
        return False


def pozisyon_al(sym_key: str, is_long: bool) -> dict:
    """
    Oncelikle lokal cache'e bakar.
    Bulamazsa MEXC API'den sorgular.
    Returns: pozisyon dict veya None
    """
    with pozisyon_kilit:
        mevcut = aktif_pozisyonlar.get(sym_key)

    if mevcut:
        # Lokal'de var — MEXC'te hala acik mi dogrula
        symbol = sym_key.replace("_HEDGE", "")
        mexc_acik = mexc_pozisyon_sorgula(symbol, mevcut["is_long"])
        if mexc_acik:
            return mevcut
        else:
            # MEXC'te kapanmis (TP/SL vurulmus) — lokali temizle
            print(f"[POZISYON] {sym_key} MEXC'te kapali, lokal temizleniyor.")
            with pozisyon_kilit:
                aktif_pozisyonlar.pop(sym_key, None)
            pozisyon_kaydet()
            return None
    else:
        # Lokalde yok — MEXC API'ye sor (deploy sonrasi durum)
        symbol = sym_key.replace("_HEDGE", "")
        mexc_acik = mexc_pozisyon_sorgula(symbol, is_long)
        if mexc_acik:
            print(f"[POZISYON] {sym_key} MEXC'te bulundu, lokale eklendi.")
            poz = {
                "is_long": is_long, "timeframe": "?",
                "sinyal": "MEXC_RECOVERY", "order_id": "",
                "vol": 0, "tp1": None, "sl": None, "tp1_vuruldu": False,
            }
            with pozisyon_kilit:
...
def webhook():
        ...
        if isinstance(data, dict) and ("message" in data or "channel_post" in data):
            ...
            if text.startswith("/istatistik"):
                ...
                if chat_id in yetkili:
                    ...
                    hedef = MEXC_NOTIFY_CHAT_ID if MEXC_NOTIFY_CHAT_ID else TELEGRAM_CHAT_ID
                    _telegram_mesaj_gonder(hedef, "✅ <b>Otomatik işlem AÇILDI</b>\nBir sonraki sinyalden itibaren MEXC'te emir açılacak.")
                ...
                if chat_id in yetkili:
                    ...
                    hedef = MEXC_NOTIFY_CHAT_ID if MEXC_NOTIFY_CHAT_ID else TELEGRAM_CHAT_ID
                    _telegram_mesaj_gonder(hedef, "🛑 <b>Otomatik işlem KAPATILDI</b>\nMEXC'e artık emir gönderilmeyecek.")
                    print("[KOMUT] AUTO_TRADE_ENABLED = False")

            elif text.startswith("/gunluk_pnl") or text.startswith("/gunluk_PNL"):
                if chat_id in yetkili:
                    _pnl_hedef = MEXC_NOTIFY_CHAT_ID if MEXC_NOTIFY_CHAT_ID else TELEGRAM_CHAT_ID
                    threading.Thread(target=_gunluk_pnl_gonder,
                                     args=(_pnl_hedef,),
                                     daemon=True).start()
    ...
    try:
        ...
        if isinstance(data, dict) and "symbol" in data:
            symbol_raw = data.get("symbol", data.get("ticker", "BTCUSDT"))
            symbol    = symbol_raw
    ...
    if AUTO_TRADE_ENABLED:
        try:
            ...
            if is_trade_signal:
                ...
                # .P filtresi: sadece futures paritelerinde islem ac
                is_futures = str(symbol_raw).upper().endswith(".P") or str(symbol_raw).upper().endswith("USDT.P")

                # TF hiyerarsisi: buyuk sayi = daha buyuk TF
                TF_SIRALAMA = {"1":1,"3":2,"5":3,"15":4,"30":5,
                               "60":6,"1H":6,"240":7,"4H":7,"D":8,"1D":8,"W":9,"1W":9}
                tf_yeni = TF_SIRALAMA.get(str(timeframe), 0)

                # Mark Price al
                mark_price = mexc_get_mark_price(symbol)

                # SENARYO D: TP1 zaten gecilmis mi?
                tp1_gecildi = False
                ...
                if not is_futures:
                    # .P yoksa futures degil — sadece sinyal bildirimi, islem yok
                    print(f"[MEXC] {symbol} futures degil (.P yok), islem atlandi.")
                elif tp1_gecildi:
                    # SENARYO D: Atla
                    msg = "TP1 zaten gecildi, emir acilmadi\n"
                    msg += f"Mark Price: {fmt_fiyat(mark_price)} | TP1: {fmt_fiyat(tp1)}"
                    mexc_bildirim_gonder(symbol, sinyal, bilgi_msg=msg)
                else:
                    mevcut = pozisyon_al(sym_key, is_long_signal)

                    if mevcut:
                        mevcut_long = mevcut["is_long"]
                        tf_mevcut = TF_SIRALAMA.get(str(mevcut.get("timeframe","1")), 0)

                        if mevcut_long != is_long_signal:
                            # SENARYO C: Ters yon — HEDGE
                            sonuc = mexc_place_order(symbol=symbol, side=sinyal, tp=tp1, sl=sl)
                            if sonuc and sonuc["success"]:
                                ...
                                yon_mev = "LONG" if mevcut_long else "SHORT"
                                msg = "HEDGE Pozisyon Acildi\n\n"
                                msg += f"⚡ {symbol}\n"
                                msg += f"{yon_mev} hala acik\n"
                                msg += f"{yon_yeni} acildi — HEDGE\n\n"
                                msg += "Her iki pozisyon kendi TP/SL'i ile calisiyor.\n"
                                msg += f"Order ID: <code>{sonuc['order_id']}</code>"
                                mexc_bildirim_gonder(symbol, sinyal, bilgi_msg=msg)
                            elif sonuc:
                                mexc_bildirim_gonder(symbol, sinyal, hata_msg=sonuc["msg"])

                        elif tf_yeni > tf_mevcut:
                            # SENARYO A: Buyuk TF, ayni yon — TP/SL guncelle
                            # Mevcut pozisyonu MEXC'te guncelle (yeni TP/SL set et)
                            order_id_mevcut = mevcut.get("order_id", "")
                            guncellendi = mexc_update_tpsl(symbol, is_long_signal, tp1, sl)
                            with pozisyon_kilit:
                                aktif_pozisyonlar[sym_key]["timeframe"] = timeframe
                                aktif_pozisyonlar[sym_key]["sinyal"] = sinyal
                                aktif_pozisyonlar[sym_key]["tp1"] = tp1
                                aktif_pozisyonlar[sym_key]["sl"] = sl
                                aktif_pozisyonlar[sym_key]["tp1_vuruldu"] = False
                            pozisyon_kaydet()
                            yon_str = "LONG" if is_long_signal else "SHORT"
                            msg = "Pozisyon Guncellendi\n\n"
                            msg += f"⚡ {symbol} | {yon_str}\n"
                            msg += f"Yeni TF: {timeframe}\n"
                            msg += f"Yeni TP1: {fmt_fiyat(tp1)} | Yeni SL: {fmt_fiyat(sl)}\n"
                            if not guncellendi:
                                msg += "⚠️ TP/SL API guncellemesi basarisiz olabilir, elle kontrol et."
                            mexc_bildirim_gonder(symbol, sinyal, bilgi_msg=msg)

                        else:
                            # SENARYO B: Kucuk TF, ayni yon
                            # Kucuk TF'nin TP1'i gecildi mi kontrol et
                            tp1_kucuk_gecildi = False
                            tp1_kucuk = mevcut.get("tp1")
                            if tp1_kucuk and mark_price:
                                try:
                                    tp1_kucuk_f = float(str(tp1_kucuk).replace(",", "."))
                                    if is_long_signal and mark_price >= tp1_kucuk_f:
                                        tp1_kucuk_gecildi = True
                                    elif not is_long_signal and mark_price <= tp1_kucuk_f:
                                        tp1_kucuk_gecildi = True
                                except:
                                    pass

                            yon_str = "LONG" if is_long_signal else "SHORT"
                            if tp1_kucuk_gecildi:
                                # TP gecildi — buyuk TF pozisyonu koru, bildir
                                mev_tf = mevcut.get("timeframe","?")
                                msg = "Pozisyon Korundu\n\n"
                                msg += f"⚡ {symbol} | {yon_str}\n"
                                msg += f"{timeframe}dk sinyali geldi ama kucuk TF TP1 zaten gecildi.\n"
                                msg += f"Mevcut pozisyon ({mev_tf}dk TP/SL) gecerli kalmaya devam ediyor."
                                mexc_bildirim_gonder(symbol, sinyal, bilgi_msg=msg)
                            else:
                                # TP gecilmedi — kucuk TF'e guncelle (pozisyon zayiflat)
                                guncellendi = mexc_update_tpsl(symbol, is_long_signal, tp1, sl)
                                with pozisyon_kilit:
                                    aktif_pozisyonlar[sym_key]["timeframe"] = timeframe
                                    aktif_pozisyonlar[sym_key]["sinyal"] = sinyal
                                ...
                                pozisyon_kaydet()
                                mev_tf = mevcut.get("timeframe","?")
                                msg = "Pozisyon Zayiflandi\n\n"
                                msg += f"⚡ {symbol} | {yon_str}\n"
                                msg += f"{timeframe}dk sinyali geldi, kucuk TF TP1 gecilmedi.\n"
                                msg += f"TP/SL guncellendi: {fmt_fiyat(tp1)} / {fmt_fiyat(sl)}\n"
                                if not guncellendi:
                                    msg += "⚠️ API guncellemesi basarisiz, elle kontrol et."
                                mexc_bildirim_gonder(symbol, sinyal, bilgi_msg=msg)
                    else:
                        ...
                        if sonuc and sonuc["success"]:
                            ...
                            mexc_bildirim_gonder(
                                ...
                                margin=MEXC_MARGIN_USDT, order_id=sonuc["order_id"],
                                price=None, tp1=tp1, sl=sl,
