...
def _olta_skor_parse(metin):
    ...
    def parse_sayi(s):
        # "69,890" veya "69.890" veya "1,982" → float
        s = s.replace(" ", "")
        if "," in s and "." not in s:
            parca = s.split(",")
            tam_kisim = parca[0]
            if len(parca) == 2 and len(parca[1]) == 3 and len(tam_kisim) >= 2:
                # 69,890 veya 2,024 → tam kisim 2+ rakam ve sonrasi 3 rakam
                # Ama 1,982 → tam kisim 1 rakam → ondalik
                return float(s.replace(",", ""))  # binlik virgul (69890, 2024)
            else:
                return float(s.replace(",", "."))  # ondalik virgul (1.982)
...
def _olta_skor_kontrol(metin):
    ...
    def _blok_kontrol(satirlar, yon):
        ...
        cikti = [yon_etiket]
        limit_map = {"5m": 288, "15m": 96, "1h": 24, "4h": 6, "1d": 2}
        for s in satirlar:
            tf = s["tf"]; iv = s["iv"]
            ...
            try:
                ...
                continue
            if not closes:
                cikti.append("  {:<5} ⚠️ veri yok".format(tf))
                continue
...
BOT_VERSIYON = "v419"
print(f"[BASLANGIC] ========== BOT VERSIYON: v419 ==========")
