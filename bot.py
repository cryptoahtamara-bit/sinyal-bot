            ...
            if is_trade:
                ...
                if not is_futures:
                    ...
                    if not mevcut and hedge_mevcut and hedge_mevcut.get("is_long") == is_long:
                        mevcut = hedge_mevcut
                        mevcut["_is_hedge_pos"] = True  # revize sırasında hedge olduğunu işaretle
                        ...
                        if mevcut_long != is_long:
                            ...
                            if tp1_hedge is None:
                                ...
                                try:
                                    ...
                                    hedge_margin = round(onceki_margin * 2, 4)
                                ...
                                print(f"[HEDGE] Önceki marjin: {onceki_margin} → Hedge marjin: {hedge_margin} {'(manuel)' if manuel_acilis else '(bot)'}") 
...
def _liq_heatmap_gercek(symbol, saat=6):
    ...
    try:
        ...
        grid_long  = gaussian_filter(grid_long,  sigma=[0.8, 0.5])
        grid_short = gaussian_filter(grid_short, sigma=[0.8, 0.5])
        ...
        try:
            ...
            def _bant_etiketleri(band_arr, renk_baz, yon_str):
                """Yoğun bantları grupla, ortasına fiyat + dolar etiketi yaz."""
                esik = band_arr.max() * 0.35
                if esik <= 0:
                    return
                i_e = 0
                etiketler = []
                while i_e < n_pb:
                    if band_arr[i_e] >= esik:
                        ...
                        etiketler.append((merkez_fiyat, toplam_usd))
                        i_e = j_e
                    else:
                        i_e += 1

                for ep_fiyat, ep_usd in sorted(etiketler, key=lambda x: -x[1])[:5]:
                    if ep_usd >= 1e9:
                    ...
                    # Fiyata yakınsa parlak, değilse normal
                    yakin = abs(ep_fiyat - fiyat) / fiyat < 0.015
                    renk  = "#FFD700" if yakin else renk_baz
                    # Grafiğin %20-%80 x aralığında ortaya yaz
                    ax.annotate(
                        etiket,
                        xy=(n_tb * 0.18, ep_fiyat),
                        color=renk, fontsize=6.5, fontweight="bold",
                        va="center", ha="left",
                        bbox=dict(boxstyle="round,pad=0.2", facecolor="#050A14",
                                  alpha=0.80, edgecolor=renk_baz, linewidth=0.4),
                        zorder=10
                    )

            _bant_etiketleri(band_long_usd,  "#FF6B6B", "LONG")
            _bant_etiketleri(band_short_usd, "#69FF69", "SHORT")
...
BOT_VERSIYON = "v381"
print(f"[BASLANGIC] ========== BOT VERSIYON: v381 ==========")
