...
MIN_KARLI_MESAFE_PCT = 0.0015   # %0.15 — genel eşik
...
def sinyal_kaydet(symbol, sinyal, timeframe, price, tp1, tp2, tp3, tp4, tp5, sl, message_id):
    kayit = {
        ...
        "istatistik_sayildi": True,  # /trade_kapat olsa bile istatistiğe dahil edilir
    }
    with gunluk_kilit:
...
def top3_guncelle(snapshot=None):
    ...
    if not sirala:
        with top3_kilit:
            top3_whitelist = []
        print("[WHITELIST] Veri yok — işlem açılmayacak (%50+ yok)")
        return []
                ...
                if text.startswith("/skor") and reply_text:
                    ...
                    def _skor_gonder(rt):
                        try:
                            ...
                            sonuc = _olta_skor_kontrol(rt)
                            if sonuc:
                                print(f"[SKOR] Sonuc gonderiliyor ({len(sonuc)} karakter)")
                                _telegram_topic_mesaj_gonder(TOPIC_OLTA, sonuc)
                            else:
                                print("[SKOR] Sonuc bos!")
                                _telegram_topic_mesaj_gonder(TOPIC_OLTA, "⚠️ Skor hesaplanamadı.")
                        except Exception as e:
        ...
        try:
            ...
            if is_trade:
                ...
                if not is_futures:
                    ...
                    print(f"[MEXC] {timeframe}dk sinyal, {sym_key} atlandı — whitelist bos (%50+ yok)")
                elif sym_key.replace("USDT","") not in top3_whitelist and not _wl_hedge:
                    print(f"[MEXC] {timeframe}dk sinyal, {sym_key} %%50+ whitelist'te degil — islem atlandı. Whitelist: {top3_whitelist}")
                else:
                    ...
                    if not mevcut and hedge_mevcut and hedge_mevcut.get("is_long") == is_long:
                        ...
                        print(f"[MEXC] Hedge pozisyonu bulundu — revize edilecek: {sym_key}_HEDGE")
                    ...
                    if not mevcut and tp1_gecildi:
                        ...
                        if mevcut_long != is_long and _acik_hedge and _acik_hedge.get("is_long") == is_long:
                            # SENARYO 2a: Ters yön ama zaten açık HEDGE var — revize et
                            tp1_revize, _ = tp1_karlilik_kontrol(mp, tp1, tp2, is_long, tp3, tp4, tp5, symbol=symbol)
                            if tp1_revize:
                                mexc_update_tpsl(symbol, is_long, tp1_revize, sl)
                                with pozisyon_kilit:
                                    if sym_key + "_HEDGE" in aktif_pozisyonlar:
                                        aktif_pozisyonlar[sym_key + "_HEDGE"].update(
                                            {"timeframe": timeframe, "sinyal": sinyal,
                                             "tp1": tp1_revize, "sl": sl})
                                pozisyon_kaydet()
                                mexc_notify(symbol, sinyal,
                                    bilgi_msg=f"🔄 Mevcut HEDGE Revize Edildi\nYeni TP: {tp1_revize} | SL: {sl}")
                            else:
                                mexc_notify(symbol, sinyal,
                                    bilgi_msg=f"🛡️ HEDGE Revize Atlandı — kârlılık eşiği geçilemedi")
                            ...
                            # Hedge pozisyonunu da güncelle
                            _poz_key = (sym_key + "_HEDGE") if mevcut.get("_is_hedge_pos") else sym_key
                            # Karlılık kontrolü — yeni TP yönü teyitli mi
                            tp1_revize, _ = tp1_karlilik_kontrol(mp, tp1, tp2, is_long, tp3, tp4, tp5, symbol=symbol)
                            if tp1_revize:
                                mexc_update_tpsl(symbol, is_long, tp1_revize, sl)
                                with pozisyon_kilit:
                                ...
                                mexc_notify(symbol, sinyal,
                                    bilgi_msg=f"💪 Pozisyon Güçlendirildi{hedge_notu}\nYeni TF: {timeframe}dk\nTP: {tp1_revize} | SL: {sl}")
                            else:
                                mexc_notify(symbol, sinyal,
                                    bilgi_msg=f"🛡️ Revize atlandı — TP yön teyidi başarısız")

                        else:
                            # SENARYO 4: Alt TF, aynı yön — Revize (küçültme mantığı)
                            # Yeni TP eskisinden kötüyse → zarar yazmıyorsa kabul et (pozisyon küçülsün)
                            # Yeni TP zarar yazıyorsa (giriş fiyatının yanlış tarafı) → reddet
                            tp1_eksi   = False  # yeni TP eskisinden kötü
                            tp1_zarar  = False  # yeni TP zarar bölgesinde
                            tp1_mevcut = mevcut.get("tp1")

                            giris_fiyat = mevcut.get("mark_price") or mp
                            if tp1 and giris_fiyat:
                                try:
                                    tp1_yeni_f   = float(str(tp1).replace(",","."))
                                    mp_f_s4      = float(str(giris_fiyat).replace(",","."))

                                    # Zarar kontrolü — TP giriş fiyatının yanlış tarafında mı?
                                    if is_long and tp1_yeni_f <= mp_f_s4:
                                        tp1_zarar = True  # LONG TP giriş altında → zarar
                                    elif not is_long and tp1_yeni_f >= mp_f_s4:
                                        tp1_zarar = True  # SHORT TP giriş üstünde → zarar
                            ...
                            if tp1_zarar:
                                # Zarar yazıyor — kesinlikle reddet
                                mexc_notify(symbol, sinyal,
                                    bilgi_msg=f"🛡️ Revize Reddedildi\n{timeframe}dk sinyali, yeni TP zarar bölgesinde\nMevcut TP geçerli: {tp1_mevcut}\nReddedilen TP: {tp1}")
                            else:
                                # Küçülme veya iyileşme — kârlılık kontrolünden geçen en iyi TP'yi seç
                                tp1_s4, fee_notu_s4 = tp1_karlilik_kontrol(
                                    mp, tp1, tp2, is_long, tp3, tp4, tp5, symbol=symbol,
                                    hedge_fallback=True)
                                if tp1_s4 is None:
                                    # Hiçbir TP kârlılık eşiğini geçemedi — küçültme/revize yapma
                                    mexc_notify(symbol, sinyal,
                                        bilgi_msg=f"🛡️ Küçültme/Revize Atlandı\nTP/SL güncellenmedi — kârlılık eşiği geçilemedi\n{fee_notu_s4}")
                                else:
                                    mexc_update_tpsl(symbol, is_long, tp1_s4, sl)
                                    _poz_key4 = (sym_key + "_HEDGE") if mevcut.get("_is_hedge_pos") else sym_key
                                    with pozisyon_kilit:
                                        if _poz_key4 in aktif_pozisyonlar:
                                            aktif_pozisyonlar[_poz_key4].update(
                                                {"timeframe": timeframe, "sinyal": sinyal,
                                                 "tp1": tp1_s4, "sl": sl})
                                        else:
                                            print(f"[MEXC] {_poz_key4} güncelleme atlandı — pozisyon temizlenmiş")
                                    pozisyon_kaydet()
                                    hedge_notu4 = " (Hedge)" if mevcut.get("_is_hedge_pos") else ""
                                    if tp1_eksi:
                                        mexc_notify(symbol, sinyal,
                                            bilgi_msg=f"📉 Pozisyon Küçültüldü{hedge_notu4}\n{timeframe}dk sinyali — TP daraltıldı\nEski TP: {tp1_mevcut} → Yeni TP: {tp1_s4} | SL: {sl}")
                                    else:
                                        mexc_notify(symbol, sinyal,
                                            bilgi_msg=f"🔄 Pozisyon Revize Edildi{hedge_notu4}\n{timeframe}dk sinyali\nTP: {tp1_s4} | SL: {sl}")
                    else:
            ...
            for sym in ["BTCUSDT", "ETHUSDT"]:
                ...
                if img:
                    ...
                    _topic_foto_gonder_filigranli(TOPIC_ANALIZ, img,
                        f"📊 {sym_kisa} Likidasyon Grafiği — {zaman_str}\n/liq")
...
def _liq_yorum(veri):
    """Otomatik yorum üret — metrikler arası kombinasyon bazlı."""
    funding   = veri["funding"]
    degisim   = veri["degisim"]
    ...
    liq_long  = veri.get("liq_long_baskisi")   # gerçek likidasyon seviyeleri varsa
    ...
    # Funding
    if funding > 0.01:
        fund_yorum = f"💚 Funding: <b>+{funding:.4f}%</b> — Long ağırlıklı"
        fund_yon = "long"
    elif funding < -0.01:
        fund_yorum = f"❤️ Funding: <b>{funding:.4f}%</b> — Short ağırlıklı"
        fund_yon = "short"
    else:
        fund_yorum = f"⚪ Funding: <b>{funding:.4f}%</b> — Dengeli pozisyonlanma"
        fund_yon = "notr"
    satirlar.append(fund_yorum)

    # OI
    if oi_deg > 3:
        oi_yorum = f"📈 OI <b>+{oi_deg:.1f}%</b> — Yeni pozisyon açılıyor"
        oi_yon = "yukari"
    elif oi_deg < -3:
        oi_yorum = f"📉 OI <b>{oi_deg:.1f}%</b> — Pozisyon kapanıyor"
        oi_yon = "asagi"
    else:
        oi_yorum = f"➡️ OI <b>{oi_deg:+.1f}%</b> — Sakin seyir"
        oi_yon = "notr"
    satirlar.append(oi_yorum)
    ...
    if destek and direnc:
        # Gerçek likidasyon verisi yoksa grafikten hesapla
        liq_long_alt  = liq_long[0]  if liq_long  else low_24h
        liq_long_ust  = liq_long[1]  if liq_long  else low_24h + (high_24h - low_24h) * 0.35
        liq_short_alt = liq_short[0] if liq_short else low_24h + (high_24h - low_24h) * 0.65
        liq_short_ust = liq_short[1] if liq_short else high_24h
    ...
    # ── Kombinasyon bazlı ana yorum ──
    satirlar.append("")
    yukari_sayac = sum([fund_yon == "long", oi_yon == "yukari", mom_yon == "yukari"])
    asagi_sayac  = sum([fund_yon == "short", oi_yon == "asagi",  mom_yon == "asagi"])

    if yukari_sayac >= 3:
        satirlar.append("✅ <b>Güçlü yükseliş sinyali</b> — Funding, OI ve Momentum hizalı")
    elif asagi_sayac >= 3:
        satirlar.append("🚨 <b>Güçlü düşüş sinyali</b> — Funding, OI ve Momentum hizalı")
    elif yukari_sayac == 2:
        satirlar.append("🟡 <b>Yükseliş eğilimi</b> — Sinyallerin çoğu yukarı yönlü")
    elif asagi_sayac == 2:
        satirlar.append("🟡 <b>Düşüş eğilimi</b> — Sinyallerin çoğu aşağı yönlü")
    elif fund_yon == "long" and oi_yon == "asagi":
        satirlar.append("⚠️ <b>Dikkat:</b> Funding Long ama OI düşüyor — Uzun pozisyon kapanıyor olabilir")
    elif fund_yon == "short" and oi_yon == "yukari":
        satirlar.append("⚠️ <b>Dikkat:</b> Funding Short ama OI artıyor — Short squeeze riski var")
    elif fund_yon == "long" and mom_yon == "asagi":
        satirlar.append("⚠️ <b>Çelişki:</b> Funding Long yönlü ama Momentum düşüyor — Dikkatli ol")
    else:
        satirlar.append("⚪ <b>Yön belirsiz</b> — Sinyaller karışık, pozisyon almaktan kaçın")
    ...
    if destek and direnc and fiyat:
        ...
        # Fiyat likidasyon bölgelerine ne kadar yakın?
        uzaklik_long  = abs(fiyat - liq_long_ust)  / fiyat * 100 if destek and direnc else None
        uzaklik_short = abs(fiyat - liq_short_alt) / fiyat * 100 if destek and direnc else None

        if uzaklik_long and uzaklik_long < 1.5:
            satirlar.append(f"⚠️ Fiyat long likidasyon bölgesine çok yakın (%{uzaklik_long:.1f}) — Sert düşüş olabilir")
        elif uzaklik_short and uzaklik_short < 1.5:
            satirlar.append(f"⚠️ Fiyat short likidasyon bölgesine çok yakın (%{uzaklik_short:.1f}) — Sert yükseliş olabilir")

        # Destek/direnç sıkışması
        if destek and direnc:
            aralik_pct = abs(direnc[-1] - destek[0]) / fiyat * 100
            if aralik_pct < 2:
                satirlar.append(f"📌 Destek-direnç arası dar (%{aralik_pct:.1f}) — Kırılım bekleniyor")

    # ── Aksiyon önerisi ──
    satirlar.append("")
    satirlar.append("🎯 <b>Ne Yapmalıyım?</b>")

    if yukari_sayac >= 3:
        if destek:
            satirlar.append(f"✅ Long için: {fmt(destek[0])} destek bölgesinde giriş izle")
        satirlar.append("⛔ Short için: Güçlü yükseliş sinyali var — bekle")
    elif asagi_sayac >= 3:
        if direnc:
            satirlar.append(f"✅ Short için: {fmt(direnc[-1])} direnç bölgesinde giriş izle")
        satirlar.append("⛔ Long için: Güçlü düşüş sinyali var — bekle")
    elif yukari_sayac == 2:
        if destek:
            satirlar.append(f"🟡 Long için: {fmt(destek[0])} destekten giriş düşünülebilir — onay bekle")
        satirlar.append("⛔ Short için: Yükseliş eğilimi var — erken girme")
    elif asagi_sayac == 2:
        if direnc:
            satirlar.append(f"🟡 Short için: {fmt(direnc[-1])} dirençten giriş düşünülebilir — onay bekle")
        satirlar.append("⛔ Long için: Düşüş eğilimi var — erken girme")
    elif fund_yon == "short" and oi_yon == "yukari":
        satirlar.append("⚠️ Short squeeze riski var — Short pozisyon açmaktan kaçın")
        if destek:
            satirlar.append(f"🟡 Long için: {fmt(destek[0])} destekte fırsat olabilir")
    elif fund_yon == "long" and oi_yon == "asagi":
        satirlar.append("⚠️ Long pozisyon kapanıyor — Mevcut longları izle, yeni giriş yapma")
    else:
        satirlar.append("⏸ Bekle — Net sinyal yok, olta seviyelerini izlemeye devam et")
...
def _liq_gonder():
    ...
    for sym in ["BTCUSDT", "ETHUSDT"]:
        try:
            ...
            # Isı haritası — gerçek WebSocket verisi (yoksa tahmin)
            heatmap = _liq_heatmap_gercek(sym, saat=6)
            if heatmap:
                _topic_foto_gonder_filigranli(
                    hedef, heatmap,
...
LIQ_WS_SURE  = 6 * 3600  # 6 saat (saniye)
...
def _olta_fib_hesapla(closes, highs, lows, fiyat, interval):
    ...
    lb_map = {"5m": 20, "15m": 30, "1h": 40, "4h": 30, "1d": 20}
...
def _skor_klines_cek(symbol, interval, limit):
    """
    /skor icin hafif kline cekici.
    - MEXC futures → MEXC spot → Binance (proxy ile)
    - len >= 50 sarti YOK — elimizdeki veriyle calis
    - Her kaynak max 5sn timeout
    """
    mexc_interval_map = {
        "5m":"Min5","15m":"Min15","1h":"Min60","4h":"Hour4","1d":"Day1"
    }
    mexc_iv = mexc_interval_map.get(interval, "Min15")
    ...
    try:
        r = requests.get(
            ...
            params={"interval": mexc_iv, "limit": limit},
            timeout=5,
            proxies=BINANCE_PROXY if BINANCE_PROXY else None
        )
        if r.status_code == 200:
            d = r.json().get("data", {})
            closes  = [float(x) for x in d.get("close", [])]
            ...
            volumes = [float(x) for x in d.get("vol",   [])]
            if len(closes) >= 3:
                print(f"[SKOR] MEXC futures kline OK: {symbol} {interval} {len(closes)} mum")
                return closes, volumes, highs, lows
    except Exception as e:
        print(f"[SKOR] MEXC futures hata ({symbol} {interval}): {e}")
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            if data and isinstance(data, list):
                ...
                volumes = [float(k[5]) for k in data]
                if len(closes) >= 3:
                    print(f"[SKOR] MEXC spot kline OK: {symbol} {interval} {len(closes)} mum")
                    return closes, volumes, highs, lows
    except Exception as e:
        print(f"[SKOR] MEXC spot hata ({symbol} {interval}): {e}")

    # 3) Binance (proxy ile, kisa timeout)
    try:
        r = requests.get(
            "https://fapi.binance.com/fapi/v1/klines",
            params={"symbol": symbol, "interval": interval, "limit": limit},
        ...
        if r.status_code == 200:
            ...
            volumes = [float(k[5]) for k in data]
            if closes:
                print(f"[SKOR] Binance kline OK: {symbol} {interval} {len(closes)} mum")
                return closes, volumes, highs, lows
    except Exception as e:
        print(f"[SKOR] Binance hata ({symbol} {interval}): {e}")
...
def _olta_skor_parse(metin):
    ...
    def parse_sayi(s):
        """
        Turk fiyat formatlarini parse eder:
          "69.890" → 69890  (nokta binlik, BTC/buyuk fiyatlar)
          "1,982"  → 1.982  (virgul ondalik, ETH/kucuk fiyatlar)
          "69.89"  → 69.89  (nokta gercek ondalik, 2 haneli)
          "69,890" → 69890  (virgul binlik)
        """
        s = s.replace(" ", "")
        if "." in s and "," not in s:
            parca = s.split(".")
            # "69.890": tam=69 (2+), ondalik=890 (3 hane) → binlik nokta → 69890
            if len(parca) == 2 and len(parca[1]) == 3 and len(parca[0]) >= 2:
                return float(s.replace(".", ""))
            return float(s)  # gercek ondalik
        if "," in s and "." not in s:
            parca = s.split(",")
            # "69,890": tam=69 (2+), sonra 3 hane → binlik virgul
            # "1,982": tam=1 (1 hane) → ondalik virgul → 1.982
            if len(parca) == 2 and len(parca[1]) == 3 and len(parca[0]) >= 2:
                return float(s.replace(",", ""))
            return float(s.replace(",", "."))
        return float(s.replace(",", ""))
...
def _olta_skor_kontrol(metin):
    ...
    if not parsed:
        return "⚠️ Olta mesajı okunamadı. BTC veya BTC TUM mesajını alıntılayın."
    ...
    def _blok_kontrol(satirlar, yon):
        ...
        for s in satirlar:
            ...
            if not closes:
                cikti.append("  {:<5} ⚠️ veri alınamadı".format(tf))
                continue
            ...
            if not giris_ulasildi:
                ...
                isaret = "-" if (yon == "long" and fiyat > giris) or (yon == "short" and fiyat < giris) else "+"
                cikti.append("  {:<5} ⏳ Bekleniyor ({}%{:.1f} uzakta)".format(tf, isaret, uzaklik))
                continue
            if yon == "long":
            ...
            if sl_hit and not tp1_hit:
                pct = abs(giris - sl) / giris * 100
                cikti.append("  {:<5} ❌ SL Hit   (-%{:.1f})".format(tf, pct))
            elif tp2_hit:
                pct = abs(tp2 - giris) / giris * 100
                cikti.append("  {:<5} ⭐ TP2 Hit  (+%{:.1f})".format(tf, pct))
            elif tp1_hit:
                pct = abs(tp1 - giris) / giris * 100
                cikti.append("  {:<5} ✅ TP1 Hit  (+%{:.1f})".format(tf, pct))
            else:
                anlik = (fiyat - giris) / giris * 100 if yon == "long" else (giris - fiyat) / giris * 100
                isaret = "+" if anlik >= 0 else ""
                cikti.append("  {:<5} 📍 Açık     ({}%{:.1f} anlık)".format(tf, isaret, anlik))
        return cikti

    baslik = ("📊 <b>" + sym + " — Skor Kontrolü</b>\n"
              "💰 Anlık Fiyat: <b>" + p(fiyat) + "</b>\n"
              "🕐 Son 24 saatlik mumlar\n")

    if fmt == "tablo":
        yon = parsed["yon"]
        cikti = _blok_kontrol(parsed["satirlar"], yon)
        return baslik + "\n" + "\n".join(cikti)
    else:
        parcalar = []
        if parsed.get("satirlar"):
            parcalar += _blok_kontrol(parsed["satirlar"], parsed["yon"])
        if parsed.get("satirlar2"):
            parcalar += [""]  # bosluk
            parcalar += _blok_kontrol(parsed["satirlar2"], parsed["yon2"])
        return baslik + "\n" + "\n".join(parcalar)
...
def _olta_mtf_sorgula(symbol_raw):
    ...
    baslik   = fmt.format("TF", "Giris", "Stop", "TP1", "TP2") + "  Skor"
    ayrac    = "-" * len(baslik)
    ...
    def satir(lbl, r, side):
        ...
        if side == "long":
            skor  = r.get("long_skor", 0)
            emoji = r.get("long_emoji", "")
            rr    = r.get("long_rr", 0)
            return (fmt.format(lbl, q(r["long_giris"]), q(r["long_sl"]),
                               q(r["long_tp1"]), q(r["long_tp2"]))
                    + f"  {emoji}{skor}")
        else:
            skor  = r.get("short_skor", 0)
            emoji = r.get("short_emoji", "")
            rr    = r.get("short_rr", 0)
            return (fmt.format(lbl, q(r["short_giris"]), q(r["short_sl"]),
                               q(r["short_tp1"]), q(r["short_tp2"]))
                    + f"  {emoji}{skor}")
...
def _analiz_gorsel(veriler, zaman_str):
    ...
    try:
        ...
        # ── OLTA PANELLERİ — BTC / ETH / SOL ──
        GAP     = 0.10
        panel_w = (fig_w - GAP * 4) / 3
        panel_h = 3.20
        ...
        for pi, sym in enumerate(["BTCUSDT", "ETHUSDT", "SOLUSDT"]):
            ...
            sym_kisa = sym.replace("USDT", "")
            fiyat    = veri.get("fiyat", 0)
            ax.text(px + panel_w/2, y - 0.14,
                    f"{sym_kisa}  ·  {fmt_fiyat(fiyat)}",
                    ha="center", va="top", fontsize=9.5, color=WHITE)

            taker = veri.get("taker")
            if taker:
                bp = taker.get("taker_buy_pct", 50)
                t_col = GREEN if bp >= 55 else (RED if bp <= 45 else GRAY)
                t_lbl = "güçlü akış" if bp >= 55 else ("satıcı baskısı" if bp <= 45 else "dengeli")
                ax.text(px + panel_w/2, y - 0.32,
                        f"Taker %{bp:.1f} alıcı — {t_lbl}",
                        ha="center", va="top", fontsize=7, color=t_col, fontweight="bold")

            ax.axhline(y - 0.44, xmin=(px+0.04)/fig_w, xmax=(px+panel_w-0.04)/fig_w,
                       color=BORDER, linewidth=0.5)

            ly    = y - 0.54
            pad_l = px + 0.10
            pad_r = px + panel_w - 0.10
            FS_L  = 7.5   # label font
            FS_V  = 8.5   # değer font
            LH    = 0.26  # satır yüksekliği

            def satir(label, val1, val2=None, renk=WHITE):
                nonlocal ly
                ax.text(pad_l, ly, label, ha="left", va="top", fontsize=FS_L, color=GRAY, fontweight="bold")
                vstr = fmt_fiyat(val1)
                if val2 is not None:
                    vstr += f" – {fmt_fiyat(val2)}"
                ax.text(pad_r, ly, vstr, ha="right", va="top",
                        fontsize=FS_V, color=renk, fontweight="bold")
                ly -= LH
            ...
            ly -= 0.22
            satir("Giriş:", olta["long_giris_alt"], olta["long_giris_ust"], renk=GOLD)
            satir("Stop:",  olta["long_stop"],  renk=RED)
            satir("TP1:",   olta["long_tp1"],   renk=GREEN)
            satir("TP2:",   olta["long_tp2"],   renk=GREEN)

            ax.axhline(ly + 0.08, xmin=(px+0.04)/fig_w, xmax=(px+panel_w-0.04)/fig_w,
            ...
            ly -= 0.22
            satir("Giriş:", olta["short_giris"], renk=GOLD)
            satir("Stop:",  olta["short_stop"],  renk=RED)
            satir("TP:",    olta["short_tp"],    renk=GREEN)
...
BOT_VERSIYON = "v421"
print(f"[BASLANGIC] ========== BOT VERSIYON: v421 ==========")
...
_liq_ws_baslat()  # Likidasyon WebSocket toplayıcı
