...
def tp_kontrol_gonder(symbol, sinyal, timeframe, tp1, tp2, tp3, tp4, tp5, sl,
    ...
    def mesaj_olustur(tp1_ok, tp2_ok, tp3_ok, tp4_ok, tp5_ok, sl_ok, erken=False):
        ...
        _SINYAL_NORM_B = {"LONG":"Long","SHORT":"Short","STRONG BUY":"Strong Buy","STRONG SELL":"Strong Sell"}
        _sinyal_norm_b = _SINYAL_NORM_B.get(sinyal.upper(), sinyal)
        with _aktif_filtreler_kilit:
            _filtre_b = _aktif_filtreler.get(_sinyal_norm_b)
        if _filtre_b:
            # Mevcut koşullarla eşleştir (gunluk_sinyaller'den son kayıt)
            def _norm_b(s):
                if not s: return ""
                return (s.replace("ı","i").replace("ş","s").replace("ğ","g")
                         .replace("ç","c").replace("ö","o").replace("ü","u"))
            _TF_MAP_B = {"1":"1DK","3":"3DK","5":"5DK","15":"15DK","30":"30DK","60":"1SA","240":"4SA","D":"1G"}
            def _fg_b(fg):
            ...
            if _kayit_b:
                _tf = _TF_MAP_B.get(str(_kayit_b.get("timeframe","")), str(_kayit_b.get("timeframe","")))
                if _tf: _mevcut_b.add(_norm_b(_tf))
                _p = _kayit_b.get("piyasa_yonu");  
                if _p: _mevcut_b.add(_norm_b(_p))
                _f = _kayit_b.get("funding_yonu"); 
                if _f: _mevcut_b.add(_norm_b(_f))
                _o = _kayit_b.get("oi_yonu");      
                if _o: _mevcut_b.add(_norm_b("OI " + _o))
                _g = _fg_b(_kayit_b.get("fg_deger")); 
                if _g: _mevcut_b.add(_norm_b(_g))
            _kparts_b = [_norm_b(p) for p in _filtre_b["kosul_parts"]]
            if all(p in _mevcut_b for p in _kparts_b):
                _filtre_notu_b = f"Güçlü Sinyal %{int(round(_filtre_b['tp_yuzde']))} ✅"
            else:
                _filtre_notu_b = "⚠️ Zayıf Sinyal"
        else:
            _filtre_notu_b = None
