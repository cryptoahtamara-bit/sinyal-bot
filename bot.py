...
def fmt_tp(val):
    """TP değerini 500 kuralına göre formatla (500 altı 4 hane, 500 üstü 2 hane)"""
    if val is None:
        return None
    try:
        f = float(str(val).replace(",", "."))
        return f"{f:.2f}" if f >= 500 else f"{f:.4f}"
    except:
        return str(val)
...
def tp_kontrol_gonder(symbol, sinyal, tp1, tp2, tp3, message_id, dakika, sinyal_ts, sinyal_fiyat=None):
    ...
    sinyal_etiket = ""
    if sinyal.upper() in ["STRONG BUY", "STRONG_BUY"]:
        sinyal_etiket = "🔥 STRONG BUY"
    elif sinyal.upper() in ["STRONG SELL", "STRONG_SELL"]:
        sinyal_etiket = "💀 STRONG SELL"
    elif "LONG" in sinyal.upper() or "BUY" in sinyal.upper():
        sinyal_etiket = "🚀 LONG"
    elif "SHORT" in sinyal.upper() or "SELL" in sinyal.upper():
        sinyal_etiket = "📉 SHORT"
    else:
        sinyal_etiket = sinyal

    fiyat_str = ""
    if sinyal_fiyat:
        try:
            f = float(str(sinyal_fiyat).replace(",", "."))
            fiyat_str = f"{f:.4f}" if f < 500 else f"{f:.2f}"
        except:
            fiyat_str = str(sinyal_fiyat)
...
def parse_plain(raw: str):
    ...
    for line in lines:
        ...
        if line.startswith("TP1 "):
            ...
            tp3 = line[4:].strip()
        elif line.startswith("Cikis: ") or line.startswith("Çıkış: "):
            sl = line.split(": ", 1)[1].strip()
        elif line.startswith("Giris: ") or line.startswith("Giriş: "):
