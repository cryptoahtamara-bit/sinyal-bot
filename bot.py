...
MEXC_NOTIFY_CHAT_ID = os.getenv("MEXC_NOTIFY_CHAT_ID", "")
...
def mexc_update_tpsl(symbol, is_long, tp1, sl):
    ...
    try:
        ...
        body = {
            ...
            "executeCycle": 87600,
            "vol": vol,
            "lossTrend": 2,    # 1=Last Price, 2=Fair Price, 3=Index Price
            "profitTrend": 2,  # 1=Last Price, 2=Fair Price, 3=Index Price
        }
        if tp1:
...
def top3_guncelle(snapshot=None):
    ...
    # Daha temiz mantık: ilk 3'ü al, 3. ile eşit olanları da ekle
    top3 = []
    for i, item in enumerate(sirala):
        if i < 3:
            top3.append(item["sym"])
        elif item["oran"] == sirala[2]["oran"]:
            top3.append(item["sym"])
        else:
            break
    ...
    with top3_kilit:
        top3_whitelist = top3
    print(f"[TOP3] Whitelist güncellendi: {top3_whitelist}")
    return top3
    ...
    if AUTO_TRADE_ENABLED:
        try:
            ...
            if is_trade:
                ...
                if not is_futures:
                ...
                elif tf_yeni < 3 and sym_key.replace("USDT","") not in top3_whitelist:
                    # 1dk ve 3dk — sadece top3 koinler işlem açabilir
                    print(f"[MEXC] {timeframe}dk sinyal, {sym_key} top3'de degil — islem atlandı")
                    print(f"[MEXC] Top3 whitelist: {top3_whitelist}")
                elif tp1_gecildi:
                    ...
                    if mevcut:
                        ...
                        if mevcut_long != is_long:
                            ...
                            sonuc = mexc_place_order(symbol, sinyal, tp1, sl)
...
def _analiz_gorsel(veriler, zaman_str):
    ...
    v283: font boyutları 2x, olta paneli düzeltildi (closure bug fix).
    """
    try:
        import matplotlib
        matplotlib.use("Agg")
        ...
        def fmt_fiyat(f):
            if f is None: return "—"
            if f >= 10000: return f"${f/1000:.1f}k"
        ...
        # Figür boyutu — yükseklik artırıldı (daha büyük font + olta paneli)
        fig_w = 14.0
        fig_h = 22.0
        fig, ax = plt.subplots(figsize=(fig_w, fig_h))
        ...
        kartlar = [
            ...
            ("MARKET BREADTH", f"{breadth}/10",
             GREEN if breadth >= 7 else (ORANGE if breadth >= 4 else RED),
             "EMA20 üstü"),
            ("BTC CVD (1H)",
             (f"+{btc_cvd['cvd']:.0f}" if btc_cvd and btc_cvd['cvd'] >= 0
            ...
            ("BTC FUNDING",
             f"{btc_fund:+.4f}%" if btc_fund != 0 else "0.0000%",
        ...
        for i, (lbl, val, col, sub) in enumerate(kartlar):
            kx = i * kart_w
            ax.add_patch(FancyBboxPatch(
                (kx + 0.10, y - kart_h + 0.08), kart_w - 0.20, kart_h - 0.10,
                boxstyle="round,pad=0.05", linewidth=0.7,
                edgecolor=BORDER, facecolor=CARD))
            ax.text(kx + kart_w/2, y - 0.18, lbl,
                    ha="center", va="top", fontsize=10, color=DGRAY, fontweight="bold")
            ax.text(kx + kart_w/2, y - 0.60, val,
                    ha="center", va="top", fontsize=20, color=col, fontweight="bold")
            ax.text(kx + kart_w/2, y - 1.00, sub,
        ...
        for cx, lbl, ha in cols:
            ax.text(cx, y, lbl, ha=ha, va="top", fontsize=11, color=DGRAY, fontweight="bold")
        y -= 0.18
        ...
        for i, sym in enumerate(SEMBOLLER):
            ...
            ax.add_patch(FancyBboxPatch(
                (0.08, ry - row_h + 0.12), fig_w - 0.16, row_h - 0.10,
                boxstyle="round,pad=0.03", linewidth=0, facecolor=bg_c))
            my = ry - row_h / 2 + 0.04

            # Coin adı
            sym_kisa = sym.replace("USDT", "")
            ax.text(0.20, my, sym_kisa, ha="left", va="center",
                    fontsize=14, color=(GOLD if sym=="BTCUSDT" else WHITE), fontweight="bold")

            # Fiyat
            ax.text(1.60, my, fmt_fiyat(veri.get("fiyat")), ha="right", va="center",
                    fontsize=12, color=GRAY)

            # 24s%
            d24 = veri.get("degisim_24h")
            ax.text(2.60, my, f"{d24:+.1f}%" if d24 is not None else "—",
                    ha="right", va="center", fontsize=13, color=renk_pct(d24))
            ...
            if skor is not None:
                ...
                ax.add_patch(FancyBboxPatch(
                    (3.10, my - 0.18), 0.90, 0.36,
                    boxstyle="round,pad=0.04", linewidth=0.6,
                    edgecolor=s_col, facecolor=BG))
                ax.text(3.55, my, str(skor), ha="center", va="center",
                        fontsize=13, color=s_col, fontweight="bold")

            # RSI
            rsi = veri.get("rsi")
            r_col = ORANGE if rsi and (rsi >= 70 or rsi <= 30) else GRAY
            ax.text(4.55, my, f"{rsi:.1f}" if rsi else "—",
                    ha="right", va="center", fontsize=13, color=r_col)

            # Funding
            fund = veri.get("funding", 0)
            ax.text(5.55, my, f"{fund:+.4f}" if fund != 0 else "0",
            ...
            if oi_d:
                dp = oi_d["delta_pct"]
                ax.text(6.70, my, f"{dp:+.1f}%", ha="right", va="center",
                        fontsize=12, color=renk_pct(dp))
            else:
                ax.text(6.70, my, "—", ha="right", va="center", fontsize=12, color=GRAY)
            ...
            if taker:
                ...
                ax.text(7.85, my, f"{bp:.0f}%B", ha="right", va="center",
                        fontsize=12, color=t_col)
            else:
                ax.text(7.85, my, "—", ha="right", va="center", fontsize=12, color=GRAY)

            # CVD
            cvd = veri.get("cvd")
            if cvd:
                u = cvd["uyum"]
                if u == "uyum" and cvd["cvd"] >= 0:
                    cvd_str, c_col = "↑ uyum", GREEN
                elif u == "uyum" and cvd["cvd"] < 0:
                    cvd_str, c_col = "↓ uyum", RED
                elif u == "uyumsuz_yukari":
                    cvd_str, c_col = "↓ UYM⚠", ORANGE
                elif u == "uyumsuz_asagi":
                    cvd_str, c_col = "↑ UYM⚠", ORANGE
                else:
                    cvd_str, c_col = "→ düz", GRAY
                ax.text(9.00, my, cvd_str, ha="right", va="center",
                        fontsize=12, color=c_col)
            else:
                ax.text(9.00, my, "—", ha="right", va="center", fontsize=12, color=GRAY)

            # Likidasyon
            lik = veri.get("likidasyonlar")
            if lik and lik["toplam_usd"] > 0:
                l_col = RED if lik["agirlik"]=="long_baskin" else (
                    GREEN if lik["agirlik"]=="short_baskin" else GRAY)
                ax.text(10.80, my, f"${lik['toplam_usd']:.1f}M", ha="right", va="center",
                        fontsize=12, color=l_col)
            else:
                ax.text(10.80, my, "—", ha="right", va="center", fontsize=12, color=DGRAY)

            # Hacim çarpanı
            hc     = veri.get("hac_carpan", 0)
            hc_col = ORANGE if hc > 5 else GRAY
            hc_str = f"{hc:.1f}x⚠" if hc > 5 else (f"{hc:.1f}x" if hc > 0 else "—")
            ax.text(12.50, my, hc_str, ha="right", va="center",
                    fontsize=12, color=hc_col)

        y -= len(SEMBOLLER) * row_h + 0.20
        ax.axhline(y, xmin=0.01, xmax=0.99, color=BORDER, linewidth=0.6)
        y -= 0.18

        # ── OLTA PANELLERİ — BTC / ETH / SOL ──
        # Closure bug fix: satir() fonksiyonu dışarı alındı, ax/px/ly parametreyle geçiyor
        def _satir_yaz(ax_, px_, ly_, panel_w_, label, val1, val2=None, renk=WHITE):
            """Tek satır yaz, yeni ly döndür."""
            ax_.text(px_ + 0.22, ly_, label,
                     ha="left", va="top", fontsize=12, color=DGRAY)
            vstr = fmt_fiyat(val1)
            if val2 is not None:
                vstr += f" – {fmt_fiyat(val2)}"
            ax_.text(px_ + panel_w_ - 0.16, ly_, vstr,
                     ha="right", va="top", fontsize=13, color=renk, fontweight="bold")
            return ly_ - 0.40

        panel_w = (fig_w - 0.60) / 3
        panel_h = 4.20  # yükseklik artırıldı
        for pi, sym in enumerate(["BTCUSDT", "ETHUSDT", "SOLUSDT"]):
            px   = 0.18 + pi * (panel_w + 0.12)
            veri = coin_verileri.get(sym, {})
            olta = _analiz_olta_seviyeleri(sym, veri)

            ax.add_patch(FancyBboxPatch(
                (px, y - panel_h), panel_w, panel_h - 0.06,
            ...
            # Senaryo notu
            nota = olta["senaryo_notu"]
            if nota:
                ax.text(px + panel_w/2, y - 0.46, nota[:60],
                        ha="center", va="top", fontsize=10, color=ORANGE)

            ax.axhline(y - 0.58,
                       xmin=(px + 0.04)/fig_w,
                       xmax=(px + panel_w - 0.04)/fig_w,
                       color=BORDER, linewidth=0.5)

            ly = y - 0.72

            # LONG başlık
            ax.text(px + 0.20, ly, "LONG OLTA",
                    ha="left", va="top", fontsize=13, color=GREEN, fontweight="bold")
            ly -= 0.36

            ly = _satir_yaz(ax, px, ly, panel_w, "Giriş:",
                            olta["long_giris_alt"], olta["long_giris_ust"], GREEN)
            ly = _satir_yaz(ax, px, ly, panel_w, "Stop:",
                            olta["long_stop"], renk=RED)
            ly = _satir_yaz(ax, px, ly, panel_w, "TP1:",
                            olta["long_tp1"], renk=LGREEN)
            ly = _satir_yaz(ax, px, ly, panel_w, "TP2:",
                            olta["long_tp2"], renk=LGREEN)

            ax.axhline(ly + 0.08,
                       xmin=(px + 0.06)/fig_w,
                       xmax=(px + panel_w - 0.06)/fig_w,
                       color=BORDER, linewidth=0.4)
            ly -= 0.08

            # SHORT başlık
            ax.text(px + 0.20, ly, "SHORT OLTA",
                    ha="left", va="top", fontsize=13, color=RED, fontweight="bold")
            ly -= 0.36

            ly = _satir_yaz(ax, px, ly, panel_w, "Giriş:",
                            olta["short_giris"], renk=RED)
            ly = _satir_yaz(ax, px, ly, panel_w, "Stop:",
                            olta["short_stop"], renk=ORANGE)
            ly = _satir_yaz(ax, px, ly, panel_w, "TP:",
                            olta["short_tp"], renk=RED)

        y -= panel_h + 0.18

        # ── ÖZET METİN ──
        ozet = _analiz_ozet_metin(coin_verileri, veriler)
        ozet_h = 1.80
        ax.add_patch(FancyBboxPatch(
            (0.10, y - ozet_h), fig_w - 0.20, ozet_h - 0.06,
            boxstyle="round,pad=0.06", linewidth=0.7,
            edgecolor="#1a3a1a", facecolor="#0f1a0f"))
        ax.text(0.26, y - 0.14, "ÖZET",
                ha="left", va="top", fontsize=12, color=GREEN, fontweight="bold")

        satirlar_ozet = textwrap.wrap(ozet, width=115)
        for si, sat in enumerate(satirlar_ozet[:5]):
            ax.text(0.26, y - 0.38 - si * 0.28, sat,
                    ha="left", va="top", fontsize=11, color="#E8E8E6")
        y -= ozet_h + 0.16
        ...
        for sym2, col, lbl in legend_items:
            ax.text(lx, y - 0.18, sym2, ha="left", va="top", fontsize=11, color=col)
            ax.text(lx + 0.22, y - 0.18, lbl, ha="left", va="top", fontsize=10, color=DGRAY)
            lx += 2.20
        ax.text(fig_w - 0.24, y - 0.18,
                "CVD=Kümülatif Alış-Satış Farkı  ·  LİK=Son 30dk Likidasyon  ·  HAC.Ç=Futures/Spot",
                ha="right", va="top", fontsize=10, color=DGRAY)
        ...
        print(f"[ANALIZ] Gorsel hatasi: {e}")
        traceback.print_exc()
        return None
...
def _analiz_ozet_telegram(coin_verileri, veriler, zaman_str):
    """Telegram HTML formatında özet metin — görsel ile birlikte gönderilir."""
    def fmt(f):
        if f is None: return "—"
        if f >= 10000: return f"${f:,.0f}"
        if f >= 100:   return f"${f:,.0f}"
    ...
    satirlar = [f"📡 <b>Genişletilmiş Piyasa Analizi</b> — {zaman_str}\n"]
    ...
    if ort_skor >= 70:
        satirlar.append(f"📊 Piyasa skoru <b>{ort_skor}</b> — güçlü yükseliş, breadth {breadth}/10")
    elif ort_skor >= 55:
        satirlar.append(f"📊 Piyasa skoru <b>{ort_skor}</b> — yükseliş, breadth {breadth}/10")
    else:
        satirlar.append(f"📊 Piyasa skoru <b>{ort_skor}</b> — breadth {breadth}/10")
    ...
    for sym in ["BTCUSDT", "ETHUSDT", "SOLUSDT"]:
        ...
        olta = _analiz_olta_seviyeleri(sym, v)
        kisa = sym.replace("USDT", "")
        em   = "₿" if sym=="BTCUSDT" else ("Ξ" if sym=="ETHUSDT" else "◎")
        fiyat= v.get("fiyat", 0)
        skor = v.get("skor", 0)
        cvd  = v.get("cvd")
        lik  = v.get("likidasyonlar")

        s_ikon = "🟢" if skor>=70 else ("🟡" if skor>=55 else ("⚪" if skor>=40 else "🔴"))
        satirlar.append(f"{s_ikon} <b>{em} {kisa}/USDT</b> — {fmt(fiyat)} | Skor: <b>{skor}</b>")

        if cvd and "uyumsuz" in cvd["uyum"]:
            satirlar.append(f"  ⚠️ CVD uyumsuz — {'sahte yükseliş riski' if cvd['uyum']=='uyumsuz_yukari' else 'birikim sinyali'}")
        if lik and lik["toplam_usd"] >= 1.0:
            lik_ikon = "📉" if lik["agirlik"]=="long_baskin" else "📈"
            satirlar.append(f"  {lik_ikon} Son 30dk likidasyon: <b>${lik['toplam_usd']:.1f}M</b> ({lik['agirlik'].replace('_',' ')})")

        satirlar.append(
            f"  🎯 Long olta: <b>{fmt(olta['long_giris_alt'])}–{fmt(olta['long_giris_ust'])}</b> "
            f"· Stop: <b>{fmt(olta['long_stop'])}</b> · TP: <b>{fmt(olta['long_tp1'])}</b>/<b>{fmt(olta['long_tp2'])}</b>"
        )
        satirlar.append(
            f"  📉 Short olta: <b>{fmt(olta['short_giris'])}</b> red "
            f"· Stop: <b>{fmt(olta['short_stop'])}</b> · TP: <b>{fmt(olta['short_tp'])}</b>"
        )
        if olta["senaryo_notu"]:
            satirlar.append(f"  💬 {olta['senaryo_notu']}")
        satirlar.append("")
...
def _analiz_gonder(chat_id=None):
    ...
    try:
        ...
        if img:
            ...
            print("[ANALIZ] Gorsel gonderildi.")
        else:
            print("[ANALIZ] Gorsel uretilmedi.")
...
BOT_VERSIYON = "v283"
print(f"[BASLANGIC] ========== BOT VERSIYON: v283 ==========")
