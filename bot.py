...
def send_telegram_and_schedule_tp(caption, symbol, timeframe, sinyal,
    ...
    if not filtre:
        print(f"[TP_FILTRE] {sinyal} icin aktif filtre yok, sinyal atlandi: {symbol}")
        return
    ...
    # fg kategorisi Türkçe karakter normalize
    def _norm(s):
        if not s: return ""
    ...
    if not tum_eslesme:
        eslesen = [p for p in kosul_parts if p in mevcut_kosullar]
        print(f"[TP_FILTRE] {sinyal} {symbol} filtre gecemedi. Eslesen:{eslesen}/{len(kosul_parts)}")
        return

    print(f"[TP_FILTRE] {sinyal} {symbol} filtre gecti (%{tp_yuzde}). TOPIC_ALARM'a gonderiliyor.")

    tp_yuzde_int = int(round(tp_yuzde))
    caption = (
        f"❗ BEN KÜL YUTMAM ❗\n\n"
        f"TP Gerçekleşme Oranı %{tp_yuzde_int} ✅\n\n"
        + caption
    )
    # ── Filtre Kontrolü Sonu ─────────────────────────────────────────────────
