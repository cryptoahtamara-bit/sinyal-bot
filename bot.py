...
def _tp_kosul_yorum_uret(veri):
    ...
    def oran_val(v):
        try:
            # satir() -> (top, tp, devam, sl, oran) -> oran indeks 4
    ...
    if tip_veri:
        ...
        maddeler.append(f"🏆 En başarılı sinyal tipi: <b>{en_iyi[0]}</b> → <b>{en_iyi[1][4]}</b> TP ({en_iyi[1][0]} sinyal)")
        if en_kotu[0] != en_iyi[0]:
            maddeler.append(f"📉 En düşük sinyal tipi: <b>{en_kotu[0]}</b> → <b>{en_kotu[1][4]}</b> TP ({en_kotu[1][0]} sinyal)")
    else:
    ...
    if kosul_veri:
        ...
        maddeler.append(f"✅ En başarılı piyasa koşulu: <b>{en_iyi_k[0]}</b> → <b>{en_iyi_k[1][4]}</b> TP ({en_iyi_k[1][0]} sinyal)")
        if en_kotu_k[0] != en_iyi_k[0]:
            maddeler.append(f"❌ En düşük piyasa koşulu: <b>{en_kotu_k[0]}</b> → <b>{en_kotu_k[1][4]}</b> TP ({en_kotu_k[1][0]} sinyal)")
    else:
    ...
    if coin_veri:
        ...
        maddeler.append(f"🥇 En başarılı coin: <b>{en_iyi_c[0]}</b> → <b>{en_iyi_c[1][4]}</b> TP ({en_iyi_c[1][0]} sinyal)")
        maddeler.append(f"🔴 En düşük performanslı coin: <b>{en_kotu_c[0]}</b> → <b>{en_kotu_c[1][4]}</b> TP ({en_kotu_c[1][0]} sinyal)")
    ...
    if tf_veri:
        ...
        maddeler.append(f"⏱ En güvenilir zaman dilimi: <b>{en_iyi_tf[0]}</b> → <b>{en_iyi_tf[1][4]}</b> TP ({en_iyi_tf[1][0]} sinyal)")
        for tf, tv in tf_veri.items():
            if oran_val(tv) < 25:
                maddeler.append(f"⚠️ Dikkat: <b>{tf}</b> → <b>{tv[4]}</b> — düşük performans, dikkatli olunmalı")
    ...
    if komb_veri:
        ...
        maddeler.append(f"💡 En güvenilir kombinasyon: <b>{en_iyi_komb[0]}</b> → <b>{en_iyi_komb[1][4]}</b> TP ({en_iyi_komb[1][0]} sinyal)")
    else:
