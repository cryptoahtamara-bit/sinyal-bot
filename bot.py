...
def _panel_b_gorsel(snapshot=None):
    ...
    try:
        ...
        # Coin Bazlı Güçlü Sinyallerin TP Oranı (Bugün)
        try:
        ...
        ax.add_patch(plt.Rectangle((0.02, 0.03), 0.96, 0.29, transform=ax.transAxes,
                     facecolor=CARD_BG, edgecolor="none", zorder=1))
        ax.text(0.04, 0.30, "Coin Bazlı Güçlü Sinyallerin TP Oranı (Bugün)", fontsize=11.5,
                fontweight="bold", color=GOLD, ha="left", va="top")
        yy = 0.24; xx_col = [0.04, 0.36, 0.68]
        for i, item in enumerate(coin_sirala[:12]):
            gx = xx_col[i % 3]
            gy = yy - (i // 3) * 0.055
            gn, gtp = item.get("guclu_n",0), item.get("guclu_tp",0)
            if gn == 0:
                metin, renk = "Az Veri", HDR_COL
            else:
                oran = gtp/gn*100
                renk = GREEN if oran>=60 else (AMBER if oran>=35 else RED)
                metin = f"%{oran:.0f} ({gtp}/{gn})"
            ax.text(gx, gy, f"{item['sym']}  ", fontsize=10, color=TEXT_W, ha="left", va="top")
            ax.text(gx+0.12, gy, metin, fontsize=10, color=renk, ha="left", va="top")
