...
def mexc_test():
    ...
    sonuc = {"version": "v183", "proxy": bool(MEXC_PRIVATE_PROXY)}
...
def webkey_test():
    ...
    # Test 4: Her field tek tek — hangisi WAF'i tetikliyor?
    if MEXC_API_KEY and MEXC_API_SECRET:
        test_bodies = {
            "sadece_symbol":   {"symbol": "BTC_USDT"},
            "sadece_vol":      {"vol": 0.001},
            "sadece_side":     {"side": 1},
            "sadece_leverage": {"leverage": 20},
            "sadece_type":     {"type": 5},
            "symbol_vol":      {"symbol": "BTC_USDT", "vol": 0.001},
            "symbol_side":     {"symbol": "BTC_USDT", "side": 1},
            "tam_body":        {"symbol": "BTC_USDT", "price": 0, "vol": 0.001, "side": 1, "type": 5, "openType": 2, "leverage": 20},
        }
        for isim, body in test_bodies.items():
            try:
                hdrs_t, _ = mexc_headers(body)
                r_t = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
                                    headers=hdrs_t, json=body, timeout=8)
                sonuc[f"body_{isim}"] = {"status": r_t.status_code}
            except Exception as e:
                sonuc[f"body_{isim}"] = {"error": str(e)[:50]}
...
print(f"[BASLANGIC] ========== BOT VERSIYON: v183 ==========")
