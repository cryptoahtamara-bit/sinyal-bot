...
def _tplog_gorsel_uret(kayitlar, tarih_str):
    ...
    try:
        ...
        for k in kayitlar:
            ...
            satirlar.append((saat, kosul, tp, guven, yon, skor))

        n_satir = len(satirlar)
        fig_h   = max(3.5, 1.2 + n_satir * 0.45)
        fig, ax = plt.subplots(figsize=(14, fig_h))
        fig.patch.set_facecolor(BG)
        ax.set_facecolor(BG)
        ax.axis("off")
        ...
        ax.text(0.5, 0.98, f"{n_satir} kayıt",
                transform=ax.transAxes, ha="center", va="bottom",
                fontsize=9, color=TEXT_G)

        COLS  = ["Saat", "En İyi Koşul", "TP %", "Güven", "Tavsiye", "Skor"]
        XPOS  = [0.01, 0.10, 0.68, 0.76, 0.87, 0.96]
        ...
        for i, (saat, kosul, tp, guven, yon, skor) in enumerate(satirlar):
            ...
            try:
                ...
                tp_renk = TEXT_G
            yon_renk = YON_RENK.get(yon, AMBER)
            yon_lbl  = YON_EMOJI.get(yon, yon)
            degerler = [saat, kosul, tp, guven, yon_lbl, f"{skor:.0f}"]
            renkler  = [TEXT_G, TEXT_W, tp_renk, TEXT_G, yon_renk, PURPLE]
            for val, x, al, renk in zip(degerler, XPOS, ALIGN, renkler):
                ax.text(x, y_mid, str(val),
                        transform=ax.transAxes, ha=al, va="center",
                        fontsize=8, color=renk)

        long_say  = sum(1 for s in satirlar if s[4] == "LONG")
        short_say = sum(1 for s in satirlar if s[4] == "SHORT")
        notr_say  = sum(1 for s in satirlar if s[4] == "NÖTR")
