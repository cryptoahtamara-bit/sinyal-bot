...
def _olta_fib_hesapla(closes, highs, lows, fiyat, interval):
    ...
    KURAL: long_giris < fiyat, short_giris > fiyat, her TF farkli swing kullanir.
    """
    if not closes or len(closes) < 30:
        return None

    # Her TF farkli lookback — farkli swing high/low → farkli seviyeler
    lb_map = {"5m": 30, "15m": 50, "1h": 60, "4h": 45, "1d": 30}
    lb = min(lb_map.get(interval, 50), len(highs) - 1)
    swing_high = max(highs[-lb:])
    swing_low  = min(lows[-lb:])
    rng = swing_high - swing_low
    if rng <= 0 or rng < fiyat * 0.002:
        return None
    ...
    # ── LONG: giris MUTLAKA fiyatin ALTINDA ──
    # fiyatin altindaki en yakin fib destek seviyesi
    l_giris_adaylar = sorted(
        [f for f in [fib_618, fib_500, fib_382, fib_236] if f < fiyat],
        reverse=True
    )
    if not l_giris_adaylar:
        l_giris_adaylar = [fiyat * 0.985]
    long_giris = rd(l_giris_adaylar[0])

    # SL: long_giris'in altindaki sonraki fib seviyesi — minimum %1 mesafe
    l_sl_adaylar = sorted(
        [f for f in [fib_786, fib_618, fib_500, fib_382, fib_236, swing_low]
         if f < long_giris - rng * 0.015],
        reverse=True
    )
    long_sl = rd(l_sl_adaylar[0]) if l_sl_adaylar else rd(long_giris * 0.985)
    if long_sl >= long_giris:
        long_sl = rd(long_giris * 0.985)

    # TP: fiyatin USTUNDEKI fib seviyeleri — benzersiz, minimum mesafe
    l_tp_tum = sorted([f for f in [fib_786, fib_100, fib_1272, fib_1618] if f > fiyat])
    l_tp_adaylar = []
    for f in l_tp_tum:
        if not l_tp_adaylar or abs(f - l_tp_adaylar[-1]) > rng * 0.03:
            l_tp_adaylar.append(f)
        if len(l_tp_adaylar) == 2:
            break
    long_tp1 = rd(l_tp_adaylar[0]) if len(l_tp_adaylar) > 0 else rd(fiyat * 1.015)
    long_tp2 = rd(l_tp_adaylar[1]) if len(l_tp_adaylar) > 1 else rd(fiyat * 1.030)
    if long_tp1 == long_tp2 or long_tp2 <= long_tp1:
        long_tp2 = rd(long_tp1 * 1.022)

    # ── SHORT: giris MUTLAKA fiyatin USTUNDE ──
    s_giris_adaylar = sorted(
        [f for f in [fib_618, fib_786, fib_100] if f > fiyat]
    )
    if not s_giris_adaylar:
        s_giris_adaylar = [fiyat * 1.015]
    short_giris = rd(s_giris_adaylar[0])

    # SL: short_giris'in USTUNDEKI sonraki fib — minimum %1 mesafe
    s_sl_adaylar = sorted(
        [f for f in [fib_786, fib_100, fib_1272, fib_1618]
         if f > short_giris + rng * 0.015]
    )
    short_sl = rd(s_sl_adaylar[0]) if s_sl_adaylar else rd(short_giris * 1.015)
    if short_sl <= short_giris:
        short_sl = rd(short_giris * 1.015)

    # TP: fiyatin ALTINDAKI fib seviyeleri — genis aday listesi, benzersiz degerler
    s_tp_tum = sorted(
        [f for f in [fib_618, fib_500, fib_382, fib_236, swing_low] if f < fiyat],
        reverse=True
    )
    # Birbirinden en az rng*0.03 uzak olan seviyeleri sec
    s_tp_adaylar = []
    for f in s_tp_tum:
        if not s_tp_adaylar or abs(f - s_tp_adaylar[-1]) > rng * 0.03:
            s_tp_adaylar.append(f)
        if len(s_tp_adaylar) == 2:
            break
    short_tp1 = rd(s_tp_adaylar[0]) if len(s_tp_adaylar) > 0 else rd(fiyat * 0.985)
    short_tp2 = rd(s_tp_adaylar[1]) if len(s_tp_adaylar) > 1 else rd(fiyat * 0.970)
    if short_tp1 == short_tp2 or short_tp2 >= short_tp1:
        short_tp2 = rd(short_tp1 * 0.978)
...
BOT_VERSIYON = "v415"
print(f"[BASLANGIC] ========== BOT VERSIYON: v415 ==========")
