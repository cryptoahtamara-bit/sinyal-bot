...
def send_telegram_and_schedule_tp(caption, symbol, timeframe, sinyal,
    ...
    if filtre_notu:
        nl = chr(10)
        satirlar = caption.split(nl)
        ilk_satir_idx = next((i for i, s in enumerate(satirlar) if s.strip()), 0)
        satirlar.insert(ilk_satir_idx + 1, "")
        satirlar.insert(ilk_satir_idx + 2, filtre_notu)
        satirlar.insert(ilk_satir_idx + 3, "")
        caption = nl.join(satirlar)
    # Filtre Notu Sonu
