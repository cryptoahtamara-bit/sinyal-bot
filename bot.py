...
def mexc_sign(body_dict, ts):
    # MEXC Futures Contract API: ApiKey + timestamp + body_json
    # GET istekleri icin body_dict=None veya {} -> bos string kullanilir
    if body_dict:
...
def mexc_private_post(url, body, timeout=15):
    """Private POST — once ana domain, basarisizsa alternatif domain dene"""
    hdrs, ts = mexc_headers(body)
    # Once ana domain
    try:
        r = requests.post(url, headers=hdrs, json=body, timeout=timeout)
        print(f"[MEXC POST] Ana domain status: {r.status_code}")
        if r.status_code != 403:
            return r
    except Exception as e:
        print(f"[MEXC POST] Ana domain hata: {e}")
    # 403 veya hata — alternatif domain dene
    url_alt = url.replace("contract.mexc.com", "futures.mexc.com")
    print(f"[MEXC POST] Alternatif domain deneniyor: {url_alt}")
    hdrs2, _ = mexc_headers(body)
    return requests.post(url_alt, headers=hdrs2, json=body, timeout=timeout)
...
def mexc_place_order(symbol, sinyal, tp1=None, sl=None):
    ...
    lev = mexc_get_max_leverage(symbol)
    print(f"[MEXC] Leverage: {lev}")

    vol = round((MEXC_MARGIN_USDT * lev) / mp, 4)
    if vol <= 0:
        return {"success": False, "msg": f"Lot sifir: {vol}"}
...
def _gunluk_pnl_gonder(hedef_chat_id):
    if not MEXC_API_KEY or not MEXC_API_SECRET:
        _telegram_mesaj_gonder(hedef_chat_id, "MEXC API anahtari tanimli degil.")
        return
    # 3 deneme yap (proxy bazen bloklu olabiliyor)
    res = None
        ...
        try:
            ...
            if not res.get("success"):
                ...
                if "Network error" in msg and deneme < 2:
                    ...
                    time.sleep(2)
                    continue
                _telegram_mesaj_gonder(hedef_chat_id, f"MEXC bakiye sorgu hatasi: {msg}")
                return
    ...
    try:
        ...
        if img_bytes:
            requests.post(f"{base}/sendPhoto",
                          data={"chat_id": hedef_chat_id, "caption": caption,
                                "parse_mode": "HTML"},
...
def webhook():
    ...
    try:
        ...
        if isinstance(data, dict) and ("message" in data or "channel_post" in data):
            ...
            yetkili   = [x for x in [TELEGRAM_CHAT_ID, TELEGRAM_LOG_ID, MEXC_NOTIFY_CHAT_ID] if x]
            ...
            if text.startswith("/istatistik"):
                ...
                if chat_id in yetkili:
                    ...
                    _telegram_mesaj_gonder(MEXC_NOTIFY_CHAT_ID,
                ...
                if chat_id in yetkili:
                    hedef_pnl = MEXC_NOTIFY_CHAT_ID if MEXC_NOTIFY_CHAT_ID else TELEGRAM_CHAT_ID
                    threading.Thread(target=_gunluk_pnl_gonder,
                                     args=(hedef_pnl,), daemon=True).start()
        ...
        try:
            ...
            if is_trade:
                ...
                if not is_futures:
                    ...
                    print(f"[MEXC] SENARYO D — TP1 gecildi, emir acilmadi")
                    mexc_notify(symbol, sinyal,
                        bilgi_msg="TP1 zaten gecildi, emir acilmadi\nMark: " + str(mp) + " | TP1: " + str(tp1))
                else:
                    ...
                    if mevcut:
                        ...
                        if mevcut_long != is_long:
                            ...
                            pozisyon_kaydet()
                            mexc_notify(symbol, sinyal,
                                bilgi_msg="Pozisyon Guncellendi\nYeni TF: " + str(timeframe) + "\nTP1: " + str(tp1) + " | SL: " + str(sl))

                        else:
                            # SENARYO B: Kucuk TF, ayni yon
                            tp1_kucuk_gecildi = False
                            tp1_k = mevcut.get("tp1")
                            if tp1_k and mp:
                                try:
                                    tp1_kf = float(str(tp1_k).replace(",","."))
                                    if is_long and mp >= tp1_kf:     tp1_kucuk_gecildi = True
                                    elif not is_long and mp <= tp1_kf: tp1_kucuk_gecildi = True
                                except: pass

                            if tp1_kucuk_gecildi:
                                mexc_notify(symbol, sinyal,
                                    bilgi_msg="Pozisyon Korundu\n" + str(timeframe) + "dk sinyali geldi, kucuk TF TP1 gecildi.\nMevcut pozisyon gecerli.")
                            else:
                                mexc_update_tpsl(symbol, is_long, tp1, sl)
                                with pozisyon_kilit:
                                    aktif_pozisyonlar[sym_key].update(
                                ...
                                pozisyon_kaydet()
                                mexc_notify(symbol, sinyal,
                                    bilgi_msg="Pozisyon Zayifladi\n" + str(timeframe) + "dk TP/SL aktif\nTP1: " + str(tp1) + " | SL: " + str(sl))
                    else:
...
def mexc_test():
    ...
    sonuc = {"version": "v191", "proxy": bool(MEXC_PRIVATE_PROXY)}
...
def webkey_test():
    ...
    # Test 4: Imza format kombinasyonlari
    if MEXC_API_KEY and MEXC_API_SECRET:
        test_body = {"symbol": "BTC_USDT", "price": 0, "vol": 0.001, "side": 1, "type": 5, "openType": 2, "leverage": 20}
        ts = str(int(time.time() * 1000))
        ...
        # Format 1: APIKey + ts + sorted query string (mevcut)
        param_str_sorted = "&".join([f"{k}={v}" for k, v in sorted(test_body.items())])
        sig1 = hmac.new(MEXC_API_SECRET.encode(), (MEXC_API_KEY + ts + param_str_sorted).encode(), hashlib.sha256).hexdigest()
        try:
            r = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
                              headers=make_hdrs(sig1), json={}, params=dict(sorted(test_body.items())), timeout=8)
            sonuc["f1_sorted_qs"] = {"status": r.status_code, "body": r.text[:150]}
        except Exception as e:
            sonuc["f1_sorted_qs"] = {"error": str(e)[:50]}

        # Format 2: Sadece ts + sorted query string (APIKey yok imzada)
        sig2 = hmac.new(MEXC_API_SECRET.encode(), (ts + param_str_sorted).encode(), hashlib.sha256).hexdigest()
        try:
            r = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
                              headers=make_hdrs(sig2), json={}, params=dict(sorted(test_body.items())), timeout=8)
            sonuc["f2_ts_sorted_qs"] = {"status": r.status_code, "body": r.text[:150]}
        except Exception as e:
            sonuc["f2_ts_sorted_qs"] = {"error": str(e)[:50]}

        # Format 3: APIKey + ts + bos string (imza bodysiz)
        sig3 = hmac.new(MEXC_API_SECRET.encode(), (MEXC_API_KEY + ts).encode(), hashlib.sha256).hexdigest()
        try:
            r = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
                              headers=make_hdrs(sig3), json={}, params=test_body, timeout=8)
            sonuc["f3_no_body_sign"] = {"status": r.status_code, "body": r.text[:150]}
        except Exception as e:
            sonuc["f3_no_body_sign"] = {"error": str(e)[:50]}

        # Format 4: Sirasiz query string
        param_str_unsorted = "&".join([f"{k}={v}" for k, v in test_body.items()])
        sig4 = hmac.new(MEXC_API_SECRET.encode(), (MEXC_API_KEY + ts + param_str_unsorted).encode(), hashlib.sha256).hexdigest()
        try:
            r = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
                              headers=make_hdrs(sig4), json={}, params=test_body, timeout=8)
            sonuc["f4_unsorted_qs"] = {"status": r.status_code, "body": r.text[:150]}
        except Exception as e:
            sonuc["f4_unsorted_qs"] = {"error": str(e)[:50]}
...
print(f"[BASLANGIC] ========== BOT VERSIYON: v191 ==========")
...
def _pnl_zamanlayici():
    ...
    while True:
        try:
            ...
            if MEXC_NOTIFY_CHAT_ID and MEXC_API_KEY and MEXC_API_SECRET:
                print("[PNL] Saatlik otomatik PNL gonderiliyor...")
                _gunluk_pnl_gonder(MEXC_NOTIFY_CHAT_ID)
        except Exception as e:
