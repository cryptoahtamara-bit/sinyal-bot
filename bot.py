...
def _liq_heatmap_gercek(symbol, saat=6):
    ...
    try:
        ...
        try:
            ...
            def _grid_minmax(grid_1d, fiyat_ref, taraf, n_pb_, price_min_, price_max_):
                if grid_1d is None or grid_1d.max() <= 0:
                    return None
                esik = grid_1d.max() * 0.25  # %25 eşik — dağınık bantları keser
            ...
            # DEBUG — gerçek grid dağılımını logla
            bant_w_dbg = (price_max - price_min) / n_pb
            print(f"[LIQ_DEBUG] sym={sym} fiyat={fiyat:.2f} price_min={price_min:.2f} price_max={price_max:.2f} n_pb={n_pb}")
            print("[LIQ_DEBUG] LONG grid:")
            for _i in range(n_pb):
                _v = grid_long_1d_tmp[_i]
                if _v > 0.01:
                    _fa = price_min + _i * bant_w_dbg
                    print(f"  [{_i:2d}] ${_fa:.1f} = {_v:.4f}")
            print("[LIQ_DEBUG] SHORT grid:")
            for _i in range(n_pb):
                _v = grid_short_1d_tmp[_i]
                if _v > 0.01:
                    _fa = price_min + _i * bant_w_dbg
                    print(f"  [{_i:2d}] ${_fa:.1f} = {_v:.4f}")
            ...
            print(f"[LIQ_DEBUG] liq_long_zone={liq_long_zone} liq_short_zone={liq_short_zone}")
...
BOT_VERSIYON = "v512"
print(f"[BASLANGIC] ========== BOT VERSIYON: v512 ==========")
