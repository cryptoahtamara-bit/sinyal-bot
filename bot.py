...
def _liq_veri_cek(symbol):
    ...
    try:
        ...
        if len(klines) >= 20:
            ...
            destek_adaylar = sorted([l for l in lows if l < fiyat_ref], reverse=True)
            destek = destek_adaylar[:3] if destek_adaylar else sorted(lows)[:3]
            # Fiyatin ustundeki high'lar = direnc (en yakin once — kucukten buyuge)
            direnc_adaylar = sorted([h for h in highs if h > fiyat_ref])
            direnc = direnc_adaylar[:3] if direnc_adaylar else sorted(highs)[-3:]
...
def _liq_yorum(veri):
    ...
    if destek and direnc:
        ...
        # Long likidasyon: fiyat altindaki destek seviyeleri
        # Short likidasyon: fiyat ustundeki direnc seviyeleri
        if liq_long:
            liq_long_alt  = liq_long[0]
            liq_long_ust  = liq_long[1]
        elif destek and len(destek) >= 2:
            liq_long_alt  = destek[-1]   # fiyata en yakin destek (en buyuk)
            liq_long_ust  = destek[0]    # en uzak destek (en kucuk)
            if liq_long_alt > liq_long_ust:
                liq_long_alt, liq_long_ust = liq_long_ust, liq_long_alt
        else:
            liq_long_alt  = low_24h
            liq_long_ust  = low_24h + (high_24h - low_24h) * 0.35
        if liq_short:
            liq_short_alt = liq_short[0]
            liq_short_ust = liq_short[1]
        elif direnc and len(direnc) >= 2:
            liq_short_alt = direnc[0]    # fiyata en yakin direnc (en kucuk)
            liq_short_ust = direnc[-1]   # en uzak direnc (en buyuk)
        else:
            liq_short_alt = low_24h + (high_24h - low_24h) * 0.65
            liq_short_ust = high_24h
        ...
        satirlar.append(f"🛡 <b>Destek:</b> {fmt(destek[0])} / {fmt(destek[1] if len(destek) > 1 else destek[0])}")
        satirlar.append(f"🎯 <b>Direnç:</b> {fmt(direnc[0])} / {fmt(direnc[1] if len(direnc) > 1 else direnc[0])}")
...
BOT_VERSIYON = "v477"
print(f"[BASLANGIC] ========== BOT VERSIYON: v477 ==========")
