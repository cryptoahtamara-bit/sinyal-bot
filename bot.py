                ...
                if chat_id in yetkili and thread_id == TOPIC_RAPOR:
                    ...
                    def _tplog_gonder(parametre=parametre):
                        try:
                            ...
                            tarih_prefix = f"{yil}-{ay:02d}-{gun:02d}"
                            ...
                            try:
                                ...
                                log = []
                            kayitlar = [k for k in log if k.get("zaman", "").startswith(tarih_prefix)]
                            if not kayitlar:
...
def _tp_kosul_yorum_uret(veri):
    """TP olan işlemlerde piyasa koşulları nasıldı — confluance + kombinasyon yorumu."""
    MIN_KOMB  = 20
    MIN_KOSUL = 5
    MIN_UCLU  = 5
    maddeler = []
    ...
    def guven_etiketi(sinyal_sayisi):
        """Sinyal sayısına göre güven etiketi döner."""
        if sinyal_sayisi < 10:
            return "⚠️ düşük veri"
        elif sinyal_sayisi < 30:
            return "🔸 orta güven"
        elif sinyal_sayisi < 60:
            return "✅ güvenilir"
        else:
            return "💎 yüksek güven"
    ...
    # 1. En başarılı ikili kombinasyonlar — min 20 sinyal
    komb_veri = {k: v for k, v in veri["kombinasyon"].items() if v[0] >= MIN_KOMB and oran_val(v) >= 0}
    if komb_veri:
        sirali = sorted(komb_veri.items(), key=lambda x: oran_val(x[1]), reverse=True)
        maddeler.append("🏆 En güvenilir kombinasyonlar:")
        for i, (k, v) in enumerate(sirali[:3]):
            emoji = ["  1️⃣", "  2️⃣", "  3️⃣"][i]
            etiket = guven_etiketi(v[0])
            maddeler.append(f"{emoji} <b>{k}</b> → {v[4]} TP ({v[1]} TP / {v[0]} sinyal) {etiket}")
    else:
        maddeler.append("⏳ Kombinasyon verisi birikmesi devam ediyor (min. 20 sinyal)")
    maddeler.append(AYIRAC)

    # 2. Üçlü kombinasyon — Coin + TF + Piyasa Koşulu
    uclu_veri = veri.get("uclu_kombinasyon", {})
    uclu_veri_fil = {k: v for k, v in uclu_veri.items() if v[0] >= MIN_UCLU and oran_val(v) >= 0}
    if uclu_veri_fil:
        uclu_sirali = sorted(uclu_veri_fil.items(), key=lambda x: oran_val(x[1]), reverse=True)
        maddeler.append("🔗 En başarılı üçlü kombinasyonlar (Coin + TF + Koşul):")
        for i, (k, v) in enumerate(uclu_sirali[:3]):
            emoji = ["  1️⃣", "  2️⃣", "  3️⃣"][i]
            etiket = guven_etiketi(v[0])
            maddeler.append(f"{emoji} <b>{k}</b> → {v[4]} TP ({v[1]} TP / {v[0]} sinyal) {etiket}")
    else:
        maddeler.append("⏳ Üçlü kombinasyon: Veri birikmesi devam ediyor (min. 5 sinyal)")
    maddeler.append(AYIRAC)

    # 3. Dörtlü kombinasyon — Coin + TF + Piyasa + Funding
    dortu_veri = veri.get("dortu_kombinasyon", {})
    dortu_veri_fil = {k: v for k, v in dortu_veri.items() if v[0] >= MIN_UCLU and oran_val(v) >= 0}
    if dortu_veri_fil:
        dortu_sirali = sorted(dortu_veri_fil.items(), key=lambda x: oran_val(x[1]), reverse=True)
        maddeler.append("🔷 En başarılı dörtlü kombinasyonlar (Coin + TF + Piyasa + Funding):")
        for i, (k, v) in enumerate(dortu_sirali[:3]):
            emoji = ["  1️⃣", "  2️⃣", "  3️⃣"][i]
            etiket = guven_etiketi(v[0])
            maddeler.append(f"{emoji} <b>{k}</b> → {v[4]} TP ({v[1]} TP / {v[0]} sinyal) {etiket}")
    else:
        maddeler.append("⏳ Dörtlü kombinasyon: Veri birikmesi devam ediyor (min. 5 sinyal)")
    maddeler.append(AYIRAC)

    # 4. Sinyal tipi + Coin + Piyasa yönü
    sp_veri = veri.get("sinyal_piyasa_kombinasyon", {})
    sp_veri_fil = {k: v for k, v in sp_veri.items() if v[0] >= MIN_UCLU and oran_val(v) >= 0}
    if sp_veri_fil:
        sp_sirali = sorted(sp_veri_fil.items(), key=lambda x: oran_val(x[1]), reverse=True)
        maddeler.append("🎯 En başarılı Sinyal tipi + Coin + Piyasa kombinasyonları:")
        for i, (k, v) in enumerate(sp_sirali[:3]):
            emoji = ["  1️⃣", "  2️⃣", "  3️⃣"][i]
            etiket = guven_etiketi(v[0])
            maddeler.append(f"{emoji} <b>{k}</b> → {v[4]} TP ({v[1]} TP / {v[0]} sinyal) {etiket}")
    else:
        maddeler.append("⏳ Sinyal+Coin+Piyasa kombinasyonu: Veri birikmesi devam ediyor (min. 5 sinyal)")
    maddeler.append(AYIRAC)

    # 5. Funding + OI + Sinyal tipi
    oi_veri = veri.get("oi_funding_kombinasyon", {})
    oi_veri_fil = {k: v for k, v in oi_veri.items() if v[0] >= MIN_UCLU and oran_val(v) >= 0}
    if oi_veri_fil:
        oi_sirali = sorted(oi_veri_fil.items(), key=lambda x: oran_val(x[1]), reverse=True)
        maddeler.append("⚡ En başarılı Funding + OI + Sinyal tipi kombinasyonları:")
        for i, (k, v) in enumerate(oi_sirali[:3]):
            emoji = ["  1️⃣", "  2️⃣", "  3️⃣"][i]
            etiket = guven_etiketi(v[0])
            maddeler.append(f"{emoji} <b>{k}</b> → {v[4]} TP ({v[1]} TP / {v[0]} sinyal) {etiket}")
    else:
        maddeler.append("⏳ Funding+OI+Sinyal kombinasyonu: Veri birikmesi devam ediyor (min. 5 sinyal)")
    maddeler.append(AYIRAC)

    # 6. Coin + TF + Hacim
    hacim_veri = veri.get("hacim_kombinasyon", {})
    hacim_veri_fil = {k: v for k, v in hacim_veri.items() if v[0] >= MIN_UCLU and oran_val(v) >= 0}
    if hacim_veri_fil:
        hacim_sirali = sorted(hacim_veri_fil.items(), key=lambda x: oran_val(x[1]), reverse=True)
        maddeler.append("📊 En başarılı Coin + TF + Hacim kombinasyonları:")
        for i, (k, v) in enumerate(hacim_sirali[:3]):
            emoji = ["  1️⃣", "  2️⃣", "  3️⃣"][i]
            etiket = guven_etiketi(v[0])
            maddeler.append(f"{emoji} <b>{k}</b> → {v[4]} TP ({v[1]} TP / {v[0]} sinyal) {etiket}")
    else:
        maddeler.append("⏳ Coin+TF+Hacim kombinasyonu: Veri birikmesi devam ediyor (min. 5 sinyal)")
    maddeler.append(AYIRAC)

    # 7. F&G kategori + Sinyal tipi
    fg_sinyal_veri = veri.get("fg_sinyal_kombinasyon", {})
    fg_sinyal_veri_fil = {k: v for k, v in fg_sinyal_veri.items() if v[0] >= MIN_UCLU and oran_val(v) >= 0}
    if fg_sinyal_veri_fil:
        fg_sinyal_sirali = sorted(fg_sinyal_veri_fil.items(), key=lambda x: oran_val(x[1]), reverse=True)
        maddeler.append("😱 En başarılı F&G + Sinyal tipi kombinasyonları:")
        for i, (k, v) in enumerate(fg_sinyal_sirali[:3]):
            emoji = ["  1️⃣", "  2️⃣", "  3️⃣"][i]
            etiket = guven_etiketi(v[0])
            maddeler.append(f"{emoji} <b>{k}</b> → {v[4]} TP ({v[1]} TP / {v[0]} sinyal) {etiket}")
    else:
        maddeler.append("⏳ F&G+Sinyal kombinasyonu: Veri birikmesi devam ediyor (min. 5 sinyal)")
    maddeler.append(AYIRAC)

    # 8. Hacim + Piyasa yönü
    hacim_piyasa_veri = veri.get("hacim_piyasa_kombinasyon", {})
    hacim_piyasa_veri_fil = {k: v for k, v in hacim_piyasa_veri.items() if v[0] >= MIN_UCLU and oran_val(v) >= 0}
    if hacim_piyasa_veri_fil:
        hacim_piyasa_sirali = sorted(hacim_piyasa_veri_fil.items(), key=lambda x: oran_val(x[1]), reverse=True)
        maddeler.append("🔊 En başarılı Hacim + Piyasa yönü kombinasyonları:")
        for i, (k, v) in enumerate(hacim_piyasa_sirali[:3]):
            emoji = ["  1️⃣", "  2️⃣", "  3️⃣"][i]
            etiket = guven_etiketi(v[0])
            maddeler.append(f"{emoji} <b>{k}</b> → {v[4]} TP ({v[1]} TP / {v[0]} sinyal) {etiket}")
    else:
        maddeler.append("⏳ Hacim+Piyasa kombinasyonu: Veri birikmesi devam ediyor (min. 5 sinyal)")
    maddeler.append(AYIRAC)

    # 9. RSI + Sinyal tipi
    rsi_sinyal_veri = veri.get("rsi_sinyal_kombinasyon", {})
    rsi_sinyal_veri_fil = {k: v for k, v in rsi_sinyal_veri.items() if v[0] >= MIN_UCLU and oran_val(v) >= 0}
    if rsi_sinyal_veri_fil:
        rsi_sinyal_sirali = sorted(rsi_sinyal_veri_fil.items(), key=lambda x: oran_val(x[1]), reverse=True)
        maddeler.append("📈 En başarılı RSI + Sinyal tipi kombinasyonları:")
        for i, (k, v) in enumerate(rsi_sinyal_sirali[:3]):
            emoji = ["  1️⃣", "  2️⃣", "  3️⃣"][i]
            etiket = guven_etiketi(v[0])
            maddeler.append(f"{emoji} <b>{k}</b> → {v[4]} TP ({v[1]} TP / {v[0]} sinyal) {etiket}")
    else:
        maddeler.append("⏳ RSI+Sinyal kombinasyonu: Veri birikmesi devam ediyor (min. 5 sinyal)")
    maddeler.append(AYIRAC)

    # 10. RSI + Piyasa yönü
    rsi_piyasa_veri = veri.get("rsi_piyasa_kombinasyon", {})
    rsi_piyasa_veri_fil = {k: v for k, v in rsi_piyasa_veri.items() if v[0] >= MIN_UCLU and oran_val(v) >= 0}
    if rsi_piyasa_veri_fil:
        rsi_piyasa_sirali = sorted(rsi_piyasa_veri_fil.items(), key=lambda x: oran_val(x[1]), reverse=True)
        maddeler.append("📉 En başarılı RSI + Piyasa yönü kombinasyonları:")
        for i, (k, v) in enumerate(rsi_piyasa_sirali[:3]):
            emoji = ["  1️⃣", "  2️⃣", "  3️⃣"][i]
            etiket = guven_etiketi(v[0])
            maddeler.append(f"{emoji} <b>{k}</b> → {v[4]} TP ({v[1]} TP / {v[0]} sinyal) {etiket}")
    else:
        maddeler.append("⏳ RSI+Piyasa kombinasyonu: Veri birikmesi devam ediyor (min. 5 sinyal)")
    maddeler.append(AYIRAC)

    # 11. Piyasa koşulları confluance analizi — güven etiketiyle
    kosul_veri = {k: v for k, v in veri["kosul"].items()
                  if v[0] >= MIN_KOSUL and oran_val(v) >= 0 and "Seans" not in k}
    piyasa_maddeler = []
    if kosul_veri:
        # Funding etkisi
        fund_pos = kosul_veri.get("Funding — Pozitif")
        fund_neg = kosul_veri.get("Funding — Negatif")
        if fund_pos and fund_neg and oran_val(fund_pos) >= 0 and oran_val(fund_neg) >= 0:
            fark = round(oran_val(fund_pos) - oran_val(fund_neg), 1)
            if abs(fark) >= 3:
                min_sinyal = min(fund_pos[0], fund_neg[0])
                etiket = guven_etiketi(min_sinyal)
                if fark > 0:
                    piyasa_maddeler.append(
                        f"💰 Funding <b>Pozitif</b> ortamda TP oranı <b>{abs(fark)} puan</b> daha yüksek "
                        f"(Pozitif {fund_pos[4]} vs Negatif {fund_neg[4]}) {etiket}"
                    )
                else:
                    piyasa_maddeler.append(
                        f"💰 Funding <b>Negatif</b> ortamda TP oranı <b>{abs(fark)} puan</b> daha yüksek "
                        f"(Negatif {fund_neg[4]} vs Pozitif {fund_pos[4]}) {etiket}"
                    )

        # Piyasa yönü etkisi
        piy_yuk = kosul_veri.get("Piyasa — Yükseliş")
        piy_dus = kosul_veri.get("Piyasa — Düşüş")
        if piy_yuk and piy_dus and oran_val(piy_yuk) >= 0 and oran_val(piy_dus) >= 0:
            fark = round(oran_val(piy_yuk) - oran_val(piy_dus), 1)
            if abs(fark) >= 3:
                daha_iyi = "Yükseliş" if fark > 0 else "Düşüş"
                min_sinyal = min(piy_yuk[0], piy_dus[0])
                etiket = guven_etiketi(min_sinyal)
                # daha_iyi olan önce gösterilsin
                if fark > 0:
                    karsilastirma = f"({piy_yuk[4]} vs {piy_dus[4]})"
                else:
                    karsilastirma = f"({piy_dus[4]} vs {piy_yuk[4]})"
                piyasa_maddeler.append(
                    f"📊 Piyasa <b>{daha_iyi}</b> trendinde TP oranı <b>{abs(fark)} puan</b> daha yüksek "
                    f"{karsilastirma} {etiket}"
                )
        ...
        if fg_korku and fg_acgoz and oran_val(fg_korku) >= 0 and oran_val(fg_acgoz) >= 0:
            fark = round(oran_val(fg_korku) - oran_val(fg_acgoz), 1)
            if abs(fark) >= 3:
                daha_iyi = "Korku" if fark > 0 else "Açgözlülük"
                min_sinyal = min(fg_korku[0], fg_acgoz[0])
                etiket = guven_etiketi(min_sinyal)
                piyasa_maddeler.append(
                    f"😱 F&G <b>{daha_iyi}</b> ortamında TP oranı <b>{abs(fark)} puan</b> daha yüksek "
                    f"({fg_korku[4]} vs {fg_acgoz[4]}) {etiket}"
                )

    if piyasa_maddeler:
        maddeler.extend(piyasa_maddeler)
        maddeler.append(AYIRAC)
    else:
        maddeler.append("⏳ Piyasa koşulu: Anlamlı fark bulunamadı (min. 3 puan fark bekleniyor)")
        maddeler.append(AYIRAC)

    # 7. Tüm kombinasyonların confluance analizi — en iyi TP koşulu
    confluance = _tp_confluance_skoru_hesapla(veri)
    if confluance:
        top3 = confluance.get("top3_mega", [])
        yon_emoji = {"LONG": "🟢", "SHORT": "🔴", "NÖTR": "⚪"}.get(confluance.get("tavsiye_yon","NÖTR"), "⚪")
        ...
        if top3:
            for i, item in enumerate(top3):
                emoji = ["  🥇", "  🥈", "  🥉"][i]
                maddeler.append(
                    f"{emoji} {item['kombinasyon']} → "
                    f"{item['tp_orani']} TP ({item['tp_sayisi']}/{item['sinyal']})"
                )
        else:
            maddeler.append("  ⏳ Yeterli mega kombinasyon verisi yok (min. 5 sinyal)")
    else:
...
def _tp_confluance_skoru_hesapla(veri):
    """
    ...
    # Tavsiye yön — top5 kombinasyonlarından anahtar kelime oylama
    LONG_KELIMELER  = ["Yükseliş", "Pozitif", "Strong Buy", "Long", "OI Artıyor", "Aşırı Korku", "Korku", "Ortalama Üstü", "Aşırı Satım", "Satım Yakın"]
    SHORT_KELIMELER = ["Düşüş", "Negatif", "Strong Sell", "Short", "OI Azalıyor", "Açgözlülük", "Aşırı Açgözlülük", "Ortalama Altı", "Aşırı Alım", "Alım Yakın"]

    long_agirlik  = 0.0
    short_agirlik = 0.0
    for item in top5:
        komb = item["kombinasyon"]
        skor = item["agirlikli_skor"]
        for kw in LONG_KELIMELER:
            if kw in komb:
                long_agirlik += skor
        for kw in SHORT_KELIMELER:
            if kw in komb:
                short_agirlik += skor

    fark_oran = abs(long_agirlik - short_agirlik) / max(long_agirlik + short_agirlik, 1) * 100
    if long_agirlik > short_agirlik:
        tavsiye_yon = "LONG"
    elif short_agirlik > long_agirlik:
        tavsiye_yon = "SHORT"
    else:
        tavsiye_yon = "NÖTR"

    if fark_oran < 10:
        tavsiye_yon = "NÖTR"  # Fark çok küçükse nötr say

    if fark_oran >= 40:
        tavsiye_guven = "yüksek güven"
    elif fark_oran >= 20:
        tavsiye_guven = "güvenilir"
    elif fark_oran >= 10:
        tavsiye_guven = "orta güven"
    else:
        tavsiye_guven = "düşük veri"

    # ── Mega kombinasyon: tüm faktörleri birleştir ──────────────────────────
    # Sinyal kayıtlarından coin+TF+sinyal_tipi+piyasa+funding+oi+rsi+hacim+fg
    # bazında TP oranı hesapla, en iyi 3'ü bul
    with gunluk_kilit:
    ...
    mega = {}
    for s in _kayitlar:
        coin  = s.get("symbol","").replace("USDT","")
        ...
        rsi   = s.get("rsi_kategori")
        hacim = s.get("hacim_durumu")
        fg    = _fg_kat(s.get("fg_deger"))
        # En az coin+tf+sinyal_tipi olmalı
        if not (coin and tf and st):
            continue
        # Faktörleri kısa etiketlerle birleştir
        parts = [coin, tf, st]
        if piy:   parts.append(piy)
        if fund:  parts.append(fund)
        if oi:    parts.append(oi)
        if rsi:   parts.append(rsi)
        if hacim: parts.append(hacim)
        if fg:    parts.append(fg)
        key = " + ".join(parts)
        if key not in mega:
            mega[key] = {"tp": 0, "toplam": 0}
        mega[key]["toplam"] += 1
        if tp_mi(s):
            mega[key]["tp"] += 1

    MIN_MEGA = 8
    mega_fil = {k: v for k, v in mega.items() if v["toplam"] >= MIN_MEGA}
    mega_sirali = sorted(
        mega_fil.items(),
        key=lambda x: (
            (x[1]["tp"] / x[1]["toplam"]) * math.log(max(x[1]["toplam"], 1)),
            x[1]["toplam"],   # tiebreaker: daha fazla sinyal önce
        ),
        reverse=True
    )
    top3_mega = []
    for k, v in mega_sirali[:3]:
...
def _tplog_gorsel_uret(kayitlar, tarih_str):
    """
    Verilen günün confluance log kayıtlarından PNG görsel üretir.
    Her saat satırı: 1. en iyi + top2 + top3 kombinasyonu gösterir.
    Sütunlar: Saat | # | Kombinasyon | TP % | TP/Sin | Güven | Tavsiye | Skor
    Döner: (img_bytes, caption) veya None
    """
    try:
        import matplotlib
        matplotlib.use("Agg")
        ...
        YON_RENK  = {"LONG": GREEN, "SHORT": RED, "NÖTR": AMBER}
        YON_EMOJI = {"LONG": "▲ LONG", "SHORT": "▼ SHORT", "NÖTR": "● NÖTR"}
        SIRA_EMOJI = ["1.", "2.", "3."]

        # Her saat için 3 alt satır üret
        # alt_satirlar: list of (saat, sira, kosul, tp, tp_sin, guven, yon, skor, ilk_mi)
        alt_satirlar = []
        for k in kayitlar:
            saat     = k.get("zaman", "")[-5:]
            yon      = k.get("tavsiye_yon", "NÖTR")
            skor     = k.get("agirlikli_skor", 0)
            top5     = k.get("top5", [])

            # top5 yoksa en_iyi_kosul'dan tek satır yap
            if not top5:
                kosul = k.get("en_iyi_kosul", "—")
                if len(kosul) > 34:
                    kosul = kosul[:32] + "…"
                tp    = k.get("tp_orani", "—")
                guven = k.get("guven", "—")
                alt_satirlar.append((saat, "1.", kosul, tp, "—", guven, yon, skor, True))
            else:
                for idx, item in enumerate(top5[:3]):
                    kosul = item.get("kombinasyon", "—")
                    if len(kosul) > 34:
                        kosul = kosul[:32] + "…"
                    tp      = item.get("tp_orani", "—")
                    tp_say  = item.get("tp_sayisi", "—")
                    sin_say = item.get("sinyal", "—")
                    tp_sin  = f"{tp_say}/{sin_say}"
                    guven   = item.get("guven", "—")
                    ilk_mi  = (idx == 0)
                    # Tavsiye ve skor sadece ilk satırda
                    alt_satirlar.append((
                        saat if ilk_mi else "",
                        SIRA_EMOJI[idx],
                        kosul, tp, tp_sin, guven,
                        yon if ilk_mi else "",
                        skor if ilk_mi else "",
                        ilk_mi
                    ))

        n_satir = len(alt_satirlar)
        fig_h   = max(4.0, 1.2 + n_satir * 0.38)
        ...
        ax.text(0.5, 1.01, f"📋  TP CONFLUANCE LOG  —  {tarih_str}",
                transform=ax.transAxes, ha="center", va="bottom",
                fontsize=13, fontweight="bold", color=TEXT_W)
        ax.text(0.5, 0.98, f"{n_kayit} kayıt  |  her saat için top 3 kombinasyon",
                transform=ax.transAxes, ha="center", va="bottom",
                fontsize=8.5, color=TEXT_G)

        # Sütunlar: Saat | # | Kombinasyon | TP % | TP/Sin | Güven | Tavsiye | Skor
        COLS  = ["Saat", "#", "Kombinasyon",               "TP %", "TP/Sin", "Güven",  "Tavsiye",  "Skor"]
        XPOS  = [0.01,  0.06, 0.10,                        0.62,   0.69,     0.78,     0.88,       0.97 ]
        ALIGN = ["left","left","left",                      "left", "center", "left",   "center",   "right"]

        toplam_satirlar = n_satir + 1
        satir_h = 1.0 / (toplam_satirlar + 1)

        # Başlık satırı
        y_baslik = 1.0 - satir_h * 0.5
        hdr_box = FancyBboxPatch((0, 1.0 - satir_h), 1.0, satir_h,
                                 boxstyle="square,pad=0",
                                 facecolor=HEADER, edgecolor="none",
                                 transform=ax.transAxes, clip_on=False)
        ax.add_patch(hdr_box)
        for col, x, al in zip(COLS, XPOS, ALIGN):
            ax.text(x, y_baslik, col,
        ...
        grup_idx = -1
        prev_saat = None
        for i, (saat, sira, kosul, tp, tp_sin, guven, yon, skor, ilk_mi) in enumerate(alt_satirlar):
            if saat:
                prev_saat = saat
                grup_idx += 1

            y_top = 1.0 - satir_h * (i + 1)
            y_mid = y_top - satir_h * 0.5
            bg    = ROW_ODD if grup_idx % 2 == 0 else ROW_EVEN
            row_box = FancyBboxPatch((0, y_top - satir_h), 1.0, satir_h,
                                     boxstyle="square,pad=0",
                                     facecolor=bg, edgecolor="none",
                                     transform=ax.transAxes, clip_on=False)
            ax.add_patch(row_box)

            # Alt satırlar için ince ayraç çizgisi
            if not ilk_mi:
                line_y = y_top  # axes koordinatı (0-1 arası)
                ax.plot([0.06, 0.99], [line_y, line_y],
                        color=TEXT_DIM, linewidth=0.3,
                        transform=ax.transAxes, clip_on=False)

            # TP rengi
            try:
                tp_val  = float(str(tp).replace("%", ""))
                tp_renk = GREEN if tp_val >= 50 else AMBER if tp_val >= 35 else RED
            except Exception:
                tp_renk = TEXT_G

            yon_renk  = YON_RENK.get(yon, AMBER) if yon else TEXT_G
            yon_lbl   = YON_EMOJI.get(yon, yon) if yon else ""
            skor_lbl  = f"{skor:.0f}" if skor != "" else ""
            sira_renk = [GREEN, TEAL, TEXT_G][["1.", "2.", "3."].index(sira)] if sira in ["1.", "2.", "3."] else TEXT_G

            degerler = [saat, sira, kosul, tp, tp_sin, guven, yon_lbl, skor_lbl]
            renkler  = [TEXT_G, sira_renk, TEXT_W if ilk_mi else TEXT_G,
                        tp_renk, TEXT_G, TEXT_G, yon_renk, PURPLE]
            fontlar  = [8, 7.5, 8 if ilk_mi else 7.5, 8, 7.5, 7.5, 8, 8]

            for val, x, al, renk, fs in zip(degerler, XPOS, ALIGN, renkler, fontlar):
                if str(val):
                    ax.text(x, y_mid, str(val),
                            transform=ax.transAxes, ha=al, va="center",
                            fontsize=fs, color=renk)

        # Alt özet
        long_say  = sum(1 for k in kayitlar if k.get("tavsiye_yon") == "LONG")
        short_say = sum(1 for k in kayitlar if k.get("tavsiye_yon") == "SHORT")
        notr_say  = sum(1 for k in kayitlar if k.get("tavsiye_yon") == "NÖTR")
        ozet_txt  = f"▲ LONG: {long_say}    ▼ SHORT: {short_say}    ● NÖTR: {notr_say}"
        ozet_box  = FancyBboxPatch((0, 0), 1.0, satir_h * 0.9,
                                   boxstyle="square,pad=0",
                                   facecolor=HEADER, edgecolor="none",
                                   transform=ax.transAxes, clip_on=False)
        ax.add_patch(ozet_box)
        ax.text(0.5, satir_h * 0.45, ozet_txt,
        ...
        plt.close(fig)
        buf.seek(0)
        return buf.read(), f"📋 TP Confluance Log — {tarih_str}\n/tplog"
    except Exception as e:
        print(f"[TPLOG_GORSEL] Hata: {e}")
        return None
