    ...
    try:
        ...
        satir_gruplari = [
            ("SİNYAL ADEDİ", [
                ...
                ("Long",     lambda r: (str(r["tip_n"]["Long"]), HDR_COL)),
                ("Short",    lambda r: (str(r["tip_n"]["Short"]), HDR_COL)),
                ("St. Sell", lambda r: (str(r["tip_n"]["Strong Sell"]), HDR_COL)),
                ("St. Buy",  lambda r: (str(r["tip_n"]["Strong Buy"]), HDR_COL)),
            ]),
            ("TP ORANI", [
        ...
        def _piy_hucre(r, piy):
            ...
            if v["n"]:
                return (f"{v['n']} · {oran_str(v['tp'], v['n'])}", TEXT_W)
            return ("—", HDR_COL)
        ...
        fig_w = max(11.0, label_w*11 + n_coin * 1.55)
        ...
        for gi, (grup_adi, satirlar_metrik) in enumerate(satir_gruplari):
            ...
            for si, (etiket, fn) in enumerate(satirlar_metrik):
                ...
                ax.text(0.02, yy, etiket, fontsize=10, color=HDR_COL, ha="left", va="top")
                for x, r in zip(coin_x, satirlar):
                    deger, renk = fn(r)
                    ax.text(x, yy, deger, fontsize=9.5, color=renk, ha="center", va="top")
                yy -= satir_h
