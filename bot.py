...
def top3_guncelle(snapshot=None):
    """Saatlik raporda top3 whitelist güncelle."""
    global top3_whitelist
    sirala = koin_sirala_bugun(snapshot)
    if not sirala:
        return []
    # İlk sıradaki oranı baz al, eşit olanları da al
    en_yuksek_oran = sirala[0]["oran"]
    # Top 3 bul — eşitlik durumunda hepsi dahil
    top3 = []
    for item in sirala:
        if len(top3) >= 3 and item["oran"] < en_yuksek_oran:
            break
        if len(top3) < 3 or item["oran"] == sirala[len(top3)-1]["oran"]:
            top3.append(item["sym"])
        en_yuksek_oran = item["oran"] if len(top3) < 3 else sirala[2]["oran"]

    # %50 ve üzeri başarı oranına sahip tüm koinler whitelist'e gir
    elli_plus = [item["sym"] for item in sirala if item["oran"] >= 50.0]

    with top3_kilit:
        top3_whitelist = elli_plus
    print(f"[TOP3] %50+ whitelist güncellendi: {top3_whitelist}")
    return elli_plus
...
def _analiz_ozet_telegram(coin_verileri, veriler, zaman_str):
    """Telegram HTML formatında piyasa özet yorumu — olta tekrarı yok, sadece veri analizi."""
    def fmt(f):
        if f is None: return "—"
        try: f = float(f)
        except: return "—"
    ...
    breadth  = veriler.get("_breadth", 0)
    btc_cvd  = veriler.get("btc_cvd", 0) or 0
    btc_fund = veriler.get("btc_funding", 0) or 0
    btc_dom  = veriler.get("btc_dom", 0) or 0
    fear     = veriler.get("fear_greed", {})
    fear_val = fear.get("value", 0) if fear else 0
    fear_lbl = fear.get("label", "") if fear else ""

    skorlar  = [v.get("skor", 0) for v in coin_verileri.values() if v.get("skor") is not None]
    ort_skor = round(sum(skorlar)/len(skorlar)) if skorlar else 0
    uyumsuz  = [sym.replace("USDT","") for sym,v in coin_verileri.items()
                if v.get("cvd") and "uyumsuz" in v["cvd"].get("uyum","")]

    # Genel piyasa durumu
    if ort_skor >= 70 and breadth >= 7:
        durum_ikon = "🟢"
        durum_metin = f"Güçlü yükseliş — {breadth}/10 coin EMA20 üstünde"
    elif ort_skor >= 55:
        durum_ikon = "🟡"
        durum_metin = f"Yükseliş eğilimli — {breadth}/10 coin EMA20 üstünde"
    elif ort_skor >= 40:
        durum_ikon = "⚪"
        durum_metin = f"Yön belirsiz — {breadth}/10 coin EMA20 üstünde"
    else:
        durum_ikon = "🔴"
        durum_metin = f"Satış baskısı — {breadth}/10 coin EMA20 üstünde"

    satirlar.append(f"{durum_ikon} <b>Genel Durum:</b> {durum_metin}")
    satirlar.append(f"📊 Ort. Skor: <b>{ort_skor}</b> | Fear&amp;Greed: <b>{fear_val}</b> ({fear_lbl}) | BTC Dom: <b>%{btc_dom:.1f}</b>")

    # CVD uyumsuzlukları — sahte hareket sinyali
    if uyumsuz:
        uyumsuz_str = ", ".join(uyumsuz)
        satirlar.append(f"\n⚠️ <b>CVD Uyumsuzluğu:</b> {uyumsuz_str}")
        satirlar.append("  CVD fiyatla zıt hareket ediyor — bu coinlerde yükseliş/düşüş suni olabilir.")
        satirlar.append("  Fiyat ↑ CVD ↓ → Satış gizleniyor (sahte pump riski)")
        satirlar.append("  Fiyat ↓ CVD ↑ → Birikim var (sahte dump, dipten alma fırsatı)")
    else:
        satirlar.append("\n✅ <b>CVD:</b> Tüm coinlerde fiyat-CVD uyumlu — momentum gerçek")

    # BTC Funding — yüksekse long taraf aşırı
    if abs(btc_fund) >= 0.01:
        fund_yorum = "long kalabalığı — düzeltme riski" if btc_fund > 0 else "short kalabalığı — sıkışma riski"
        satirlar.append(f"\n💰 <b>BTC Funding:</b> {btc_fund:+.4f}% — {fund_yorum}")
    elif abs(btc_fund) >= 0.005:
        satirlar.append(f"\n💰 <b>BTC Funding:</b> {btc_fund:+.4f}% — dengeli")
    
    # CVD 1H — anlık alım baskısı
    if abs(btc_cvd) > 1000:
        cvd_ikon = "📈" if btc_cvd > 0 else "📉"
        cvd_yorum = "alım baskısı birikim var" if btc_cvd > 0 else "satış baskısı hakim"
        satirlar.append(f"\n{cvd_ikon} <b>BTC CVD (1H):</b> {btc_cvd:+,.0f} — {cvd_yorum}")
    ...
    if toplam_lik >= 0.5:
        if long_lik > short_lik * 2:
            lik_yorum = f"longlar ezildi ({long_lik:.1f}M) — dikkat, zayıf yapı"
        elif short_lik > long_lik * 2:
            lik_yorum = f"shortlar ezildi ({short_lik:.1f}M) — güçlü alım baskısı"
        else:
            lik_yorum = f"dengeli ({long_lik:.1f}M long, {short_lik:.1f}M short)"
        satirlar.append(f"\n💥 <b>Son 30dk Likidasyon:</b> {toplam_lik:.1f}M USD — {lik_yorum}")

    # Market yönü sonucu
    satirlar.append("\n" + "─"*30)
    taker_long_sayisi = sum(1 for v in coin_verileri.values()
                           if v.get("taker") and v["taker"].get("taker_buy_pct",50) >= 55)
    taker_short_sayisi = sum(1 for v in coin_verileri.values()
                            if v.get("taker") and v["taker"].get("taker_buy_pct",50) <= 45)

    if taker_long_sayisi >= 6 and ort_skor >= 60:
        yon = "📈 <b>YUKARI</b> — taker alıcı çoğunlukta, momentum güçlü"
    elif taker_short_sayisi >= 6 or ort_skor < 40:
        yon = "📉 <b>AŞAĞI</b> — satış baskısı ve taker satıcı çoğunlukta"
    elif uyumsuz and len(uyumsuz) >= 3:
        yon = "⚠️ <b>DİKKATLİ</b> — CVD uyumsuzluğu yaygın, suni hareket riski yüksek"
    else:
        yon = "↔️ <b>YATAY</b> — net yön yok, pozisyon açmak için sinyal bekle"

    satirlar.append(f"🧭 <b>Market Yönü:</b> {yon}")
    satirlar.append(f"<i>Kaynak: Binance Futures + Spot  ·  {zaman_str}</i>")
    return "\n".join(satirlar)
