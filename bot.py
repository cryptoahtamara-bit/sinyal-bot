...
#      Long icin ayri, Short icin ayri 2 mesaj
#      Fibonacci %23.6/%38.2/%50/%61.8/%78.6/%100 + extension %127.2/%161.8
...
PNL_RAPOR_ENABLED  = True   # /pnl_kapat ve /pnl_ac komutlariyla kontrol edilir
    ...
    if AUTO_TRADE_ENABLED:
        try:
            ...
            if is_trade:
                ...
                # Hedge muafiyeti: aktif_pozisyonlar dict'i + API her ikisine bak
                # Ters yön mevcut pozisyon varsa whitelist atlanır
                _wl_mevcut_api  = pozisyon_al(sym_key, is_long) or pozisyon_al(sym_key, not is_long)
                _wl_mevcut_dict = None
                with pozisyon_kilit:
                    _wl_mevcut_dict = (aktif_pozisyonlar.get(sym_key) or
                                       aktif_pozisyonlar.get(sym_key + "_HEDGE"))
                _wl_mevcut = _wl_mevcut_api or _wl_mevcut_dict
                _wl_hedge  = bool(_wl_mevcut) and (
                    (_wl_mevcut_api  and _wl_mevcut_api["is_long"]  != is_long) or
                    (_wl_mevcut_dict and _wl_mevcut_dict["is_long"] != is_long)
                )
...
def _olta_fib_hesapla(closes, highs, lows, fiyat, interval):
    """
    Belirtilen TF icin Fibonacci destekli olta seviyeleri hesapla.
    """
    if not closes or len(closes) < 30:
        return None
    lb_map = {"5m": 40, "15m": 50, "1h": 60, "4h": 60, "1d": 60}
    lb = lb_map.get(interval, 50)
    lb = min(lb, len(highs) - 1)
    ...
    def rd(v):
        return round(v, dec)
    # LONG
    long_candidates = [f for f in [fib_618, fib_500, fib_382] if f < fiyat * 1.002]
    long_giris = rd(max(long_candidates)) if long_candidates else rd(fiyat * 0.985)
    long_sl = rd(min(fib_786, long_giris * 0.988))
    if long_sl >= long_giris:
        long_sl = rd(long_giris * 0.988)
    tp_adaylar = sorted([f for f in [fib_100, fib_1272, fib_1618] if f > fiyat * 1.005])
    long_tp1 = rd(tp_adaylar[0]) if len(tp_adaylar) > 0 else rd(fiyat * 1.015)
    long_tp2 = rd(tp_adaylar[1]) if len(tp_adaylar) > 1 else rd(fiyat * 1.030)
    long_tp3 = rd(tp_adaylar[2]) if len(tp_adaylar) > 2 else rd(fiyat * 1.050)
    # SHORT
    short_candidates = [f for f in [fib_618, fib_786, fib_100] if f > fiyat * 0.998]
    short_giris = rd(min(short_candidates)) if short_candidates else rd(fiyat * 1.015)
    short_sl = rd(max(swing_high, short_giris) * 1.012)
    short_tp_adaylar = sorted([f for f in [fib_382, fib_236, fib_0] if f < fiyat * 0.995], reverse=True)
    short_tp1 = rd(short_tp_adaylar[0]) if len(short_tp_adaylar) > 0 else rd(fiyat * 0.985)
    short_tp2 = rd(short_tp_adaylar[1]) if len(short_tp_adaylar) > 1 else rd(fiyat * 0.970)
    short_tp3 = rd(short_tp_adaylar[2]) if len(short_tp_adaylar) > 2 else rd(fiyat * 0.950)
    return {
        "long_giris":  long_giris,  "long_sl":     long_sl,
        "long_tp1":    long_tp1,    "long_tp2":    long_tp2,    "long_tp3":    long_tp3,
        "short_giris": short_giris, "short_sl":    short_sl,
        "short_tp1":   short_tp1,   "short_tp2":   short_tp2,   "short_tp3":   short_tp3,
...
def _olta_mtf_sorgula(symbol_raw):
    """
    Multi-timeframe Fibonacci olta sorgulama.
    5dk, 15dk, 1s, 4s, Gun icin ayri Fib hesabi yapar.
    Iki mesaj doner: (long_msg, short_msg)
    """
    import concurrent.futures
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as ex:
        ...
        for fut in concurrent.futures.as_completed(futures):
            lbl, r = fut.result()
            results[lbl] = r
    def fmt_row(lbl, r, side):
        if not r:
            return "  {:<5} | veri yok".format(lbl)
        if side == "long":
            return ("  {:<5} | {:<13} | {:<13} | {:<13} | {:<13} | {}".format(
                lbl, p(r["long_giris"]), p(r["long_sl"]),
                p(r["long_tp1"]), p(r["long_tp2"]), p(r["long_tp3"])))
        else:
            return ("  {:<5} | {:<13} | {:<13} | {:<13} | {:<13} | {}".format(
                lbl, p(r["short_giris"]), p(r["short_sl"]),
                p(r["short_tp1"]), p(r["short_tp2"]), p(r["short_tp3"])))
    baslik = "  {:<5} | {:<13} | {:<13} | {:<13} | {:<13} | {}".format("TF","Giris","Stop","TP1","TP2","TP3")
    ayrac  = "  " + "-" * 80
    long_satirlar  = [fmt_row(lbl, results.get(lbl), "long")  for lbl, _, _ in TF_LIST]
    short_satirlar = [fmt_row(lbl, results.get(lbl), "short") for lbl, _, _ in TF_LIST]
    nl = "\n"
    long_msg  = ("\U0001f7e2 <b>" + sym + " \u2014 LONG OLTALAR</b>\n"
                 "\U0001f4b0 Anlik Fiyat: <b>" + p(fiyat) + "</b>\n"
                 "\U0001f4d0 Fibonacci Multi-Timeframe\n\n"
                 "<pre>" + baslik + "\n" + ayrac + "\n" + nl.join(long_satirlar) + "\n" + ayrac + "</pre>\n"
                 "\U0001f3a3 Giris | Stop | TP1 | TP2 | TP3")
    short_msg = ("\U0001f534 <b>" + sym + " \u2014 SHORT OLTALAR</b>\n"
                 "\U0001f4b0 Anlik Fiyat: <b>" + p(fiyat) + "</b>\n"
                 "\U0001f4d0 Fibonacci Multi-Timeframe\n\n"
                 "<pre>" + baslik + "\n" + ayrac + "\n" + nl.join(short_satirlar) + "\n" + ayrac + "</pre>\n"
                 "\U0001f3a3 Giris | Stop | TP1 | TP2 | TP3")
...
BOT_VERSIYON = "v411"
print(f"[BASLANGIC] ========== BOT VERSIYON: v411 ==========")
