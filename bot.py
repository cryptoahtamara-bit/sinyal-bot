...
def _analiz_ozet_telegram(coin_verileri, veriler, zaman_str):
    ...
    satirlar.append("<i>Mevcut koşullar (Funding, OI, CVD, Likidasyon, F&G, Breadth) geçmiş büyük BTC hareketleriyle karşılaştırıldı.</i>")
    ...
    try:
        ...
        # Fear & Greed ve Breadth
        mevcut_fg_deger   = fg_skoru          # sayısal F&G (0-100)
        mevcut_breadth_n  = ema_ustunde_sayi   # EMA üstündeki coin sayısı
        mevcut_toplam_n   = len(analiz_verileri)

        # Benzerlik hesapla (6 kriter)
        ...
        fg_str    = f"{mevcut_fg_deger}" if mevcut_fg_deger is not None else "—"
        fg_kat    = _fg_kategori(mevcut_fg_deger) or "—"
        brd_kat   = _breadth_kategori(mevcut_breadth_n, mevcut_toplam_n)
        satirlar.append(
            f"📊 <b>Mevcut:</b> Funding <b>{btc_fund:+.4f}%</b>  ·  "
            f"OI <b>{oi_str}</b>  ·  "
            f"CVD <b>{mevcut_cvd_uyum.replace('_', ' ')}</b>  ·  "
            f"Lik. <b>{mevcut_lik_taraf}</b>  ·  "
            f"F&G <b>{fg_str}</b> ({fg_kat.replace('_', ' ')})  ·  "
            f"Breadth <b>{brd_kat}</b>"
        )
        satirlar.append("")
        ...
        satirlar.append(
            ...
            "Fear &amp; Greed kategorisi ve Breadth (EMA üstü coin oranı). "
...
BOT_VERSIYON = "v446"
print(f"[BASLANGIC] ========== BOT VERSIYON: v446 ==========")
