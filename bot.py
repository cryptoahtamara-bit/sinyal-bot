...
def tp_sonuc_guncelle(message_id, tp1_ok, tp2_ok, tp3_ok, tp4_ok, tp5_ok, tp6_ok, tp7_ok, tp8_ok, tp9_ok, tp10_ok, sl_ok):
    with gunluk_kilit:
...
def tp_kontrol_gonder(symbol, sinyal, timeframe, tp1, tp2, tp3, tp4, tp5, tp6, tp7, tp8, tp9, tp10, sl,
    ...
    def mesaj_olustur(tp1_ok, tp2_ok, tp3_ok, tp4_ok, tp5_ok, tp6_ok, tp7_ok, tp8_ok, tp9_ok, tp10_ok, sl_ok, erken=False):
        ...
        tp_kontroller = [
            ...
            (tp4, tp4_ok, "TP4"), (tp5, tp5_ok, "TP5"), (tp6, tp6_ok, "TP6"),
            (tp7, tp7_ok, "TP7"), (tp8, tp8_ok, "TP8"), (tp9, tp9_ok, "TP9"),
            (tp10, tp10_ok, "TP10"),
        ]
    ...
    while gecen_sure < toplam_sure:
        ...
        if (any([tp1_ok, tp2_ok, tp3_ok, tp4_ok, tp5_ok, tp6_ok, tp7_ok, tp8_ok, tp9_ok, tp10_ok]) or sl_ok) and not erken_bildirim_gonderildi:
            ...
            tp_sonuc_guncelle(message_id, tp1_ok, tp2_ok, tp3_ok, tp4_ok, tp5_ok, tp6_ok, tp7_ok, tp8_ok, tp9_ok, tp10_ok, sl_ok)
            return
...
def parse_plain(raw: str):
    ...
    tp1, tp2, tp3, tp4, tp5, tp6, tp7, tp8, tp9, tp10, sl = (None,) * 11
    ...
    for line in lines:
        ...
        if line.upper().startswith("TP"):
            parts = line.split(maxsplit=1)
            if len(parts) == 2:
                tp_label = parts[0].upper()
                tp_value = parts[1].strip()
                tp_map = {
                    "TP1": "tp1", "TP2": "tp2", "TP3": "tp3", "TP4": "tp4", "TP5": "tp5",
                    "TP6": "tp6", "TP7": "tp7", "TP8": "tp8", "TP9": "tp9", "TP10": "tp10"
                }
                if tp_label in tp_map:
                    locals()[tp_map[tp_label]] = tp_value
...
def webhook():
    ...
    print(f"[SINYAL] {symbol} {sinyal} @ {price} ({timeframe}) | TP1={tp1} TP2={tp2} TP3={tp3} TP4={tp4} TP5={tp5} TP6={tp6} TP7={tp7} TP8={tp8} TP9={tp9} TP10={tp10} SL={sl}")
