...
def _tp_kosul_rapor_gorsel(veri=None, snapshot=None):
    """7 bölümlü TP koşul raporunu PNG olarak üretir."""
    try:
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as plt
    ...
    BOLUM_RENKLER = [BLUE, GREEN, AMBER, PURPLE, TEAL, CORAL, PINK]
    BOLUM_BASLIKLAR = [
        "1. Sinyal Tipi — Karşılaştırma",
        "2. Piyasa Koşulları — Karşılaştırma",
        "3. TP Seviyesi — Dağılımı",
        "4. Coin Bazlı — Karşılaştırma",
        "5. Zaman Dilimi — Karşılaştırma",
        "6. En Kârlı Kombinasyonlar",
        "7. Öneri Mekanizması — En Güvenilir Koşullar",
    ]

    def oran_renk(oran_str):
        try:
            val = float(oran_str.replace("%",""))
    ...
    def ciz_tablo(ax, baslik, renk, satirlar, sutunlar, y_start, satir_h=0.38, sol=0.1, sag=11.9):
        genislik = sag - sol
        # Başlık bandı
        ax.add_patch(FancyBboxPatch((sol, y_start - 0.35), genislik, 0.38,
                                    boxstyle="round,pad=0.02", linewidth=0, facecolor=renk+"33", zorder=1))
        ax.text(sol + 0.15, y_start - 0.15, baslik, va="center", fontsize=11,
        ...
        for ci, sut in enumerate(sutunlar):
            ax.text(sol + ci*col_w + (col_w*0.1 if ci==0 else col_w/2),
                    y, sut, va="center",
                    ha="left" if ci==0 else "center",
                    fontsize=9, color=TEXT_G, fontweight="bold")
        y -= 0.30

        for ri, satir in enumerate(satirlar):
            bg = ROW_ODD if ri % 2 == 0 else ROW_EVEN
            ax.add_patch(FancyBboxPatch((sol, y - satir_h + 0.04), genislik, satir_h - 0.06,
                                        boxstyle="round,pad=0.01", linewidth=0, facecolor=bg, zorder=0))
            for ci, hucre in enumerate(satir):
                ...
                ax.text(sol + ci*col_w + (col_w*0.1 if ci==0 else col_w/2),
                        ...
                        ha="left" if ci==0 else "center",
                        fontsize=10, color=renk_h,
                        fontweight="bold" if ci == len(satir)-1 else "normal")
            y -= satir_h

        return y - 0.2

    # Yükseklik hesapla
    n_kosul = len(veri["kosul"])
    n_coin  = len(veri["coin"])
    n_tf    = len(veri["tf"])
    n_komb  = min(len(veri["kombinasyon"]), 12)
    n_oneri = max(len(veri["oneri"]), 1)
    toplam_satir = 4 + n_kosul + 4 + n_coin + n_tf + n_komb + n_oneri
    fig_h = 3.5 + toplam_satir * 0.38 + 7 * 0.8
    ...
    ax.text(6, fig_h - 0.5, "TP Koşul Raporu", ha="center", fontsize=16,
            fontweight="bold", color=TEXT_W)
    ax.text(6, fig_h - 0.95, f"Toplam: {veri['toplam']} sinyal  |  TP: {veri['toplam_tp']}  |  {simdi}",
            ha="center", fontsize=10, color=TEXT_G)

    y = fig_h - 1.4

    # Bölüm 1 — Sinyal tipi
    satirlar1 = [(t,)+v for t,v in veri["sinyal_tipi"].items()]
    y = ciz_tablo(ax, BOLUM_BASLIKLAR[0], BOLUM_RENKLER[0],
                  satirlar1, ["Sinyal Tipi","Toplam","TP","TP Olmayan","Oran"], y)

    # Bölüm 2 — Piyasa koşulları
    satirlar2 = [(t,)+v for t,v in veri["kosul"].items()]
    y = ciz_tablo(ax, BOLUM_BASLIKLAR[1], BOLUM_RENKLER[1],
                  satirlar2, ["Koşul","Toplam","TP","TP Olmayan","Oran"], y)

    # Bölüm 3 — TP seviyesi
    satirlar3 = [(lbl, v[0], v[1], "", "") for lbl,v in veri["tp_seviye"].items()]
    satirlar3.append(("En çok ulaşılan", veri["en_cok_tp"], "", "", ""))
    y = ciz_tablo(ax, BOLUM_BASLIKLAR[2], BOLUM_RENKLER[2],
                  satirlar3, ["Seviye","Toplam TP","Oran","",""], y)

    # Bölüm 4 — Coin
    satirlar4 = [(t,)+v for t,v in veri["coin"].items()]
    y = ciz_tablo(ax, BOLUM_BASLIKLAR[3], BOLUM_RENKLER[3],
                  satirlar4, ["Coin","Toplam","TP","TP Olmayan","Oran"], y)

    # Bölüm 5 — TF
    satirlar5 = [(t,)+v for t,v in veri["tf"].items()]
    y = ciz_tablo(ax, BOLUM_BASLIKLAR[4], BOLUM_RENKLER[4],
                  satirlar5, ["TF","Toplam","TP","TP Olmayan","Oran"], y)

    # Bölüm 6 — Kombinasyonlar (en yüksek oranlı 12)
    komb_sirali = sorted(veri["kombinasyon"].items(), key=lambda x: x[1][1], reverse=True)[:12]
    satirlar6 = [(k,)+v for k,v in komb_sirali]
    y = ciz_tablo(ax, BOLUM_BASLIKLAR[5], BOLUM_RENKLER[5],
                  satirlar6, ["Kombinasyon","Toplam","TP","TP Olmayan","Oran"], y)
    ...
    y = ciz_tablo(ax, BOLUM_BASLIKLAR[6], BOLUM_RENKLER[6],
                  satirlar7, ["Kombinasyon","Toplam","TP","TP Olmayan","Oran"], y)

    # Alt not
    ax.text(6, 0.15, "TP Olmayan = SL + Devam Eden  |  Öneri: min. 20 sinyal  |  %50+ Önerilen  |  %35- Kaçınılan",
            ha="center", fontsize=8, color=TEXT_G)
...
def _tp_kosul_rapor_zamanlayici():
    ...
    while True:
        try:
            ...
            veri = _tp_kosul_rapor_hesapla(snapshot=snapshot)
            img  = _tp_kosul_rapor_gorsel(veri=veri)
            if img:
                _topic_foto_gonder_filigranli(TOPIC_RAPOR, img, "TP Koşul Raporu")
            else:
