...
def _liq_heatmap_gercek(symbol, saat=6):
    ...
    try:
        ...
        try:
            ...
            # Haritadaki etiketlerle aynı mantıkla likidasyon bölgelerini hesapla
            def _bolge_hesapla(band_arr, fiyat_ref, taraf):
                """band_arr'dan en yoğun grubu bul, fiyat altı=long, üstü=short filtrele."""
                esik = band_arr.max() * 0.30
                if esik <= 0: return None
                gruplar = []
                i = 0
                while i < n_pb:
                    if band_arr[i] >= esik:
                        j = i
                        while j < n_pb and band_arr[j] >= esik:
                            j += 1
                        fiyat_alt = price_min + (i / n_pb) * (price_max - price_min)
                        fiyat_ust = price_min + (j / n_pb) * (price_max - price_min)
                        merkez    = (fiyat_alt + fiyat_ust) / 2
                        hacim     = band_arr[i:j].sum()
                        # Taraf filtresi
                        if taraf == "long" and merkez >= fiyat_ref: i = j; continue
                        if taraf == "short" and merkez <= fiyat_ref: i = j; continue
                        gruplar.append((hacim, round(fiyat_alt, 2), round(fiyat_ust, 2)))
                        i = j
                    else:
                        i += 1
                if not gruplar: return None
                gruplar.sort(reverse=True)
                _, alt, ust = gruplar[0]
                return (alt, ust)
        ...
        try:
            liq_long_zone  = _bolge_hesapla(band_long_usd,  fiyat, "long")  if band_long_usd  is not None else None
            liq_short_zone = _bolge_hesapla(band_short_usd, fiyat, "short") if band_short_usd is not None else None
...
BOT_VERSIYON = "v508"
print(f"[BASLANGIC] ========== BOT VERSIYON: v508 ==========")
