...
def _ls_gonder(chat_id=None):
    """L/S grafiği + yorum gönder. (v281 — direkt Binance'den çeker, /kod etiketi ekler)"""
    ...
    try:
        ...
        yorum_raw = _ls_yorum(btc_ls, eth_ls)
        yorum = f"/ls\n\n{yorum_raw}"
        _telegram_topic_mesaj_gonder(TOPIC_ANALIZ, yorum)
        print("[LS] Tamamlandi.")
    except Exception as e:
...
def _trend_gonder(chat_id=None):
    ...
    metin_raw = _trend_metin(sonuclar, btc_dom, eth_dom, total_mcap, mcap_change, fg_deger, fg_sinif, zaman_str)
    metin = f"/trendyonu\n\n{metin_raw}"
...
def _marketyonu_gonder():
    ...
    try:
        ...
        if not veri_map:
            ...
            return
        yorum_raw = _marketyonu_yorum(veri_map)
        mesaj = f"/marketyonu\n\n{yorum_raw}"
        _telegram_topic_mesaj_gonder(TOPIC_ANALIZ, mesaj)
...
BOT_VERSIYON = "v281"
print(f"[BASLANGIC] ========== BOT VERSIYON: v281 ==========")
