...
def mexc_test():
    ...
    sonuc = {"version": "v190", "proxy": bool(MEXC_PRIVATE_PROXY)}
...
def webkey_test():
    ...
    # Test 4: Query string imzasi ile gercek order
    if MEXC_API_KEY and MEXC_API_SECRET:
        test_body = {"symbol": "BTC_USDT", "price": 0, "vol": 0.001, "side": 1, "type": 5, "openType": 2, "leverage": 20}
        ts = str(int(time.time() * 1000))

        # Yontem 1: Parametreler alfabetik sirali, imza query string uzerinden
        try:
            sorted_params = dict(sorted(test_body.items()))
            param_str = "&".join([f"{k}={v}" for k, v in sorted_params.items()])
            sign_str = MEXC_API_KEY + ts + param_str
            sig = hmac.new(MEXC_API_SECRET.encode(), sign_str.encode(), hashlib.sha256).hexdigest()
            hdrs1 = {
                "ApiKey": MEXC_API_KEY,
                "Request-Time": ts,
                "Signature": sig,
            ...
            r1 = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
                               headers=hdrs1, json={}, params=sorted_params, timeout=8)
            sonuc["query_imza_alfabetik"] = {"status": r1.status_code, "body": r1.text[:200]}
        except Exception as e:
            sonuc["query_imza_alfabetik"] = {"error": str(e)[:50]}

        # Yontem 2: Imza JSON body uzerinden, parametreler query string'de
        try:
            sign_str2 = MEXC_API_KEY + ts + json.dumps(test_body, separators=(",",":"), ensure_ascii=False)
            sig2 = hmac.new(MEXC_API_SECRET.encode(), sign_str2.encode(), hashlib.sha256).hexdigest()
            hdrs2 = {
                "ApiKey": MEXC_API_KEY,
                "Request-Time": ts,
                "Signature": sig2,
            ...
            r2 = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
                               headers=hdrs2, json={}, params=test_body, timeout=8)
            sonuc["query_imza_json"] = {"status": r2.status_code, "body": r2.text[:200]}
        except Exception as e:
            sonuc["query_imza_json"] = {"error": str(e)[:50]}
...
print(f"[BASLANGIC] ========== BOT VERSIYON: v190 ==========")
