...
def _tp_kosul_yorum_uret(veri):
    ...
    # Tüm kombinasyonların confluance analizi — en iyi TP koşulu
...
def _tp_confluance_skoru_hesapla(veri):
    ...
    mega_coin  = {}   # coin dahil (granüler)
    mega_nokoin = {}  # coin hariç (geniş, daha güvenilir)
    ...
    for s in _kayitlar:
        ...
        # Coin'siz faktörler (TF + sinyal + piyasa koşulları)
        nokoin_parts = [tf, st]
        if piy:   nokoin_parts.append(piy)
        if fund:  nokoin_parts.append(fund)
        if oi:    nokoin_parts.append(oi)
        if rsi:   nokoin_parts.append(rsi)
        if hacim: nokoin_parts.append(hacim)
        if fg:    nokoin_parts.append(fg)
        nk = " + ".join(nokoin_parts)
        if nk not in mega_nokoin:
            mega_nokoin[nk] = {"tp": 0, "toplam": 0}
        mega_nokoin[nk]["toplam"] += 1
        if tp_mi(s):
            mega_nokoin[nk]["tp"] += 1
        ...
        if not coin:
            continue
        coin_parts = [coin] + nokoin_parts
        ck = " + ".join(coin_parts)
        if ck not in mega_coin:
            mega_coin[ck] = {"tp": 0, "toplam": 0}
        mega_coin[ck]["toplam"] += 1
        if tp_mi(s):
            mega_coin[ck]["tp"] += 1

    def _sirala(d, min_sin):
        fil = {k: v for k, v in d.items() if v["toplam"] >= min_sin}
        return sorted(
            fil.items(),
            key=lambda x: (
    ...
    MIN_MEGA = 5
    # Önce coin'siz dene (daha güvenilir, daha fazla sinyal)
    sirali_nokoin = _sirala(mega_nokoin, MIN_MEGA * 2)  # coin'siz için daha yüksek eşik
    sirali_coin   = _sirala(mega_coin,   MIN_MEGA)

    # Beraberse coin'siz tercih et, değilse en yüksek skoru al
    # top3: önce coin'siz sonuçları al, coin'siz yetmezse coin'li ekle
    seen_keys = set()
    candidates = []
    for k, v in sirali_nokoin[:10]:
        oran = v["tp"] / v["toplam"] * 100
        skor = oran * math.log(max(v["toplam"], 1))
        candidates.append((k, v, skor))
        seen_keys.add(k)

    for k, v in sirali_coin[:10]:
        oran = v["tp"] / v["toplam"] * 100
        skor = oran * math.log(max(v["toplam"], 1))
        # Coin'li ekleme: bu kombinasyonun coin'siz versiyonu zaten listede değilse
        nokoin_key = " + ".join(k.split(" + ")[1:])  # coin'i çıkar
        if nokoin_key not in seen_keys:
            candidates.append((k, v, skor))

    candidates.sort(key=lambda x: (x[2], x[1]["toplam"]), reverse=True)

    top3_mega = []
    for k, v, _ in candidates[:3]:
...
def _tplog_gorsel_uret(kayitlar, tarih_str):
    ...
    Her saat: En İyi TP Koşulları (top3_mega) gösterilir.
    Sütunlar: Saat | Koşul | TP% (TP/Sin)
    ...
    try:
        ...
        YON_EMOJI = {"LONG": "▲", "SHORT": "▼", "NÖTR": "●"}
        SIRA_EMOJI = ["🥇", "🥈", "🥉"]

        # Her saat + top3_mega satırlarını hazırla
        # satır: (saat_str, sira_emoji, kosul, tp_orani, tp_sayisi, sinyal, yon, ilk_mi)
        satirlar = []
        for k in kayitlar:
            saat   = k.get("zaman", "")[-5:]
            yon    = k.get("tavsiye_yon", "NÖTR")
            top3   = k.get("top3_mega") or []

            if not top3:
                # top3_mega yok — eski top5'ten al
                top5 = k.get("top5", [])
                if top5:
                    item = top5[0]
                    kosul = item.get("kombinasyon", "—")
                    if len(kosul) > 30: kosul = kosul[:28] + "…"
                    satirlar.append((saat, "🥇", kosul,
                                     item.get("tp_orani","—"),
                                     str(item.get("tp_sayisi","—")),
                                     str(item.get("sinyal","—")),
                                     yon, True))
                else:
                    satirlar.append((saat, "—", "—", "—", "—", "—", yon, True))
            else:
        ...
        TABLO_SAG = 0.90  # tablo sağ sınırı
        # figsize: genişlik TABLO_SAG oranıyla ayarlanır — sağda boşluk kalmaz
        fig_w   = 12 * TABLO_SAG
        fig_h   = max(3.0, 1.0 + n_satir * 0.32)
        fig, ax = plt.subplots(figsize=(fig_w, fig_h))
        fig.patch.set_facecolor(BG)
        ax.set_facecolor(BG)
        ax.axis("off")
        ...
        ax.text(0.5, 1.01, f"📋  En İyi TP Koşulları  —  {tarih_str}",
                transform=ax.transAxes, ha="center", va="bottom",
                fontsize=12, fontweight="bold", color=TEXT_W)
        ax.text(0.5, 0.98, f"{n_kayit} saat  |  her saat için top 3 mega kombinasyon",
                transform=ax.transAxes, ha="center", va="bottom",
                fontsize=8, color=TEXT_G)

        # Sütun konumları — dar, Telegram'a uygun
        # Saat | Sıra | Koşul          | TP%  | TP/Sin | Yön
        COLS  = ["Saat",  "#", "Koşul",  "TP%",  "TP/Sin", "Yön"]
        XPOS  = [0.005, 0.055, 0.085,    0.55,   0.635,    0.76]
        ...
        hdr_box = FancyBboxPatch((0, 1.0 - satir_h), TABLO_SAG, satir_h,
        ...
        for col, x, al in zip(COLS, XPOS, ALIGN):
            ax.text(x, y_baslik, col,
                    ...
                    fontsize=8.5, fontweight="bold", color=TEXT_W)

        # Veri satırları
        grup_idx = -1
        for i, (saat, sira, kosul, tp, tp_say, sin_say, yon, ilk_mi) in enumerate(satirlar):
            if saat:
                grup_idx += 1
            ...
            row_box = FancyBboxPatch((0, y_top - satir_h), TABLO_SAG, satir_h,
                                     boxstyle="square,pad=0",
                                     facecolor=bg, edgecolor="none",
                                     transform=ax.transAxes, clip_on=False)
            ...
            if not ilk_mi:
                ax.plot([0.055, TABLO_SAG - 0.01], [y_top, y_top],
                        color=TEXT_DIM, linewidth=0.25,
                        transform=ax.transAxes, clip_on=False)
            ...
            yon_renk  = YON_RENK.get(yon, AMBER) if yon else TEXT_W
            yon_lbl   = (YON_EMOJI.get(yon,"") + " " + yon).strip() if yon else ""
            sira_renk = {SIRA_EMOJI[0]: GREEN, SIRA_EMOJI[1]: TEAL, SIRA_EMOJI[2]: TEXT_W}.get(sira, TEXT_W)
            tp_sin_lbl = f"{tp_say}/{sin_say}" if tp_say != "—" else "—"

            degerler = [saat, sira, kosul, tp, tp_sin_lbl, yon_lbl]
            renkler  = [TEXT_W, sira_renk, TEXT_W, tp_renk, TEXT_W, yon_renk]
            fontlar  = [8, 8, 8 if ilk_mi else 7.5, 8, 8, 8]

            for val, x, al, renk, fs in zip(degerler, XPOS, ALIGN, renkler, fontlar):
                if str(val):
                    ax.text(x, y_mid, str(val),
                            transform=ax.transAxes, ha=al, va="center",
                            fontsize=fs, fontweight="bold", color=renk)
        ...
        ozet_box  = FancyBboxPatch((0, 0), TABLO_SAG, satir_h * 0.85,
                                   boxstyle="square,pad=0",
                                   facecolor=HEADER, edgecolor="none",
                                   transform=ax.transAxes, clip_on=False)
        ax.add_patch(ozet_box)
        ax.text(TABLO_SAG / 2, satir_h * 0.42, ozet_txt,
                transform=ax.transAxes, ha="center", va="center",
                fontsize=8.5, color=TEXT_W)
        ...
        plt.close(fig)
        buf.seek(0)
        return buf.read(), f"📋 En İyi TP Koşulları — {tarih_str}\n/tplog"
    except Exception as e:
