...
def mexc_test():
    ...
    sonuc = {"version": "v185", "proxy": bool(MEXC_PRIVATE_PROXY)}
...
def webkey_test():
    ...
    # Test 4: Imzayi bos body ile hesapla, gercek body gonder
    if MEXC_API_KEY and MEXC_API_SECRET:
        test_body = {"symbol": "BTC_USDT", "price": 0, "vol": 0.001, "side": 1, "type": 5, "openType": 2, "leverage": 20}

        # Yontem 1: Imza bos {}, body gercek
        try:
            hdrs1, _ = mexc_headers({})  # imza bos body uzerinden
            r1 = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
                               headers=hdrs1, json=test_body, timeout=8)
            sonuc["imza_bos_body_gercek"] = {"status": r1.status_code, "body": r1.text[:200]}
        except Exception as e:
            sonuc["imza_bos_body_gercek"] = {"error": str(e)[:50]}

        # Yontem 2: Imza None, body gercek
        try:
            hdrs2, _ = mexc_headers(None)  # imza bos string uzerinden
            r2 = requests.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
                               headers=hdrs2, json=test_body, timeout=8)
            sonuc["imza_none_body_gercek"] = {"status": r2.status_code, "body": r2.text[:200]}
        except Exception as e:
            sonuc["imza_none_body_gercek"] = {"error": str(e)[:50]}

        # Yontem 3: Once bos body gonder (200 al), sonra gercek body gonder ayni session ile
        try:
            session = requests.Session()
            hdrs3, _ = mexc_headers({})
            session.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
                        headers=hdrs3, json={}, timeout=8)
            hdrs4, _ = mexc_headers(test_body)
            r4 = session.post(f"{MEXC_BASE_URL}/api/v1/private/order/submit",
                              headers=hdrs4, json=test_body, timeout=8)
            sonuc["session_ikinci_istek"] = {"status": r4.status_code, "body": r4.text[:200]}
        except Exception as e:
            sonuc["session_ikinci_istek"] = {"error": str(e)[:50]}
...
print(f"[BASLANGIC] ========== BOT VERSIYON: v185 ==========")
