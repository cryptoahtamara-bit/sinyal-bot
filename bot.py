...
def mexc_place_order(symbol, sinyal, tp1=None, sl=None):
    ...
    # Vol hesabi — contractSize kullanarak kontrat adedi hesapla
    # Kontrat adedi = (marjin * leverage) / (mp * contractSize)
    notional = MEXC_MARGIN_USDT * lev  # toplam pozisyon degeri USDT
    vol_raw = notional / (mp * contract_size)

    # Vol hassasiyeti: contractSize'dan hesapla
    if contract_size >= 1:
        vol = round(vol_raw, vol_decimals)
    else:
        # contractSize kucukse vol tam sayi (kontrat adedi), minimum 1
        vol = max(1, round(vol_raw))

    if vol <= 0:
        vol = 1  # son care: en az 1 kontrat

    print(f"[MEXC] Vol hesabi: notional={notional} cs={contract_size} vol_raw={vol_raw:.4f} -> vol={vol}")

    body = {"symbol": sym, "price": 0, "vol": vol, "side": side,
            "type": 5, "openType": 2, "leverage": lev}

    # TP/SL validation — MEXC kurallari:
    # LONG: takeProfitPrice > mp, stopLossPrice < mp
    # SHORT: takeProfitPrice < mp, stopLossPrice > mp
    # NOT: Oncelikle TP/SL olmadan emir ac, success aldiktan sonra TP/SL eklenebilir
    # Simdilik TP/SL gonderme — 5003 hatasini onle
    print(f"[MEXC] TP/SL gonderilmiyor (5003 hatasi onleme modu)")
    _ = tp1  # kullanilmadi uyarisi icin
...
def mexc_test():
    ...
    sonuc = {"version": "v209", "proxy": bool(MEXC_PRIVATE_PROXY)}
...
print(f"[BASLANGIC] ========== BOT VERSIYON: v209 ==========")
