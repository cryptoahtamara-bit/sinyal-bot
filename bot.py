...
def tp1_karlilik_kontrol(mark_price, tp1, tp2, is_long, tp3=None, tp4=None, tp5=None, symbol=None):
    """
    ...
    NOT: Tüm TP'ler arasında geçerli olan varsa TP5 öncelikli seçilir.
    ...
    # TP5 varsa önce onu dene — geçiyorsa direkt kullan, geçmiyorsa işlem açma
    if tp5 and str(tp5) != "null":
        try:
            tp5_f = float(str(tp5).replace(",", "."))
            if is_long and tp5_f <= mp_f:
                print(f"[FEE] TP5 yön hatası: LONG TP5 ({tp5_f}) <= giriş ({mp_f}) — işlem açılmıyor")
                notu = f"TP5 yanlış yönde (mark={mark_price}), işlem açılmıyor"
                print(f"[FEE] {notu}")
            elif not is_long and tp5_f >= mp_f:
                print(f"[FEE] TP5 yön hatası: SHORT TP5 ({tp5_f}) >= giriş ({mp_f}) — işlem açılmıyor")
                notu = f"TP5 yanlış yönde (mark={mark_price}), işlem açılmıyor"
                print(f"[FEE] {notu}")
            else:
                ...
                if mesafe_pct >= esik:
                    ...
                    notu = f"TP5 çok yakın (mark={mark_price}), işlem açılmıyor"
                    print(f"[FEE] {notu}")
        except Exception as e:
            print(f"[FEE] TP5 parse hatasi: {e}")
            notu = f"TP5 parse hatası, işlem açılmıyor"
        return None, notu
...
def _olta_sorgula(symbol_raw):
    ...
    Veri kaynağı: 15dk OHLCV (MEXC Futures → MEXC Spot → Binance Futures → Binance Spot)
    """
    ...
    # 2) OHLCV verisi çek (15dk, 220 mum)
    try:
    ...
    # 4) Destek / Direnç: son 50 mumdaki swing high/low
    recent_highs = highs[-50:]
    recent_lows  = lows[-50:]

    # Swing high: komşulardan yüksek
    direnc_list = []
    for i in range(2, len(recent_highs) - 2):
        if recent_highs[i] > recent_highs[i-1] and recent_highs[i] > recent_highs[i+1] \
                and recent_highs[i] > recent_highs[i-2] and recent_highs[i] > recent_highs[i+2]:
            direnc_list.append(recent_highs[i])

    destek_list = []
    for i in range(2, len(recent_lows) - 2):
        if recent_lows[i] < recent_lows[i-1] and recent_lows[i] < recent_lows[i+1] \
                and recent_lows[i] < recent_lows[i-2] and recent_lows[i] < recent_lows[i+2]:
            destek_list.append(recent_lows[i])

    # Fiyata yakın olanları al (en fazla 3'er tane)
    direnc_ust = sorted([d for d in direnc_list if d > fiyat])[:3]
    destek_alt = sorted([d for d in destek_list if d < fiyat], reverse=True)[:3]
    ...
    # LONG GİRİŞ
    long_candidates = []
    if ema20 and ema20 < fiyat:
        long_candidates.append(ema20)
    for d in destek_alt:
        if d < fiyat:
            long_candidates.append(d)
    long_ref = max(long_candidates) if long_candidates else fiyat * 0.985
    ...
    # SHORT GİRİŞ
    short_ref = direnc_ust[0] if direnc_ust else (highs[-1] if highs else fiyat * 1.015)
    short_giris = round(short_ref * 0.999, dec)
    short_stop  = round(short_ref * 1.012, dec)

    # SHORT TP
    short_tp = round(destek_alt[0], dec) if destek_alt else round(fiyat * 0.975, dec)
    ...
    mesaj = (
        ...
        f"\n"
        f"🟢 <b>LONG OLTA</b>\n"
        f"  Giriş: {p(long_giris_alt)} – {p(long_giris_ust)}\n"
        f"  Stop:  {p(long_stop)}\n"
        f"  TP1:   {p(long_tp1)}\n"
        f"  TP2:   {p(long_tp2)}\n"
        f"\n"
        f"🔴 <b>SHORT OLTA</b>\n"
        f"  Giriş: {p(short_giris)}\n"
        f"  Stop:  {p(short_stop)}\n"
        f"  TP:    {p(short_tp)}\n"
    )
    if destek_alt:
        mesaj += f"\n🛡 Destek: {' / '.join(p(d) for d in destek_alt[:3])}"
    if direnc_ust:
        mesaj += f"\n🧱 Direnç: {' / '.join(p(r) for r in direnc_ust[:3])}"
...
def _analiz_olta_seviyeleri(sym, veri):
    ...
    dec = 2 if fiyat < 10 else (1 if fiyat < 100 else 0)
