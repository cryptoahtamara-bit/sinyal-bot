...
def mexc_private_post(url, body, timeout=15):
    """Private POST — imza {} (bos dict JSON), parametreler query string (WAF bypass)"""
    # Imza: APIKey + ts + "{}" — MEXC bos body gordigunde bunu dogruluyor
    ts = str(int(time.time() * 1000))
...
def mexc_test():
    ...
    sonuc = {"version": "v199", "proxy": bool(MEXC_PRIVATE_PROXY)}
    ...
    if MEXC_API_KEY and MEXC_API_SECRET:
        ...
        sign_str = MEXC_API_KEY + ts_dbg + "{}"
        sig = hmac.new(MEXC_API_SECRET.encode(), sign_str.encode(), hashlib.sha256).hexdigest()
        sonuc["imza_debug"] = {"sign_str": "APIKey + ts + '{}'", "signature": sig[:20] + "..."}
        try:
            hdrs_dbg = {
                "ApiKey": MEXC_API_KEY, "Request-Time": ts_dbg, "Signature": sig,
                "Content-Type": "application/json",
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
                "Origin": "https://futures.mexc.com",
                "Referer": "https://futures.mexc.com/exchange/BTC_USDT",
...
print(f"[BASLANGIC] ========== BOT VERSIYON: v199 ==========")
