...
def istatistik_mesaji(snapshot=None):
    ...
    en_sym = max(
        ((sym, st["basarili"], round(st["basarili"]/st["toplam"]*100, 1))
         for sym, st in sym_stats.items() if st["basarili"] > 0),
        key=lambda x: (x[1], x[2]),  # önce TP sayısı, eşitlikte oran
        default=("", 0, -1)
    )
    ...
    if en_sym[0]:
        mesaj += f"🥇 En Başarılı Sembol: <b>{en_sym[0]}</b> ({en_sym[1]} TP, %{en_sym[2]})\n"
    if en_tf[0]:
    ...
    try:
        ...
        for st, veri_st in sinyal_sonuclari.items():
            ...
            if not en_iyi:
                ...
                continue
            best      = en_iyi[0]
            kosul_str = best["kombinasyon"]           # "1DK + Düşüş + Negatif + OI Artıyor + Aşırı Korku"
            tp_yuzde  = float(str(best["tp_orani"]).replace("%", ""))
            yeni_filtreler[st] = {
                "kosul_parts": [p.strip() for p in kosul_str.split("+")],
                "tp_yuzde"   : tp_yuzde,
            }
